...
为了提升行存表在分析类的复杂查询上的查询性能，openGauss数据库提供行存表使用向量化执行引擎的能力。通过设置GUC参数[try\_vector\_engine\_strategy](../database_reference/optimizer_method_configuration.md#section145867222412)，可以将包含行存表的查询语句转换为向量化执行计划执行。
