...
function install_gaussdb()
{
    ...
    export LD_LIBRARY_PATH=${BINARYLIBS_PATH}/${PLAT_FORM_STR}/kerberos/comm/lib:$LD_LIBRARY_PATH
    export LD_LIBRARY_PATH=${BINARYLIBS_PATH}/${PLAT_FORM_STR}/libcgroup/comm/lib:$LD_LIBRARY_PATH
    export LD_LIBRARY_PATH=${BINARYLIBS_PATH}/${PLAT_FORM_STR}/openssl/comm/lib:$LD_LIBRARY_PATH
    export LD_LIBRARY_PATH=${BINARYLIBS_PATH}/${PLAT_FORM_STR}/libcurl/comm/lib:$LD_LIBRARY_PATH
    ...
    if [ "$product_mode"x == "multiple"x ]; then
        if [ "$version_mode"x == "release"x ]; then
            ...
            CMAKE_OPT="-DENABLE_MULTIPLE_NODES=ON -DENABLE_THREAD_SAFETY=ON"
        ...
        if [ "$version_mode"x == "release"x ]; then
            ...
            CMAKE_OPT="-DENABLE_MULTIPLE_NODES=OFF -DENABLE_THREAD_SAFETY=ON -DENABLE_PRIVATEGAUSS=ON"
            export DEBUG_TYPE=release
        elif [ "$version_mode"x == "memcheck"x ]; then
        ...
        if [ "$version_mode"x == "release"x ]; then
            ...
            if [ "$PLATFORM_ARCH"X == "aarch64"X ] ; then
                ...
                GAUSSDB_EXTRA_FLAGS=" -D__USE_NUMA -D__ARM_LSE"
                CMAKE_OPT="-DENABLE_MULTIPLE_NODES=OFF -DENABLE_THREAD_SAFETY=ON"
                export DEBUG_TYPE=release
            fi
...
function target_file_copy_for_non_server()
{
    ...
    if [ "$3"x = "client"x ]; then
        # copy Roach tools
        mkdir -p $2/gs_roach
        mkdir -p $2/gs_roach/lib/postgresql
        mkdir -p $2/gs_roach/script/util
        mkdir -p $2/gs_roach/share/doc/postgresql/extension
        mkdir -p $2/gs_roach/share/postgresql/extension
        mkdir -p $2/gs_roach/share/roach
            
        cp -r ${CMAKE_BUILD_DIR}/gs_roach $2/gs_roach/
        cp -r ${BUILD_DIR}/lib/postgresql/roach_api.so $2/gs_roach/lib/postgresql
        cp -r ${CMAKE_BUILD_DIR}/lib_roach_show.so $2/gs_roach/lib
        cp -r ${CMAKE_BUILD_DIR}/roach_contrib.so $2/gs_roach/lib
        cp -r ${BUILD_DIR}/share/postgresql/extension/roach_api--1.0.sql $2/gs_roach/share/postgresql/extension
        cp -r ${BUILD_DIR}/share/postgresql/extension/roach_api.control $2/gs_roach/share/postgresql/extension
        cp -r ${ROOT_DIR}/src/distribute/bin/roach/src/conf/*.conf $2/gs_roach/share/roach
        cp -f ${script_dir}/other/roach/util/GSroach* $2/gs_roach/script/util
        if [ "$version_mode"x == "release"x ]; then
            mkdir -p $2/gs_roach/symbols
            #cp -r ${CMAKE_BUILD_DIR}/roach/symbols $2/gs_roach/
        fi
        if [ $? -ne 0 ]; then
            if [ "$version_mode"x == "release"x ]; then
