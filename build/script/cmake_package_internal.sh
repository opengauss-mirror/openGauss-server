...
if [ X"$kernel" == X"euleros" ]; then
    ...
    echo "Only support EulerOS, OPENEULER(aarch64), CentOS and Kylin platform."
...
if [ "$PLATFORM_ARCH"X == "aarch64"X ] ; then
    if [ "$dist_version" == "EULER" ]; then
        ...
        ARCHITECTURE_EXTRA_FLAG=_kylinv10_sp1_$PLATFORM_ARCH
        GAUSSDB_EXTRA_FLAGS=" -D__USE_NUMA"
    else
    ...
    if [ "$dist_version" == "KYLIN" ]; then
        ARCHITECTURE_EXTRA_FLAG=_kylinv10_sp1_${PLATFORM_ARCH}_intel
    else
...
UNIX_ODBC="${BINARYLIBS_PATH}/${PLAT_FORM_STR}/unixodbc"
...
if [ "$product_mode"x == "single"x ]; then
    declare UPGRADE_PRIV_SQL_DIR="${ROOT_DIR}/privategauss/include/catalog/upgrade_sql"
fi
...
function mpp_pkg_pre_check()
{
    ...
    if [  X"$package_type" == X"server" -o X"$package_type" == X"all" ] && [ X"$zip_package" = X"on" ] && [ ! -d "${ROOT_DIR}"/script/script/gspylib/ ]; then
        ...
        echo "  1) git clone git@lfg-y.codehub.huawei.com:2222/Gauss/GaussDBKernel/GaussDBKernel-om.git -b $(git branch | grep '*' | sed -e 's/*//g' -e 's/^ //g')"
...
{
    local gs_version=$(grep DEF_GS_VERSION ${CMAKE_BUILD_DIR}/pg_config.h | awk -F '"' '{print $2}')
...
function install_inspection()
{
    ...
    cp -rf ${BINARYLIBS_PATH}/install_tools${ARCHITECTURE_EXTRA_FLAG}/_cffi_backend*.so ${package_path}/inspection/gspylib/inspection/lib/
...
function install_gaussdb()
{
    ...
    if [ "$product_mode"x == "opengauss"x ]; then
        if [ "$version_mode"x == "release"x ]; then
            CMAKE_OPT="-DENABLE_MULTIPLE_NODES=OFF -DENABLE_THREAD_SAFETY=ON -DENABLE_MOT=ON"
...
function target_file_copy()
{
    ...
    cp -rf $BUILD_DIR/script/stage_step/ $2/script/
...
function make_package()
{
    ...
    elif [ "$pkgname"x = "libpq"x -a "$version_mode" = "release" ]; then
        # copy include file
        head=$(cat $releasefile | grep "\[header\]" -n | awk -F: '{print $1}')
        if [ ! -n "$head" ]; then
            die "error: ono find header in the $releasefile file "
    ...
    case "$pkgname" in
        ...
        server)
            ...
            mkdir -p ${BUILD_DIR}/temp/script/gspylib/inspection/lib/sqlparse/ &&
            ...
            cp -rf ${BINARYLIBS_PATH}/install_tools${ARCHITECTURE_EXTRA_FLAG}/_cffi_backend*.so ${BUILD_DIR}/temp/script/gspylib/inspection/lib/
            ...
            cp -rf ${BINARYLIBS_PATH}/install_tools${ARCHITECTURE_EXTRA_FLAG}/sqlparse/ ${BUILD_DIR}/temp/script/gspylib/inspection/lib/
