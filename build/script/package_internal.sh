...
while [ $# -gt 0 ]; do
    case "$1" in
         ...
         *)
            ...
            echo "./package_internal.sh --help or ./package_package.sh -h"
            exit 1
    esac
done
...
if [ "$product_mode"x == "single"x ]; then
    declare UPGRADE_PRIV_SQL_DIR="${ROOT_DIR}/privategauss/src/include/catalog/upgrade_sql"
fi
...
function install_gaussdb()
{
    ...
    shared_opt="--gcc-version=${gcc_version}.0 --prefix="${BUILD_DIR}" --3rd=${binarylibs_path} --enable-thread-safety ${enable_readline} --without-zlib"
    if [ "$product_mode"x == "opengauss"x ]; then
