...
sub CreateSolution
{
	...
	{
		return newm VS2005Solution(@_);
	}
	elsif ($visualStudioVersion eq '9.00')
	{
		return newm VS2008Solution(@_);
	}
	elsif ($visualStudioVersion eq '10.00')
	{
		return newm VS2010Solution(@_);
	}
	elsif ($visualStudioVersion eq '11.00')
	{
		return newm VS2012Solution(@_);
	}
	else
	{
...
sub CreateProject
{
	...
	if ($visualStudioVersion eq '8.00')
	{
		return newm VC2005Project(@_);
	}
	elsif ($visualStudioVersion eq '9.00')
	{
		return newm VC2008Project(@_);
	}
	elsif ($visualStudioVersion eq '10.00')
	{
		return newm VC2010Project(@_);
	}
	elsif ($visualStudioVersion eq '11.00')
	{
		return newm VC2012Project(@_);
	}
	else
	{
		croak "The requested Visual Studio version is not supported.";
...
"Unable to determine Visual Studio version: The nmake command wasn't found.";
		...
		{
			chomp;
			#As you may use visual studio with non-English edition, so here we remove the $ in re.
			if (/(\d+)\.(\d+)\.\d+(\.\d+)?/)
			{
