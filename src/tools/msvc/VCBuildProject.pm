...
EOF
	...
	foreach my $fileNameWithPath (sort keys %{ $self->{files} })
	{
		confess "Bad format filename '$fileNameWithPath'\n"
		  unless ($fileNameWithPath =~ /^(.*)\\([^\\]+)\.[r]?[cyl](p{2})?$/);
		my $dir  = $1;
		my $file = $2;
...
sub WriteConfiguration
{
  ...
  <Configuration Name="$cfgname|$self->{platform}" OutputDirectory=".\\$cfgname\\$self->{name}" IntermediateDirectory=".\\$cfgname\\$self->{name}"
	...
	<Tool Name="VCCLCompilerTool" Optimization="$p->{opt}"
		...
		PreprocessorDefinitions="NOCRYPT;WIN32;_WINDOWS;__WINDOWS__;__WIN32__;EXEC_BACKEND;WIN32_STACK_RLIMIT=4194304;_CRT_SECURE_NO_DEPRECATE;_CRT_NONSTDC_NO_DEPRECATE$self->{defines}$p->{defs}"
