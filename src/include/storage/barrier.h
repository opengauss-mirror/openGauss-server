...
/*
 * aarch64 architecure memory barrier. In order to minimize the impact on
...
#define pg_memory_barrier(x)                          \
    do {                                              \
        S_LOCK(&t_thrd.storage_cxt.dummy_spinlock);   \
        S_UNLOCK(&t_thrd.storage_cxt.dummy_spinlock); \
    } while (0)
#endif
