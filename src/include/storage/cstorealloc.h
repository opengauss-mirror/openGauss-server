 ...
 * 
 * IDENTIFICATION
 *        src/include/storage/cstorealloc.h
 *
 * ---------------------------------------------------------------------------------------
 */
...
public:
    ...
    static uint64 AcquireSpace(const CFileNode& cnode, Size size);

    // Acquire space for CU from free space
    static uint64 TryAcquireSpaceFromFSM(CStoreFreeSpace* fsm, Size size);
