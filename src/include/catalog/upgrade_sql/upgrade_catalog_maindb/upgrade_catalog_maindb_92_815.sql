...
CREATE OR REPLACE FUNCTION pg_catalog.compress_address_header(IN relname regclass, IN segment_no bigint, OUT extent bigint, OUT nblocks bigint, OUT alocated_chunks integer, OUT chunk_size integer, OUT algorithm bigint) RETURNS record LANGUAGE INTERNAL STRICT NOT FENCED NOT SHIPPABLE AS 'compress_address_header';

DROP FUNCTION IF EXISTS pg_catalog.compress_address_details(IN relname regclass, IN segment_no bigint, OUT extent bigint, OUT extent_block_number bigint, OUT block_number bigint, OUT alocated_chunks integer, OUT nchunks integer, OUT chunknos _int4) CASCADE;
SET LOCAL inplace_upgrade_next_system_object_oids = IUO_PROC, 1001;
CREATE OR REPLACE FUNCTION pg_catalog.compress_address_details(IN relname regclass, IN segment_no bigint, OUT extent bigint, OUT extent_block_number bigint, OUT block_number bigint, OUT alocated_chunks integer, OUT nchunks integer, OUT chunknos _int4) RETURNS record LANGUAGE INTERNAL STRICT NOT FENCED NOT SHIPPABLE AS 'compress_address_details';
...
CREATE OR REPLACE FUNCTION pg_catalog.compress_buffer_stat_info(OUT ctrl_cnt bigint, OUT main_cnt bigint, OUT free_cnt bigint, OUT recycle_times bigint) RETURNS record LANGUAGE INTERNAL STRICT NOT FENCED NOT SHIPPABLE AS 'compress_buffer_stat_info';
...
CREATE OR REPLACE FUNCTION pg_catalog.compress_ratio_info(IN input_path text, OUT path text, OUT is_compress boolean, OUT file_count bigint, OUT logic_size bigint, OUT physic_size bigint, OUT compress_ratio text) RETURNS record LANGUAGE INTERNAL STRICT NOT FENCED NOT SHIPPABLE AS 'compress_ratio_info';
...
CREATE OR REPLACE FUNCTION pg_catalog.compress_statistic_info(IN input_path text, IN step smallint, OUT path text, OUT extent_count bigint, OUT dispersion_count bigint, OUT void_count bigint) RETURNS record LANGUAGE INTERNAL STRICT NOT FENCED NOT SHIPPABLE AS 'compress_statistic_info';
...
CREATE OR REPLACE FUNCTION pg_catalog.pg_read_binary_file_blocks(IN input text, IN blocknum bigint, IN blockcount bigint, OUT path text, OUT blocknum integer, OUT len integer, OUT algorithm integer, OUT chunk_size integer, OUT data bytea) RETURNS SETOF record AS 'pg_read_binary_file_blocks' LANGUAGE INTERNAL STRICT NOT FENCED NOT SHIPPABLE COST 100 ROWS 20;
