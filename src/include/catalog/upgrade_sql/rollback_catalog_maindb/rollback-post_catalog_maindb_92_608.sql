...
DROP FUNCTION IF EXISTS pg_catalog.gs_get_history_memory_detail() cascade;DROP FUNCTION IF EXISTS gs_is_dw_io_blocked() CASCADE;
...
$do$
    DECLARE
        v5r2c10_and_later_version boolean;
    ...
    BEGIN
        ...
        IF has_version_proc = true  then
            select working_version_num >= 92549 as v5r2c10_and_later_version from working_version_num() into v5r2c10_and_later_version;
            IF v5r2c10_and_later_version = true then
                need_upgrade = true;
            end IF;
        END IF;
...
DROP FUNCTION IF EXISTS pg_catalog.array_count(anyarray, integer) CASCADE;
        ...
        END IF;
    END
$do$;DROP FUNCTION IF EXISTS pg_catalog.gs_lwlock_status() CASCADE;
DO
$do$
    DECLARE
        has_version_proc boolean;
        v5r2c10_start_version_num bigint := 92550;
        base_version_without_index boolean := false;
    BEGIN
        select case when count(*)=1 then true else false end as has_version_proc from (select * from pg_proc where proname = 'working_version_num' limit 1) into has_version_proc;
        IF has_version_proc = true  then
            select (working_version_num >= v5r2c10_start_version_num) from working_version_num() into base_version_without_index;
        END IF;
        IF base_version_without_index = true then
            -- DROP INDEX pg_partition_tblspc_relfilenode_index
            DROP INDEX IF EXISTS pg_partition_tblspc_relfilenode_index;
        END IF;
    END
$do$;
