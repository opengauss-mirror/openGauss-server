...
comment on function PG_CATALOG.gs_undo_dump_parsepage_mv(relpath text, blkno bigint, reltype text, rmem boolean) is 'parse uheap data page and undo to output file based on given filepath';DROP EXTENSION IF EXISTS hdfs_fdw;/*------ add sys fuction gs_is_recycle_obj ------*/
...
RETURNS bool LANGUAGE INTERNAL as 'gs_is_recycle_obj';SET LOCAL inplace_upgrade_next_system_object_oids = IUO_PROC, 8010;
...
(
    ...
    OUT detail text
) RETURNS SETOF record LANGUAGE INTERNAL STABLE as 'gs_verify_urq';
...
(
	reloid oid,
    OUT result text
) RETURNS SETOF record LANGUAGE INTERNAL STABLE as 'gs_repair_urq';
...
CREATE OR REPLACE FUNCTION pg_catalog.gs_verify_undo_record(text, int8, int8, boolean, OUT zone_id oid, OUT detail text)
RETURNS SETOF record LANGUAGE INTERNAL as 'gs_verify_undo_record';
...
CREATE OR REPLACE FUNCTION pg_catalog.gs_verify_undo_translot(text, int8, int8, boolean, OUT zone_id oid, OUT detail text)
RETURNS SETOF record LANGUAGE INTERNAL as 'gs_verify_undo_translot';
...
CREATE OR REPLACE FUNCTION pg_catalog.gs_verify_undo_meta(text, int8, int8, boolean, OUT zone_id oid, OUT detail text)
RETURNS SETOF record LANGUAGE INTERNAL as 'gs_verify_undo_meta';