...
DROP INDEX IF EXISTS pg_catalog.statement_history_time_idx CASCADE;
...
/*------ add sys fuction statement_trace_decode ------*/
DROP FUNCTION IF EXISTS pg_catalog.statement_trace_decode() CASCADE;
set local inplace_upgrade_next_system_object_oids = IUO_PROC, 5736;
...
DROP FUNCTION IF EXISTS dbe_perf.standby_statement_history(boolean) CASCADE;
DROP FUNCTION IF EXISTS dbe_perf.standby_statement_history(boolean, timestamp with time zone[]) CASCADE;
