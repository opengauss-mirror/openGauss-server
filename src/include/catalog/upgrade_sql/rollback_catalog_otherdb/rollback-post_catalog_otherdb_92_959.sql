...
RETURNS float4 AS
$$select pg_catalog.to_binary_float($1, ' ', false, false)$$
...
RETURNS float4 AS
$$select pg_catalog.to_binary_float($1, 0, false, false)$$
LANGUAGE SQL IMMUTABLE COST 1;
...
$$select pg_catalog.to_binary_float($1, 0, false, false)$$
LANGUAGE SQL IMMUTABLE COST 1;
COMMENT ON FUNCTION pg_catalog.to_binary_float(float8, text, bool, bool) IS 'convert float8 to a single precision floating-point number, with default return expr on convert error';
...
LANGUAGE INTERNAL
IMMUTABLE;
COMMENT ON FUNCTION pg_catalog.to_binary_float(text, float8, bool, bool) IS 'convert text to a single precision floating-point number, with default return expr on convert error';
