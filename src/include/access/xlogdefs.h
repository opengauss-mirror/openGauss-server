...
typedef struct XLogRecPtrOld {
    uint32 xlogid;  /* log file #, 0 based */
    uint32 xrecoff; /* byte offset of location in log file */
} XLogRecPtrOld;

#define InvalidXLogRecPtrOld \
    (XLogRecPtrOld)          \
    {                        \
        0, 0                 \
    }
...
/*
 * Macro for caculate the difference between two given record pointer.
 */
#define XLByteDifference(a, b) (a - b)

/*
 * Macro for advancing a record pointer by the specified number of bytes.
 */
#define XLByteAdvance(recptr, nbytes) (recptr) += nbytes
