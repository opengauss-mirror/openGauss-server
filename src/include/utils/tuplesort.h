...
extern Tuplesortstate* tuplesort_begin_heap(TupleDesc tupDesc, int nkeys, AttrNumber* attNums, Oid* sortOperators,
    Oid* sortCollations, const bool* nullsFirstFlags, int64 workMem, SortCoordinate coordinate, bool randomAccess,
    int64 maxMem = 0, int planId = 0, int dop = 1);
extern Tuplesortstate* tuplesort_begin_cluster(
    TupleDesc tupDesc, Relation indexRel, int workMem, SortCoordinate coordinate, bool randomAccess, int maxMem);
...
extern Tuplesortstate* tuplesort_begin_index_hash(
    Relation indexRel, uint32 hash_mask, int workMem, SortCoordinate coordinate, bool randomAccess, int maxMem);
extern Tuplesortstate* tuplesort_begin_datum(Oid datumType, Oid sortOperator, Oid sortCollation,
    bool nullsFirstFlag, int workMem, SortCoordinate coordinate, bool randomAccess);
#ifdef PGXC
...
extern void tuplesort_initialize_shared(SharedSort *shared, int nWorkers, void *seg);
