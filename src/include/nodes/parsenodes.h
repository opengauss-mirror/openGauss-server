...
typedef enum RowMovementValue { ROWMOVEMENT_DISABLE, ROWMOVEMENT_ENABLE, ROWMOVEMENT_DEFAULT } RowMovementValue;
...
typedef enum TdTruncCastStatus { UNINVOLVED_QUERY = 0, NOT_CAST_BECAUSEOF_GUC, TRUNC_CAST_QUERY } TdTruncCastStatus;
...
#define MAX_TABLE_LIKE_OPTIONS (9)
 ...
 * ----------------------
 */
typedef enum TempType { Temp_None, Temp_Rel, Temp_Toast } TempType;
...
/* Additional Property for AlterTableCmd */
typedef enum AlterTableStatProperty { 
...
typedef struct SplitPartitionState {
    NodeTag type;
...
typedef struct AddPartitionState {
    NodeTag type;
    List* partitionList;
    bool isStartEnd;
} AddPartitionState;
