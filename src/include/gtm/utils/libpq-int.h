...
#include "gtm/utils/pqexpbuffer.h"
...
struct gtm_conn {
    ...
    char* pghostaddr;        /* the IPv4 address of the machine on which
                              ...
                              * over above. */
    char* gtmlocalhost;      /* for gtm HA to connect to standby */
    char* gtmlocalport;      /* for gtm HA to connect to standby */
    ...
    pthread_t remote_thdpid; /* remote node thread pid */

    /* Optional file to write trace info to */
    FILE* Pfdebug;

    /* Status indicators */
    GTMConnStatusType status;
...
/* === in fe-misc.c === */
extern THR_LOCAL bool g_gtm_interrupt_happened;
...
extern int64 gtmpqGetInt64(uint64* result, GTM_Conn* conn);
extern int gtmpqPutInt(int value, size_t bytes, GTM_Conn* conn);
extern int gtmpqPutInt64(uint64 value, GTM_Conn* conn);
...
extern int gtmpqWaitTimed(int forRead, int forWrite, GTM_Conn* conn, time_t finish_time);
...
// In gtmctl-protocol.c
GTM_Result* GTMCTLPQgetResult(GTM_Conn* conn);
int gtmpqPutMsgBytes(const void* buf, size_t len, GTM_Conn* conn);
