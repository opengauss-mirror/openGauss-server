...
#define InvalidTgOwnerId ((Oid)(-1)) /* An invalid trigger owner oid, for old version pg_trigger */
...
extern THR_LOCAL PGDLLIMPORT int SessionReplicationRole;
...
extern ObjectAddress CreateTrigger(CreateTrigStmt* stmt, const char* queryString, Oid relOid, Oid refRelOid, Oid constraintOid,
...
extern TupleTableSlot* ExecBRUpdateTriggers(EState* estate, EPQState* epqstate, ResultRelInfo* relinfo,
    Oid oldPartitionOid, int2 bucketid,
#ifdef PGXC
    HeapTupleHeader datanode_tuphead,
#endif
...
#endif
    List* recheckIndexes);
extern TupleTableSlot* ExecIRUpdateTriggers(
    EState* estate, ResultRelInfo* relinfo, HeapTuple trigtuple, TupleTableSlot* slot);
 ...
 */
extern bool RI_FKey_keyequal_upd_pk(Trigger* trigger, Relation pk_rel, HeapTuple old_row, HeapTuple new_row);
extern bool RI_FKey_keyequal_upd_fk(Trigger* trigger, Relation fk_rel, HeapTuple old_row, HeapTuple new_row);
...
extern bool pgxc_should_exec_br_trigger(Relation rel, int16 trigevent, EState* estate = NULL);
