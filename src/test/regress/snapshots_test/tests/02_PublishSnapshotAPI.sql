...
BEGIN
    ...
    PERFORM _db4ai_test.assert_exception('db4ai.publish_snapshot(null,null)',
        'i_name cannot be NULL or empty');
    PERFORM _db4ai_test.assert_exception('db4ai.publish_snapshot(null,''name'')',
        'i_name must contain exactly one ''@'' character',
        'reference a snapshot using the format: snapshot_name@version');
    PERFORM _db4ai_test.assert_exception('db4ai.publish_snapshot(null,''name@org@com'')',
        'i_name must contain exactly one ''@'' character',
        'reference a snapshot using the format: snapshot_name@version');
    PERFORM _db4ai_test.assert_exception('db4ai.publish_snapshot(''_db4ai test'',''name@1.0.0'')',
    ...
    PERFORM _db4ai_test.assert_equal(
        'db4ai.publish_snapshot(''_db4ai test'', '' test  @1.1.0'')',
    ...
    PERFORM _db4ai_test.assert_equal(
        'SELECT (schema, name) FROM '
            'db4ai.publish_snapshot(null, ''test@2.0.0'')',
