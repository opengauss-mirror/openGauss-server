...
----
--- test 6: Vec Merge Sort, work_mem is not enough to pre read all data.
----
CREATE TABLE merge_sort_row(id INTEGER, hphm NVARCHAR2(50));
CREATE TABLE src(a INTEGER);
INSERT INTO src VALUES (1);
INSERT INTO merge_sort_row SELECT generate_series(1, 5000), 'merge sort string' || generate_series(1, 5000) FROM src;
DROP TABLE src;

SET WORK_MEM=64;
SELECT COUNT(*) FROM (SELECT hphm, ROW_NUMBER() OVER (ORDER BY hphm) AS kdbh FROM (SELECT hphm FROM merge_sort_row GROUP BY 1));

CREATE TABLE merge_sort_column WITH (ORIENTATION = COLUMN) AS SELECT * FROM merge_sort_row;
SELECT COUNT(*) FROM (SELECT hphm, ROW_NUMBER() OVER (ORDER BY hphm) AS kdbh FROM (SELECT hphm FROM merge_sort_column GROUP BY 1));

RESET WORK_MEM;
DROP TABLE merge_sort_row;
DROP TABLE merge_sort_column;
