...
-- Comment the following test, since we don't support 
-- return empty string. Refer the result of PT
-- SELECT regexp_matches('foobarbequebaz', $re$(bar)(.*)(beque)$re$);
-- no match
SELECT regexp_matches('foobarbequebaz', $re$(bar)(.+)(beque)$re$);
...
-- some corner cases
-- Comment the following 3 test, since we don't support 
-- return empty string. Refer the result of PT
...
-- test template substring
SELECT substring(f1,100005,10)  is null from toasttest;
...
SELECT translate('', '14', 'ax') is null;
...
select substring('CCCCChina' from 'A*') is null;
select substring('sdnnnn*f*(ja)1@$%' from 'n*') is null;
drop table test_substring1;
drop table test_substring2;
create table test_substring1(col_str varchar2(20));
insert into test_substring1 values('123Thomas');
insert into test_substring1 values('THAAAhomas');
select col_str,substring(col_str from '[a-zA-Z]*') as result from test_substring1 order by result,col_str;
create table test_substring2(col_str varchar2(20));
insert into test_substring2 values('sdnf^ja^i@#$%');
select col_str,substring(col_str from '\^') as result from test_substring2;
select col_str,substring(col_str from '^') as result from test_substring2;
select substring('111aa11an44444k3aaaaa56aaaa','a{0,0}') is null;
drop table test_substring1;
drop table test_substring2;

SELECT SUBSTRING('asdf' from 2 for 0) is null;
CREATE TABLE COMPARISIOIN_OPERATOR_ISNULL_TABLE_000 (
 C_ID INTEGER,
 C_CHAR CHAR(102400),
 ...
 C_DATE DATE,
 C_BOOL BOOLEAN );
INSERT INTO COMPARISIOIN_OPERATOR_ISNULL_TABLE_000 VALUES(1,'ABCDEFG','abcdefg',111,1.11,'2000-01-01',TRUE);
INSERT INTO COMPARISIOIN_OPERATOR_ISNULL_TABLE_000 VALUES(2,'BCDEFGH','bcdefgh',222,2.22,'2000-02-02',FALSE);
INSERT INTO COMPARISIOIN_OPERATOR_ISNULL_TABLE_000 VALUES(3,NULL,NULL,NULL,NULL,NULL,NULL);
INSERT INTO COMPARISIOIN_OPERATOR_ISNULL_TABLE_000 VALUES(4,'','1',0,0,'2000-02-02',FALSE);
INSERT INTO COMPARISIOIN_OPERATOR_ISNULL_TABLE_000 VALUES(5,'NULL','NULL',0,0,NULL,NULL);
INSERT INTO COMPARISIOIN_OPERATOR_ISNULL_TABLE_000 VALUES(6,'  ','  ',0,0,NULL,NULL);

SELECT SUBSTRING(C_CHAR FROM 2 FOR 0) IS NULL AS NUM FROM COMPARISIOIN_OPERATOR_ISNULL_TABLE_000 WHERE C_ID = 1;

DROP TABLE COMPARISIOIN_OPERATOR_ISNULL_TABLE_000;
