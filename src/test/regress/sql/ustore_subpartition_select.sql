--prepare
DROP SCHEMA ustore_subpartition_select CASCADE;
CREATE SCHEMA ustore_subpartition_select;
SET CURRENT_SCHEMA TO ustore_subpartition_select;
...
create table pjade(jid int,jn int,name varchar2) WITH (STORAGE_TYPE=USTORE) partition by range(jid) subpartition by range(jn)
(
...
create table cjade(jid int,jn int,name varchar2) WITH (STORAGE_TYPE=USTORE);
