...
CREATE TABLE films (
    ...
    date_prod   date,
    kind        varchar(10),
    CONSTRAINT code_title PRIMARY KEY(code, title, did)
)distribute by hash(code, title); 
...
create unique index test_index_films on films(code);
drop table films;

CREATE TABLE parent_fk (
    a int, 
    b int, 
    c int, 
    CONSTRAINT code_title PRIMARY KEY(a, b) 
    ) distribute BY hash(a, b); 

--Have not include all distribute keys
CREATE TABLE child_fk (a int, b int, FOREIGN KEY (b, c) references parent_fk(a, b), c int) distribute BY hash(a, b); 
CREATE TABLE child_fk (a int, b int, FOREIGN KEY (a, b) references parent_fk(a, b), c int);

--The position of distribute key is different
CREATE TABLE child_fk (a int, b int, FOREIGN KEY (b, a) references parent_fk(a, b), c int) distribute BY hash(a, b); 
--OK
CREATE TABLE child_fk (a int, b int, FOREIGN KEY (a, b) references parent_fk(a, b), c int) distribute BY hash(a, b); 
...
CREATE TABLE parent_fk (
 ...
 b int,
 c int,
 CONSTRAINT code_title PRIMARY KEY(a, b, c)
 ) distribute BY hash(b, c);

 --The num is different, FOREIGN KEY and references
 CREATE TABLE child_fk (a int, b int, FOREIGN KEY (a, b, c) references parent_fk(a, b), c int);
--Have not include all distribute keys
 CREATE TABLE child_fk (a int, b int, FOREIGN KEY (a, b) references parent_fk(a, b), c int);
--OK
CREATE TABLE child_fk (a int, b int, FOREIGN KEY (a, b, c) references parent_fk(a, b, c), c int);
drop table child_fk;
CREATE TABLE child_fk (a int, b int, FOREIGN KEY (a, c, b) references parent_fk(a, c, b), c int);
drop table child_fk;
CREATE TABLE child_fk (a int, b int, FOREIGN KEY (b, c, a) references parent_fk(a, c, b), c int);
drop table child_fk;
