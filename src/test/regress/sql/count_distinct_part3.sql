...
explain (costs off, nodes off) select count(distinct a),max(distinct b),c from t_distinct group by c union (select count(distinct a),max(distinct b),d from t_distinct group by d) order by 3,2,1 limit 10;
...
explain (costs off, nodes off) select count(distinct(a)), count(distinct(b)), d from t_distinct group by d order by d;
select count(distinct(a)), count(distinct(b)), d from t_distinct group by d order by d;
explain (costs off, nodes off) select count(distinct(a)), count(distinct(b)), d from t_distinct group by c, d order by c, d limit 10;
...
explain (costs off, nodes off) select count(distinct(a)), count(distinct(a*0)), b, c, d from t_distinct group by b, c, d order by 3 desc, 4 desc, 5 desc limit 10;
select count(distinct(a)), count(distinct(a*0)), b, c, d from t_distinct group by b, c, d order by 3 desc, 4 desc, 5 desc limit 10;
explain (costs off, nodes off) select count(distinct(a)), count(distinct(b)), c, d from t_distinct group by c, d order by 3 desc, 4 desc limit 10;
...
analyze t_distinct1;
-- distribute by d
explain (verbose on, costs off, nodes off) select sum(a),count(distinct(b)),avg(c),d from t_distinct1 group by b,c,d;
select sum(a),count(distinct(b)),avg(c),d from t_distinct1 group by b,c,d order by 1 desc limit 10;
explain (verbose on, costs off, nodes off) select avg(a::numeric(7, 2)) over(partition by b, d) from t_distinct1;
select avg(a::numeric(7, 2)) over(partition by b, d) from t_distinct1 order by 1 desc limit 10;
-- all expr is skew
explain (verbose on, costs off, nodes off) select avg(a::numeric(7, 2)),count(distinct(c)) from t_distinct1 group by b,c;
select avg(a::numeric(7, 2)),count(distinct(c)) from t_distinct1 group by b,c order by 1 desc limit 10;
explain (verbose on, costs off, nodes off) select abs(b), count(distinct(a)) from t_distinct1 group by abs(b) having abs(b)+sum(c)>1000;
