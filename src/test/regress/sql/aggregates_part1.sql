...
-- group by clause
explain (costs off, nodes off) select e, sum(c) from t_agg1 group by e;
select e, sum(c) from t_agg1 group by e;
explain (costs off, nodes off) select e, sum(b+c), avg(d) from t_agg1 group by e;
select e, sum(b+c), avg(d) from t_agg1 group by e;
explain (costs off, nodes off) select f, e, max(a), min(b) from t_agg1 group by f, e;
select f, e, max(a), min(b) from t_agg1 group by f, e;
explain (costs off, nodes off) select e, min(b), rank() over (partition by f order by f) from t_agg1 group by f, e;
select e, min(b), rank() over (partition by f order by f) from t_agg1 group by f, e;

-- distinct clause
explain (costs off, nodes off) select distinct(e) from t_agg1;
select distinct(e) from t_agg1;
explain (costs off, nodes off) select distinct(e+f) from t_agg1;
select distinct(e+f) from t_agg1;

-- join
explain (costs off, nodes off) select sum(t_agg1.a) from t_agg1 join t_agg2 on t_agg1.c=t_agg2.b group by t_agg1.e;
select sum(t_agg1.a) from t_agg1 join t_agg2 on t_agg1.c=t_agg2.b group by t_agg1.e;
explain (costs off, nodes off) select a, sum(c) from (select t_agg1.e a, t_agg1.b b, t_agg2.a-t_agg2.b c from t_agg1 join t_agg2 on t_agg1.b=t_agg2.c) group by a;
...
explain (costs off, nodes off) select a, sum(c) from (select t_agg1.b b, t_agg1.e a, sum(t_agg2.a-t_agg2.b) c from t_agg1 join t_agg2 on t_agg1.b=t_agg2.c group by 1, 2) group by a;
...
explain (costs off, nodes off) select a, sum(c) from (select t_agg1.b b, t_agg1.e a, sum(t_agg2.a-t_agg2.b) c from t_agg1 join t_agg2 on t_agg1.b=t_agg2.c group by 1, 2) group by a having sum(c)=-20;
...
-- group by clause
explain (costs off, nodes off) select b, sum(a+c), avg(d) from t_agg1 group by b;
select b, sum(a+c), avg(d) from t_agg1 group by b order by b limit 10;
explain (costs off, nodes off) select b, c, max(a), min(d) from t_agg1 group by b, c;
select b, c, max(a), min(d) from t_agg1 group by b, c order by b, c limit 10;
explain (costs off, nodes off) select b, d, min(c), rank() over (partition by d order by d) from t_agg1 group by b, d;
select b, d, min(c), rank() over (partition by d order by d) from t_agg1 group by b, d order by b, d limit 10;

-- distinct clause
explain (costs off, nodes off) select distinct(b) from t_agg1;
select distinct(b) from t_agg1 order by 1 limit 10;
explain (costs off, nodes off) select distinct(b+c) from t_agg1;
...
explain (costs off, nodes off) select sum(x) from (select sum(a) x from t_agg1 group by b, c);
select sum(x) from (select sum(a) x from t_agg1 group by b, c);
explain (costs off, nodes off) select sum(c+d) from (select 2*b x, c, d from t_agg1) group by x;
select sum(c+d) from (select 2*b x, c, d from t_agg1) group by x order by x limit 10;
explain (costs off, nodes off) select sum(x) from (select 1, sum(a) x from t_agg1 group by 1);
select sum(x) from (select 1, sum(a) x from t_agg1 group by 1);
explain (costs off, nodes off) select avg(z) from (select 2*a x, a+1 y, a-1 z from t_agg1) group by x, y;
