...
create table t1 (id bigserial primary key, val vector(5));
ALTER TABLE t1 SET (parallel_workers = 4);
INSERT INTO t1 (val) VALUES ('[0,0,0,0,0]'), ('[1,2,3,0,1]'), ('[1,1,1,0,3]');
INSERT INTO t1 (val) VALUES ('[4,0,0,0,1]'), ('[4,2,3,0,2]'), ('[4,1,1,0,3]');
INSERT INTO t1 (val) VALUES ('[25,20,20,22,21]'), ('[25,22,23,26,3]'), ('[25,21,21,21,5]');
INSERT INTO t1 (val) VALUES ('[24,20,20,20,6]'), ('[24,22,23,20,8]'), ('[24,21,21,20,9]');
INSERT INTO t1 (val) VALUES ('[3,3,4,5,6]'), ('[6,7,8,9,2]'), ('[3,5,6,7,8]');
INSERT INTO t1 (val) VALUES ('[4,5,6,7,8]'), ('[1,2,3,4,5]'), ('[6,7,8,9,2]');
drop index if exists idx_t1;
create index idx_t1 on t1 using ivfflat (val vector_cosine_ops) with(lists=3, enable_rabitq=on, rabitq_refine_type='none', rabitq_fht=on);
drop index if exists idx_t1;
create index idx_t1 on t1 using ivfflat (val vector_cosine_ops) with(lists=3, enable_rabitq=on, rabitq_refine_type='SQ8', rabitq_fht=on);
drop index if exists idx_t1;
create index idx_t1 on t1 using ivfflat (val vector_cosine_ops) with(lists=3, enable_rabitq=on, rabitq_refine_type='FP32', rabitq_fht=on);
set rbq_refinek=5;
...
drop index if exists idx_t1;
create index idx_t1 on t1 using ivfflat (val vector_l2_ops) with(lists=3, enable_rabitq=on, rabitq_refine_type='FP32', rabitq_fht=on);
explain (costs off) select id from t1 order by val <-> '[6,7,3,10,1]' limit 5;
drop index if exists idx_t1;
