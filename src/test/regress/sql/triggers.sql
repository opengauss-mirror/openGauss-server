...
--self add
--1. scenes for ddl in trigger
create table test_ddl_in_trigger_table1 (a int);
create or replace function test_ddl_in_trigger_func() returns trigger language plpgsql as $$
begin
	execute 'create table test_ddl_in_trigger_table2 (b int);';
	return null;
end $$;
create trigger test_ddl_in_trigger_trig before insert on test_ddl_in_trigger_table1
  for each row execute procedure test_ddl_in_trigger_func();
insert into test_ddl_in_trigger_table1 values (1);
select * from test_ddl_in_trigger_table1;
select * from test_ddl_in_trigger_table2;
drop table if exists test_ddl_in_trigger_table1;
drop table if exists test_ddl_in_trigger_table2;
drop function test_ddl_in_trigger_func;
--2. scenes for select in trigger
create table test_select_in_trigger_table1(a serial,name text,comment text);
create table test_select_in_trigger_table2(slotname char(20),pfname text,slotlink char(20),backlink char(20));
create function test_select_in_trigger_table2_func() returns trigger as $proc$
declare
    pfrec       record;
    ps          alias for new;
begin
    select into pfrec * from test_select_in_trigger_table1 where name = ps.pfname;
    if not found then
...
create trigger tg_test_select_in_trigger_table2_trig before insert or update
    on test_select_in_trigger_table2 for each row execute procedure test_select_in_trigger_table2_func();
insert into test_select_in_trigger_table1 values(2,'a','feawre');
insert into test_select_in_trigger_table2 values('est','a','dfe','afe');
select * from test_select_in_trigger_table1;
select * from test_select_in_trigger_table2;
drop table if exists test_select_in_trigger_table1;
drop table if exists test_select_in_trigger_table2;
drop function test_select_in_trigger_table2_func;
--3. scenes for update in trigger
create table test_update_in_trigger_table1(slotname char(20),pfname text,slotlink char(20),backlink char(20)) distribute by hash(slotlink);
insert into test_update_in_trigger_table1 values('PStest','a name');
create function test_update_in_trigger_table1_func() returns trigger as '
begin
    if new.slotname != old.slotname then
        delete from test_update_in_trigger_table1 where slotname = old.slotname;
        insert into test_update_in_trigger_table1 values (new.slotname,new.pfname,new.slotlink,new.backlink);
        return null;
    end if;
    return new;
end;
' language plpgsql;
create trigger tg_test_select_in_trigger_table2_trig before update
    on test_update_in_trigger_table1 for each row execute procedure test_update_in_trigger_table1_func();
update test_update_in_trigger_table1 set pfname='this is a new name';
select * from test_update_in_trigger_table1;
drop table if exists test_update_in_trigger_table1;
drop function test_update_in_trigger_table1_func;
--4. scenes for insert into select in trigger
create table test_insert_into_select_trigger_table1(id int);
create table test_insert_into_select_trigger_table2(id int)with(orientation = column);
create table test_insert_into_select_trigger_table_temp(id int);
insert into test_insert_into_select_trigger_table_temp values(100);
insert into test_insert_into_select_trigger_table_temp values(200);
insert into test_insert_into_select_trigger_table_temp values(300);
create or replace function test_insert_into_select_trigger_table2_func() returns trigger as $$
begin
	insert into test_insert_into_select_trigger_table2 values(new.id);
	raise notice '% % % %', TG_RELNAME, TG_OP, TG_WHEN, TG_LEVEL;
	return new;
end;$$ language plpgsql;
...
--5. test update trigger with drop column
create table test_update_trigger_with_drop_column(slotname char(20),comment text,slotlink char(20)) distribute by hash(slotlink);
insert into test_update_trigger_with_drop_column values('aa','bb','cc');
create or replace function test_update_trigger_with_drop_column_func() returns trigger as '
begin
    if new.slotname != old.slotname then
        delete from test_update_trigger_with_drop_column where slotname = old.slotname;
        return null;
    end if;
    return new;
end;
...
create trigger test_update_trigger_with_drop_column_trig before update on test_update_trigger_with_drop_column
  for each row execute procedure test_update_trigger_with_drop_column_func();
alter table test_update_trigger_with_drop_column drop column comment;
alter table test_update_trigger_with_drop_column add column comment text;
update test_update_trigger_with_drop_column set comment='fjeoajfe.joaej';
select * from test_update_trigger_with_drop_column;
drop table test_update_trigger_with_drop_column;
--6. test fire trigger to modify orientation/column table
create table test_trigger_modify_column_src_tbl(id1 int, id2 int, id3 int);
create table test_trigger_modify_column_des_tbl(id1 int, id2 int, id3 int) with(orientation = column);
create or replace function tri_insert_func() returns trigger as
$$
declare
begin
        insert into test_trigger_modify_column_des_tbl values(NEW.id1, NEW.id2, NEW.id3);
        return new;
end
$$ language plpgsql;
...
declare
begin
        update test_trigger_modify_column_des_tbl set id3 = new.id3 where id1=old.id1;
        return new;
end
$$ language plpgsql;
...
declare
begin
        delete from test_trigger_modify_column_des_tbl where id1=old.id1;
        return old;
end
$$ language plpgsql;
create trigger insert_trigger
  before insert on test_trigger_modify_column_src_tbl
  for each row
  execute procedure tri_insert_func();
create trigger update_trigger
  before update on test_trigger_modify_column_src_tbl
  for each row
  execute procedure tri_update_func();
create trigger delete_trigger
  before delete on test_trigger_modify_column_src_tbl
  for each row
  execute procedure tri_delete_func();
insert into test_trigger_modify_column_src_tbl values(100,100,100);
select * from test_trigger_modify_column_src_tbl order by id1;
select * from test_trigger_modify_column_des_tbl order by id1;
update test_trigger_modify_column_src_tbl set id3=1000 where id3=100;
select * from test_trigger_modify_column_src_tbl order by id1;
select * from test_trigger_modify_column_des_tbl order by id1;
delete from test_trigger_modify_column_src_tbl where id2=100;
select * from test_trigger_modify_column_src_tbl order by id1;
select * from test_trigger_modify_column_des_tbl order by id1;
drop table if exists test_trigger_modify_column_src_tbl cascade;
drop table if exists test_trigger_modify_column_des_tbl cascade;
--8. test error trigger function body
create table tri_over1 (over1 INT, function1 INT, split1 INT, return1 int, isnull1 int) partition by range (over1)(partition over1 values less than(100),partition p2 values less than(maxvalue));
create table tri_function1 (over1 INT, function1 INT, split1 INT, return1 int, isnull1 int) ;
set check_function_bodies = false;
CREATE OR REPLACE FUNCTION tri_over1() RETURNS TRIGGER AS
$$
DECLARE
BEGIN
insert into tri_over1 values(% ,% ,% , % , %);
RETURN NEW;
END
$$ LANGUAGE PLPGSQL;
CREATE TRIGGER overrt  BEFORE INSERT ON tri_function1 FOR EACH ROW EXECUTE PROCEDURE tri_over1();
insert into tri_function1 values(1,1,1,1,1);
drop table tri_over1;
drop table tri_function1;
