...
--prepare
DROP SCHEMA ustore_subpartition_createtable CASCADE;
CREATE SCHEMA ustore_subpartition_createtable;
SET CURRENT_SCHEMA TO ustore_subpartition_createtable;
...
create table range_range_copy WITH (STORAGE_TYPE=USTORE) as select * from range_range subpartition(p_201901_a) where month_code in(201902,201903);
...
(
  ...
  c int,
  primary key(c,c_int)
) with (parallel_workers=10, STORAGE_TYPE=USTORE)
