...
create table random_plan_t1(c1 int, c2 char(10));
create table random_plan_t2(c1 int, c2 char(10));
...
explain (verbose on, costs off, nodes off) select count(*) from (select distinct a.c1,b.c1 from random_plan_t1 a left join random_plan_t2 b on a.c1=b.c1 group by a.c1,b.c1 order by a.c1,b.c1);
...
explain (verbose on, costs off, nodes off) select distinct(c1+c2) from random_plan_t3;
--set current plan_mode_seed
set plan_mode_seed=877315715;
--explain last plan, query_level==2 and distributed_key!=NULL
explain (costs off, nodes off) select t.c1, sum(t.c3) from (select t3.c2 c2, t3.c3 c1, sum(t3.c2+t4.c1) c3 from random_plan_t3 t3 join random_plan_t4 t4 on t3.c2=t4.c3 group by 1, 2) as t group by t.c1;
--set current plan_mode_seed
set plan_mode_seed=1183126883;
--explain last plan, query_level==2 and distributed_key==NULL
explain (costs off, nodes off) select x from (select c4, sum(c1) x from random_plan_t3 group by 1);
--drop table
