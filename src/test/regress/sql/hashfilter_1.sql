...
explain (costs off, nodes off) select t1.* from t1 right join t2 on t2.col_int = trunc(t1.col_numeric) and t1.col_varchar like 'a%' where t2.col_date is not null and t2.col_int < 750 order by t1.col_int;
