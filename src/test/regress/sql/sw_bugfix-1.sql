...
-- test connnect by nocycle without start-with
create table c (c_id number,c_d_id number,c_w_id number);
...
-- ends up with cycle detected
select c_id,c_d_id,c_w_id from c a connect by nocycle c_d_id = prior c_d_id;

-- ends up with cycle detected and output which tuple is cycled
select c_id,c_d_id,c_w_id, connect_by_iscycle from c a connect by nocycle c_d_id = prior c_d_id;

-- ends up with cycle exception
select c_id,c_d_id,c_w_id from c a connect by c_d_id = prior c_d_id;
