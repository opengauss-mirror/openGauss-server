...
explain (costs off, nodes off) select count(distinct(c)) from (select a, ''::text as c from (select t1.a from t_distinct t1 join t_distinct t2 on t1.a=t2.a join t_distinct t3 on t2.a=t3.a) where a>5);
