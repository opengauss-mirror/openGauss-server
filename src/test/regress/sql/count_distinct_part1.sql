...
-- Case 1.1 top level
explain (costs off, nodes off) select d, count(distinct(a)), avg(a::numeric(7, 2)) from t_distinct group by d;
select d, count(distinct(a)), avg(a::numeric(7, 2)) from t_distinct group by d order by d;
explain (costs off, nodes off) select c, count(distinct(b)), avg(a::numeric(7, 2)) from t_distinct group by c;
select c, count(distinct(b)), avg(a::numeric(7, 2)) from t_distinct group by c order by c limit 10;
explain (costs off, nodes off) select 1, 1, b, count(distinct(a)), avg(a::numeric(7, 2)) from t_distinct group by 1,2,3 order by 1,2,3;
select 1, 1, b, count(distinct(a)), avg(a::numeric(7, 2)) from t_distinct group by 1,2,3 order by 1,2,3 limit 10;
explain (costs off, nodes off) select d, count(distinct(b)), avg(a::numeric(7, 2)) from t_distinct group by d;
select d, count(distinct(b)), avg(a::numeric(7, 2)) from t_distinct group by d order by d;
explain (costs off, nodes off) select c, count(distinct(a)), count(distinct(b)), count(distinct(d)) from t_distinct group by c;
...
explain (costs off, nodes off) select c, count(distinct(a)) ca, count(distinct(b)) cb, count(distinct(d)) cd from t_distinct group by c having count(distinct(b))+count(distinct(d))=47;
...
explain (costs off, nodes off) select a, count(distinct(b+d)), max(c+b) from t_distinct group by a;
select a, count(distinct(b+d)), max(c+b) from t_distinct group by a order by a limit 10;
explain (costs off, nodes off) select 1, count(distinct(b)), max(c), min(d), avg(a) from t_distinct group by 1;
select 1, count(distinct(b)), max(c), min(d), avg(a) from t_distinct group by 1;
explain (costs off, nodes off) SELECT sum(c),avg(c),max(c),min(c),avg(c),avg(c),sum(c),sum(c),count(distinct(c)) FROM t_distinct GROUP BY d order by d;
SELECT sum(c),avg(c),max(c),min(c),avg(c),avg(c),sum(c),sum(c),count(distinct(c)) FROM t_distinct GROUP BY d order by d;
explain (costs off, nodes off) select sum(a) like '%1%', count(distinct c), d from t_distinct group by d order by 1,2,3;
select sum(a) like '%1%', count(distinct c), d from t_distinct group by d order by 1,2,3;
explain (costs off, nodes off) select sum(a) like '%1%', count(distinct c), c from t_distinct group by c order by 1,2,3;
select sum(a) like '%1%', count(distinct c), c from t_distinct group by c order by 1,2,3 limit 10;
explain (costs off, nodes off) select a from (select a, b from t_distinct union all select b, a from t_distinct) group by a having count(distinct(b+1)) = 1 order by 1 limit 5; 
...
-- Case 1.2 sub level
explain (costs off, nodes off) select c, ca, cb from (select c, count(distinct(a)) ca, max(b) cb, min(c+d) cd from t_distinct group by c) x where cd=7;
select c, ca, cb from (select c, count(distinct(a)) ca, max(b) cb, min(c+d) cd from t_distinct group by c) x where cd=7;
explain (costs off, nodes off) select c, ca, cb from (select c, count(distinct(a)) ca, count(distinct(b)) cb, count(distinct(d)) cd from t_distinct group by c) x where cd=7;
...
explain (costs off, nodes off) select avg(cab) from (select d, count(distinct(a+b)) cab, max(c+a) mca from t_distinct group by d);
select avg(cab::numeric(15,10)) from (select d, count(distinct(a+b)) cab, max(c+a) mca from t_distinct group by d);
explain (costs off, nodes off) select tt.a, x.cbd, x.ccb from t_distinct tt, (select a, count(distinct(b+d)) cbd, max(c+b) ccb from t_distinct group by a) x where x.cbd=tt.a;
...
explain (costs off, nodes off) select distinct count(distinct(a)) from t_distinct group by d;
select distinct count(distinct(a)) from t_distinct group by d order by 1;
explain (costs off, nodes off) select distinct count(distinct(b)) from t_distinct group by a;
select distinct count(distinct(b)) from t_distinct group by a order by 1;

-- Case 1.4 Can't push down
explain (costs off, nodes off) select count(distinct(a)), max(d) from t_distinct group by e; 
select count(distinct(a)), max(d) from t_distinct group by e; 
explain (costs off, nodes off) select count(distinct a order by a nulls last) from t_distinct;
select count(distinct a order by a nulls last) from t_distinct;

-- Case 1.5 not hashable
explain (costs off, nodes off) select count(distinct(e)) from t_distinct group by e limit 5;
select count(distinct(e)) from t_distinct group by e limit 5;

-- Case 2 (without group by clause)
-- Case 2.1 not hashable
explain (costs off, nodes off) select count(distinct(e)) from t_distinct;
select count(distinct(e)) from t_distinct;

-- Case 2.2 top level
-- Case 2.2.1 count_distinct within target list
explain (costs off, nodes off) select count(distinct(a)), max(b+c), avg(d::numeric(15,10)) from t_distinct;
select count(distinct(a)), max(b+c), avg(d::numeric(15,10)) from t_distinct;
explain (costs off, nodes off) select count(distinct(b)), max(a-c), avg(d::numeric(15,10)) from t_distinct;
select avg(distinct(b)), max(a-c), avg(d::numeric(15,10)) from t_distinct;
explain (costs off, nodes off) select count(distinct(b+c)), max(a-c), avg(d::numeric(15,10)) from t_distinct;
select sum(distinct(b+c)), max(a-c), avg(d::numeric(15,10)) from t_distinct;

-- Case 2.2.2 count_distinct within other place
explain (costs off, nodes off) select max(b) from t_distinct order by count(distinct(d));
select max(b) from t_distinct order by count(distinct(d));
explain (costs off, nodes off) select max(b) from t_distinct having count(distinct(d))=7;
select max(b) from t_distinct having count(distinct(d))=7;
explain (costs off, nodes off) select avg(a+b+c+d) from t_distinct having count(distinct(a))=max(a)+1;
select avg((a+b+c+d)::numeric(15,10)) from t_distinct having count(distinct(a))=max(a)+1;
explain (costs off, nodes off) select avg(c) from t_distinct order by count(distinct(a));
select avg(c::numeric(15,10)) from t_distinct order by count(distinct(a));
explain (costs off, nodes off) select avg(c) from t_distinct order by count(distinct(d));
select avg(c::numeric(15,10)) from t_distinct order by count(distinct(d));

-- test sublink in multi count(distinct)
explain (costs off, nodes off) select 1 from t_distinct t1 where (a,b) not in
 (select coalesce(t1.a+t2.b, t1.c+t2.d), coalesce(max(distinct(t2.c)), min(distinct(t2.d))) from t_distinct t2 group by 1);
explain (costs off, nodes off) select 1 from t_distinct t1 where (a,b,c) not in
