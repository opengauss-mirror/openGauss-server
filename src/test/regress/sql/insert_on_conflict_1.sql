...
(
  PARTITION parted_conflict_test_1 VALUES (1,2),
  PARTITION parted_conflict_test_2 VALUES (3),
  PARTITION parted_conflict_test_3 VALUES (4),
  PARTITION parted_conflict_test_4 VALUES (5,6)
  (
    SUBPARTITION parted_conflict_test_4_1 VALUES ( 5 ),
    SUBPARTITION parted_conflict_test_4_2 VALUES ( 6 )
  )
);
...
drop table parted_conflict_test cascade;

-- test behavior of inserting a conflicting tuple into an intermediate
-- partitioning level
CREATE TABLE parted_conflict (a int primary key, b text)
PARTITION BY RANGE (a) SUBPARTITION BY RANGE (a)
(
  PARTITION parted_conflict_1 VALUES less than (1000)
  (
      SUBPARTITION parted_conflict_1_1 VALUES less than (500),
      SUBPARTITION parted_conflict_1_2 VALUES less than (1000)
  )
);

insert into parted_conflict values (40, 'forty'),(600, 'seiscientos');
SELECT * FROM parted_conflict;
insert into parted_conflict partition (parted_conflict_1) values (40, 'cuarenta') on conflict (a) do update set b = excluded.b;
SELECT * FROM parted_conflict;
...
CREATE INDEX parted_conflict_1_idx ON parted_conflict_1(a) local;
...
(
  PARTITION parted_conflict_1 VALUES less than (500),
  PARTITION parted_conflict_2 VALUES less than (1000),
  PARTITION parted_conflict_3 VALUES less than (maxvalue)
);
CREATE UNIQUE INDEX parted_conflict_idx_b_b ON parted_conflict(a,b) local;

insert into parted_conflict values (50, 'cincuenta', 1), (501, 'cincuenta', 2), (1001, 'setecientos', 3);
...
RETURNS TRIGGER AS $$
BEGIN
    RAISE NOTICE 'insert updated: a = %, b = %, c = %', NEW.a, NEW.b, NEW.c;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
drop table parted_conflict cascade;
