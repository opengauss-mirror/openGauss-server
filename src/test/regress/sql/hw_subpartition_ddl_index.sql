...
--
DROP SCHEMA hw_subpartition_ddl_index CASCADE;
CREATE SCHEMA hw_subpartition_ddl_index;
SET CURRENT_SCHEMA TO hw_subpartition_ddl_index;
...
(
    product_id     INT4 CONSTRAINT cc1 CHECK (product_id < 2500) NOT NULL,
    customer_id    INT4,
    time_id        DATE CONSTRAINT cc2 CHECK (time_id IS NOT NULL) UNIQUE,
    channel_id     CHAR(1),
    type_id        INT4,
    quantity_sold  NUMERIC(3),
    amount_sold    NUMERIC(10,2),
    comment        VARCHAR2,
    UNIQUE (product_id, customer_id, comment)
)
...
INSERT INTO range_list_sales1 SELECT generate_series(1001,2000),
                                     generate_series(1,1000),
                                     date_pli('2008-01-01', generate_series(1001,2000)),
                                     generate_series(1,1000)%10,
                                     generate_series(1,1000)%10,
                                     generate_series(1,1000)%1000,
                                     generate_series(1,1000),
                                     'test' || generate_series(1001,2000);
