...
-- 非 A_FORMAT 兼容库验证：B/C/PG/D 库下溢出应走非 A 固定模板。
...
CREATE DATABASE to_char_fast_d DBCOMPATIBILITY 'D';
...
select case when
    ...
    to_char(99999999999999999.999::numeric, '999,999,999.99') = ' ###,###,###.##' and
    to_char(-99999999999999999.999::numeric, '999,999,999.99') = '-###,###,###.##'
then 'ok' else 'failed' end as to_char_fast_d_default;
...
select case when
    ...
    to_char(99999999999999999.999::numeric, '999,999,999.99') = ' ###,###,###.##' and
    to_char(-99999999999999999.999::numeric, '999,999,999.99') = '-###,###,###.##'
then 'ok' else 'failed' end as to_char_fast_d_options;
...
select case when
    ...
    to_char(999999999.995::numeric, '999,999,999.99') = ' ###,###,###.##' and
    to_char(-999999999.995::numeric, '999,999,999.99') = '-###,###,###.##'
then 'ok' else 'failed' end as to_char_fast_d_display_leading_zero;
...
select case when
    ...
    to_char(10000000000.00::numeric, '999,999,999.99') = ' ###,###,###.##' and
    to_char(-10000000000.00::numeric, '999,999,999.99') = '-###,###,###.##'
then 'ok' else 'failed' end as to_char_fast_d_hide_tailing_zero;
