...
create table t11(c1 int, c2 int, c3 int) with (storage_type = ustore);
create table t12(c1 int, c2 int, c3 int) with (storage_type = ustore);
create table t13(c1 int, c2 int, c3 int) with (storage_type = ustore);
create table t14(A4 int, B4 int, c4 int) with (storage_type = ustore);
create table t_a (id int, name varchar(10), code int) with (storage_type = ustore);
create table t_b (id int, name varchar(10), code int) with (storage_type = ustore);
...
-- ignored plus operator in or clause
explain (costs off) select * from t11, t12 where t11.c1(+) > 1 OR t11.c2 > 10;
explain (costs off) select * from t11, t12 where t11.c1(+) > 1 OR t11.c2(+) > 10;

explain (costs off) select * from t11, t12 where t11.c1(+) > 1 OR t12.c2(+) > 10;

explain (costs off) select * from t11, t12 where t11.c1 > 1 OR (t12.c2(+) > 10 AND t11.c1 = t12.c1);

--other join condition kind
select * from t11, t12 where t11.c1 < t12.c2(+) order by 1,2,3,4,5,6 limit 3;

-- other non-equal sign
explain (costs off) select * from t11, t12 where t11.c1 > t12.c2(+);
explain (costs off) select * from t11, t12 where t11.c1 <> t12.c2(+);
explain (costs off) select * from t11, t12 where t11.c1 <= t12.c2(+);
explain (costs off) select * from t11, t12 where t11.c1 >= t12.c2(+);
...
explain (verbose on, costs off) select * from t11 inner join t12 on t11.c1 = t12.c2(+);
select * from t11 inner join t12 on t11.c1 = t12.c2(+) order by 1,2,3,4,5,6;

explain (verbose on, costs off) select * from t11 left join t12 on t11.c1 = t12.c2(+);
select * from t11 left join t12 on t11.c1 = t12.c2(+) order by 1,2,3,4,5,6;

explain (verbose on, costs off) select * from t11 right join t12 on t11.c1(+) = t12.c2;
select * from t11 right join t12 on t11.c1(+) = t12.c2 order by 1,2,3,4,5,6;

explain (verbose on, costs off) select t11.c1, t12.c2, t13.c2 from t11 INNER JOIN t12 INNER JOIN t13 on t13.c2 = t12.c3(+) on t11.c3 = t13.c1;
select t11.c1, t12.c2, t13.c2 from t11 INNER JOIN t12 INNER JOIN t13 on t13.c2 = t12.c3(+) on t11.c3 = t13.c1 order by 1,2,3;
...
explain (verbose on, costs off) select * from t11 full join t12 on t11.c1 = t12.c2(+);
select * from t11 full join t12 on t11.c1 = t12.c2(+) order by 1,2,3,4,5,6;

explain (verbose on, costs off) select * from t11 full join t12 on t11.c1(+) = t12.c2;
select * from t11 full join t12 on t11.c1(+) = t12.c2 order by 1,2,3,4,5,6;

-- const
explain (verbose on, costs off) select * from t11 inner join t12 on t11.c1(+) = 1;
select * from t11 inner join t12 on t11.c1(+) = 1 order by 1,2,3,4,5,6;

explain (verbose on, costs off) select * from t11 inner join t12 on t11.c1(+) = 1 AND t11.c2 = t12.c2(+);
select * from t11 inner join t12 on t11.c1(+) = 1 AND t11.c2 = t12.c2(+) order by 1,2,3,4,5,6;

-- ereport
select * from t11 left join t12 on t11.c1(+) = t12.c2 order by 1,2,3,4,5,6;
select * from t11 right join t12 on t11.c1 = t12.c2(+) order by 1,2,3,4,5,6;
select * from t11 left join t12 on t11.c1(+) = 1;
select * from t11 right join t12 on t12.c1(+) = 1;

----with out Identifier------
create table t1(a1 int, b1 int, c1 int) with (storage_type = ustore);
create table t2(a2 int, b2 int, c2 int) with (storage_type = ustore);
...
create table t15 (like t11) with (storage_type = ustore);
...
create table join_nestloop_bugfix_t1(c1 int, c2 text) with (storage_type = ustore);
create table join_nestloop_bugfix_t2(c1 int, c2 text) with (storage_type = ustore);
...
--test (+) with itself
create table public.join_t1(col int, col2 int) with (storage_type = ustore);
create schema join_schema;
create table join_schema.join_t1(col int, col2 int) with (storage_type = ustore);
