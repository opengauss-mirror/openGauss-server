...
function show_env_detail
{
    ...
    log "openGauss" "User for Fastcheck: ${g_execute_user}"
    log "openGauss" "Port for Fastcheck: ${g_check_port}"
...
function mk_fastcheck
{
    ...
    #makefastcheck
    makefastcheck_start_time=$(date +%s.%N)
    log "openGauss" "makefastcheck_start_time="${makefastcheck_start_time}""
    log "openGauss" "Begin make fastcheck..."
    CUR_USER=`whoami`
    if [ "${CUR_USER}" == "root" ]; then
        ...
        if [ "${check_mode}" = "fastcheck_single" -o "${check_mode}" == "fastcheck_single_mot" ]; then
            ...
            log "openGauss" "Command: su - ${g_execute_user} -c \"source $g_conf_file && export && cd \"${g_src_dir}/src/test/regress\" && make ${check_mode} p=${g_check_port}\""
            result=`su - ${g_execute_user} -c "source $g_conf_file && export && cd \"${g_src_dir}/src/test/regress\" && make ${check_mode} p=${g_check_port}" >> ${g_log_file} 2>&1`
        elif [ "${check_mode}" = "fastcheck" -o "${check_mode}" = "fastcheck_old" ];then
            log "openGauss" "Begin do the ${check_mode} on part--${check_part}..."
            log "openGauss" "Command: su - ${g_execute_user} -c \"source $g_conf_file && export && cd \"${g_src_dir}/src/test/regress\" && make ${check_mode} p=${g_check_port} PART=${check_part}\""
            if [ "${check_part}" == "all" ]; then
                result=`su - ${g_execute_user} -c "source $g_conf_file && export && cd \"${g_src_dir}/src/test/regress\" && make ${check_mode} p=${g_check_port} PART=''" >> ${g_log_file} 2>&1`
            else
                result=`su - ${g_execute_user} -c "source $g_conf_file && export && cd \"${g_src_dir}/src/test/regress\" && make ${check_mode} p=${g_check_port} PART=${check_part}" >> ${g_log_file} 2>&1`
            fi
        fi
        if [ $? -ne 0 ]; then
            makefastcheck_stop_time=$(date +%s.%N)
            log "openGauss" "makefastcheck_stop_time="${makefastcheck_stop_time}""
            error "openGauss" "Make fastcheck failed."
            log "openGauss" "Make fastcheck detail:"
            sed -n '/make fastcheck .* PART/,$'p ${g_log_file}|sed 1d|sed '$d'|sed '$d'
            cat ${g_src_dir}/src/test/regress/log/initdb.log 2>&1 | tee -a ${g_log_file}
            cat ${g_src_dir}/src/test/regress/regression.diffs 2>&1 | tee -a ${g_log_file}
            cp ${g_src_dir}/src/test/regress/regression.diffs ${g_log_file%.log}.regression.diffs
            cp ${g_src_dir}/src/test/regress/log/postmaster_datanode1.log ${g_log_file%.log}.postmaster_datanode1.log
            flag_makefastcheck_fail=1
            return 1
        else
            makefastcheck_stop_time=$(date +%s.%N)
            log "openGauss" "makefastcheck_stop_time="${makefastcheck_stop_time}""
            flag_makefastcheck_fail=0
            log "openGauss" "Make fastcheck succeed."
        fi
    else
...
function gen_report
{
    if [ "${check_mode}" = "fastcheck" -o "${check_mode}" = "fastcheck_old" -o "${check_mode}" = "fastcheck_single" -o "${check_mode}" == "fastcheck_single_mot" ];then
        printf "\
==============================================================================================
| Work Dir           | ${g_boot_dir}
| llt part           | ${check_part}
...
"
        ...
        judge_time " make ${check_mode} " "${flag_makefastcheck_fail}" "$makefastcheck_start_time" "$makefastcheck_stop_time"
        interval_total_time="$(cal_interval_time $total_start_time $total_stop_time)"
        echo ----------------------------------------------------------------------------------------------
        echo "FASTCHECK TOTAL TIME: $interval_total_time"
        echo
    fi
}
function clean_env
{
    ...
    ps -ef|grep ${g_src_dir}/src/test/regress|grep -v grep|awk '{print $2}'|xargs kill -9 > /dev/null 2>&1

    rm -rf ${g_src_dir}/src/backend/catalog/postgres.bki
    rm -rf ${g_src_dir}/src/test/regress/tmp_check
    rm -rf ${g_src_dir}/src/test/regress/log
...
function main
{
    ...
    echo ========================================================================================== | tee -a ${g_log_file}
    if [ "${check_mode}" = "commitcheck" ]; then
        check_ci
    elif [ "${check_mode}" = "fastcheck" -o "${check_mode}" = "fastcheck_old" -o "${check_mode}" = "fastcheck_single" -o "${check_mode}" == "fastcheck_single_mot" ];then
        mk_fastcheck
        if [ $? -ne 0 ]; then
            error "openGauss" "Fail make fastcheck"
        fi
    fi
    echo | tee -a ${g_log_file}
    ...
    if [ "${flag_makeinstall_fail}" == 1 ] || [ "${flag_makefastcheck_fail}" == 1 ] || [ "${flag_makeclean_fail}" == 1 ];then
        error "openGauss" "Exit the make fastcheck whith error."
        exit 1
    fi
}
...
if [ "X${arch}" == "Xaarch64" ]; then
    g_conf_file=${real_path}/config_openeuler.ini
else
