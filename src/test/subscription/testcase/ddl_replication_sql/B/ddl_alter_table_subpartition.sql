...
-- 检查：分区/子分区数量、分区/子分区类型、分区与子分区关系、分区范围、relfilenode、Toast
...
-- 建表时指定PRIMARY KEY/UNIQUE索引（分区索引），索引键必须包含所有分区键，否则变为全局索引
...
-- DEFAULT[子]分区定义异常情况：
---- 不能只有一个DEFAULT分区（是从语法上限制）
---- 第一个[子]分区不能为DEFAULT
-- 注：但是自动生成的子分区可以只有一个DEFAULT子分区
create table t_subpart_error (id integer, name integer)
partition by hash(id)
subpartition by hash(name)
    subpartition template
...
----------------------------
-- 下面部分只做了语法的简单检查，避免coredump，具体功能还不支持
----------------------------
-- TODO SET 子分区模板
...
-- TODO HASH分区也有对应的拆分分区语法


-- TODO SPLIT INDEX PARTITION



----------------------------
-- TODO MERGE [SUB]PARTITIONS [FOR]
-- MERGE TO 只支持Range分区，合并相邻的所有分区
----------------------------
...
-- TODO List分区 MODIFY ADD/DROP VALUES (...)
...
-- 本地分区索引，索引只有一级分区，可以混用关键字PARTITION和SUBPARTITION，
-- 后面指定的是索引的分区名，而不是表的分区名（与oracle有差别）
...
where (p1.parttype = 'r' and p1.parentid not in (select oid from pg_class where relkind = 'r' and parttype != 'n')) -- 分区表不存在pg_class条目
    or (p1.parttype = 'r' and not exists (select oid from pg_partition where parttype='p' and parentid=p1.parentid)) -- 分区表不存在分区
    or (p1.parttype = 'p' and not exists (select oid from pg_partition where parttype='r' and parentid=p1.parentid)) -- 表分区不存在主表
    or (p1.parttype = 'p' and exists (select oid from pg_class where parttype='s' and oid=p1.parentid) and not exists (select oid from pg_partition where parttype='s' and parentid=p1.oid)) -- 二级分区表的分区不存在子分区
    or (p1.parttype = 's' and not exists (select oid from pg_partition where parttype='p' and oid=p1.parentid)) -- 二级分区不存在父分区
    or (p1.parttype = 'x' and p1.parentid not in (select oid from pg_class where relkind = 'i' and parttype != 'n')) -- 分区索引不存在主索引
    or (p1.indextblid != 0 and p1.indextblid not in (select oid from pg_partition where parttype != 'r')); -- 分区索引不存在主表
