...
test -f regression.diffs.hacheck.mot && rm regression.diffs.hacheck.mot
...
do
	...
	for line in `cat ha_schedule_multi_single_mot$2 | grep -v ^#`
	do
		printf "%-50s" $line
		...
		if [ $( grep "$failed_keyword" ./results/$line.log | grep -v "the database system is shutting down" | wc -l ) -eq 0 ]; then
			...
			printf "%-10s%-10s\n" ".... FAILED" $interval
			cp ./results/$line.log regression.diffs.hacheck.mot
			exit 1
		fi
	done
done
...
./run_python.sh $scripts_dir/pgxc_multi_single.py -o > ./results/stop_database_multi_single_mot.log 2>&1
