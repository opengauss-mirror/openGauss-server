...
class Pterodb():
	...
	def init_env(self):
		...
		for i in range(1, self.data_node_num + 2):
			if (i == 1):
				# primary
				datanode_cmd_init = install_path + "/bin/gs_initdb -D " + self.data_dir + "/" + \
									self.dname_prefix + \
				...
				# primary
				conf_file = self.data_dir + "/" + \
							self.dname_prefix + str(i) + "/postgresql.conf"
				...
				# standby
				datanode_cmd_standby = install_path + "/bin/gs_initdb -D " + self.data_dir + "/" + self.dname_prefix + \
									   str(i-1) + "_standby" + " --nodename=" + \
									   self.dname_prefix + str(1) + " -w " + g_passwd
				print(datanode_cmd_standby)
				os.system(datanode_cmd_standby)
				# standby
				conf_file_standby = self.data_dir + "/" + self.dname_prefix + \
									str(i-1) + "_standby" + "/postgresql.conf"
				self.__modify_conf_port(
					conf_file_standby, i+self.coordinator_num-1, i)
				self.__turn_on_pg_log(conf_file_standby)
				self.__modify_conf_standby(conf_file_standby, i, i)
				self.__modify_conf_application_name(
					conf_file_standby, "dn_s" + str(i - 1))
		...
		for i in range(1, self.data_node_num + 2):
			if (i != n):
				...
				string = "replconninfo%d = 'localhost=%s localport=%d localheartbeatport=%d localservice=%d remotehost=%s remoteport=%d remoteheartbeatport=%d remoteservice=%d'\n" % \
						 (j, g_local_ip, self.ha_port_arr[n-1], self.heartbeat_port_arr[n-1], self.service_port_arr[n-1],
						  g_local_ip, self.ha_port_arr[i-1], self.heartbeat_port_arr[i-1], self.service_port_arr[i-1])
				print(string)
				file_handler.write(string)
				j = j + 1
	...
	def __create_regress_group(self):
		...
		for i in range(0, self.coordinator_num):
			cmd = install_path + "/bin/gsql -p " + \
				  str(port + 3 * i) + " postgres < " + create_regress_group
			print(cmd)
			os.system(cmd)
	...
	def __create_node(self):
		...
		for i in range(1, self.coordinator_num+1):
			...
			Sql = Sql + "CREATE NODE coordinator" + \
				  str(i) + " WITH (HOST = 'localhost', type = 'coordinator', PORT = " + str(port) + ");\n"
		...
		for i in range(1, self.data_node_num+2):
			...
			if (i == 1):
				Sql = Sql + "CREATE NODE " + self.dname_prefix + str(1) + " WITH (type = 'datanode', RW = 'true', HOST = 'localhost', PORT = " + str(
					port + (i-1)*3) + ",sctp_port = " + str(g_base_port+(i+self.coordinator_num-1)*128) + ", control_port = " + str(port + (i-1)*3 + 2) + ");\n"
			else:
				Sql = Sql + "CREATE NODE " + self.dname_prefix + str(1) + " WITH (type = 'datanode', RW = 'false', HOST = 'localhost', PORT = " + str(
					port + (i-1)*3) + ",sctp_port = " + str(g_base_port+(i+self.coordinator_num-1)*128) + ", control_port = " + str(port + (i-1)*3 + 2) + ");\n"
		...
		for i in range(1, self.coordinator_num+1):
			port = g_base_port + 3 * (i - 1)
			cmd = install_path + "/bin/gsql -p " + \
				  str(port) + " postgres < " + create_nodes_sql
			print(cmd)
			os.system(cmd)
	...
	def __create_default_db(self):
		...
		cmd = install_path + "/bin/gsql -p " + \
			  str(g_base_port + 3) + " postgres -c 'create database test'"
		os.system(cmd)
