...
test -f regression.diffs.hacheck.cascade_mot && rm regression.diffs.hacheck.cascade_mot
...
sh deploy_multi_cascade_mot.sh > ./results/deploy_standby_multi_cascade_mot.log 2>&1

for((i=1;i<=$loop_num;i++))
do
	printf "run the ha_schedule %d time\n" $i
	printf "%-50s%-10s%-10s\n" "testcase" "result" "time(s)"
	for line in `cat ha_schedule_multi_cascade_mot$2 | grep -v ^#`
	do
		printf "%-50s" $line
		...
		if [ $( grep "$failed_keyword" ./results/$line.log | grep -v "the database system is shutting down" | wc -l ) -eq 0 ]; then
			...
			printf "%-10s%-10s\n" ".... FAILED" $interval
			cp ./results/$line.log regression.diffs.hacheck.cascade_mot
			exit 1
		fi
	done
done
...
./run_python.sh $scripts_dir/pgxc_multi_cascade_mot.py -o > ./results/stop_database_multi_cascade_mot.log 2>&1
