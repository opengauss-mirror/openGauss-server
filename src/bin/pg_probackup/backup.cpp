...
static int	standby_message_timeout_local = 10 * 1000;	/* 10 sec = default */
			 ...
			 int timeout_elevel, bool in_stream_dir)
{
	...
	uint32		try_count = 0,
				timeout;
	char		*wal_delivery_str = (char *)(in_stream_dir ? "streamed":"archived");
...
check_external_for_tablespaces(parray *external_list, PGconn *backup_conn)
{
	...
	char	   *query = (char *)"SELECT pg_catalog.pg_tablespace_location(oid) "
