...
static void _PrintTocData(ArchiveHandle* AH, TocEntry* te, RestoreOptions* ropt)
{
        ...
        while (blkType != EOF && id != te->dumpId) {
            switch (blkType) {
                ...
                default: /* Always have a default */
                    exit_horribly(modulename, "unrecognized data block type (%d) while searching archive\n", blkType);
                    break;
            }
            _readBlockHeader(AH, &blkType, &id);
        }
    ...
    switch (blkType) {
        ...
        case BLK_BLOBS:
            _LoadBlobs(AH, (ropt->dropSchema ? true : false));
            break;
    ...
    while (blkLen != 0) {
        ...
        if (cnt != blkLen) {
            if (feof(AH->FH)) {
                ...
                exit_horribly(modulename, "could not read from input file: %s\n", strerror(errno));
            }
        }
...
static void _CloseArchive(ArchiveHandle* AH)
{
    ...
    if (AH->mode == archModeWrite) {
         ...
         */
        if (ctx->hasSeek && tpos >= 0 && fseeko(AH->FH, tpos, SEEK_SET) == 0) {
            WriteToc(AH);
        }
    }
...
static void _Clone(ArchiveHandle* AH)
{
    ...
    errno_t rc = memcpy_s(AH->formatData, sizeof(lclContext), ctx, sizeof(lclContext));
