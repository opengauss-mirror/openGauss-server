...
static int utf_charcheck(const unsigned char* c)
{
    if ((*c & 0x80) == 0) {
        ...
        if (((c[1] & 0xc0) == 0x80) && (((c[0] & 0x0f) != 0x00) || ((c[1] & 0x20) == 0x20)) &&
            ...
            if (((z == 0x0f) &&
                    (((yx & 0xffe) == 0xffe) || (((yx & 0xf80) == 0xd80) && (lx >= 0x30) && (lx <= 0x4f)))) ||
        ...
        /* four-byte char */
        if (((c[1] & 0xc0) == 0x80) && (u > 0x00) && (u <= 0x10) && ((c[2] & 0xc0) == 0x80) &&
            ((c[3] & 0xc0) == 0x80)) {
 ...
 */
void pg_wcssize(
    const unsigned char* pwcs, size_t len, int encoding, int* result_width, int* result_height, int* result_format_size)
{
    int w;
    int chlen = 0;
    int linewidth = 0;
    ...
    /* Set results */
    if (result_width != NULL) {
        *result_width = width;
    }
    if (result_height != NULL) {
        *result_height = height;
    }
    if (result_format_size != NULL) {
 ...
 */
void pg_wcsformat(const unsigned char* pwcs, size_t len, int encoding, struct lineptr* lines, int count, int max_bytes)
{
    int w = 0;
    int chlen = 0;
    int linewidth = 0;
        ...
        if (chlen == 1) { /* single-byte char */
            if (*pwcs == '\n') { /* Newline */
                *ptr++ = '\0';
                ...
                rc = strcpy_s((char*)ptr, max_bytes - linewidth + 1, "\\r");
            ...
            if (encoding == PG_UTF8) {
                rc = sprintf_s((char*)ptr, max_bytes - linewidth + 1, "\\u%04X", utf8_to_unicode(pwcs));
                securec_check_ss_c(rc, "\0", "\0");
            } else {
                /*
                 ...
                 */
                rc = sprintf_s((char*)ptr, max_bytes - linewidth + 1, "\\u????");
                securec_check_ss_c(rc, "\0", "\0");
            }
            ptr += 6;
            linewidth += 6;
