...
const printTextFormat pg_asciiformat = {"ascii",
    {{"-", "+", "+", "+"}, {"-", "+", "+", "+"}, {"-", "+", "+", "+"}, {"", "|", "|", "|"}},
    "|",
    "|",
    "|",
...
const printTextFormat pg_asciiformat_old = {"old-ascii",
    {{"-", "+", "+", "+"}, {"-", "+", "+", "+"}, {"-", "+", "+", "+"}, {"", "|", "|", "|"}},
    ":",
    ";",
    " ",
...
static void* pg_local_calloc(int count, size_t size)
{
    void* tmp = NULL;
...
static int integer_digits(const char* my_str)
{
    ...
    frac_len = strchr(my_str, '.') != NULL ? strlen(strchr(my_str, '.')) : 0;
...
static int additional_numeric_locale_len(const char* my_str)
{
    ...
    if (int_len > 0) {
        ...
        len = (int_len / groupdigits - (int_len % groupdigits == 0)) * strlen(thousands_sep);
}
...
{
    if (my_str == NULL) {
        fprintf(stderr, _("my_str is NULL!\n"));
        exit(EXIT_FAILURE);
    }
    ...
    int groupdigits = atoi(grouping);
    int new_str_start = 0;
    char* new_str = (char*)pg_local_malloc(strlen_with_numeric_locale(my_str) + 1);
    int new_str_sz = strlen_with_numeric_locale(my_str) + 1;

    leading_digits = (int_len % groupdigits != 0) ? int_len % groupdigits : groupdigits;
    ...
    for (i = 0, j = new_str_start;; i++, j++) {
        /* Hit decimal point? */
        if (my_str[i] == '.') {
            check_strcpy_s(strcpy_s(&new_str[j], new_str_sz, decimal_point));
            j += strlen(decimal_point);
            /* add fractional part */
            check_strcpy_s(strcpy_s(&new_str[j], new_str_sz, &my_str[i] + 1));
            break;
        }
        ...
        if (i != 0 && (i - leading_digits) % groupdigits == 0) {
            check_strcpy_s(strcpy_s(&new_str[j], new_str_sz, thousands_sep));
...
static printTableFooter* footers_with_default(const printTableContent* cont)
{
    if (cont->footers == NULL && cont->opt->default_footer) {
        ...
        check_sprintf_s(sprintf_s(
            default_footer, sizeof(default_footer), ngettext("(%lu row)", "(%lu rows)", total_records), total_records));
...
static void print_unaligned_text(const printTableContent* cont, FILE* fout)
{
    ...
    if (cont->opt->start_table) {
        /* print title */
        if (!opt_tuples_only && (cont->title != NULL)) {
        ...
        /* print headers */
        if (!opt_tuples_only) {
            for (ptr = cont->headers; *ptr != NULL; ptr++) {
                if (ptr != cont->headers) {
    ...
    /* print cells */
    for (i = 0, ptr = cont->cells; *ptr != NULL; i++, ptr++) {
        if (need_recordsep) {
            print_separator(cont->opt->recordSep, fout);
...
/* draw "line" */
static void _print_horizontal_line(const unsigned int ncolumns, const unsigned int* widths, unsigned short border,
    printTextRule pos, const printTextFormat* format, FILE* fout)
{
...
static void print_aligned_text(const printTableContent* cont, FILE* fout)
{
    ...
    unsigned int *width_header = NULL, *max_width = NULL, *width_wrap = NULL, *width_average = NULL;
    unsigned int *max_nl_lines = NULL, /* value split by newlines */
        *curr_nl_line = NULL, *max_bytes = NULL;
    unsigned char** format_buf = NULL;
    unsigned int width_total = 0;
    unsigned int total_header_width = 0;
    unsigned int extra_row_output_lines = 0;
    unsigned int extra_output_lines = 0;

    const char* const* ptr = NULL;

    struct lineptr** col_lineptrs = NULL; /* pointers to line pointer per column */

    bool* header_done = NULL;       /* Have all header lines been output? */
    int* bytes_output = NULL;       /* Bytes output for column value */
    printTextLineWrap* wrap = NULL; /* Wrap status for each column */
    ...
    if (cont->ncolumns > 0) {
        ...
        width_header = (unsigned int*)pg_local_calloc(col_count, sizeof(*width_header));
        width_average = (unsigned int*)pg_local_calloc(col_count, sizeof(*width_average));
        max_width = (unsigned int*)pg_local_calloc(col_count, sizeof(*max_width));
        width_wrap = (unsigned int*)pg_local_calloc(col_count, sizeof(*width_wrap));
        max_nl_lines = (unsigned int*)pg_local_calloc(col_count, sizeof(*max_nl_lines));
        curr_nl_line = (unsigned int*)pg_local_calloc(col_count, sizeof(*curr_nl_line));
        col_lineptrs = (lineptr**)pg_local_calloc(col_count, sizeof(*col_lineptrs));
        max_bytes = (unsigned int*)pg_local_calloc(col_count, sizeof(*max_bytes));
        format_buf = (unsigned char**)pg_local_calloc(col_count, sizeof(*format_buf));
        header_done = (bool*)pg_local_calloc(col_count, sizeof(*header_done));
        bytes_output = (int*)pg_local_calloc(col_count, sizeof(*bytes_output));
        wrap = (printTextLineWrap*)pg_local_calloc(col_count, sizeof(*wrap));
    } else {
        width_header = NULL;
        width_average = NULL;
        max_width = NULL;
    ...
    for (i = 0; i < col_count; i++) {
        ...
        pg_wcssize((const unsigned char*)cont->headers[i],
            ...
            &nl_lines,
            &bytes_required);
        if ((unsigned int)(width) > max_width[i]) {
            max_width[i] = width;
        }
        if ((unsigned int)(nl_lines) > max_nl_lines[i]) {
            max_nl_lines[i] = nl_lines;
        }
        if ((unsigned int)(bytes_required) > max_bytes[i]) {
            max_bytes[i] = bytes_required;
        }
        if ((unsigned int)(nl_lines) > extra_row_output_lines) {
    ...
    for (i = 0, ptr = cont->cells; *ptr != NULL; ptr++, i++, cell_count++) {
        int width, nl_lines, bytes_required;

        pg_wcssize((const unsigned char*)*ptr, strlen(*ptr), encoding, &width, &nl_lines, &bytes_required);

        /* To avoid DIVIDE_BY_ZERO exception */
        if (0 != col_count) {
            if ((unsigned int)(width) > max_width[i % col_count]) {
                max_width[i % col_count] = width;
            }
            if ((unsigned int)(nl_lines) > max_nl_lines[i % col_count]) {
                max_nl_lines[i % col_count] = nl_lines;
            }
            if ((unsigned int)(bytes_required) > max_bytes[i % col_count]) {
    ...
    for (i = 0; i < col_count; i++) {
        ...
        col_lineptrs[i] = (lineptr*)pg_local_calloc(max_nl_lines[i] + 1, sizeof(**col_lineptrs));

        format_buf[i] = (unsigned char*)pg_local_malloc(max_bytes[i] + 1);
    ...
    if (cont->opt->format == PRINT_WRAPPED) {
         ...
         * headers
         */
        if (output_columns > 0 && (unsigned int)(output_columns) >= total_header_width) {
            /* While there is still excess width... */
            while (width_total > (unsigned int)(output_columns)) {
                double max_ratio = 0;
                int worst_col = -1;
    ...
    if (cont->opt->expanded == 2 && output_columns > 0 &&
        ((unsigned int)(output_columns) < total_header_width || (unsigned int)(output_columns) < width_total)) {
    ...
    if (!is_pager) {
        /* scan all cells, find maximum width, compute cell_count */
        for (i = 0, ptr = cont->cells; *ptr != NULL; ptr++, cell_count++) {
            int width, nl_lines, bytes_required;
    ...
    if (cont->opt->start_table) {
        ...
        if (!opt_tuples_only) {
            ...
            while (more_col_wrapping) {
                ...
                for (i = 0; i < (unsigned int)(cont->ncolumns); i++) {
    ...
    for (i = 0, ptr = cont->cells; *ptr != NULL; i += col_count, ptr += col_count) {
         ...
         * Format each cell.
         */
        for (j = 0; j < col_count && ptr[j] != NULL; j++) {
            pg_wcsformat(
                (const unsigned char*)ptr[j], strlen(ptr[j]), encoding, col_lineptrs[j], max_nl_lines[j], max_bytes[j]);
            curr_nl_line[j] = 0;
        }

        check_memset_s(memset_s(bytes_output, col_count * sizeof(int), 0, col_count * sizeof(int)));
        ...
        do {
            ...
            for (j = 0; j < col_count; j++) {
                ...
                if (this_line->ptr == NULL) {
                     ...
                     */
                    if ((unsigned int)(chars_to_output) > width_wrap[j])
...
static void print_aligned_vertical_line(const printTableContent* cont, unsigned long record, unsigned int hwidth,
    unsigned int dwidth, printTextRule pos, FILE* fout)
{
    const printTextFormat* format = get_line_style(cont->opt);
    const printTextLineFormat* lformat = &format->lrule[pos];
    unsigned short opt_border = cont->opt->border;
...
static void print_aligned_vertical(const printTableContent* cont, FILE* fout)
{
    ...
    struct lineptr* hlineptr = NULL;
    struct lineptr* dlineptr = NULL;
    bool is_pager = false;
    ...
    for (i = 0; i < (unsigned int)(cont->ncolumns); i++) {
        ...
        if ((unsigned int)(width) > hwidth)
            hwidth = width;
        if ((unsigned int)(height) > hheight)
            hheight = height;
        if ((unsigned int)(fs) > hformatsize)
            hformatsize = fs;
    }

    /* find longest data cell */
    for (i = 0, ptr = cont->cells; *ptr != NULL; ptr++, i++) {
        int width, height, fs;

        pg_wcssize((const unsigned char*)*ptr, strlen(*ptr), encoding, &width, &height, &fs);
        if ((unsigned int)(width) > dwidth)
            dwidth = width;
        if ((unsigned int)(height) > dheight)
            dheight = height;
        if ((unsigned int)(fs) > dformatsize)
            dformatsize = fs;
    }
     ...
     * structures
     */
    dlineptr = (lineptr*)pg_local_malloc((sizeof(*dlineptr)) * (dheight + 1));
    hlineptr = (lineptr*)pg_local_malloc((sizeof(*hlineptr)) * (hheight + 1));

    dlineptr->ptr = (unsigned char*)pg_local_malloc(dformatsize);
    hlineptr->ptr = (unsigned char*)pg_local_malloc(hformatsize);
    ...
    for (i = 0, ptr = cont->cells; *ptr != NULL; i++, ptr++) {
        ...
        if ((cont->ncolumns != 0) && (i % cont->ncolumns == 0)) {
            if (!opt_tuples_only)
        ...
        if (cont->ncolumns != 0)
            pg_wcsformat((const unsigned char*)cont->headers[i % cont->ncolumns],
                ...
                hformatsize);
        /* Format the data */
        pg_wcsformat((const unsigned char*)*ptr, strlen(*ptr), encoding, dlineptr, dheight, dformatsize);
        ...
        while (!dcomplete || !hcomplete) {
            ...
            if (!hcomplete) {
                fprintf(fout, "%-s%*s", hlineptr[line_count].ptr, hwidth - hlineptr[line_count].width, "");

                if (hlineptr[line_count + 1].ptr == NULL)
                    hcomplete = 1;
            } else
            ...
            if (!dcomplete) {
                ...
                if (dlineptr[line_count + 1].ptr == NULL)
                    dcomplete = 1;
            } else {
                if (opt_border < 2)
                    fputc('\n', fout);
...
static void print_latex_text(const printTableContent* cont, FILE* fout)
{
    ...
    if (cont->opt->start_table) {
        ...
        for (i = 0; i < (unsigned int)(cont->ncolumns); i++) {
            ...
            if (opt_border != 0 && i < (unsigned int)(cont->ncolumns - 1))
                fputs(" | ", fout);
        }
        if (opt_border == 2)
            fputs(" |", fout);
        ...
        /* print headers */
        if (!opt_tuples_only) {
            for (i = 0, ptr = cont->headers; i < (unsigned int)(cont->ncolumns); i++, ptr++) {
                if (i != 0)
                    fputs(" & ", fout);
                fputs("\\textit{", fout);
...
#endif
            pagerprog = GetEnvStr("PAGER");
            if (pagerprog != NULL) {
                check_env_value(pagerprog);
            }
#ifndef WIN32
            pqsignal(SIGPIPE, SIG_IGN);
#endif
            pagerpipe = popen((pagerprog != NULL) ? pagerprog : DEFAULT_PAGER, "w");
            if (pagerpipe != NULL) {
                if (pagerprog != NULL) {
                    free(pagerprog);
                    pagerprog = NULL;
...
{
    if (pagerpipe != NULL && pagerpipe != stdout) {
        /*
    ...
    printTableContent* const content, const printTableOpt* opt, const char* title, const int ncolumns, const int nrows)
{
    ...
    content->aligns = (char*)pg_local_calloc(ncolumns + 1, sizeof(*content->align));
...
void printTableAddCell(printTableContent* const content, char* cell, const bool translate, const bool mustfree)
{
    ...
    if (content->nrows > 0 && ((content->cellsadded / content->nrows) >= content->ncolumns)) {
        fprintf(stderr,
...
void printTableAddFooter(printTableContent* const content, const char* footer)
{
    ...
    f = (printTableFooter*)pg_local_calloc(1, sizeof(*f));
...
void printTableCleanup(printTableContent* const content)
{
    ...
    if (content->footers != NULL) {
        for (content->footer = content->footers; content->footer != NULL;) {
            printTableFooter* f = NULL;
...
void printTable(const printTableContent* cont, FILE* fout, FILE* flog)
{
    ...
    if (cont->opt->format != PRINT_ALIGNED && cont->opt->format != PRINT_WRAPPED)
...
void printQuery(const PGresult* result, const printQueryOpt* opt, FILE* fout, FILE* flog)
{
    ...
    for (i = 0; i < cont.ncolumns; i++) {
        ...
        printTableAddHeader(&cont, PQfname(result, i), opt->translate_header, align);
    }
    ...
    for (r = 0; r < cont.nrows; r++) {
        for (c = 0; c < cont.ncolumns; c++) {
            ...
            if (PQgetisnull(result, r, c))
                cell = (char*)(opt->nullPrint != NULL ? opt->nullPrint : "");
            else {
            ...
            translate = ((opt->translate_columns != NULL) && opt->translate_columns[c]);
    ...
    if (opt->footers != NULL) {
        ...
        for (footer = opt->footers; *footer != NULL; footer++)
