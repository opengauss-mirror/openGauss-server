    ...
    for (i = 0; i < sqlda->sqld; i++) {
        if (sqlda->sqlvar[i].sqlind && *(sqlda->sqlvar[i].sqlind) == -1)
            ...
            switch (sqlda->sqlvar[i].sqltype) {
                case SQLCHAR:
                    printf(
                        "name sqlda descriptor: '%s' value '%s'\n", sqlda->sqlvar[i].sqlname, sqlda->sqlvar[i].sqldata);
                    break;
                case SQLINT:
                    printf("name sqlda descriptor: '%s' value %d\n",
                        sqlda->sqlvar[i].sqlname,
                        *(int*)sqlda->sqlvar[i].sqldata);
                    break;
                case SQLFLOAT:
...
#line 79 "sqlda.pgc"
    ...
    strcpy(msg, "insert");
    {
        ECPGdo(__LINE__,
            ...
            0,
            ECPGst_normal,
            "insert into t1 values ( 1 , 'a' , 1.0 , 1 , 'a' ) , ( 2 , null , null , null , null ) , ( 4 , 'd' , 4.0 , "
            "4 , 'd' )",
            ECPGt_EOIT,
            ECPGt_EORT);
#line 85 "sqlda.pgc"
...
#line 98 "sqlda.pgc"
    ...
    strcpy(msg, "open");
    {
        ECPGdo(__LINE__,
            ...
            ECPGt_char_variable,
            (ECPGprepared_statement(NULL, "st_id1", __LINE__)),
            (long)1,
            (long)1,
            (1) * sizeof(char),
            ECPGt_NO_INDICATOR,
...
#line 135 "sqlda.pgc"
    ...
    strcpy(msg, "open");
    {
        ECPGdo(__LINE__,
            ...
            ECPGt_char_variable,
            (ECPGprepared_statement(NULL, "st_id2", __LINE__)),
            (long)1,
            (long)1,
            (1) * sizeof(char),
            ECPGt_NO_INDICATOR,
