        ...
        while ((start_pattern = strstr(outbuf, mapping[i].format)) != NULL) {
            ...
            switch (replace_type) {
                ...
                case PGTYPES_TYPE_UINT_2_LZ: {
                    ...
                    errno_t rc = memcpy_s(start_pattern, strlen(t), t, strlen(t));
...
int PGTYPESdate_defmt_asc(date* d, const char* fmt, char* str)
{
    ...
    if ((d == NULL) || (str == NULL) || (fmt == NULL)) {
        errno = PGTYPES_DATE_ERR_EARGS;
        return -1;
    }

    /* analyze the fmt string */
    fmt_ystart = (char*)strstr(fmt, "yy");
    fmt_mstart = (char*)strstr(fmt, "mm");
    fmt_dstart = (char*)strstr(fmt, "dd");

    if ((fmt_ystart == NULL) || (fmt_mstart == NULL) || (fmt_dstart == NULL)) {
        errno = PGTYPES_DATE_ERR_EARGS;
        return -1;
    }
    ...
    if (token_count != 3) {
        ...
        for (i = 0; list[i] != NULL; i++) {
            ...
            if ((start_pos = strstr(str_copy, month_lower_tmp)) != NULL) {
