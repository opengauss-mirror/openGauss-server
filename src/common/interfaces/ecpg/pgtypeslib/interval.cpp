 ...
 */
static void AdjustFractSeconds(double frac, struct /* pg_ */ tm* tm, fsec_t* fsec, int scale)
{
    int sec;
 ...
 * and changesd struct pg_tm to struct tm
 */
static void AdjustFractDays(double frac, struct /* pg_ */ tm* tm, fsec_t* fsec, int scale)
{
    int extra_days;
 ...
 * and changesd struct pg_tm to struct tm
 */
static inline void ClearPgTm(struct /* pg_ */ tm* tm, fsec_t* fsec)
{
    tm->tm_year = 0;
    tm->tm_mon = 0;
    tm->tm_mday = 0;
 ...
 */
static int DecodeISO8601Interval(char* str, int* dtype, struct /* pg_ */ tm* tm, fsec_t* fsec)
{
    bool datepart = true;
...
int EncodeInterval(struct /* pg_ */ tm* tm, fsec_t fsec, int style, char* str)
{
...
#ifdef HAVE_INT64_TIMESTAMP
    span->time = (((((((tm->tm_mday * INT64CONST(24)) + tm->tm_hour) * INT64CONST(60)) + tm->tm_min) * INT64CONST(60)) +
                      tm->tm_sec) *
                     USECS_PER_SEC) +
                 fsec;
#else
    span->time = (((((tm->tm_mday * (double)HOURS_PER_DAY) + tm->tm_hour) * (double)MINS_PER_HOUR) + tm->tm_min) *
