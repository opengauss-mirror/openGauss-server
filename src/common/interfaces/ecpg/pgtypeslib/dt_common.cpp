...
static THR_LOCAL datetkn* datacache[MAXDATEFIELDS] = {NULL};

static THR_LOCAL datetkn* deltacache[MAXDATEFIELDS] = {NULL};
...
char* pgtypes_date_months[] = {"January",
    "February",
    "March",
    "April",
    "May",
...
static datetkn* datebsearch(const char* key, datetkn* base, unsigned int nel)
{
    if (nel > 0) {
 ...
 */
static int DecodeSpecial(int field, const char* lowtoken, int* val)
{
    int type;
    datetkn* tp = NULL;
...
static int DecodeNumberField(
    int len, char* str, unsigned int fmask, unsigned int* tmask, struct tm* tm, fsec_t* fsec, int* is2digits)
{
    char* cp;
 ...
 */
static int DecodeNumber(int flen, char* str, unsigned int fmask, unsigned int* tmask, struct tm* tm, fsec_t* fsec,
    int* is2digits, bool EuroDates)
{
    int val;
    char* cp;
    ...
    *tmask = 0;
    ...
    else if ((EuroDates || (fmask & DTK_M(MONTH))) && !(fmask & DTK_M(YEAR)) && !(fmask & DTK_M(DAY)) && val >= 1 &&
             val <= 31) {
        *tmask = DTK_M(DAY);
        tm->tm_mday = val;
 ...
 */
static int DecodeDate(char* str, unsigned int fmask, unsigned int* tmask, struct tm* tm, bool EuroDates)
{
    fsec_t fsec;
...
int DecodeTime(const char* str, unsigned int* tmask, struct tm* tm, fsec_t* fsec)
{
    ...
    *tmask = DTK_TIME_M;
    ...
    /* do a sanity check */
#ifdef HAVE_INT64_TIMESTAMP
    if (tm->tm_hour < 0 || tm->tm_min < 0 || tm->tm_min > 59 || tm->tm_sec < 0 || tm->tm_sec > 59 ||
        *fsec >= USECS_PER_SEC)
        return -1;
#else
    ...
    for (i = 0; i < nf; i++) {
        switch (ftype[i]) {
                 ...
                 ***/
                    ...
                    if (isdigit((unsigned char)*field[i]) || ptype != 0) {
                        ...
                        *cp = '\0';
                         ...
                         * time
                         */
                        if ((ftype[i] = DecodeNumberField(
                                 strlen(field[i]), field[i], fmask, &tmask, tm, fsec, &is2digits)) < 0)
                            return -1;
            ...
            case DTK_TZ: {
                 ...
                 */
                if (i > 0 && (fmask & DTK_M(TZ)) != 0 && ftype[i - 1] == DTK_TZ &&
    ...
    while (*pfmt) {
        ...
        switch (*pfmt) {
            case 'a':
                ...
                err = 1;
                j = 0;
                while (pgtypes_date_weekdays_short[j] != NULL) {
            ...
            case 'B':
                ...
                err = 1;
                j = 0;
                while (pgtypes_date_months[j] != NULL) {
