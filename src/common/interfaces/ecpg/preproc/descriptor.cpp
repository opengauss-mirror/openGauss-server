...
{
    struct assignment* newm = (struct assignment*)mm_alloc(sizeof(struct assignment));

    newm->next = assignments;
    newm->variable = (char*)mm_alloc(strlen(var) + 1);
    strcpy(newm->variable, var);
    newm->value = value;
    assignments = newm;
}
...
void add_descriptor(char* name, char* connection)
{
    ...
    newm = (struct descriptor*)mm_alloc(sizeof(struct descriptor));

    newm->next = descriptors;
    newm->name = (char*)mm_alloc(strlen(name) + 1);
    strcpy(newm->name, name);
    if (connection) {
        newm->connection = (char*)mm_alloc(strlen(connection) + 1);
        strcpy(newm->connection, connection);
    } else
        newm->connection = connection;
    descriptors = newm;
}
...
void output_get_descr(char* desc_name, char* index)
{
    ...
    for (results = assignments; results != NULL; results = results->next) {
        ...
        ECPGdump_a_type(
            yyout, v->name, v->type, v->brace_level, NULL, NULL, -1, NULL, NULL, mm_strdup("0"), NULL, NULL);
    }
    drop_assignments();
    fputs("ECPGd_EODT);\n", yyout);
...
struct variable* descriptor_variable(const char* name, int input)
{
    ...
    static struct variable varspace[2] = {
        {descriptor_names[0], &descriptor_type, 0, NULL}, {descriptor_names[1], &descriptor_type, 0, NULL}};
