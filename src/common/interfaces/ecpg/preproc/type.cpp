...
{
    struct ECPGstruct_member* newm = NULL;
    ...
    while (rm) {
        ...
        switch (rm->type->type) {
            case ECPGt_struct:
            case ECPGt_union:
                type = ECPGmake_struct_type(
                    rm->type->u.members, rm->type->type, rm->type->type_name, rm->type->struct_sizeof);
                break;
            case ECPGt_array:
                ...
                if (rm->type->u.element->type == ECPGt_struct || rm->type->u.element->type == ECPGt_union)
                    type = ECPGmake_struct_type(rm->type->u.element->u.members,
                        rm->type->u.element->type,
                        rm->type->u.element->type_name,
                        rm->type->u.element->struct_sizeof);
                else
                    type = ECPGmake_array_type(
                        ECPGmake_simple_type(
                            rm->type->u.element->type, rm->type->u.element->size, rm->type->u.element->counter),
                        rm->type->size);
                break;
            default:
        ...
        ECPGmake_struct_member(rm->name, type, &newm);
...
struct ECPGtype* ECPGmake_struct_type(
    struct ECPGstruct_member* rm, enum ECPGttype type, char* type_name, char* struct_sizeof)
{
 ...
 */
static void ECPGdump_a_simple(FILE* o, const char* name, enum ECPGttype type, char* varcharsize, char* arrsiz,
    const char* siz, const char* prefix, int);
static void ECPGdump_a_struct(FILE* o, const char* name, const char* ind_name, char* arrsiz, struct ECPGtype* type,
    struct ECPGtype* ind_type, const char* prefix, const char* ind_prefix);
    ...
    const char* struct_sizeof, const char* ind_struct_sizeof)
{
    ...
    if (type->type != ECPGt_descriptor && type->type != ECPGt_sqlda && type->type != ECPGt_char_variable &&
        type->type != ECPGt_const && brace_level >= 0) {
        char* str = NULL;
    ...
    switch (type->type) {
        case ECPGt_array:
            ...
            switch (type->u.element->type) {
                case ECPGt_array:
                    mmerror(
                        PARSE_ERROR, ET_ERROR, "nested arrays are not supported (except strings)"); /* array of array */
                    break;
                case ECPGt_struct:
                case ECPGt_union:
                    ECPGdump_a_struct(o,
                ...
                default:
                    ...
                    if (ind_type != NULL) {
                        if (ind_type->type == ECPGt_NO_INDICATOR)
                            ECPGdump_a_simple(
                                o, ind_name, ind_type->type, ind_type->size, mm_strdup("-1"), NULL, ind_prefix, 0);
                        else {
                            ECPGdump_a_simple(o,
                                ind_name,
...
static void ECPGdump_a_simple(FILE* o, const char* name, enum ECPGttype type, char* varcharsize, char* arrsize,
    const char* siz, const char* prefix, int counter)
{
    if (type == ECPGt_NO_INDICATOR)
        ...
        switch (type) {
            ...
            case ECPGt_varchar:
                 ...
                 * bounds
                 */
                if (((atoi(arrsize) > 0) || (atoi(arrsize) == 0 && strcmp(arrsize, "0") != 0)) && siz == NULL)
            ...
            case ECPGt_string:
                 ...
                 */
                if ((atoi(varcharsize) > 1 || (atoi(arrsize) > 0) ||
