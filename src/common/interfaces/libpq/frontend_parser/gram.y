...
%type <node>	column_item opt_table_partitioning_clause
				...
				list_subpartitioning_clause subpartition_item opt_default_fmt_clause opt_default_nls_clause default_on_err_expr
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACCOUNT ACTION ADD_P ADMIN AFTER
	...
	CLUSTER COALESCE COLLATE COLLATION COLUMN COLUMN_ENCRYPTION_KEY COLUMN_ENCRYPTION_KEYS COLUMN_NAME COLUMNS COMMENT COMMENTS COMMIT CONVERT_P
	CONNECT COMMITTED COMPACT COMPATIBLE_ILLEGAL_CHARS COMPILE COMPLETE COMPLETION COMPRESS CONDITION CONCURRENTLY CONFIGURATION CONFLICT CONNECTBY CONNECTION CONSISTENT CONSTANT CONSTRAINT 
	CONSTRAINT_CATALOG CONSTRAINT_NAME CONSTRAINT_SCHEMA CONSTRAINTS CONTENT_P CONTINUE_P CONTVIEW CONVERSION_P COORDINATOR COORDINATORS COPY COST CREATE
...
/* PGXC_END */
	...
	END_P ENDS ENFORCED ENUM_P ERROR_P ERRORS ESCAPE EOL ESCAPING EVENT EVENTS EVERY EXCEPT EXCHANGE
	...
	IDENTIFIED IDENTITY_P IF_P IGNORE IGNORE_EXTRA_DATA ILIKE IMMEDIATE IMMUTABLE IMPLICIT_P IN_P IMCSTORED
	INCLUDE INCLUDING INCREMENT INCREMENTAL INDEX INDEXES INFILE INFINITE_P INHERIT INHERITS INITIAL_P INITIALLY INITRANS INLINE_P
...
/* PGXC_END */
	...
	XACT_ABORT XML_P XMLATTRIBUTES XMLCONCAT XMLELEMENT XMLEXISTS XMLFOREST XMLNAMESPACES XMLPARSE
...
opt_type_modifiers: '(' expr_list ')'				{ $$ = $2; }
                    ...
                    | '(' Iconst CHAR_P ')'         { $$ = list_make1(makeIntConst($2, @2)); }
                    | '(' Iconst CHARACTER ')'      { $$ = list_make1(makeIntConst($2, @2)); }
					| /* EMPTY */					{ $$ = NIL; }
		;
			...
			| CURRENT_DATE
				{
					...
					$$ = makeTypeCast(n, SystemTypeName("date"), NULL, NULL, NULL, @1);
				}
			| CURRENT_TIME
				{
					/*
