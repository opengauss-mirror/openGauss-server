 ...
 *
 * src/common/interfaces/libpq/pqexpbuffer.cpp
 *
 * -------------------------------------------------------------------------
 */
...
void* libpq_realloc(void* src, size_t old_len, size_t new_len)
{
    void* dst = malloc(new_len);
    if (dst == NULL)
        return NULL;
    ...
    if (src != NULL) {
        ...
        errno_t ss_rc = memcpy_s(dst, new_len, src, old_len);
...
int enlargePQExpBuffer(PQExpBuffer str, size_t needed)
{
    ...
    newdata = (char*)libpq_realloc(str->data, str->maxlen, newlen);
    if (newdata != NULL) {
        str->data = newdata;
        str->maxlen = newlen;
        return 1;
...
void appendPQExpBuffer(PQExpBuffer str, const char* fmt, ...)
{
    ...
    for (;;) {
        ...
        if (str->maxlen > str->len + 16) {
            avail = str->maxlen - str->len - 1;
            va_start(args, fmt);
            // it will Double the buffer size, so the length is more than SECUREC_STRING_MAX_LEN
            nprinted = vsnprintf(str->data + str->len, avail, fmt, args);
            va_end(args);
