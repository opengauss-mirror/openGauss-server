...
const bool CacheLoader::fill_cached_types(PGconn *conn)
{
    ...
    if (res->resultStatus != PGRES_TUPLES_OK) {
        PQclear(res);
        printfPQExpBuffer(&conn->errorMessage,
            libpq_gettext("fill_cached_types - query to initialize did not return data\n"));
        return false;
    }
    int typid_num = PQfnumber(res, "oid");
    int db_num = PQfnumber(res, "current_database");
    int schema_num = PQfnumber(res, "nspname");
    int typname_num = PQfnumber(res, "typname");
    m_cached_types_list.clear();

    for (int n = 0; n < PQntuples(res); n++) {
        CachedType *curr_type =
            new CachedType(ntohl(*(Oid *)PQgetvalue(res, n, typid_num)), PQgetvalue(res, n, typname_num),
            PQgetvalue(res, n, schema_num), PQgetvalue(res, n, db_num), conn->client_logic->m_cached_column_manager);
...
bool CacheLoader::fill_cached_procs(PGconn *conn)
{
    ...
    if (res->resultStatus != PGRES_TUPLES_OK) {
        PQclear(res);
        printfPQExpBuffer(&conn->errorMessage,
            libpq_gettext("fill_cached_procs - query to initialize did not return data\n"));
        return false;
    }

    // getting the index of each column in the response
    int func_oid_num = PQfnumber(res, "func_id");
    int origin_ret_type_num = PQfnumber(res, "prorettype_orig");
    int cached_col_num = PQfnumber(res, "proargcachedcol");
    int alltypes_orig_num = PQfnumber(res, "proallargtypes_orig");
    int proname_num = PQfnumber(res, "proname");
    int pronargs_num = PQfnumber(res, "pronargs");
    int proargtypes_num = PQfnumber(res, "proargtypes");
    int proallargtypes_num = PQfnumber(res, "proallargtypes");
    int proargnames_num = PQfnumber(res, "proargnames");
    int dbname_num = PQfnumber(res, "dbname");
    int schema_num = PQfnumber(res, "nspname");

    m_proc_list.clear();
    for (int n = 0; n < PQntuples(res); n++) {
        CachedProc *curr_proc = new CachedProc();
        curr_proc->m_func_id = ntohl(*(Oid *)PQgetvalue(res, n, func_oid_num));
        curr_proc->m_proname = strdup(PQgetvalue(res, n, proname_num));
        curr_proc->m_pronargs = ntohs(*(short *)PQgetvalue(res, n, pronargs_num));
        curr_proc->m_prorettype_orig = ntohl(*(Oid *)PQgetvalue(res, n, origin_ret_type_num));
        curr_proc->m_dbname = strdup(PQgetvalue(res, n, dbname_num));
        curr_proc->m_schema_name = strdup(PQgetvalue(res, n, schema_num));
        if (PQgetlength(res, n, cached_col_num))
            parseBinaryArray(PQgetvalue(res, n, cached_col_num), (void **)&curr_proc->m_proargcachedcol);
        if (PQgetlength(res, n, proargtypes_num))
            parseBinaryArray(PQgetvalue(res, n, proargtypes_num), (void **)&curr_proc->m_proargtypes);
        if (PQgetlength(res, n, alltypes_orig_num))
            curr_proc->m_nallargtypes_orig =
                parseBinaryArray(PQgetvalue(res, n, alltypes_orig_num), (void **)&curr_proc->m_proallargtypes_orig);
        if (PQgetlength(res, n, proallargtypes_num))
            parseBinaryArray(PQgetvalue(res, n, proallargtypes_num), (void **)&curr_proc->m_proallargtypes);
        if (PQgetlength(res, n, proargnames_num))
            curr_proc->m_nallargtypes =
                parseBinaryArray(PQgetvalue(res, n, proargnames_num), (void **)&curr_proc->m_proargnames);
