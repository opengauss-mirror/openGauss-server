 ...
 *
 * IDENTIFICATION
 *	  src/common/interfaces/libpq/fe-misc.cpp
 *
 * -------------------------------------------------------------------------
 */
...
THR_LOCAL bool g_pq_interrupt_happened = false;

#ifndef WIN32
const char* libpqVersionString = DEF_GS_VERSION;
#else
const char* libpqVersionString = "";
#endif
    ...
    switch (bytes) {
        case 2:
            if (conn->inCursor + 2 > conn->inEnd)
                return EOF;
            check_memcpy_s(memcpy_s(&tmp2, sizeof(uint16), conn->inBuffer + conn->inCursor, 2));
            conn->inCursor += 2;
        ...
        case 4:
            if (conn->inCursor + 4 > conn->inEnd)
                return EOF;
            check_memcpy_s(memcpy_s(&tmp4, sizeof(uint32), conn->inBuffer + conn->inCursor, 4));
            conn->inCursor += 4;
...
/*
 * pqGetInt64
 *    read 8 byte integer and convert from network byte order
 *    to local byte order
 */
int64 pqGetInt64(int64 *result, PGconn *conn)
{
    uint32        h32;
    uint32        l32;
    ...
    if (conn->inCursor + 4 > conn->inEnd)
        return EOF;
    check_memcpy_s(memcpy_s(&h32, sizeof(uint32), conn->inBuffer + conn->inCursor, 4));
    conn->inCursor += 4;
    h32 = ntohl(h32);

    if (conn->inCursor + 4 > conn->inEnd)
        return EOF;
    check_memcpy_s(memcpy_s(&l32, sizeof(uint32), conn->inBuffer + conn->inCursor, 4));
    conn->inCursor += 4;
    l32 = ntohl(l32);
...
int pqCheckOutBufferSpace(size_t bytes_needed, PGconn* conn)
{
    ...
    if (newsize > 0 && bytes_needed <= (size_t)newsize) {
        newbuf = (char*)libpq_realloc(conn->outBuffer, conn->outBufSize, newsize);
        if (newbuf != NULL) {
            /* malloc succeeded */
...
int pqCheckInBufferSpace(size_t bytes_needed, PGconn* conn)
{
    ...
    if (newsize > 0 && bytes_needed <= (size_t)newsize) {
        ...
        if (newbuf != NULL) {
            ...
            if (conn->inBuffer != NULL) {
                ...
                memcpy(newbuf, conn->inBuffer, conn->inBufSize);
                free(conn->inBuffer);
            }
    ...
    if (newsize > 0 && bytes_needed <= (size_t)newsize) {
        ...
        if (newbuf != NULL) {
            /* malloc succeeded */
            if (conn->inBuffer != NULL) {
                check_memcpy_s(memcpy_s(newbuf, newsize, conn->inBuffer, conn->inBufSize));
                free(conn->inBuffer);
            }
            conn->inBuffer = newbuf;
            conn->inBufSize = newsize;
    ...
    switch (pqReadReady(conn)) {
        ...
        default:
            printfPQExpBuffer(&conn->errorMessage,
                libpq_gettext("libpq read failed, remote datanode %s, err: %s"),
...
static int pqSendSome(PGconn* conn, int len)
{
    ...
    if (remaining > 0)
        check_memmove_s(memmove_s(conn->outBuffer, remaining, ptr, remaining));
...
int pqWait(int forRead, int forWrite, PGconn* conn)
{
    ...
    if (timeout <= 0) {
        ret = pqWaitTimed(forRead, forWrite, conn, finish_time);
    } else {
        /* check for interrupt every 5s, use ceil algorithm */
        retry_cnt = ((timeout - 1) / connect_timeout) + 1;
        while (retry_cnt) {
            finish_time = time(NULL) + connect_timeout;
            ret = pqWaitTimed(forRead, forWrite, conn, finish_time);
            if (g_pq_interrupt_happened) {
                appendPQExpBuffer(&conn->errorMessage, libpq_gettext("connecting interrupted by pool validation.\n"));
                break;
            }
            retry_cnt--;
        }
...
static int pqSocketCheck(PGconn* conn, int forRead, int forWrite, time_t end_time)
{
    ...
    if (conn == NULL)
        return -1;
    if (conn->sock < 0) {
        printfPQExpBuffer(&conn->errorMessage, libpq_gettext("socket not open\n"));
        return -1;
    }
