 ...
 *
 * IDENTIFICATION
 *	  src/common/interfaces/libpq/fe-protocol2.cpp
 *
 * -------------------------------------------------------------------------
 */
        ...
        switch (conn->setenv_state) {
            ...
            case SETENV_STATE_CLIENT_ENCODING_SEND: {
                ...
                if (val != NULL) {
                    if (pg_strcasecmp(val, "default") == 0) {
                        check_sprintf_s(sprintf_s(setQuery, sizeof(setQuery), "SET client_encoding = DEFAULT"));
                    } else {
                        check_sprintf_s(sprintf_s(setQuery, sizeof(setQuery), "SET client_encoding = '%.60s'", val));
                    }
#ifdef CONNECTDEBUG
            ...
            case SETENV_STATE_OPTION_SEND: {
                ...
                if (conn->next_eo->envName != NULL) {
                    ...
                    if (check_client_env(val) != NULL) {
                        if (pg_strcasecmp(val, "default") == 0)
                            check_sprintf_s(
                                sprintf_s(setQuery, sizeof(setQuery), "SET %s = DEFAULT", conn->next_eo->pgName));
                        else
                            check_sprintf_s(
                                sprintf_s(setQuery, sizeof(setQuery), "SET %s = '%.60s'", conn->next_eo->pgName, val));
#ifdef CONNECTDEBUG
            ...
            case SETENV_STATE_QUERY1_SEND: {
                 ...
                 */
                if (!PQsendQuery(conn, "start transaction; select version(); end"))
                    goto error_return;
            ...
            case SETENV_STATE_QUERY2_WAIT: {
                ...
                if (res != NULL) {
                    ...
                    if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) == 1) {
                        ...
                        val = PQgetvalue(res, 0, 0);
                        if ((val != NULL) && *val) /* null should not happen, but */
                         ...
                         * fallback.
                         */
                        val = gs_getenv_r("PGCLIENTENCODING");
                        if ((check_client_env(val) != NULL) && *val)
                            pqSaveParameterStatus(conn, "client_encoding", val);
                        else
...
static int getRowDescriptions(PGconn* conn)
{
    ...
    for (i = 0; i < nfields; i++) {
        ...
        if (pqGets(&conn->workBuffer, conn) || pqGetInt(&typid, 4, conn) || pqGetInt(&typlen, 2, conn) ||
...
static int getAnotherTuple(PGconn* conn, bool binary)
{
    ...
    if (nfields > conn->rowBufLen) {
        rowbuf = (PGdataValue*)libpq_realloc(rowbuf, conn->rowBufLen, nfields * sizeof(PGdataValue));
        if (rowbuf == NULL) {
            errmsg = NULL; /* means "out of memory", see below */
            goto advance_and_error;
        }
...
static int getNotify(PGconn* conn)
{
    ...
    if (newNotify != NULL) {
        ...
        check_strcpy_s(strcpy_s(newNotify->relname, nmlen + 1, conn->workBuffer.data));
...
int pqGetCopyData2(PGconn* conn, char** buffer, int async)
{
    ...
    for (;;) {
        ...
        *buffer = (char*)malloc(msgLength + 1);
        ...
        check_memcpy_s(memcpy_s(*buffer, msgLength + 1, &conn->inBuffer[conn->inStart], msgLength));
...
int pqGetline2(PGconn* conn, char* s, int maxlen)
{
    ...
    if (conn->sock < 0 || conn->asyncStatus != PGASYNC_COPY_OUT) {
        *s = '\0';
        return EOF;
    }
...
int pqEndcopy2(PGconn* conn)
{
    ...
    if (conn->asyncStatus != PGASYNC_COPY_IN && conn->asyncStatus != PGASYNC_COPY_OUT) {
        printfPQExpBuffer(&conn->errorMessage,
            libpq_gettext("no COPY in progress, remote datanode %s, errno: %s\n"),
...
{
    StartupPacket* startpacket = NULL;
    ...
    *packetlen = sizeof(StartupPacket);
    ...
    check_memset_s(memset_s(startpacket, sizeof(StartupPacket), 0, sizeof(StartupPacket)));

    startpacket->protoVersion = htonl(conn->pversion);

    check_strcpy_s(strcpy_s(startpacket->user, SM_USER, conn->pguser));
    check_strcpy_s(strcpy_s(startpacket->database, SM_DATABASE, conn->dbName));
    check_strcpy_s(strcpy_s(startpacket->tty, SM_TTY, conn->pgtty));

    if (conn->pgoptions != NULL) {
        /* make sure the length of options is not out of startpacket's options size */
        if (strlen(conn->pgoptions) >= SM_OPTIONS) {
            if (conn->errorMessage.data == NULL)
                printfPQExpBuffer(&conn->errorMessage, libpq_gettext("fill startpacket options failed\n"));
            else
                appendPQExpBuffer(&conn->errorMessage, libpq_gettext("fill startpacket options failed\n"));

            libpq_free(startpacket);
            return NULL;
        }

        check_strcpy_s(strcpy_s(startpacket->options, SM_OPTIONS, conn->pgoptions));
    }
