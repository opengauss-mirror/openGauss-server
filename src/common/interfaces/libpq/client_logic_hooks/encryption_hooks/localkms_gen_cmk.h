...
extern KmsErrType get_real_key_path_kms(const char *cmk_path, RealCmkPath *real_cmk_path);
...
extern KmsErrType read_content_from_file(char *real_path, unsigned char *buf, const size_t buf_len, size_t *content_len);
extern bool rm_one_file(const char *real_cmk_path);
extern bool rm_cmk_store_file(RealCmkPath real_cmk_path);
...
extern KmsErrType read_iv_and_salt(char *real_cmk_path, unsigned char *iv, const size_t iv_len, unsigned char *salt,
    const size_t salt_len);
extern KmsErrType read_cmk_plain_kms(char *real_cmk_path, unsigned char *cmk_plain, size_t *plain_len);

/* encrypt cek with cmk plain */
extern KmsErrType encrypt_cek_use_ras2048(const unsigned char *cek_plain, const size_t plain_len, 
    char *real_pub_cmk_path, const size_t path_len, unsigned char *cek_cipher, size_t &cipher_len);
extern KmsErrType decrypt_cek_use_rsa2048(const unsigned char *cek_cipher, const size_t cipher_len,
    char *real_priv_cmk_path, const size_t path_len, unsigned char *cek_plain, size_t *cek_plain_len);
