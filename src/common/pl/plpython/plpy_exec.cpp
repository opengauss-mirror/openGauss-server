...
Datum PLy_exec_function(FunctionCallInfo fcinfo, PLyProcedure* proc)
{
    ...
    PG_TRY();
    {
        ...
        if (proc->result.out.d.typoid == VOIDOID) {
            ...
            Assert((proc->result.out.d.typoid == RECORDOID && proc->result.out.d.typmod != -1) ||
                   (proc->result.out.d.typoid != RECORDOID && proc->result.out.d.typmod == -1));

            desc = lookup_rowtype_tupdesc(proc->result.out.d.typoid, proc->result.out.d.typmod);
 ...
 * BEFORE the event and is ROW level.  openGauss expects the function
