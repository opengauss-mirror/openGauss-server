...
static void compute_tsvector_stats(
    ...
    VacAttrStats* stats, AnalyzeAttrFetchFunc fetchfunc, int samplerows, double totalrows, Relation rel)
{
     ...
     */
    ss_rc = memset_s(&hash_ctl, sizeof(hash_ctl), 0, sizeof(hash_ctl));
    ...
    lexemes_tab = hash_create(
        "Analyzed lexemes table", num_mcelem, &hash_ctl, HASH_ELEM | HASH_FUNCTION | HASH_COMPARE | HASH_CONTEXT);
    ...
    for (vector_no = 0; vector_no < samplerows; vector_no++) {
        ...
        bool isnull = false;
        TSVector vector;
        WordEntry* curentryptr = NULL;
        char* lexemesptr = NULL;
        int j;

        vacuum_delay_point();

        value = fetchfunc(stats, vector_no, &isnull, rel);
        ...
        for (j = 0; j < vector->size; j++) {
            ...
            if (found) {
                ...
                item->key.lexeme = (char*)palloc(hash_key.length);
                errno_t rc = memcpy_s(item->key.lexeme, hash_key.length, hash_key.lexeme, hash_key.length);
                securec_check(rc, "", "");
            }
