...
/*
 * update tabOid1's slices information using tabOid2,
 * used in cluster redis tools.
 */
void UpdateSliceForRedisTable(Oid tabOid1, Oid tabOid2)
{
    int i;
    HeapTuple tup, newtup;
    Relation relation;
    ...
    slicelist1 = SearchSysCacheList2(PGXCSLICERELID, ObjectIdGetDatum(tabOid1), CharGetDatum(PGXC_SLICE_TYPE_SLICE));
    slicelist2 = SearchSysCacheList2(PGXCSLICERELID, ObjectIdGetDatum(tabOid2), CharGetDatum(PGXC_SLICE_TYPE_SLICE));
    ...
    for (i = 0; i < slicelist1->n_members; i++) {
        tup = &slicelist1->members[i]->tuple;
        simple_heap_delete(relation, &tup->t_self);
    }
    ReleaseSysCacheList(slicelist1);
    
    /* copy tabOid2 slice tuples to tabOid1 */
    for (i = 0; i < slicelist2->n_members; i++) {
        tup = &slicelist2->members[i]->tuple;
        bool isnull = false;
        Datum val = fastgetattr(tup, Anum_pgxc_slice_type, RelationGetDescr(relation), &isnull);
        if (DatumGetChar(val) == PGXC_SLICE_TYPE_TABLE) {
            continue;
        ...
        values[Anum_pgxc_slice_relid - 1] = tabOid1;
        replaces[Anum_pgxc_slice_relid - 1] = true;
        newtup = heap_modify_tuple(tup, RelationGetDescr(relation), values, nulls, replaces);
        simple_heap_insert(relation, newtup);
        CatalogUpdateIndexes(relation, newtup);
