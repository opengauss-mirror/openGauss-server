...
static Node* remove_dummy_true_conditions(Node* node);
...
/*
 * - remove_dummy_true_conditions()
 *
 * - brief: Function to recursively walk through the expression tree and remove
 *   dummy true conditions like "1=1". These conditions are generated during the
 *   outer join conversion process as placeholders for join conditions that have
 *   been moved to the ON clause. The function preserves user-defined conditions
 *   while removing system-generated placeholders.
 *
 *   The function handles nested AND expressions and ensures the expression tree
 *   remains valid after removal. It's used to clean up the WHERE clause after
 *   successful Oracle outer join conversion.
 *
 * - param[in] node: The root node of the expression tree to process.
 *
 * - return: Processed expression tree with dummy true conditions removed.
 *           Returns NULL if the entire expression becomes empty after removal.
 */
static Node* remove_dummy_true_conditions(Node* node)
{
    if (node == NULL) {
    ...
    if (IsA(node, A_Expr)) {
        ...
        /*
         * Handle AND expressions recursively - we need to process both left
         * and right sides to remove dummy conditions from nested structures
         */
        if (expr->kind == AEXPR_AND) {
            Node* left = remove_dummy_true_conditions(expr->lexpr);
            Node* right = remove_dummy_true_conditions(expr->rexpr);

            /*
             * After processing both sides, handle different combinations:
             * - If both sides become NULL, the entire AND expression is removed
             * - If one side becomes NULL, return the other side
             * - If both sides remain, rebuild the AND expression
             */
            if (left == NULL && right == NULL) {
                ...
                A_Expr* new_expr = makeA_Expr(AEXPR_AND, NULL, left, right, expr->location);
                return (Node*)new_expr;
            }
        }
        /*
         * Handle operator expressions - check if this is a "1=1" dummy condition
         * generated by the system during outer join conversion
         */
        else if (expr->kind == AEXPR_OP) {
            /*
             * Check if this is a constant equality expression (like "1=1")
             * Only remove system-generated placeholders, not user-defined conditions
             */
            if (IsA(expr->lexpr, Const) && IsA(expr->rexpr, Const)) {
                Const* left_const = (Const*)expr->lexpr;
                Const* right_const = (Const*)expr->rexpr;

                /*
                 * Verify it's an integer equality comparison with value 1 on both sides
                 * This pattern matches the dummy conditions generated by
                 * preprocess_plus_outerjoin_walker()
                 */
                if (left_const->consttype == INT4OID && right_const->consttype == INT4OID &&
                    DatumGetInt32(left_const->constvalue) == 1 && DatumGetInt32(right_const->constvalue) == 1) {
                    return NULL; /* Remove this dummy true condition */
                }
            }
        }
    }
...
void transformOperatorPlus(ParseState* pstate, Node** whereClause)
{
    ...
    if (ctx.contain_plus_outerjoin) {
        ...
        if (*whereClause != NULL) {
            *whereClause = remove_dummy_true_conditions(*whereClause);
        }
    } else {
        /*
