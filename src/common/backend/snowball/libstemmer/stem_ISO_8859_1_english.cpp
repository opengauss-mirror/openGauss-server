...
{
    {
        int m1 = z->l - z->c;
        (void)m1; /* or, line 51 */
        if (out_grouping_b(z, g_v_WXY, 89, 121, 0) ||
            in_grouping_b(z, g_v, 97, 121, 0) ||
            out_grouping_b(z, g_v, 97, 121, 0)) {
            goto lab1;
        }
        goto lab0;
    lab1:
        z->c = z->l - m1;
        if (out_grouping_b(z, g_v, 97, 121, 0) ||
            in_grouping_b(z, g_v, 97, 121, 0) ||
            z->c > z->lb) {
...
static int r_Step_2(struct SN_env* z)
{
    ...
    switch (among_var) {
        ...
        case 16:
            if (in_grouping_b(z, g_valid_LI, 99, 116, 0)) {
                return 0;
            }
            {
