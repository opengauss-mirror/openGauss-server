...
void PostgresInitializer::InitSession()
{
    ...
    if (RecoveryInProgress() && g_instance.attr.attr_storage.real_recovery_parallelism > 1 &&
        g_instance.attr.attr_storage.EnableHotStandby && u_sess->proc_cxt.clientIsGsql) {
    ...
    StartXact();
    if (EnableLocalSysCache() && IS_THREAD_POOL_WORKER) {
