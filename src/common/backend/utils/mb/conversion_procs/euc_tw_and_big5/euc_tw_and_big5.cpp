...
extern "C" Datum euc_tw_to_big5(PG_FUNCTION_ARGS);
extern "C" Datum big5_to_euc_tw(PG_FUNCTION_ARGS);
extern "C" Datum euc_tw_to_mic(PG_FUNCTION_ARGS);
extern "C" Datum mic_to_euc_tw(PG_FUNCTION_ARGS);
extern "C" Datum big5_to_mic(PG_FUNCTION_ARGS);
extern "C" Datum mic_to_big5(PG_FUNCTION_ARGS);
...
static void mic2euc_tw(const unsigned char* mic, unsigned char* p, int len)
{
    ...
    while (len > 0) {
        ...
        if (c1 == LC_CNS11643_1) {
            ...
            *p++ = mic[1];
            *p++ = mic[2];
        } else if (c1 == 0x9d && mic[1] >= LC_CNS11643_3 && mic[1] <= LC_CNS11643_7) { /* LCPRV2? */
            *p++ = SS2;
