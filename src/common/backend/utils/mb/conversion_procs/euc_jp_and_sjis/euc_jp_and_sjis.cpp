...
extern "C" Datum euc_jp_to_sjis(PG_FUNCTION_ARGS);
extern "C" Datum sjis_to_euc_jp(PG_FUNCTION_ARGS);
extern "C" Datum euc_jp_to_mic(PG_FUNCTION_ARGS);
extern "C" Datum mic_to_euc_jp(PG_FUNCTION_ARGS);
extern "C" Datum sjis_to_mic(PG_FUNCTION_ARGS);
extern "C" Datum mic_to_sjis(PG_FUNCTION_ARGS);
    ...
    while (len > 0) {
        ...
        if (c1 == LC_JISX0201K)
            ...
            *p++ = c2 - ((c1 & 1) ? ((c2 < 0xe0) ? 0x61 : 0x60) : 2);
            ...
            report_untranslatable_char(PG_MULE_INTERNAL, PG_SJIS, (const char*)mic, len);
        }
        mic += l;
        len -= l;
    }
    ...
    while (len > 0) {
        ...
        if (c1 == LC_JISX0201K) {
            ...
            *p++ = mic[2];
            ...
            report_untranslatable_char(PG_MULE_INTERNAL, PG_EUC_JP, (const char*)mic, len);
        }
        mic += l;
        len -= l;
    }
 ...
 * EUC_JP -> SJIS
 */
static void euc_jp2sjis(const unsigned char* euc, unsigned char* p, int len)
{
    int c1, c2, k;
    int l;
 ...
 * SJIS ---> EUC_JP
 */
static void sjis2euc_jp(const unsigned char* sjis, unsigned char* p, int len)
{
    int c1, c2, i, k, k2;
    int l;
