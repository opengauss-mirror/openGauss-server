...
/*
 * Get global memory request details
 */
static AllocChunk DumpSharedMemoryAllocInfo(const char* ctxName, int *memctxInfoLen)
{
    ...
    FreeStringInfoHuge(&memInfo);
    return memctxInfoRes;
}

/*
 * Get session memory request details
 */
...
/*
 * Traverse all threads, get thread memory request details
 */
...
static void SortMemoryContextInfo(MemoryDumpData* memctxInfoRes, int64 memctxInfoCnt, int64* resLen)
{
    ...
    *resLen = i + 1;
    ...
    if (*resLen > TOP_MEMORY_CONTEXT_NUM) {
    ...
    const char* logdir, const char* filename_pattern)
{
    ...
    filename = (char*)palloc0(MAXPGPATH);
...
static int MemoryUsageCompare(const void* p1, const void* p2)
{
    ...
    if (m1->usedSize > m2->usedSize) {
        return -1;
    } else if (m1->usedSize == m2->usedSize) {
        return 0;
    } else {
        return 1;
    }
...
/*
 * clear connection by session id
 */
void TerminateConnectionBySessid(uint64 sessid)
{
    if (ENABLE_THREAD_POOL) {
        ThreadPoolSessControl *sess_ctrl = g_threadPoolControler->GetSessionCtrl();
        int ctrlIdx = sess_ctrl->FindCtrlIdxBySessId(sessid);
        sess_ctrl->SendSignal(ctrlIdx, SIGTERM);
    } else {
        kill_backend(sessid, false);
    }
}
 ...
 */
static void CleanConnectionByMemoryOverload()
{
    int sessNum = 0;
    ...
    for (int i = 0; i < sessNum; i++) {
        ...
        TerminateConnectionBySessid(sessUsage[i].sessid);
...
static void CleanConnectionByMemory()
{
    ...
    PG_TRY();
    {
        ...
        CleanConnectionByMemoryOverload();
        while (true) {
            if (processMemInChunks < maxChunksPerProcess *
 ...
 * Thread pool full clear all connections
 */
static void CleanConnectionByThreadpoolOverload()
{
    int sessNum = 0;
    SessMemoryUsage* sessUsage = NULL;
    if (ENABLE_THREAD_POOL) {
...
static void CleanConnectionByThreadpool()
{
    ...
    {
        g_instance.comm_cxt.rejectRequest = true;
        CleanConnectionByThreadpoolOverload();
        while (true) {
            int currentConn = g_instance.conn_cxt.CurConnCount;
            int threadpoolNum = g_threadPoolControler->GetThreadActualNum();
            if (threadpoolNum > 0) {
