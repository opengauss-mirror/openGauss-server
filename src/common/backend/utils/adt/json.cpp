...
static void report_parse_error(const JsonParseStack* stack, JsonLexContext* lex);
static void report_invalid_token(JsonLexContext* lex);
static int report_json_context(const JsonLexContext* lex);
...
Datum json_recv(PG_FUNCTION_ARGS)
{
     ...
     */
    result = (text*)palloc(nbytes + 1 + VARHDRSZ);
    SET_VARSIZE(result, nbytes + VARHDRSZ);
    errno_t ss_rc = memcpy_s(VARDATA(result), nbytes + 1, str, nbytes);
...
{
    JsonLexContext lex;
    JsonParseStack *stack = NULL;
    JsonParseStack *stacktop = NULL;
    uint stacksize;
        ...
        switch (stack->state) {
            ...
            case JSON_PARSE_ARRAY_START:
                if (lex.token_type != JSON_VALUE_INVALID) {
                    ...
                    op = JSON_STACKOP_POP;
                } else if (lex.token_start[0] == '[' || lex.token_start[0] == '{') {
                    stack->state = JSON_PARSE_ARRAY_NEXT;
            ...
            case JSON_PARSE_OBJECT_START:
                ...
                } else if (lex.token_type == JSON_VALUE_INVALID && lex.token_start[0] == '}') {
                    op = JSON_STACKOP_POP;
                } else {
                    report_parse_error(stack, &lex);
                }
            ...
            case JSON_PARSE_OBJECT_LABEL:
                if (lex.token_type == JSON_VALUE_INVALID && lex.token_start[0] == ':') {
            ...
            default:
                ereport(ERROR,
                    (errcode(ERRCODE_UNRECOGNIZED_NODE_TYPE),
                        errmsg("unexpected json parse state: %d", (int)stack->state)));
        }
        ...
        switch (op) {
            ...
            case JSON_STACKOP_PUSH_WITH_PUSHBACK:
                ...
                if (stack >= &stacktop[stacksize]) {
                    ...
                    stacksize += 32;
                    stacktop = (JsonParseStack*)repalloc(stacktop, sizeof(JsonParseStack) * stacksize);
...
static void report_parse_error(const JsonParseStack* stack, JsonLexContext* lex)
{
    ...
    errno_t ss_rc = memcpy_s(token, toklen + 1, lex->token_start, toklen);
    securec_check(ss_rc, "\0", "\0");
    token[toklen] = '\0';
...
static int report_json_context(const JsonLexContext* lex)
{
    ...
    errno_t ss_rc = memcpy_s(ctxt, ctxtlen + 1, context_start, ctxtlen);
    securec_check(ss_rc, "\0", "\0");
    ctxt[ctxtlen] = '\0';
...
static char* extract_mb_char(const char* s)
{
    ...
    errno_t ss_rc = memcpy_s(res, len + 1, s, len);
    securec_check(ss_rc, "\0", "\0");
    res[len] = '\0';
    ...
    TYPCATEGORY tcategory, Oid typoutputfunc, bool use_line_feeds)
{
    ...
    for (i = 1; i <= dims[dim]; i++) {
        ...
        if (dim + 1 == ndims) {
            datum_to_json(vals[*valcount], nulls[*valcount], result, tcategory, typoutputfunc);
            (*valcount)++;
        } else {
            /*
...
static void array_to_json_internal(Datum array, StringInfo result, bool use_line_feeds)
{
    ...
    get_type_io_data(
        element_type, IOFunc_output, &typlen, &typbyval, &typalign, &typdelim, &typioparam, &typoutputfunc);
