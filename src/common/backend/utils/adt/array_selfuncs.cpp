...
static Selectivity mcelem_array_selec(ArrayType* array, TypeCacheEntry* typentry, Datum* mcelem, int nmcelem,
    float4* numbers, int nnumbers, float4* hist, int nhist, Oid operatorType, FmgrInfo* cmpfunc);
static Selectivity mcelem_array_contain_overlap_selec(Datum* mcelem, int nmcelem, float4* numbers, int nnumbers,
    Datum* array_data, int nitems, Oid operatorType, FmgrInfo* cmpfunc);
 ...
 */
Selectivity scalararraysel_containment(
    PlannerInfo* root, Node* leftop, Node* rightop, Oid elemtype, bool isEquality, bool useOr, int varRelid)
{
    ...
    if (isvalidstats) {
        ...
        if (get_attstatsslot(vardata.statsTuple,
            ...
            if (useOr)
                selec = mcelem_array_contain_overlap_selec(
                    values, nvalues, numbers, nnumbers, &constval, 1, OID_ARRAY_CONTAINS_OP, cmpfunc);
            else
            ...
            if (useOr)
                selec =
                    mcelem_array_contain_overlap_selec(NULL, 0, NULL, 0, &constval, 1, OID_ARRAY_CONTAINS_OP, cmpfunc);
            else
                selec = mcelem_array_contained_selec(
                    NULL, 0, NULL, 0, &constval, 1, NULL, 0, OID_ARRAY_CONTAINED_OP, cmpfunc);
        }
        ...
        if (useOr)
            selec = mcelem_array_contain_overlap_selec(NULL, 0, NULL, 0, &constval, 1, OID_ARRAY_CONTAINS_OP, cmpfunc);
        else
            selec =
                mcelem_array_contained_selec(NULL, 0, NULL, 0, &constval, 1, NULL, 0, OID_ARRAY_CONTAINED_OP, cmpfunc);
...
Datum arraycontsel(PG_FUNCTION_ARGS)
{
    ...
    Oid operatorType = PG_GETARG_OID(1);
    ...
    rc = memset_s(&vardata, sizeof(VariableStatData), 0, sizeof(VariableStatData));
    ...
    if (!get_restriction_variable(root, args, varRelid, &vardata, &other, &varonleft))
        PG_RETURN_FLOAT8(DEFAULT_SEL(operatorType));

    /*
     * Can't do anything useful if other is NULL
     */
    if (other == NULL) {
        ReleaseVariableStats(vardata);
        PG_RETURN_FLOAT8(DEFAULT_SEL(operatorType));
     ...
     */
    if (!varonleft) {
        if (operatorType == OID_ARRAY_CONTAINS_OP)
            operatorType = OID_ARRAY_CONTAINED_OP;
        else if (operatorType == OID_ARRAY_CONTAINED_OP)
            operatorType = OID_ARRAY_CONTAINS_OP;
    }
    ...
    if (element_typeid != InvalidOid && element_typeid == get_base_element_type(vardata.vartype)) {
        selec = calc_arraycontsel(&vardata, ((Const*)other)->constvalue, element_typeid, operatorType);
    } else {
        selec = DEFAULT_SEL(operatorType);
    }
...
static Selectivity calc_arraycontsel(VariableStatData* vardata, Datum constval, Oid elemtype, Oid operatorType)
{
    ...
    typentry = lookup_type_cache(elemtype, TYPECACHE_CMP_PROC_FINFO);
    if (!OidIsValid(typentry->cmp_proc_finfo.fn_oid))
        return DEFAULT_SEL(operatorType);
    cmpfunc = &typentry->cmp_proc_finfo;
    ...
    if (HeapTupleIsValid(vardata->statsTuple) &&
        ...
        if (get_attstatsslot(vardata->statsTuple,
            ...
            /* No most-common-elements info, so do without */
            selec = mcelem_array_selec(array, typentry, NULL, 0, NULL, 0, NULL, 0, operatorType, cmpfunc);
        }
    ...
    float4* numbers, int nnumbers, float4* hist, int nhist, Oid operatorType, FmgrInfo* cmpfunc)
{
     ...
     */
    if (null_present && operatorType == OID_ARRAY_CONTAINS_OP) {
    ...
    if (operatorType == OID_ARRAY_CONTAINS_OP || operatorType == OID_ARRAY_OVERLAP_OP)
        selec = mcelem_array_contain_overlap_selec(
            mcelem, nmcelem, numbers, nnumbers, elem_values, nonnull_nitems, operatorType, cmpfunc);
    else if (operatorType == OID_ARRAY_CONTAINED_OP)
        selec = mcelem_array_contained_selec(
            mcelem, nmcelem, numbers, nnumbers, elem_values, nonnull_nitems, hist, nhist, operatorType, cmpfunc);
    else {
        ereport(ERROR,
            (errcode(ERRCODE_ARRAY_ELEMENT_ERROR),
                errmodule(MOD_OPT),
                errmsg("arraycontsel called for unrecognized operator %u", operatorType)));
 ...
 *
 * Notice : this estimate probably could be improved by using the distinct
    ...
    Datum* array_data, int nitems, Oid operatorType, FmgrInfo* cmpfunc)
{
    ...
    for (i = 0; i < nitems; i++) {
        ...
        if (i > 0 && element_compare(&array_data[i - 1], &array_data[i], cmpfunc) == 0)
            continue;
...
#define EFFORT 100
    ...
    pfree_ext(dist);
    pfree_ext(mcelem_dist);
    pfree_ext(hist_part);
    pfree_ext(elem_selec);
