...
static void range_gist_class_split(
    TypeCacheEntry* typcache, GistEntryVector* entryvec, GIST_SPLITVEC* v, SplitLR* classes_groups);
static void range_gist_single_sorting_split(
    TypeCacheEntry* typcache, GistEntryVector* entryvec, GIST_SPLITVEC* v, bool use_upper_bound);
static void range_gist_double_sorting_split(TypeCacheEntry* typcache, GistEntryVector* entryvec, GIST_SPLITVEC* v);
static void range_gist_consider_split(ConsiderSplitContext* context, RangeBound* right_lower, int min_left_count,
...
Datum range_gist_union(PG_FUNCTION_ARGS)
{
    ...
    for (i = 1; i < entryvec->n; i++) {
        result_range = range_super_union(typcache, result_range, DatumGetRangeType(ent[i].key));
    }
...
Datum range_gist_penalty(PG_FUNCTION_ARGS)
{
    ...
    RangeType* newm = DatumGetRangeType(newentry->key);
    TypeCacheEntry* typcache = NULL;
    bool has_subtype_diff = false;
    RangeBound orig_lower, new_lower, orig_upper, new_upper;
    bool orig_empty = false;
    bool new_empty = false;

    if (RangeTypeGetOid(orig) != RangeTypeGetOid(newm))
    ...
    range_deserialize(typcache, newm, &new_lower, &new_upper, &new_empty);
...
Datum range_gist_picksplit(PG_FUNCTION_ARGS)
{
    ...
    if (non_empty_classes_count == 1) {
        ...
        if (count_in_classes[CLS_NORMAL] > 0) {
            ...
            nonInfCount =
                count_in_classes[CLS_NORMAL] + count_in_classes[CLS_CONTAIN_EMPTY] + count_in_classes[CLS_EMPTY];
            infCount = total_count - nonInfCount;

            nonEmptyCount = count_in_classes[CLS_NORMAL] + count_in_classes[CLS_LOWER_INF] +
                            count_in_classes[CLS_UPPER_INF] + count_in_classes[CLS_LOWER_INF | CLS_UPPER_INF];
    ...
    switch (strategy) {
        case RANGESTRAT_BEFORE:
            if (RangeIsEmpty(key) || RangeIsEmpty(DatumGetRangeType(query)))
                return false;
            return (!range_overright_internal(typcache, key, DatumGetRangeType(query)));
        case RANGESTRAT_OVERLEFT:
            if (RangeIsEmpty(key) || RangeIsEmpty(DatumGetRangeType(query)))
                return false;
            return (!range_after_internal(typcache, key, DatumGetRangeType(query)));
        case RANGESTRAT_OVERLAPS:
            return range_overlaps_internal(typcache, key, DatumGetRangeType(query));
        case RANGESTRAT_OVERRIGHT:
            if (RangeIsEmpty(key) || RangeIsEmpty(DatumGetRangeType(query)))
                return false;
            return (!range_before_internal(typcache, key, DatumGetRangeType(query)));
        case RANGESTRAT_AFTER:
            if (RangeIsEmpty(key) || RangeIsEmpty(DatumGetRangeType(query)))
                return false;
            return (!range_overleft_internal(typcache, key, DatumGetRangeType(query)));
        case RANGESTRAT_ADJACENT:
            if (RangeIsEmpty(key) || RangeIsEmpty(DatumGetRangeType(query)))
                return false;
            if (range_adjacent_internal(typcache, key, DatumGetRangeType(query)))
                return true;
            return range_overlaps_internal(typcache, key, DatumGetRangeType(query));
        case RANGESTRAT_CONTAINS:
            return range_contains_internal(typcache, key, DatumGetRangeType(query));
        ...
        default:
            ereport(
                ERROR, (errcode(ERRCODE_UNRECOGNIZED_NODE_TYPE), errmsg("unrecognized range strategy: %d", strategy)));
...
static bool range_gist_consistent_leaf(TypeCacheEntry* typcache, StrategyNumber strategy, RangeType* key, Datum query)
{
    switch (strategy) {
        ...
        case RANGESTRAT_AFTER:
            return range_after_internal(typcache, key, DatumGetRangeType(query));
        case RANGESTRAT_ADJACENT:
            return range_adjacent_internal(typcache, key, DatumGetRangeType(query));
        case RANGESTRAT_CONTAINS:
            return range_contains_internal(typcache, key, DatumGetRangeType(query));
        case RANGESTRAT_CONTAINED_BY:
            return range_contained_by_internal(typcache, key, DatumGetRangeType(query));
        case RANGESTRAT_CONTAINS_ELEM:
            return range_contains_elem_internal(typcache, key, query);
        case RANGESTRAT_EQ:
    ...
    TypeCacheEntry* typcache, GistEntryVector* entryvec, GIST_SPLITVEC* v, SplitLR* classes_groups)
{
    ...
    for (i = FirstOffsetNumber; i <= maxoff; i = OffsetNumberNext(i)) {
        ...
        if (classes_groups[classtype] == SPLIT_LEFT)
            ...
            Assert(classes_groups[classtype] == SPLIT_RIGHT);
            PLACE_RIGHT(range, i);
        }
    }
...
static void range_gist_single_sorting_split(
    TypeCacheEntry* typcache, GistEntryVector* entryvec, GIST_SPLITVEC* v, bool use_upper_bound)
{
    SingleBoundSortItem* sortItems = NULL;
    RangeType* left_range = NULL;
    RangeType* right_range = NULL;
    OffsetNumber i, maxoff, split_idx;
...
static void range_gist_double_sorting_split(TypeCacheEntry* typcache, GistEntryVector* entryvec, GIST_SPLITVEC* v)
{
    ...
    NonEmptyRange* by_lower = NULL;
    NonEmptyRange* by_upper = NULL;
    CommonEntry* common_entries = NULL;
    int nentries, i1, i2;
    RangeBound* right_lower = NULL;
    RangeBound* left_upper = NULL;

    errno_t rc = memset_s(&context, sizeof(ConsiderSplitContext), 0, sizeof(ConsiderSplitContext));
    ...
    for (i = FirstOffsetNumber; i <= maxoff; i = OffsetNumberNext(i)) {
        ...
        range_deserialize(
            typcache, range, &by_lower[i - FirstOffsetNumber].lower, &by_lower[i - FirstOffsetNumber].upper, &empty);
        Assert(!empty);
    }
     ...
     */
    rc = memcpy_s(by_upper, nentries * sizeof(NonEmptyRange), by_lower, nentries * sizeof(NonEmptyRange));
    securec_check(rc, "\0", "\0");
