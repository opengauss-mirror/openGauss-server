...
static float calc_rank_and(const float* w, TSVector t, TSQuery q)
{
    ...
    WordEntryPosVector* POSNULL_PTR = (WordEntryPosVector*)palloc(sizeof(WordEntryPosVector) + sizeof(WordEntryPos));
    POSNULL_PTR->npos = 1;
    POSNULL_PTR->pos[0] = 0;
    ...
    WEP_SETPOS(POSNULL_PTR->pos[0], MAXENTRYPOS - 1);
            ...
            for (k = 0; k < i; k++) {
                ...
                for (l = 0; l < dimt; l++) {
                    for (p = 0; p < lenct; p++) {
                        ...
                        if (dist || (dist == 0 && (pos[i] == POSNULL_PTR || pos[k] == POSNULL_PTR))) {
                            float curw;
    ...
    for (i = 0; i < size; i++) {
        ...
        while (entry - firstentry < nitem) {
            if (entry->haspos) {
                ...
                dimt = POSNULL_PTR->npos;
                post = POSNULL_PTR->pos;
            }
...
static bool checkcondition_QueryOperand(const void* checkval, QueryOperand* val)
{
...
static DocRepresentation* get_docrep(TSVector txt, QueryRepresentation* qr, int* doclen)
{
    ...
    /* A flatten dummy WordEntryPos array to use when haspos is false */
    uint16 tmp_npos = 1;
    const int pos_null_length = 1;
    WordEntryPos tmp_pos[pos_null_length];
    tmp_pos[0] = 0;
            ...
            for (j = 0; j < dimt; j++) {
                if (j == 0) {
                    ...
                    for (k = 0; k < qr->query->size; k++) {
                        ...
                        if (k == i || (item[k].type == QI_VAL && compareQueryOperand(&kptr, &iptr, operand) == 0)) {
                            /*
