    ...
    if (BufFileSeekBlock(lts->pfile, blocknum) != 0 || BufFileWrite(lts->pfile, buffer, BLCKSZ) != BLCKSZ)
        ereport(ERROR,
            ...
            (errcode_for_file_access(),
                errmsg("could not write block %ld of temporary file: %m", blocknum),
...
static long ltsRewindIndirectBlock(LogicalTapeSet* lts, IndirectBlock* indirect, bool freezing)
{
    ...
    if (indirect->nextup != NULL) {
        ...
        if (indirblock == -1L) {
            ereport(ERROR, (errcode(ERRCODE_FETCH_DATA_FAILED), errmsg("Failed to obtain address of the block!")));
        }
...
static long ltsRecallNextBlockNum(LogicalTapeSet* lts, IndirectBlock* indirect, bool frozen)
{
    ...
    if (indirect->nextSlot >= BLOCKS_PER_INDIR_BLOCK || indirect->ptrs[indirect->nextSlot] == -1L) {
        long indirblock;
...
{
    LogicalTape* lt = NULL;
    IndirectBlock* ib = NULL;
    IndirectBlock* nextib = NULL;
    int i;
...
void LogicalTapeWrite(LogicalTapeSet* lts, int tapenum, void* ptr, size_t size)
{
    ...
    while (size > 0) {
        ...
        nthistime = BLCKSZ - (size_t)lt->pos;
        if (nthistime > size)
            nthistime = size;
        Assert(nthistime > 0);
...
void LogicalTapeRewind(LogicalTapeSet* lts, int tapenum, bool forWrite)
{
    ...
    if (!forWrite) {
        ...
        if (datablocknum != -1L) {
            ...
            lt->nbytes = (lt->curBlockNumber < lt->numFullBlocks) ? BLCKSZ : lt->lastBlockBytes;
        }
    } else {
        /*
...
size_t LogicalTapeRead(LogicalTapeSet* lts, int tapenum, void* ptr, size_t size)
{
    ...
    while (size > 0) {
        if (lt->pos >= lt->nbytes) {
            ...
            long datablocknum = ltsRecallNextBlockNum(lts, lt->indirect, lt->frozen);
...
bool LogicalTapeSeek(LogicalTapeSet* lts, int tapenum, long blocknum, int offset)
{
     ...
     * Not-so-easy case.  Figure out whether it's possible at all.
     */
    if (blocknum < 0 || blocknum > lt->numFullBlocks || (blocknum == lt->numFullBlocks && offset > lt->lastBlockBytes))
        return false;
