...
static void ltsConcatWorkerTapes(LogicalTapeSet *lts, const TapeShare *shared, SharedFileSet *fileset);
...
static void ltsWriteBlock(LogicalTapeSet* lts, long blocknum, void* buffer)
{
    ...
    while (blocknum > lts->nBlocksWritten) {
        ...
        int rc = memset_s(zerobuf, sizeof(zerobuf), 0, sizeof(zerobuf));
        securec_check(rc, "", "");
...
static void ltsReadBlock(LogicalTapeSet* lts, long blocknum, void* buffer)
{
    if (BufFileSeekBlock(lts->pfile, blocknum) != 0 || BufFileRead(lts->pfile, buffer, BLCKSZ) != BLCKSZ)
        ereport(ERROR,
            /* XXX is it okay to assume errno is correct? */
            (errcode_for_file_access(), errmsg("could not read block %ld of temporary file: %m", blocknum)));
}
...
static long ltsGetFreeBlock(LogicalTapeSet* lts)
{
    ...
    if (lts->nFreeBlocks > 0) {
        if (!lts->blocksSorted) {
            qsort((void*)lts->freeBlocks, (size_t)lts->nFreeBlocks, sizeof(long), freeBlocks_cmp);
...
static void ltsConcatWorkerTapes(LogicalTapeSet *lts, const TapeShare *shared, SharedFileSet *fileset)
{
     ...
     * tape.
     */
    for (int i = 0; i < lts->nTapes - 1; i++) {
        char filename[MAXPGPATH];

        lt = &lts->tapes[i];
        pg_itoa(i, filename);
        BufFile *file = BufFileOpenShared(fileset, filename);
        int64 filesize = BufFileSize(file);
 ...
 */
LogicalTapeSet* LogicalTapeSetCreate(int ntapes, const TapeShare *shared, SharedFileSet *fileset, int worker)
{
    LogicalTapeSet* lts = NULL;
    LogicalTape* lt = NULL;
    int i;
     ...
     */
    Assert(ntapes > 0);
    lts = (LogicalTapeSet*)palloc(sizeof(LogicalTapeSet) + ((size_t)ntapes - 1) * sizeof(LogicalTape));
    ...
    lts->freeBlocks = (long*)palloc((size_t)lts->freeBlocksLen * sizeof(long));
    lts->nFreeBlocks = 0;
    lts->nTapes = ntapes;
...
void LogicalTapeWrite(LogicalTapeSet* lts, int tapenum, void* ptr, size_t size)
{
    ...
    while (size > 0) {
        if ((uint32)lt->pos >= TapeBlockPayloadSize) {
            /* Buffer full, dump it out */
            if (!lt->dirty) {
                /* Hmm, went directly from reading to writing? */
                ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION), errmsg("invalid logtape state: should be dirty")));
            }
            /*
             * First allocate the next block, so that we can store it in the
             * 'next' pointer of this block.
             */
            long nextBlockNumber = ltsGetFreeBlock(lts);
        ...
        nthistime = TapeBlockPayloadSize - (size_t)lt->pos;
        if (nthistime > size)
            nthistime = size;
        Assert(nthistime > 0);
...
void LogicalTapeRewind(LogicalTapeSet* lts, int tapenum, bool forWrite)
{
    ...
    if (!forWrite) {
        ...
        if (lt->firstBlockNumber != -1L && lt->buffer == NULL) {
            lt->buffer = (char*)palloc(BLCKSZ);
        }
...
size_t LogicalTapeRead(LogicalTapeSet* lts, int tapenum, void* ptr, size_t size)
{
    ...
    while (size > 0) {
        ...
        nthistime = (size_t)lt->nbytes - (size_t)lt->pos;
        if (nthistime > size)
            nthistime = size;
        Assert(nthistime > 0);
    ...
    while (size > seekpos) {
        ...
        if (prev == -1L) {
            ...
            if (lt->curBlockNumber != lt->firstBlockNumber)
                ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION), errmsg("unexpected end of tape")));
            lt->pos = 0;
            return seekpos;
        }
        ...
        if (TapeBlockGetTrailer(lt->buffer)->next != lt->curBlockNumber) {
            ereport(ERROR,
                (errcode(ERRCODE_DATA_EXCEPTION), errmsg("broken tape, next of block %ld is %ld, expected %ld", prev,
 ...
 *
 * Must be called with a block/offset previously returned by LogicalTapeTell().
 */
void LogicalTapeSeek(LogicalTapeSet* lts, int tapenum, long blocknum, int offset)
{
    ...
    lt = &lts->tapes[tapenum];
    Assert(lt->frozen);
    Assert(offset >= 0 && (Size)offset <= TapeBlockPayloadSize);
    ...
    if (offset > lt->nbytes) {
        ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION), errmsg("invalid tape seek position")));
    }
    lt->pos = offset;
}
