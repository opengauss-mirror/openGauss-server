...
struct Tuplestorestate {
     ...
     * assume that a single pfree_ext() is enough to release the tuple later, so
     ...
     * pfree_ext() it, and increase state->availMem by the amount of memory space
     * thereby released.
     */
    ...
    int64 width;           /* first records tuple count, second records total width */
...
static bool LACKMEM(Tuplestorestate* state)
{
 ...
 *
 * Parameters:
 *	@in state: tuple store state
 *	@out newmemtupsize: return allowed size of input tuple after spread
 *
 * Return:
 *	true if successful, or failed.
 */
static bool AutoSpreadMem(Tuplestorestate* state, double* growRatio)
{
    int64 usedMem = state->allowedMem - state->availMem;

    /* For concurrent case, we don't allow materialize to spread a lot */
    if (!gs_sysmemory_busy(usedMem * state->dop, true) && state->maxMem > state->allowedMem &&
        ...
        if (spreadMem > state->allowedMem * MEM_AUTO_SPREAD_MIN_RATIO) {
            *growRatio = Min(*growRatio, 1 + (double)spreadMem / state->allowedMem);
            ...
            state->spreadNum++;
            if (state->isMemCtl) {
                AllocSetContext* set = (AllocSetContext*)(state->context);
                set->maxSpaceSize += spreadMem;
            }
            MEMCTL_LOG(DEBUG2,
                "Materialize(%d) auto mem spread %ldKB succeed, and work mem is %ldKB.",
                state->planId,
                spreadMem / 1024L,
        ...
        MEMCTL_LOG(LOG,
            "Materialize(%d) auto mem spread %ldKB failed, and work mem is %ldKB.",
            state->planId,
            spreadMem / 1024L,
            state->allowedMem / 1024L);
...
static Tuplestorestate* tuplestore_begin_common(int eflags, bool interXact, int maxKBytes)
{
    ...
    state->resowner = t_thrd.utils_cxt.CurrentResourceOwner;
 ...
 * amount is paged to disk).  When in doubt, use u_sess->attr.attr_memory.work_mem.
 *
 * memSpread: if auto memory spread is allowed for this tuplestore
 * planId: id of plan that uses this state
 */
Tuplestorestate* tuplestore_begin_heap(
    bool randomAccess, bool interXact, int maxKBytes, int maxMem, int planId, int dop, bool isAddMemCtl)
{
    Tuplestorestate* state = NULL;
    int eflags;
...
void tuplestore_set_eflags(Tuplestorestate* state, int eflags)
{
    ...
    if (state->status != TSS_INMEM || state->memtupcount != 0) {
        ereport(ERROR,
            ...
            (errcode(ERRCODE_CHECK_VIOLATION), errmsg("too late to call tuplestore_set_eflags"))));
    }        
...
void tuplestore_select_read_pointer(Tuplestorestate* state, int ptr)
{
    ...
    switch (state->status) {
        ...
        case TSS_READFILE:
            ...
            if (readptr->eof_reached) {
                if (BufFileSeek(state->myfile, state->writepos_file, state->writepos_offset, SEEK_SET) != 0)
                    ereport(ERROR,
                        (errmodule(MOD_EXECUTOR),
                            (errcode(ERRCODE_FILE_READ_FAILED), errmsg("tuplestore seek failed"))));
            } else {
        ...
        default:
            ereport(ERROR,
                (errmodule(MOD_EXECUTOR),
                    (errcode(ERRCODE_UNEXPECTED_NODE_STATE), errmsg("invalid tuplestore state"))));
            break;
    }
...
static bool grow_memtuples(Tuplestorestate* state)
{
    ...
    if (gs_sysmemory_busy(memNowUsed * state->dop, true)) {
        MEMCTL_LOG(LOG,
            "Materialize(%d) early spilled, workmem: %ldKB, availmem: %ldKB",
            state->planId,
            state->allowedMem / 1024L,
    ...
    if (memtupsize * growRatio <= memtupsize) {
        MEMCTL_LOG(LOG, "Materialize(%d) mem limit reached", state->planId);
        goto noalloc;
    }

    if (!gs_sysmemory_avail((int64)(memNowUsed * (growRatio - 1)))) {
        MEMCTL_LOG(LOG,
            "Materialize(%d) mem lack, workmem: %ldKB, availmem: %ldKB,"
...
void tuplestore_puttuple(Tuplestorestate* state, HeapTuple tuple)
{
     ...
     */
    tuple = (HeapTuple)COPYTUP(state, tuple);
 ...
 */
void tuplestore_putvalues(Tuplestorestate* state, TupleDesc tdesc, Datum* values, const bool* isnull)
{
    MinimalTuple tuple;
    MemoryContext oldcxt = MemoryContextSwitchTo(state->context);
    ...
    switch (state->status) {
        case TSS_INMEM:
            ...
            if (LACKMEM(state)) {
                ...
                elog(LOG,
                    "Materialize lacks mem, workmem: %ldKB, availmem: %ldKB",
                    state->allowedMem / 1024L,
                    state->availMem / 1024L);
            }
        ...
        case TSS_READFILE:
             ...
             */
            if (state->readptrs[state->activeptr].eof_reached == false)
                BufFileTell(
                    state->myfile, &state->readptrs[state->activeptr].file, &state->readptrs[state->activeptr].offset);
            if (BufFileSeek(state->myfile, state->writepos_file, state->writepos_offset, SEEK_SET) != 0)
                ereport(ERROR,
                    (errmodule(MOD_EXECUTOR),
                        (errcode(ERRCODE_FILE_READ_FAILED), errmsg("tuplestore seek to EOF failed"))));
            state->status = TSS_WRITEFILE;
    ...
    switch (state->status) {
        ...
        case TSS_READFILE:
            ...
            if (BufFileSeek(state->myfile, 0, 0L, SEEK_SET) != 0)
                ereport(ERROR,
                    (errmodule(MOD_EXECUTOR),
                        (errcode(ERRCODE_FILE_READ_FAILED), errmsg("tuplestore seek to start failed"))));
            break;
        default:
            ereport(ERROR,
                (errmodule(MOD_EXECUTOR),
...
void tuplestore_trim(Tuplestorestate* state)
{
    ...
    for (i = 0; i < state->readptrcount; i++) {
        if (state->readptrs[i].eof_reached == false)
    ...
    if (nremove + 1 == state->memtupcount) {
        ...
        int msize = (state->memtupcount - nremove) * sizeof(void*);
        errno_t rc = memmove_s(state->memtuples, msize, state->memtuples + nremove, msize);
...
static void writetup_heap(Tuplestorestate* state, void* tup)
{
    ...
    if (BufFileWrite(state->myfile, (void*)&tuplen, sizeof(tuplen)) != sizeof(tuplen))
        ereport(ERROR, (errcode_for_file_access(), errmsg("could not write to tuplestore temporary file: %m")));

    if (BufFileWrite(state->myfile, (void*)tupbody, tupbodylen) != (size_t)tupbodylen)
        ereport(ERROR, (errcode_for_file_access(), errmsg("could not write to tuplestore temporary file: %m")));
...
static void* readtup_heap(Tuplestorestate* state, unsigned int len)
{
    ...
    tuple->t_len = tuplen;
    if (BufFileRead(state->myfile, (void*)tupbody, tupbodylen) != (size_t)tupbodylen)
        ereport(
            ERROR, (errmodule(MOD_EXECUTOR), (errcode(ERRCODE_FILE_READ_FAILED), errmsg("unexpected end of data"))));
    if (state->backward) /* need trailing length word? */
        if (BufFileRead(state->myfile, (void*)&tuplen, sizeof(tuplen)) != sizeof(tuplen))
            ereport(ERROR,
...
int64 tuplestore_get_avgwidth(Tuplestorestate* state)
{
    if (tuplestore_in_memory(state) && state->memtupcount > 0)
        state->width = state->width / state->memtupcount;
    return state->width;
}

bool tuplestore_get_busy_status(Tuplestorestate* state)
{
    return state->causedBySysRes;
}

int tuplestore_get_spread_num(Tuplestorestate* state)
{
