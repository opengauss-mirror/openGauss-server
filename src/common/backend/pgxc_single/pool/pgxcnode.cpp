...
#define LIBCOMM_BUFFER_SIZE   8192
    ...
    {\
        ereport((level), \
                (errcode(ERRCODE_CONNECTION_FAILURE), \
                 errmsg(CONCAT(str, (Local:%s Remote: %s:%d Oid:%u)), g_instance.attr.attr_common.PGXCNodeName, result->nodename.data,\
                        result->nodeport,conn->nodeoid))); \
    }\
    else  if ((conn)->remoteNodeName != NULL) \
    {\
        ereport((level), \
                (errcode(ERRCODE_CONNECTION_FAILURE), \
                 errmsg(CONCAT(str, (Local:%s Remote: %s)), g_instance.attr.attr_common.PGXCNodeName, conn->remoteNodeName))); \
    }\
}\
while(0);\
...
LibcommCancelOrStop(NODE_CONNECTION * conn, char * errbuf,int errbufsize,int timeout, uint32 request_code)
{
    ...
    tmp_sctp_addr.sctp_port = conn->libcomm_addrinfo.sctp_port;
    ...
    if(error <= 0)
    {
         ...
         * so ECOMMSCTPREMOETECLOSE in here is expected.
         */
        if(errno == ECOMMSCTPREMOETECLOSE)
            return TRUE;
    ...
    while(1)
    {
        ...
        if(errno == ECOMMSCTPEPOLLTIMEOUT)
        {
...
LibcommPutMsgEnd(NODE_CONNECTION *conn)
{
    ...
    if (conn->outCount >= LIBCOMM_BUFFER_SIZE)
    {
...
LibcommReadData(NODE_CONNECTION *conn)
{
     ...
     * buffer size, so...
     */
    if (conn->inBufSize - conn->inEnd < LIBCOMM_BUFFER_SIZE)
    {
    ...
    if (nread <= 0)
    {
        /* gs_recv return -1 with errno ECOMMSCTPNODATA if data is not arrived */
        if(errno == ECOMMSCTPNODATA)
        {
            return 0;
        }
        else
...
get_message(PGXCNodeHandle *conn, int *len, char **msg)
{
    ...
    *len -= 4;
    if (*len < 0) {
        ereport(ERROR, (errmsg("message len is too short")));
    }

    if (conn->inCursor > (INT_MAX - *len)) {
        ereport(ERROR, (errmsg("conn cursor overflow")));
    }
...
destroy_handles(void)
{
    ...
    PoolManagerReleaseConnections(NULL, true);
...
release_handles(void)
{
    ...
    /* And finally release all the connections on pooler */
    PoolManagerReleaseConnections(status_array, has_error);
...
send_some(PGXCNodeHandle *handle, int len)
{
    ...
    while (len > 0)
    {
        ...
        if(handle->is_logic_conn)
        {
            ...
            {
                sent = pgfdw_pqsecure_write(handle->pg_conn, ptr, Min(len, LIBCOMM_BUFFER_SIZE));
            }
            else
#endif
            {
                sent = gs_send(&(handle->gsock), ptr, Min(len, LIBCOMM_BUFFER_SIZE), -1, TRUE);
            }
        }
        else
        {
...
#endif
            {
                ...
                if (handle->pg_conn != NULL && handle->pg_conn->ssl != NULL)
                {
                    sent = pgfdw_pqsecure_write(handle->pg_conn, ptr, Min(len, 65536));
                }
                else
#endif
                {
...
int
pgxc_node_send_sessid(PGXCNodeHandle *handle, uint64 global_sess_id, PGXCNode_HandleGPC handle_type)
{
    Assert(false);
    DISTRIBUTED_FEATURE_NOT_SUPPORTED();
    return 0;
 ...
 */
int
pgxc_node_send_query(PGXCNodeHandle * handle, const char *query, bool isPush, bool trigger_ship, bool check_gtm_mode)
{
#ifndef ENABLE_MULTIPLE_NODES
    Assert(false);
    DISTRIBUTED_FEATURE_NOT_SUPPORTED();
...
pgxc_node_send_cmd_id(PGXCNodeHandle *handle, CommandId cid)
{
    ...
    handle->outEnd += 4;
    i32 = htonl(cid);
    rc = memcpy_s(handle->outBuffer + handle->outEnd, 4, &i32, 4);
    securec_check(rc, "\0", "\0");
    handle->outEnd += 4;
 ...
 *                          */
    if (handle->error == NULL && u_sess->top_mem_cxt != NULL) {
        old = MemoryContextSwitchTo(u_sess->top_mem_cxt);
