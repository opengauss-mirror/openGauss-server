...
/* The following is used only for buffer free list management. */
static THR_LOCAL unsigned short free_list_rand48_seed[3] = {RAND48_SEED_0, RAND48_SEED_1, RAND48_SEED_2};
static THR_LOCAL unsigned short free_list_rand48_mult[3] = {RAND48_MULT_0, RAND48_MULT_1, RAND48_MULT_2};
static THR_LOCAL unsigned short free_list_rand48_add = RAND48_ADD;

void free_list_srand48(long seed)
{
    free_list_rand48_seed[0] = RAND48_SEED_0;
    free_list_rand48_seed[1] = (unsigned short)seed;
    free_list_rand48_seed[2] = (unsigned short)((unsigned long)seed >> 16);
    free_list_rand48_mult[0] = RAND48_MULT_0;
    free_list_rand48_mult[1] = RAND48_MULT_1;
    free_list_rand48_mult[2] = RAND48_MULT_2;
    free_list_rand48_add = RAND48_ADD;
}

static void free_list_dorand48(unsigned short xseed[3])
{
    unsigned long accu;
    unsigned short temp[2];

    accu = (unsigned long)free_list_rand48_mult[0] * (unsigned long)xseed[0] + (unsigned long)free_list_rand48_add;
    temp[0] = (unsigned short)accu; /* lower 16 bits */
    accu >>= sizeof(unsigned short) * 8;
    accu += (unsigned long)free_list_rand48_mult[0] * (unsigned long)xseed[1] +
            (unsigned long)free_list_rand48_mult[1] * (unsigned long)xseed[0];
    temp[1] = (unsigned short)accu; /* middle 16 bits */
    accu >>= sizeof(unsigned short) * 8;
    accu += free_list_rand48_mult[0] * xseed[2] + free_list_rand48_mult[1] * xseed[1] +
            free_list_rand48_mult[2] * xseed[0];
    xseed[0] = temp[0];
    xseed[1] = temp[1];
    xseed[2] = (unsigned short)accu;
}

long free_list_lrand48(void)
{
    free_list_dorand48(free_list_rand48_seed);
    return ((long)free_list_rand48_seed[2] << 15) + ((long)free_list_rand48_seed[1] >> 1);
}
