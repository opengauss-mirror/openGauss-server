...
static Datum ExecEvalConst(ExprState* exprstate, ExprContext* econtext, bool* isNull, ExprDoneCond* isDone)
{
   Const* con = NULL;
    if (IsA(exprstate->expr, UserVar)) {
        ...
        GucUserParamsEntry *entry = (GucUserParamsEntry *)hash_search(KNL_UTILS_GUC_FIELD(&u_sess->utils_cxt, set_user_params_htab), uservar->name, HASH_FIND, &found);
...
*/
ExprState* ExecPrepareExpr(Expr* node, EState* estate, bool isExprIndex, MemoryContext context)
{
   ExprState* result = NULL;
   MemoryContext oldcontext;
   MemoryContext exprcontext = (context == NULL) ? estate->es_query_cxt : context;

   oldcontext = MemoryContextSwitchTo(exprcontext);
