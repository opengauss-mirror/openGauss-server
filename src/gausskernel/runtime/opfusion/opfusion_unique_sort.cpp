 ...
 * ---------------------------------------------------------------------------------------
 *
 * opfusion_unique_sort.cpp
 *        Definition of sort operator for bypass executor.
 *
 * IDENTIFICATION
 *        src/gausskernel/runtime/opfusion/opfusion_unique_sort.cpp
 *
 * ---------------------------------------------------------------------------------------
 */
...
UniqueSortFusion::UniqueSortFusion(MemoryContext context, CachedPlanSource* psrc, List* plantree_list,
...
void UniqueSortFusion::InitLocals(ParamListInfo params)
{
    Plan *node = (Plan*)m_global->m_planstmt->planTree->lefttree->lefttree;
    Sort *sortnode = (Sort *)m_global->m_planstmt->planTree->lefttree;
    ...
    m_unique_plan_node = (Unique *)m_global->m_planstmt->planTree;
    m_eqfunctions = execTuplesMatchPrepare(m_unique_plan_node->numCols, m_unique_plan_node->uniqOperators);
}

void UniqueSortFusion::InitGlobals()
{
    m_global->m_reloid = 0;
    Sort *sortnode = (Sort *)m_global->m_planstmt->planTree->lefttree;
...
bool UniqueSortFusion::execute(long max_rows, char *completionTag)
{
    max_rows = FETCH_ALL;
    bool success = false;
...

    {
        ...
        tuplesortstate =
            tuplesort_begin_heap(m_c_local.m_scanDesc, sortnode->numCols, sortnode->sortColIdx, sortnode->sortOperators,
                                 sortnode->collations, sortnode->nullsFirst, sortMem, false, maxMem,
                                 sortnode->plan.plan_node_id, SET_DOP(sortnode->plan.dop));
    }
    ...
    TupleTableSlot *previousSlot = NULL;
    bool isFirst = true;
    ...
    while (tuplesort_gettupleslot(tuplesortstate, true, slot, NULL)) {
        ...
        if (!isFirst &&
            compareTuplesNotDistinct(slot, previousSlot, m_unique_plan_node->numCols, m_unique_plan_node->uniqColIdx,
                                     m_eqfunctions, m_unique_plan_node->uniq_collations)) {
            continue;
        } else if (memoryIsAllocated) {
            tableam_tops_free_tuple(reslot->tts_tuple);
