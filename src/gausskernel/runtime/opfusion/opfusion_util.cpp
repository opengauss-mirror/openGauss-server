...
const char* getBypassReason(FusionType result)
{
    switch (result) {
      ...
      case NOBYPASS_INVALID_PLAN: {
        ...
        case NOBYPASS_SUBPARTITION_TYPE_NOT_SUPPORT: {
            return "Bypass not support in sub partition currently";
        }
...
FusionType checkPartitionType(const Relation rel)
{
    ...
    if (RelationIsSubPartitioned(rel)) {
        return NOBYPASS_SUBPARTITION_TYPE_NOT_SUPPORT;
    }
...
FusionType checkDMLRelation(const Relation rel, const PlannedStmt *plannedstmt, bool isInsert, bool isPartTbl)
{
    if (isPartTbl && ENABLE_GPC) {
        return NOBYPASS_GPC_NOT_SUPPORT_PARTITION_BYPASS;
    }
    FusionType result = BYPASS_OK;
    /*
     * recheck partition, when set partition_iterator_elimination, partIterator may be eliminated.
     */
    if (rel->rd_rel->relkind != RELKIND_RELATION || rel->rd_rel->relhasrules || rel->rd_rel->relhastriggers ||
        ...
        result = NOBYPASS_DML_RELATION_NOT_SUPPORT;
    }
...
FusionType getSelectFusionType(List *stmt_list, ParamListInfo params)
{
    ...
    if ((IsA(top_plan, IndexScan) || IsA(top_plan, IndexOnlyScan)) && top_plan->lefttree == NULL) {
        ...
        if (isPartTbl && !u_sess->attr.attr_sql.enable_partition_opfusion) {
            return NOBYPASS_PARTITION_BYPASS_NOT_OPEN;
        }
    } else {
    ...
    ttype = checkDMLRelation(rel, plannedstmt, false, isPartTbl);
    if (ttype != BYPASS_OK) {
        heap_close(rel, AccessShareLock);
        return ttype;
    }
    ttype = checkPartitionType(rel);
    if (ttype != BYPASS_OK) {
        heap_close(rel, AccessShareLock);
        return ttype;
    }
...
FusionType getInsertFusionType(List *stmt_list, ParamListInfo params)
{
    ...
    /* check subquery num */
    ttype = checkBaseResult(top_plan);
    if (ttype > BYPASS_OK) {
        return ttype;
    }
    ModifyTable *node = (ModifyTable *)top_plan;
    ...
    ttype = checkDMLRelation(rel, plannedstmt, true, RELATION_IS_PARTITIONED(rel));
    if (ttype != BYPASS_OK) {
        heap_close(rel, AccessShareLock);
        return ttype;
    }
...
FusionType getUpdateFusionType(List *stmt_list, ParamListInfo params)
{
    ...
    /* check index scan */
    ttype = checkFusionIndexScan<true, false>((Node *)indexscan, params);
    /* check failed */
    if (ttype > BYPASS_OK) {
        return ttype;
    }
    ...
    Oid relid = getrelid(res_rel_idx, plannedstmt->rtable);
    Relation rel = heap_open(relid, AccessShareLock);
    ttype = checkDMLRelation(rel, plannedstmt, false, indexscan->scan.isPartTbl);
    if (ttype != BYPASS_OK) {
        heap_close(rel, AccessShareLock);
        return ttype;
    }
...
FusionType getDeleteFusionType(List *stmt_list, ParamListInfo params)
{
    ...
    if (indexscan->scan.isPartTbl && !u_sess->attr.attr_sql.enable_partition_opfusion) {
        return NOBYPASS_PARTITION_BYPASS_NOT_OPEN;
    }
    /* check index scan */
    ttype = checkFusionIndexScan<true, false>((Node *)deletePlan, params);
    /* check failed */
    if (ttype > BYPASS_OK) {
        return ttype;
    }
