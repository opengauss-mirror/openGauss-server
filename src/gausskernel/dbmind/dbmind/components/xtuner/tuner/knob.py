...
class RecommendedKnobs:
    ...
    def output_formatted_knobs(self):
        ...
        report_msg = 'There is no report because the current mode specifies the list of ' \
                     'tuning knobs through the configuration file.'
        if self.reporter:
            report_msg = self.reporter.generate()
        print(report_msg)
        ...
        for knob in self.all_knobs:
            knob_table.add_row(
                (knob.name, knob.recommend, knob.min, knob.max, knob.restart)
            )
        print(knob_table)
