...
void mc_poller_add(struct mc_poller* self, int fd, int id)
{
    ...
    ev.events = EPOLLIN;
    ev.data.u64 = (((uint64)(fd)) << MC_POLLER_FD_ID_OFFSET) + id;
    rc = epoll_ctl(self->ep, EPOLL_CTL_ADD, fd, &ev);
    errno_assert(rc == 0);
}

void mc_poller_rm(struct mc_poller* self, int fd)
{
    //  Remove the file descriptor from the pollset.
    //
    epoll_ctl(self->ep, EPOLL_CTL_DEL, fd, NULL);
}
...
int mc_poller_hndl_list::add_fd(struct sock_id* fd_id)
{
    ...
    struct int_poller_hndl_entry* entry_hndl = NULL;
    LIBCOMM_PTHREAD_MUTEX_LOCK(&m_lock);
    LIBCOMM_MALLOC(entry_hndl, sizeof(struct int_poller_hndl_entry), struct int_poller_hndl_entry);
    if (entry_hndl == NULL) {
        LIBCOMM_PTHREAD_MUTEX_UNLOCK(&m_lock);
    ...
    entry_hndl->val.fd = fd_id->fd;
    entry_hndl->val.id = fd_id->id;
    entry_hndl->val.events = 0;
...
int mc_poller_hndl_list::del_fd(struct sock_id* fd_id)
{
    ...
    mc_poller_rm(m_poller, fd_id->fd);
