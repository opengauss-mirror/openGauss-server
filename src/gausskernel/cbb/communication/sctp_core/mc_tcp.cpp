...
int mc_tcp_bind(int fd, const struct sockaddr* sa, socklen_t salen)
{
    ...
    while (retry_time--) {
        ...
        LIBCOMM_ELOG(WARNING,
            "(mc tcp listen)\tFailed to bind host:port[%s:%u], errno[%d]:%s."
            "Maybe port %u is used, run 'netstat -anop|grep %u' or "
            "'lsof -i:%u'(need root) to see who is using.",
        ...
        if (rc > 0) {
            ...
            if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
                ...
                if (((time_now - time_enter) >
                        ((uint64)g_instance.comm_cxt.mctcp_cxt.mc_tcp_send_timeout * SEC_TO_MICRO_SEC)) &&
                    (time_now > time_enter)) {
