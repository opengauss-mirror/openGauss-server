 ...
 *
 * IDENTIFICATION
 *    src/gausskernel/cbb/communication/sctp_utils/sctp_util.cpp
 *
 * -------------------------------------------------------------------------
 */
#include <stdio.h>
...
{
    if (cmailbox->statistic == NULL || cmailbox->state == MAIL_UNKNOWN) {
        return;
    }
...
{
    if (pmailbox->statistic == NULL || pmailbox->state == MAIL_UNKNOWN) {
        return;
    }
...
uint64_t mc_timers_us(void)
{
    ...
    ns_monotonic = ((ts.tv_sec * MC_NS_IN_SEC) + (uint64_t)ts.tv_nsec) / 1000;
    return ns_monotonic;
}
...
void comm_ipc_log_get_time(char *now_date, int time_len)
{
    ...
    struct timeval tv;  /* make time string start, like setup_formatted_log_time */
    pg_time_t stamp_time;
    char msbuf[BUF_LEN];
    errno_t ss_rc = 0;
