...
static char* GetWaitStatusInfo(const SessionHistEntry *beentry)
{
    ...
    if (beentry->xid != 0) {
        GetWaitstatusXid(beentry, &waitStatus);
               ...
               beentry->waitstatus == STATE_VACUUM_FULL || beentry->waitstatus == STATE_VACUUM_GPI)) {
...
{
    uint32 classId = beentry->waitevent & 0xFF000000;
    ...
    if (classId == PG_WAIT_LOCK) {
        ...
        uint16  eventId = beentry->waitevent & 0x0000FFFF;
        ...
        (void)cJSON_AddItemToObject(root, "locktag", cJSON_CreateString(buftag));
        (void)cJSON_AddItemToObject(root, "lockmode", cJSON_CreateNull());
        (void)cJSON_AddItemToObject(root, "block_sessionid", cJSON_CreateNull());
        pfree_ext(buftag);
    } else {
...
static void GetBlockInfo(Datum* values, bool* nulls, const SessionHistEntry *beentry)
{
    ...
    if (classId == PG_WAIT_LOCK) {
        ...
        char* buftag = BuftagToString(&(beentry->bufTag));
        values[Anum_gs_asp_locktag - 1] = CStringGetTextDatum(buftag);
        nulls[Anum_gs_asp_lockmode - 1] = true;
        nulls[Anum_gs_asp_block_sessionid - 1] = true;
        pfree_ext(buftag);
    } else {
    ...
    if (u_sess -> attr.attr_storage.DefaultXactReadOnly) {
        ereport(ERROR,
            (errcode(ERRCODE_READ_ONLY_SQL_TRANSACTION),
                errmsg("cannot execute ASP flushing in a read-only transaction")));
    }
    ...
    PgBackendStatus* beentry, SessionHistEntry *ash_arrary_slot)
{
    ...
    ash_arrary_slot->bufTag = beentry->bufTag;
}
