 ...
 *
 * IDENTIFICATION
 *    src/gausskernel/storage/mot/core/src/memory/mm_global_api.cpp
 *
 * -------------------------------------------------------------------------
 */
...
static MemGlobalAllocator** globalAllocators = NULL;
    ...
    if (globalAllocators == NULL) {
        ...
        for (int i = 0; i < (int)g_memGlobalCfg.m_nodeCount; ++i) {
            ...
            if (result != 0) {
                ...
                globalAllocators[i] = NULL;
                break;
            }
            MemSessionAllocatorInit(
                &globalAllocators[i]->m_allocator, 0, 0, NULL, 0, MEM_ALLOC_GLOBAL);  // no special reservation is made
