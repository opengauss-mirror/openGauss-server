 ...
 *
 * IDENTIFICATION
 *    src/gausskernel/storage/mot/core/src/memory/mm_buffer_allocator.cpp
 *
 * -------------------------------------------------------------------------
 */
    ...
    MemAllocType allocType, int freeListDrainSize /* = MEM_DEFAULT_FREE_LIST_SIZE */)
{
    ...
    bufferAllocator->m_allocSize = L1_ALIGNED_SIZEOF(MemBufferHeap);
    bufferAllocator->m_allocSize += L1_ALIGNED_SIZEOF(MemBufferChunkSnapshot) *
                                    g_memGlobalCfg.m_maxThreadCount;  // this is a bit too much, but code is simpler
    bufferAllocator->m_allocSize +=
        L1_ALIGNED_SIZEOF(MemBufferList) * g_memGlobalCfg.m_maxThreadCount;  // this is too much, but code is simpler
    ...
    if (inplaceBuffer == nullptr) {
        ...
        bufferAllocator->m_chunkSnapshots = (MemBufferChunkSnapshot*)L1_ALIGNED_PTR(bufferAllocator->m_bufferHeap + 1);
        bufferAllocator->m_freeListArray =
            (MemBufferList*)L1_ALIGNED_PTR(bufferAllocator->m_chunkSnapshots + g_memGlobalCfg.m_maxThreadCount);
