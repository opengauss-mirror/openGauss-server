 ...
 *
 * IDENTIFICATION
 *    src/gausskernel/storage/mot/core/src/memory/mm_buffer_heap.h
 *
 * -------------------------------------------------------------------------
 */
...
struct MemBufferHeap {
     ...
     */
    uint64_t m_fullnessBitset[MAX_BITSET_COUNT];  // accommodate for MAX_BUFFERS_PER_CHUNK fullness levels
     ...
     */
    MemBufferChunkHeader* m_fullnessDirectory[MAX_BUFFERS_PER_CHUNK];

    /** @var List of all full chunks. */
    MemBufferChunkHeader* m_fullChunkList;

    /** @var The number of chunks currently allocated by this heap. */
    uint64_t m_allocatedChunkCount;

    /** @var The number of buffers currently allocated by this heap. */
    uint64_t m_allocatedBufferCount;
};
