 ...
 *
 * IDENTIFICATION
 *    src/gausskernel/storage/mot/core/src/memory/garbage_collector/mm_gc_manager.cpp
 *
 * -------------------------------------------------------------------------
 */
...
volatile GcEpochType g_gcGlobalEpoch;
/** @var Current lowest epoch among all active GC Managers */
volatile GcEpochType g_gcActiveEpoch;
...
bool GcManager::Initialize()
{
    ...
    if (m_purpose == GC_MAIN) {
        ...
        void* limboSpace = MemSessionAlloc(sizeof(LimboGroup));
#else
        void* limboSpace = calloc(1, sizeof(LimboGroup));
#endif
        if (limboSpace == nullptr) {
            ...
            m_limboHead = m_limboTail = new (limboSpace) LimboGroup;
...
#endif
        if (limboSpace != nullptr) {
            m_limboTail->m_next = new (limboSpace) LimboGroup;
        } else {
            MOT_REPORT_ERROR(MOT_ERROR_OOM,
