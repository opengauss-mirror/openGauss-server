 ...
 *
 * IDENTIFICATION
 *    src/gausskernel/storage/mot/core/src/infra/synchronization/affinity.cpp
 *
 * -------------------------------------------------------------------------
 */
...
uint32_t Affinity::GetAffineProcessor(uint64_t threadId) const
{
    ...
    switch (m_affinityMode) {
        case AffinityMode::FILL_SOCKET_FIRST: {
            if (GetGlobalConfiguration().m_maxDataNodes > 1) {
                uint64_t groupSize = ((m_numaNodes * m_physicalCoresNuma) / GetGlobalConfiguration().m_maxDataNodes);
                result = (uint32_t)((threadId % groupSize) + GetGlobalConfiguration().m_dataNodeId * groupSize);
            } else {
...
uint32_t Affinity::GetAffineNuma(uint64_t threadId) const
{
    ...
    switch (m_affinityMode) {
        case AffinityMode::FILL_SOCKET_FIRST: {
            if (GetGlobalConfiguration().m_maxDataNodes > 1) {
                uint64_t groupSize = ((m_numaNodes * m_physicalCoresNuma) / GetGlobalConfiguration().m_maxDataNodes);
                uint32_t physicalThreadId =
                    (uint32_t)((threadId % groupSize) + GetGlobalConfiguration().m_dataNodeId * groupSize);
                result = (uint32_t)GetGlobalConfiguration().GetCpuNode(physicalThreadId);
            } else {
                result =
                    (uint32_t)GetGlobalConfiguration().GetCpuNode(GetGlobalConfiguration().GetMappedCore(threadId));
            }
            break;
        }
