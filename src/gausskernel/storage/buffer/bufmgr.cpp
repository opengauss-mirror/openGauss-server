...
#ifdef ENABLE_DFX_OPT
FORCE_INLINE
uint32 hashquickany24(uint32 seed, register const unsigned char* data) 
{
...
#ifdef ENABLE_DFX_OPT
    if (sizeof(BufferTag) == 24) {
        new_tag.rnode.opt = DefaultFileNodeOpt;
        new_hash = hashquickany24(0xFFFFFFFF, (unsigned char *)&new_tag);
    } else {
        new_hash = BufTableHashCode(&new_tag);
    }
#else
...
#ifdef ENABLE_DFX_OPT
            if (sizeof(BufferTag) == 24) {
                old_tag.rnode.opt = DefaultFileNodeOpt;
                old_hash = hashquickany24(0xFFFFFFFF, (unsigned char *)&old_tag);
            } else {
                old_hash = BufTableHashCode(&old_tag);
            }
#else
