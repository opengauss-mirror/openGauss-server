...
static void CheckAutoExplainThreshold();
...
void handle_sig_alarm(SIGNAL_ARGS)
{
    ...
    if (t_thrd.postgres_cxt.auto_explain_threshold_active) {
        CheckAutoExplainThreshold();
    }
...
static void CheckAutoExplainThreshold()
{
    if (!u_sess->attr.attr_resource.enable_auto_explain ||
        u_sess->attr.attr_resource.auto_explain_log_min_duration == 0) {
        return;
    }

    TimestampTz now = GetCurrentTimestamp();
    if (now > t_thrd.postgres_cxt.auto_explain_threshold_time) {
        t_thrd.postgres_cxt.auto_explain_threshold_active = false;
        t_thrd.explain_cxt.need_auto_explain = true;
    }
}

bool enable_auto_explain_threshold_sig_alarm()
{
    /* no need to setup new timer for nested execution */
    if (t_thrd.postgres_cxt.auto_explain_threshold_active) {
        return true;
    }

    int delayms = u_sess->attr.attr_resource.auto_explain_log_min_duration;

    /* set auto-explain timer flag and finish time */
    t_thrd.postgres_cxt.auto_explain_threshold_active = true;
    t_thrd.postgres_cxt.auto_explain_threshold_time =  TimestampTzPlusMilliseconds(GetCurrentTimestamp(), delayms);

    /* set the timer to interrupt */
    struct itimerval timeval;
...
bool disable_auto_explain_threshold_sig_alarm()
{
    /* already disabled */
    if (!t_thrd.postgres_cxt.auto_explain_threshold_active) {
        return true;
    ...
    /* reset auto-explain timer flag and finish time */
    t_thrd.postgres_cxt.auto_explain_threshold_active = false;
    t_thrd.postgres_cxt.auto_explain_threshold_time = 0;
