...
#define FileIsValid(file, inter_xact)                                            \
    ((file) > 0 && (file) < (int)GetSizeVfdCache(inter_xact) && \
     (GetVfdCache(inter_xact)[file].fileName != NULL || t_thrd.proc_cxt.proc_exit_inprogress))
 ...
 * --------------------
 */
static void Delete(File file, bool interXact);
static void LruDelete(File file, bool interXact);
static void Insert(File file, bool interXact);
static int LruInsert(File file, bool interXact);
static bool ReleaseLruFile(void);
static File AllocateVfd(bool interXact);
static void FreeVfd(File file, bool interXact);

static int FileAccess(File file, bool interXact);
static File OpenTemporaryFileInTablespace(Oid tblspcOid, bool rejectError, bool interXact);
static void CleanupTempFiles(bool isProcExit, bool interXact);
...
void CloseAllTempFile(bool inter_xact)
{
    Index i;
    if (GetSizeVfdCache(inter_xact) > 0) {
        for (i = 1; i < GetSizeVfdCache(inter_xact); i++) {
            if (FileIsValid((int)i, inter_xact) && (GetVfdCache(inter_xact)[i].fdstate & FD_TEMP_FILE_LIMIT))
...
{
    Vfd *vfdP = &GetVfdCache(false)[file];
    auto chunk_size = CHUNK_SIZE_LIST[GET_COMPRESS_CHUNK_SIZE(node.opt)];
    int returnCode = FileAccess(file, false);
    if (returnCode < 0) {
...
PageCompressHeader *GetPageCompressMemoryMap(File file, uint32 chunk_size)
{
    ...
    Assert(FileIsValid(file, false));

    returnCode = FileAccess(file, false);
    if (returnCode < 0) {
        ereport(ERROR, (errcode(ERRCODE_INSUFFICIENT_RESOURCES), errmsg("Failed to open file %s: %m", vfdP->fileName)));
    }
