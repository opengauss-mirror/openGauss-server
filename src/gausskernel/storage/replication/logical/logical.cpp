...
void LogicalIncreaseXminForSlot(XLogRecPtr current_lsn, TransactionId xmin)
{
    ...
    if (updated_xmin)
        LogicalConfirmReceivedLocation(slot->data.confirmed_flush, true);
}
...
void LogicalIncreaseRestartDecodingForSlot(XLogRecPtr current_lsn, XLogRecPtr restart_lsn)
{
    ...
    if (XLByteEQ(slot->candidate_restart_valid, InvalidXLogRecPtr)) {
        ...
        ereport(DEBUG1, (errmodule(MOD_LOGICAL_DECODE),
            errmsg("%s to increase restart lsn: proposed %X/%X, after %X/%X, current candidate %X/%X, current "
                   "after %X/%X, flushed up to %X/%X",
                   (updated_lsn ? "success" : "failed"),
 ...
 */
void LogicalConfirmReceivedLocation(XLogRecPtr lsn, bool force)
{
    /*
     ...
     */
    if (!force && XLByteLE(lsn, t_thrd.slot_cxt.MyReplicationSlot->data.confirmed_flush))
        return;
...
bool LogicalAdvanceConnect()
{
    ...
    /* Note: retries here would lead in no reply for client message */
    int retryNum = u_sess->attr.attr_storage.wal_receiver_connect_retries;
...
static bool NotifyPrimaryAdvance(XLogRecPtr restart, XLogRecPtr flush)
{
    char query[256];
    PGresult* res = NULL;
    int nRet = 0;
...
static bool NotifyPrimaryCatalogXmin(TransactionId catalogXmin)
{
    char query[256];
    PGresult* res = NULL;
...
/* Notify the primary to advance logical replication slot's comfirm flush and catalog xmin */
void LogicalNotifyPrimary(XLogRecPtr flush)
{
    if (RecoveryInProgress() && OidIsValid(t_thrd.slot_cxt.MyReplicationSlot->data.database)) {
        ...
        if (NotifyPrimaryAdvance(t_thrd.slot_cxt.MyReplicationSlot->data.restart_lsn, flush)) {
            (void)NotifyPrimaryCatalogXmin(t_thrd.slot_cxt.MyReplicationSlot->data.catalog_xmin);
        }
    }
}
