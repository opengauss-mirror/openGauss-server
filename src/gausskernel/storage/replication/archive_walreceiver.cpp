...
static char* FindTargetGlobalBarrierInRecords(char* globalBarrierRecords)
{
    ...
    while (token != NULL) {
        ...
        if (strncasecmp(tmptoken, g_instance.roach_cxt.targetTimeInPITR,
            ...
            if (lastTimestamp != NULL &&
                atol(g_instance.roach_cxt.targetTimeInPITR) * msec - atol(lastTimestamp) <= requestTimeInterval) {
    ...
    if (outputStr != NULL) {
        if (strstr(outputStr, g_instance.roach_cxt.targetTimeInPITR) == NULL)
            return NULL;
        return outputStr;
    } else {
        ereport(WARNING, (errmsg("Find target PITR barrier record failed on obs record list")));
        return NULL;
    }
}
...
static char* FindGlobalBarrierRecordForPITR(ArchiveConfig *archive_config)
{
    ...
    for (int i = 0; i < tryTimes; i++) {
        rc = snprintf_s(barrierFileName, MAXPGPATH, MAXPGPATH - 1, "global_barrier_records/hadr_barrier_%013ld",
            startMsecTimeStamp + (cntFile - i) * FILE_TIME_INTERVAL);
        securec_check_ss(rc, "\0", "\0");
        if (ArchiveFileExist(barrierFileName, archive_config) == false) {
            ereport(ERROR, (errcode(ERRCODE_NO_DATA_FOUND),
    ...
    for (int i = 0; i < g_instance.attr.attr_storage.max_replication_slots; i++) {
        ...
        if (currSlotArchConfig != NULL) {
            ...
            if (endBarrierTimestamp > startTimeStamp) {
                ...
                securec_check_ss_c(rc, "", "");
                readLen = 0;
                if (ArchiveFileExist(barrierFileName, currSlotArchConfig) != true) {
                    ereport(WARNING, (errmsg("There is no barrier record file on media named: %s.", barrierFileName)));
                } else {
