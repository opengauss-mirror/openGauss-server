    ...
    switch (cmd_node->type) {
        ...
        case T_StartReplicationCmd: {
            ...
            if (cmd->kind == REPLICATION_KIND_PHYSICAL) {
                ...
                (void)ReplicationSlotValidateName(cmd->slotname, ERROR);
...
void XLogSendParallelLogical()
{
     ...
     */
    TimestampTz last_check_time = GetCurrentTimestamp();
    for (;; g_Logicaldispatcher[slotId].id = (g_Logicaldispatcher[slotId].id + 1) % GetDecodeParallelism(slotId)) {
        ...
        /* Check for input from the client per second */
        TimestampTz curTime = GetCurrentTimestamp();
        if (TimestampDifferenceExceeds(last_check_time, curTime, expTime)) {
            ProcessRepliesIfAny();
