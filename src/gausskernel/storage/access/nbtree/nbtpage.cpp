...
Buffer _bt_getroot(Relation rel, int access)
{
    ...
    if (!(metaopaque->btpo_flags & BTP_META) || metad->btm_magic != BTREE_MAGIC)
        ereport(ERROR,
            (errcode(ERRCODE_INDEX_CORRUPTED), errmsg("index \"%s\" is not a btree", RelationGetRelationName(rel))));
...
void _bt_checkpage(Relation rel, Buffer buf)
{
    ...
    Size _bt_specialsize =
        PageIs4BXidVersion(page) ? MAXALIGN(sizeof(BTPageOpaqueDataInternal)) : MAXALIGN(sizeof(BTPageOpaqueData));
 ...
 */
void _bt_delitems_vacuum(const Relation rel, Buffer buf, OffsetNumber* itemnos, int nitems, BlockNumber lastBlockVacuumed)
{
    Page page = BufferGetPage(buf);
    BTPageOpaqueInternal opaque;
...
int _bt_pagedel(Relation rel, Buffer buf, BTStack stack)
{
    ...
    if (leftsib != P_NONE) {
        ...
        while (P_ISDELETED(opaque) || opaque->btpo_next != target) {
            ...
            if (leftsib == P_NONE) {
                ereport(
                    LOG, (errmsg("no left sibling (concurrent deletion?) in \"%s\"", RelationGetRelationName(rel))));
                return 0;
            }
            lbuf = _bt_getbuf(rel, leftsib, BT_WRITE);
            page = BufferGetPage(lbuf);
...
void _bt_page_localupgrade(Page page)
{
    ...
    rc = memmove_s((char*)page + phdr->pd_special - sizeof(TransactionId),
        MAXALIGN(sizeof(BTPageOpaqueDataInternal)),
    ...
    rc = memset_s(page + phdr->pd_special + MAXALIGN(sizeof(BTPageOpaqueDataInternal)),
