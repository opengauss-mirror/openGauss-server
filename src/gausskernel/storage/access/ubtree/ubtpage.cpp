...
int UBTreePageDel(Relation rel, Buffer buf)
{
    ...
    for (;;) {
        ...
        /* Clean BTP_VACUUM_DELETING flag after delete on one page. */
        if (((t_thrd.pgxact->vacuumFlags & PROC_IN_VACUUM) || IsAutoVacuumWorkerProcess()) &&
            P_VACUUM_DELETING((UBTPageOpaqueInternal)opaque)) {
            UBTPageOpaque uOpaque = (UBTPageOpaque) PageGetSpecialPointer(BufferGetPage(buf));
            uOpaque->xact = InvalidTransactionId;
...
static bool ReserveForDeletion(Relation rel, Buffer buf)
{
    ...
    while (times--) {
        ...
        if (TransactionIdIsCurrentTransactionId(previousXact) || P_VACUUM_DELETING((UBTPageOpaqueInternal)opaque)) {
