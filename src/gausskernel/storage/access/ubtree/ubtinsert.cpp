...
static bool UBTreeIndexIsValid(Relation indexRelation);
...
static void ReportFailedToDelete(Relation rel, Buffer buf, IndexTuple itup, char* extraInfo)
{
    ...
    PrevDumpUPage((rel == NULL ? NULL : (&(rel->rd_node))), BufferGetPage(buf), BufferGetBlockNumber(buf), "ubtree");
    ereport(ERROR, (errcode(ERRCODE_INDEX_CORRUPTED),
...
/*
 * UBTreeIsValid() -- check if concrrent index is valid.
*/
static bool UBTreeIndexIsValid(Relation indexRelation)
{
    ...
    if (RelationIsPartition(indexRelation)) {
        Oid parentIndexOid = GetBaseRelOidOfParition(indexRelation);
        indexTuple = SearchSysCache1(INDEXRELID, ObjectIdGetDatum(parentIndexOid));
        if (!HeapTupleIsValid(indexTuple)) {
            ereport(ERROR, (errcode(ERRCODE_CACHE_LOOKUP_FAILED),
                errmsg("cache lookup failed for index %u", parentIndexOid)));
        }
    } else {
        indexTuple = SearchSysCache1(INDEXRELID, ObjectIdGetDatum(RelationGetRelid(indexRelation)));
        if (!HeapTupleIsValid(indexTuple)) {
            ereport(ERROR, (errcode(ERRCODE_CACHE_LOOKUP_FAILED),
                errmsg("cache lookup failed for index %u", RelationGetRelid(indexRelation))));
        }
    }
 
    indexForm = (Form_pg_index)GETSTRUCT(indexTuple);
    bool valid = IndexIsValid(indexForm);
