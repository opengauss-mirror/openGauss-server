...
static void UBTreeRecsplitloc(FindSplitData *state, OffsetNumber firstoldonright, bool newitemonleft,
    int olddataitemstoleft, Size firstoldonrightsz);
static void UBTreeDeltasortsplits(FindSplitData *state, double fillfactormult, bool usemult);
static int UBTreeSplitcmp(const void *arg1, const void *arg2);
static bool UBTreeAfternewitemoff(FindSplitData *state, OffsetNumber maxoff, int leaffillfactor, bool *usemult);
static bool UBTreeAdjacenthtid(ItemPointer lowhtid, ItemPointer highhtid);
static OffsetNumber UBTreeBestsplitloc(FindSplitData *state, int perfectpenalty, bool *newitemonleft,
    FindSplitStrat strategy);
static int UBTreeStrategy(FindSplitData *state, SplitPoint *leftpage, SplitPoint *rightpage, FindSplitStrat *strategy);
static void UBTreeIntervalEdges(const FindSplitData *state, SplitPoint **leftinterval, SplitPoint **rightinterval);
static int UBTreeSplitPenalty(FindSplitData *state, SplitPoint *split);
static inline IndexTuple UBTreeSplitLastleft(FindSplitData *state, SplitPoint *split);
static inline IndexTuple UBTreeSplitFirstright(FindSplitData *state, SplitPoint *split);
    ...
    bool *newitemonleft, IndexTuple newitem)
{
    ...
    for (offnum = P_FIRSTDATAKEY(opaque); offnum <= maxoff; offnum = OffsetNumberNext(offnum)) {
        ...
        if (offnum > newitemoff)
            UBTreeRecsplitloc(&state, offnum, true, olddataitemstoleft, itemsz);

        else if (offnum < newitemoff)
            UBTreeRecsplitloc(&state, offnum, false, olddataitemstoleft, itemsz);
        else {
    ...
    if (newitemoff > maxoff)
        UBTreeRecsplitloc(&state, newitemoff, false, olddataitemstotal, 0);
    ...
    } else if (UBTreeAfternewitemoff(&state, maxoff, leaffillfactor, &usemult)) {
        /*
    ...
    UBTreeDeltasortsplits(&state, fillfactormult, usemult);
     ...
     */
    perfectpenalty = UBTreeStrategy(&state, &leftpage, &rightpage, &strategy);
     ...
     */
    foundfirstright = UBTreeBestsplitloc(&state, perfectpenalty, newitemonleft, strategy);
    pfree(state.splits);
...
static void UBTreeDeltasortsplits(FindSplitData *state, double fillfactormult, bool usemult)
{
    ...
    qsort(state->splits, state->nsplits, sizeof(SplitPoint), UBTreeSplitcmp);
}
...
static bool UBTreeAfternewitemoff(FindSplitData *state, OffsetNumber maxoff, int leaffillfactor, bool *usemult)
{
    ...
    if (!UBTreeAdjacenthtid(&tup->t_tid, &state->newitem->t_tid))
        return false;
    ...
    FindSplitStrat strategy)
{
    ...
    for (int i = lowsplit; i < highsplit; i++) {
        ...
        penalty = UBTreeSplitPenalty(state, state->splits + i);
...
static int UBTreeStrategy(FindSplitData *state, SplitPoint *leftpage, SplitPoint *rightpage, FindSplitStrat *strategy)
{
    ...
    *strategy = SPLIT_DEFAULT;
     ...
     */
    UBTreeIntervalEdges(state, &leftinterval, &rightinterval);
    leftmost = UBTreeSplitLastleft(state, leftinterval);
    rightmost = UBTreeSplitFirstright(state, rightinterval);
     ...
     */
    leftmost = UBTreeSplitLastleft(state, leftpage);
    rightmost = UBTreeSplitFirstright(state, rightpage);
...
static int UBTreeSplitPenalty(FindSplitData *state, SplitPoint *split)
{
    ...
    lastleftuple = UBTreeSplitLastleft(state, split);
    firstrighttuple = UBTreeSplitFirstright(state, split);
