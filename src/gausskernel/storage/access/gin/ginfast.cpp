...
static int32 writeListPage(Relation index, Buffer buffer, IndexTuple *tuples, int32 ntuples, BlockNumber rightlink)
{
    ...
    for (i = 0; i < ntuples; i++) {
        ...
        ret = memcpy_s(ptr, BLCKSZ - size, tuples[i], this_size);
        securec_check(ret, "", "");
        ptr += this_size;
        size += this_size;
...
void ginHeapTupleFastInsert(GinState *ginstate, GinTupleCollector *collector)
{
    ...
    if (separateList) {
        ...
        errno_t rc = EOK;
        rc = memset_s(&sublist, sizeof(GinMetaPageData), 0, sizeof(GinMetaPageData));
    ...
    if ((uint64_t)(metadata->nPendingPages * GIN_PAGE_FREESIZE) > (uint64_t)(cleanupSize * 1024L))
        needCleanup = true;
                      ...
                      IndexBulkDeleteResult *stats)
{
    ...
    do {
        ...
        Assert(metadata->nPendingPages >= (uint32)data.ndeleted);
                      ...
                      IndexBulkDeleteResult *stats)
{
    ...
    if (forceCleanup) {
        ...
        workMemory = u_sess->attr.attr_memory.maintenance_work_mem;
    } else {
        /*
        ...
        if (!ConditionalLockPage(index, GIN_METAPAGE_BLKNO, ExclusiveLock))
            return;
        workMemory = u_sess->attr.attr_memory.work_mem;
    }
...
Datum gin_clean_pending_list(PG_FUNCTION_ARGS)
{
    ...
    if (!pg_class_ownercheck(indexoid, GetUserId()))
        aclcheck_error(ACLCHECK_NOT_OWNER, ACL_KIND_CLASS, RelationGetRelationName(indexRel));
