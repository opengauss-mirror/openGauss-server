...
void spgRedoMoveLeafsOpratorSrcPage(
...
void spgRedoMoveLeafsOpratorParentPage(
...
void spgRedoAddNodeOperatorSrcPage(RedoBufferInfo* buffer, void* recorddata, BlockNumber blknoNew)
{
    ...
    if (PageAddItem(page, (Item)dt, dt->size, xldata->offnum, false, false) != xldata->offnum)
        ereport(ERROR,
            (errcode(ERRCODE_INDEX_CORRUPTED), errmsg("failed to add item of size %u to SPGiST index page", dt->size)));
...
void spgRedoAddNodeOperatorDestPage(
...
void spgRedoPickSplitRestoreLeafTuples(
...
void spgRedoPickSplitOperatorInnerPage(
...
void spgRedoVacuumLeafOperatorPage(RedoBufferInfo* buffer, void* recorddata)
{
    ...
    spgPageIndexMultiDelete(
        &state, page, toDead, xldata->nDead, SPGIST_DEAD, SPGIST_DEAD, InvalidBlockNumber, InvalidOffsetNumber);
