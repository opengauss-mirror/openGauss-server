...
XLogRecParseState* relmap_xlog_parse_to_block(XLogReaderState* record, uint32* blocknum)
{
    ...
    *blocknum = 0;
    ...
    XLogRecSetBlockCommonState(record, BLOCK_DATA_RELMAP_TYPE, forknum, lowblknum, NULL, recordstatehead, true);
...
XLogRecParseState* relmap_redo_parse_to_block(XLogReaderState* record, uint32* blocknum)
{
    ...
    *blocknum = 0;
    ...
    if (info == XLOG_RELMAP_UPDATE) {
        ...
        ereport(PANIC,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("relmap_redo_parse_to_block: unknown op code %u", info)));
    }
    return recordstatehead;
}