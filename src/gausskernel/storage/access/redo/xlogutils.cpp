...
void DoLsnCheck(RedoBufferInfo* bufferinfo, bool willInit, XLogRecPtr lastLsn)
{
    ...
    if (!(XLByteEQ(lastLsn, pageCurLsn))) {
        ...
        if (willInit) {
            ereport(DEBUG4,
                (errmodule(MOD_REDO),
                    ...
                    errmsg("new page lsn check error,lsn in record (%lu),lsn in current page %lu,"
                        ...
                        lastLsn,
                        pageCurLsn,
                        blockinfo->rnode.spcNode,
                        blockinfo->rnode.dbNode,
                        blockinfo->rnode.relNode,
                        blockinfo->forknum,
                        lsn,
                        blockinfo->blkno)));
            ...
            ereport(PANIC,
                (errmsg("lsn check error, lsn in record (%lu) ,lsn in current page %lu, "
                        "page info:%u/%u/%u forknum %d lsn %lu blknum:%u",
                    lastLsn,
                    pageCurLsn,
                    blockinfo->rnode.spcNode,
...
void XLogBlockInitRedoBufferInfo(
    XLogBlockHead* blockhead, RedoBufferInfo* bufferinfo, Buffer bufferid, void* page, Size pagesize)
{
    /* init blockinfo */
...
XLogRedoAction XLogCheckBlockDataRedoAction(XLogBlockDataParse* datadecode, RedoBufferInfo* bufferinfo)
{
    if (XLogBlockDataHasBlockImage(datadecode)) {
        ...
        if (imagedata == NULL) {
            ereport(
                ERROR, (errcode(ERRCODE_DATA_EXCEPTION), errmsg("XLogCheckRedoAction failed to restore block image")));
        } else {
...
void XLogRecSetBlockCommonState(XLogReaderState* record, XLogBlockParseEnum blockvalid, ForkNumber forknum,
    BlockNumber blockknum, RelFileNode* relnode, XLogRecParseState* recordblockstate, bool reforirecord)
{
...
void XLogBlockParseStateRelease_debug(XLogRecParseState* recordstate, const char* func, uint32 line)
{
    ...
    ereport(LOG,
        (errcode(ERRCODE_LOG),
            errmsg("XLogBlockParseStateRelease_debug %s, lsn %X/%X, line %u",
                func,
                (uint32)(recordstate->blockparse.blockhead.end_ptr >> U64_HIGH_BITS_OFFSET),
                (uint32)(recordstate->blockparse.blockhead.end_ptr),
                line)));
#endif
    do {
...
void XLogRecSetBlockDdlState(XLogBlockDdlParse* blockddlstate, uint32 blockddltype, uint32 columnrel, Oid ownerid)
{
    ...
    RedoBufferManager* buffermanager, RelFileNode relnode, ForkNumber forkNum, BlockNumber blockNum)
{
    ...
    buffdsc = (RedoBufferDesc*)((char*)buffermanager->BufferBlockPointers + memctl->itemsize * (allocslot->buf_id - 1));
...
XLogRecParseState* XLogParseBufferAllocList(
    RedoParseManager* parsemanager, XLogRecParseState* blkstatehead, void* record)
{
...
XLogRecParseState* XLogParseBufferCopy(XLogRecParseState* srcState)
{
    ...
    errno_t rc = memcpy_s(
        &newState->blockparse, sizeof(newState->blockparse), &srcState->blockparse, sizeof(srcState->blockparse));
    securec_check(rc, "\0", "\0");
    return newState;
}
...
void XLogParseBufferRelease(XLogRecParseState* recordstate)
{
    ...
    if (!RedoMemIsValid(memctl, descstate->buff_id)) {
        ereport(PANIC,
            (errmodule(MOD_REDO),
                errcode(ERRCODE_LOG),
                errmsg("XLogRedoBufferRelease failed!, taoalblknum:%u, buf_id:%u",
                    memctl->totalblknum,
                    descstate->buff_id)));
        /* panic */
    }
...
{
    if (XLogBlockHeadGetValidInfo(blockhead) != BLOCK_DATA_HEAP_TYPE) {
    ...
    switch (rmid) {
        case RM_HEAP_ID:
            heap_redo_data_block(blockhead, blockdatarec, bufferinfo);
            break;
        case RM_HEAP2_ID:
            heap2_redo_data_block(blockhead, blockdatarec, bufferinfo);
            break;
        case RM_HEAP3_ID:
            heap3_redo_data_block(blockhead, blockdatarec, bufferinfo);
            break;
        case RM_BTREE_ID:
            btree_redo_data_block(blockhead, blockdatarec, bufferinfo);
            break;
        case RM_XLOG_ID:
...
void XLogBlockVmCommonRedo(XLogBlockHead* blockhead, void* blockrecbody, RedoBufferInfo* bufferinfo)
{
    ...
    switch (rmid) {
        case RM_HEAP_ID:
            heap_redo_vm_block(blockhead, blockvmrec, bufferinfo);
            break;
        case RM_HEAP2_ID:
            heap2_redo_vm_block(blockhead, blockvmrec, bufferinfo);
            break;
        default:
            ereport(PANIC, (errmsg("XLogBlockDataCommonRedo: unknown rmid %u", rmid)));
    }
...
void XLogBlockDdlCommonRedo(XLogBlockHead* blockhead, void* blockrecbody, RedoBufferInfo* bufferinfo)
{
    ...
    switch (blockddlrec->blockddltype) {
        case BLOCK_DDL_CREATE_RELNODE:
            smgr_redo_create(rnode, blockhead->forknum);
            break;
...
void XLogBlockDdlDoRealAction(XLogBlockHead* blockhead, void* blockrecbody, RedoBufferInfo* bufferinfo)
{
    XLogBlockDdlParse* blockddlrec = (XLogBlockDdlParse*)blockrecbody;
...
static const XLogBlockRedoExtreRto XLogExtRtoRedoTable[BLOCK_DATA_CSNLOG_TYPE + 1] = {
    {XLogBlockDataCommonRedo, BLOCK_DATA_HEAP_TYPE},
    {XLogBlockVmCommonRedo, BLOCK_DATA_VM_TYPE},
    {XLogBlockFsmCommonRedo, BLOCK_DATA_FSM_TYPE},
    {XLogBlockDdlCommonRedo, BLOCK_DATA_DDL_TYPE},
    {XLogBlockBcmCommonRedo, BLOCK_DATA_BCM_TYPE},
    {XLogBlockNewCuCommonRedo, BLOCK_DATA_NEWCU_TYPE},
    {XLogBlockClogCommonRedo, BLOCK_DATA_CLOG_TYPE},
    {XLogBlockCsnLogCommonRedo, BLOCK_DATA_CSNLOG_TYPE},
};
    ...
    bool notfound, ReadBufferMethod readmethod)
{
    ...
    if (block_valid == BLOCK_DATA_HEAP_TYPE) {
        bool willinit;
        ...
        redoaction =
            XLogReadBufferForRedoBlockExtend(&bufferinfo->blockinfo, mode, false, bufferinfo, xlogLsn, readmethod);
    } else if (block_valid == BLOCK_DATA_VM_TYPE || block_valid == BLOCK_DATA_FSM_TYPE) {
        redoaction = XLogReadBufferForRedoBlockExtend(
            &bufferinfo->blockinfo, RBM_ZERO_ON_ERROR, false, bufferinfo, xlogLsn, readmethod);
...
void UpdateFsm(RedoBufferTag* blockInfo, Size freespace)
{
    ...
    XLogReadBufferForRedoBlockExtend(
        &fsmBufInfo.blockinfo, RBM_ZERO_ON_ERROR, false, &fsmBufInfo, InvalidXLogRecPtr, g_bufferreadmethod);
...
bool XLogBlockRedoForExtremeRTO(XLogRecParseState* redoblocktate, RedoBufferInfo* bufferinfo, bool notfound)
{
    ...
    if (block_valid <= BLOCK_DATA_FSM_TYPE) {
        Assert(block_valid == XLogExtRtoRedoTable[block_valid].block_valid);
        XLogExtRtoRedoTable[block_valid].xlog_redoextrto(blockhead, blockrecbody, bufferinfo);
    } else {
        ereport(WARNING,
            (errmsg("XLogBlockRedoForExtremeRTO: unsuport type %u, lsn %lu", (uint32)block_valid, blockhead->end_ptr)));
    }
    return false;
}

static const XLogParseBlock XLogParseBlockTable[RM_MAX_ID + 1] = {
    {xlog_redo_parse_to_block, RM_XLOG_ID},
    {xact_redo_parse_to_block, RM_XACT_ID},
    {smgr_redo_parse_to_block, RM_SMGR_ID},
    {clog_redo_parse_to_block, RM_CLOG_ID},
    {dbase_redo_parse_to_block, RM_DBASE_ID},
    ...
    {NULL, RM_STANDBY_ID},
    {heap2_redo_parse_to_block, RM_HEAP2_ID},
    {heap_redo_parse_to_block, RM_HEAP_ID},
    {btree_redo_parse_to_block, RM_BTREE_ID},
    {hash_redo_parse_to_block, RM_HASH_ID},
    ...
    {heap3_redo_parse_to_block, RM_HEAP3_ID},
...
inline XLogRecParseState* XLogParseToBlockCommonFunc(XLogReaderState* record, uint32* blocknum)
{
    ...
    *blocknum = 0;
    ...
    Assert(rmid == XLogParseBlockTable[rmid].rmid);
    if (XLogParseBlockTable[rmid].xlog_parseblock != NULL)
        blockparsestate = XLogParseBlockTable[rmid].xlog_parseblock(record, blocknum);
