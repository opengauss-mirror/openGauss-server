 ...
 *
 * IDENTIFICATION
 *  src/gausskernel/storage/access/redo/bufpage.cpp
 *
 * -------------------------------------------------------------------------
 */
#include "postgres.h"
...
void PageIndexMultiDelete(Page page, OffsetNumber* itemnos, int nitems)
{
    ...
    if ((unsigned int)(pd_lower) < GetPageHeaderSize(page) || pd_lower > pd_upper || pd_upper > pd_special ||
        pd_special > BLCKSZ || (unsigned int)(pd_special) != MAXALIGN(pd_special))
        ereport(ERROR,
            (errcode(ERRCODE_DATA_CORRUPTED),
                errmsg(
                    "corrupted page pointers: lower = %d, upper = %d, special = %d", pd_lower, pd_upper, pd_special)));
    ...
    for (offnum = FirstOffsetNumber; offnum <= nline; offnum = OffsetNumberNext(offnum)) {
        ...
        if (offset < (unsigned int)(pd_upper) || (offset + size) > (unsigned int)(pd_special) ||
    ...
    if (nextitm != nitems)
        ereport(ERROR,
            (errcode(ERRCODE_INVALID_ROW_COUNT_IN_RESULT_OFFSET_CLAUSE), errmsg("incorrect index offsets supplie")));
    ...
    for (i = 0, itemidptr = itemidbase; i < nused; i++, itemidptr++) {
        ...
        rc = memmove_s(
            (char*)page + upper, itemidptr->alignedlen, (char*)page + itemidptr->itemoff, itemidptr->alignedlen);
...
void PageIndexTupleDelete(Page page, OffsetNumber offnum)
{
    ...
    if ((int)offnum <= 0 || (int)offnum > nline)
        ereport(ERROR,
            (errcode(ERRCODE_INVALID_ROW_COUNT_IN_RESULT_OFFSET_CLAUSE), errmsg("invalid index offnum: %u", offnum)));
    ...
    if (nstorage == 0) {
        ...
        for (i = 0; i < nline; i++) {
            ...
            if (ItemIdHasStorage(lp)) {
                ...
                if (itemidptr->itemoff < (int)pd_upper || itemidptr->itemoff >= (int)pd_special)
                    ereport(ERROR,
...
static bool PageCheckAndFreeDict(Page page)
{
    ...
    pd_upper = phdr->pd_upper + dictsize;
    tupsSize = pageSize - dictsize - phdr->pd_upper;
    if (tupsSize > 0) {
        rc = memmove_s((char*)page + pd_upper, tupsSize, (char*)page + phdr->pd_upper, tupsSize);
        securec_check(rc, "\0", "\0");
    }
...
Page PageGetTempPageCopySpecial(Page page, bool isbtree)
{
    ...
    rc = memcpy_s(
        PageGetSpecialPointer(temp), PageGetSpecialSize(page), PageGetSpecialPointer(page), PageGetSpecialSize(page));
    securec_check(rc, "\0", "\0");
