 ...
 * IDENTIFICATION
 *
 * src/gausskernel/storage/access/redo/csnlog.cpp
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
{
    int64 pageno = TransactionIdToCSNPage(xid); /* get page of parent */
    ...
    for (int i = 0; i < nsubxids; i++) {
        if (((int64)TransactionIdToCSNPage(subxids[i]) != pageno) || (xidnum == MAX_BLOCK_XID_NUMS)) {
