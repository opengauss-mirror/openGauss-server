 ...
 * IDENTIFICATION
 *
 * src/gausskernel/storage/access/redo/dbcommands.cpp
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
XLogRecParseState* database_xlog_common_parse_to_block(XLogReaderState* record, uint32* blocknum)
{
    ...
    XLogRecSetBlockCommonState(
        record, blockparsetype, MAIN_FORKNUM, InvalidBlockNumber, &rnode, recordstatehead, false);

    XLogRecSetBlockDataBaseState(
        &(recordstatehead->blockparse.extra_rec.blockdatabase), srcdatabaseid, srctablespaceid);
...
XLogRecParseState* dbase_redo_parse_to_block(XLogReaderState* record, uint32* blocknum)
{
    ...
    *blocknum = 0;
    ...
    if ((info == XLOG_DBASE_CREATE) || (info == XLOG_DBASE_DROP)) {
        recordstatehead = database_xlog_common_parse_to_block(record, blocknum);
    } else {
        ereport(PANIC,
            (errcode(ERRCODE_UNRECOGNIZED_NODE_TYPE), errmsg("dbase_redo_parse_to_block: unknown op code %u", info)));
    }
    return recordstatehead;
}
