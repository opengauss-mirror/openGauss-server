 ...
 * IDENTIFICATION
 *
 * src/gausskernel/storage/access/redo/clog.cpp
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
XLogRecParseState* clog_xlog_ddl_parse_to_block(XLogReaderState* record, uint32* blocknum)
{
...
XLogRecParseState* clog_redo_parse_to_block(XLogReaderState* record, uint32* blocknum)
{
    ...
    *blocknum = 0;
    ...
    if ((info == CLOG_ZEROPAGE) || (info == CLOG_TRUNCATE)) {
        recordstatehead = clog_xlog_ddl_parse_to_block(record, blocknum);
    } else {
        ereport(PANIC, (errmsg("clog_redo_parse_to_block: unknown op code %u", info)));
    }
    ...
    uint32* blocknum, TransactionId xid, int nsubxids, TransactionId* subxids, CLogXidStatus status)
{
    ...
    for (int i = 0; i < nsubxids; i++) {
        if (((int64)TransactionIdToPage(subxids[i]) != pageno) || (xidnum == MAX_BLOCK_XID_NUMS)) {
