...
void XLogRecSetXlogCommonState(
    XLogBlockXLogComParse* blockxlogstate, char* maindata, Size maindatalen, XLogRecPtr readptr)
{
...
XLogRecParseState* xlog_xlog_common_parse_to_block(XLogReaderState* record, uint32* blocknum)
{
    ...
    XLogRecSetBlockCommonState(
        record, BLOCK_DATA_XLOG_COMMON_TYPE, InvalidForkNumber, InvalidBlockNumber, NULL, recordstatehead, true);
    char* maindata = XLogRecGetData(record);
    Size maindatalen = XLogRecGetDataLen(record);
    XLogRecSetXlogCommonState(
        &recordstatehead->blockparse.extra_rec.blockxlogcommon, maindata, maindatalen, record->ReadRecPtr);
    return recordstatehead;
}
