 ...
 * **************************************************
 */
XLogRecParseState* xact_redo_rmddl_parse_to_block(
    XLogReaderState* record, XLogRecParseState* recordstatehead, uint32* blocknum, ColFileNodeRel* xnodes, int nrels)
{
        ...
        if (!IsValidColForkNum(node.forknum)) {
            ...
            if (SUPPORT_DFS_BATCH && ((IS_PGXC_COORDINATOR && IsValidPaxDfsForkNum(node.forknum)) ||
                ...
                XLogRecSetBlockCommonState(
                    record, BLOCK_DATA_DDL_TYPE, fork, InvalidBlockNumber, &node.filenode, blockstate);
                XLogRecSetBlockDdlState(&(blockstate->blockparse.extra_rec.blockddlrec),
                ...
                XLogRecSetBlockCommonState(
                    record, BLOCK_DATA_DDL_TYPE, MAX_FORKNUM, InvalidBlockNumber, &node.filenode, blockstate);
                XLogRecSetBlockDdlState(&(blockstate->blockparse.extra_rec.blockddlrec),
                    BLOCK_DDL_DROP_RELNODE,
                    false,
                    node.ownerid);
...
static XLogRecParseState* xact_xlog_commit_internal_parse(XLogReaderState* record, uint32* blocknum)
{
    ...
    *blocknum = 0;
    ...
    recordstatehead = xact_xlog_clog_parse_to_block(
        record, recordstatehead, blocknum, xid, nsubxacts, sub_xids, CLOG_XID_STATUS_COMMITTED);
    ...
    recordstatehead =
        xact_xlog_csnlog_parse_to_block(record, blocknum, xid, nsubxacts, sub_xids, commitseql, recordstatehead);
    if (recordstatehead == NULL) {
        return NULL;
    }
...
static XLogRecParseState* xact_xlog_abort_internal_parse(XLogReaderState* record, uint32* blocknum)
{
    ...
    *blocknum = 0;
    ...
    recordstatehead = xact_xlog_clog_parse_to_block(
        record, recordstatehead, blocknum, xid, nsubxacts, sub_xids, CLOG_XID_STATUS_ABORTED);
    if (recordstatehead == NULL) {
        return NULL;
    }

    recordstatehead = xact_xlog_csnlog_parse_to_block(
        record, blocknum, xid, nsubxacts, sub_xids, COMMITSEQNO_ABORTED, recordstatehead);
    if (recordstatehead == NULL) {
        return NULL;
    }
...
static XLogRecParseState* xact_xlog_prepare_internal_parse(XLogReaderState* record, uint32* blocknum)
{
    ...
    *blocknum = 0;
    if (TransactionIdIsNormal(hdr->xid)) {
        ...
        recordstatehead = xact_xlog_csnlog_parse_to_block(
            record, blocknum, xid, nsubxacts, sub_xids, COMMITSEQNO_COMMIT_INPROGRESS, recordstatehead);
        if (recordstatehead == NULL) {
            return NULL;
        }
    }
...
XLogRecParseState* xact_redo_parse_to_block(XLogReaderState* record, uint32* blocknum)
{
    ...
    if ((info == XLOG_XACT_COMMIT_COMPACT) || (info == XLOG_XACT_COMMIT) || (info == XLOG_XACT_COMMIT_PREPARED)) {
        ...
        ereport(PANIC,
            (errcode(ERRCODE_INVALID_TRANSACTION_STATE), errmsg("xact_redo_parse_to_block: unknown op code %u", info)));
    ...
    uint32* blocknum, TransactionId maxxid, CommitSeqNo maxseqnum)
{
    ...
    XLogRecSetBlockCommonState(
        record, BLOCK_DATA_XACTDATA_TYPE, InvalidForkNumber, InvalidBlockNumber, &relnode, blockstate, true);

    XLogRecSetXactCommonState(&blockstate->blockparse.extra_rec.blockxact, info, xrecinfo, xact_time);

    XLogRecSetXactRecoveryState(
        &blockstate->blockparse.extra_rec.blockxact, maxxid, maxseqnum, delayddlflag, updateminrecovery);
    XLogRecSetXactDdlState(&blockstate->blockparse.extra_rec.blockxact,
        nrels,
        (void*)xnodes,
        invalidmsgnum,
        (void*)inval_msgs,
    ...
    uint32* blocknum, TransactionId maxxid, CommitSeqNo maxseqnum)
{
    ...
    XLogRecSetBlockCommonState(
        record, BLOCK_DATA_XACTDATA_TYPE, InvalidForkNumber, InvalidBlockNumber, NULL, blockstate, true);
    ...
    XLogRecSetXactDdlState(
        &blockstate->blockparse.extra_rec.blockxact, nrels, (void*)xnodes, 0, NULL, nlibrary, (void*)libfilename);
...
void XLogRecSetPreparState(
    XLogBlockPrepareParse* blockpreparestate, TransactionId maxxid, char* maindata, Size maindatalen)
{
    ...
    XLogReaderState* record, XLogRecParseState* recordstatehead, uint32* blocknum, TransactionId maxxid)
{
    ...
    XLogRecSetBlockCommonState(
        record, BLOCK_DATA_PREPARE_TYPE, InvalidForkNumber, InvalidBlockNumber, NULL, blockstate, true);
