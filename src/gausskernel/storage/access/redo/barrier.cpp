 ...
 *
 * IDENTIFICATION
 *   src/gausskernel/storage/access/redo/barrier.cpp
 *
 * -------------------------------------------------------------------------
 */
...
XLogRecParseState* barrier_redo_parse_to_block(XLogReaderState* record, uint32* blocknum)
{
    ...
    *blocknum = 1;
    ...
    XLogRecSetBlockCommonState(record, BLOCK_DATA_BARRIER_TYPE, forknum, lowblknum, NULL, recordstatehead, false);

    XLogRecSetBarrierState(
        &(recordstatehead->blockparse.extra_rec.blockbarrier), record->ReadRecPtr, record->EndRecPtr);
