    ...
    if (!t_thrd.xlog_cxt.InRecovery) {
        if (!TransactionIdEquals(oldXid, newXid)) {
            ...
            ereport(ERROR, (
                ...
                errmodule(MOD_UPAGE),
                errmsg(USTOREFORMAT("[SHIFT-XIDBASE:PAGEVERIFY]The tuple xid is inconsistent after the xid base"
                    " is adjusted, blkno %u, offset %u, oldXid/newXid %lu/%lu, base from %lu to %lu (delta %ld)"),
...
bool UHeapPagePrepareForXid(Relation relation, Buffer buffer, TransactionId xid, bool pageReplication, bool multi)
{
    ...
    for (int i = 0; i <= 1; i++) {
        ...
        if (UHeapPageGetMaxOffsetNumber(page) == InvalidOffsetNumber && !multi) {
            ...
            ereport(LOG, (
                errmodule(MOD_UPAGE),
                errmsg("Relation is %s, prepare xid %lu, page min xid: %lu, page max xid: %lu",
    ...
    ereport(ERROR, (
        errcode(ERRCODE_CANNOT_MODIFY_XIDBASE),
        errmodule(MOD_UPAGE),
        errmsg(USTOREFORMAT("Can't fit xid into page. relation \"%s\", now xid is %lu, "
            "base is %lu, min is %u, max is %u, pageReplication is %u, multi is %u"),
            RelationGetRelationName(relation), xid, base, min, max, (uint32)pageReplication, (uint32)multi)));
    return false;
}
...
Oid UHeapInsert(RelationData *rel, UHeapTupleData *utuple, CommandId cid, BulkInsertState bistate, bool isToast)
{
    ...
    t_thrd.ustore_cxt.rnode = (rel == NULL ? NULL : (&(rel->rd_node)));
    WHITEBOX_TEST_STUB(UHEAP_INSERT_FAILED, WhiteboxDefaultErrorEmit);
    if (utuple == NULL) {
        ereport(ERROR, (
            errcode(ERRCODE_INVALID_PARAMETER_VALUE),
            errmodule(MOD_UPAGE),
            errmsg(USTOREFORMAT("The insert tuple is NULL, isToast is %u, cid is %u."), (uint32)isToast, cid)));
    }
...
reacquire_buffer:
    ...
    ereport(DEBUG5, (
        errmodule(MOD_UPAGE),
        errmsg("Ins1: Rel: %s, Buf: %d, Space: %d, tuplen: %d",
    ...
    ereport(DEBUG5, (
        errmodule(MOD_UPAGE),
        errmsg("Ins2: Rel: %s, Buf: %d, Space: %d, tuplen: %d",
        RelationGetRelationName(rel), buffer, phdr->pd_upper - phdr->pd_lower, tuple->disk_tuple_size)));
                                             ...
                                             Buffer buffer, UHeapDiskTupleData hdr)
{
    ...
    while (true) {
        ...
        if (rc == UNDO_TRAVERSAL_ABORT) {
            ...
            ereport(ERROR, (
                errmodule(MOD_UNDO),
                errmsg(USTOREFORMAT("snapshot too old! the undo record has been force discard. "
...
void RelationPutUTuple(Relation relation, Buffer buffer, UHeapTupleData *tuple)
{
    ...
    offNum = UPageAddItem(relation, &bufpage, (Item)tuple->disk_tuple,
        tuple->disk_tuple_size, InvalidOffsetNumber, false);
    if (offNum == InvalidOffsetNumber) {
        PrevDumpUPage((relation == NULL ? NULL : (&(relation->rd_node))), BufferGetPage(buffer), blockno, "uheap");
        ereport(PANIC, (
            errcode(ERRCODE_DATA_CORRUPTED),
            errmodule(MOD_UPAGE),
            errmsg(USTOREFORMAT("failed to add tuple to upage: xid %lu, oid %u, blockno %u."
            "tuple disk_tuple_size is %u, tuple->tupTableType is %u"),
            GetTopTransactionId(), RelationGetRelid(relation), blockno, (uint32)tuple->disk_tuple_size,
            tuple->tupTableType)));
    }
    ItemPointerSet(&(tuple->ctid), blockno, offNum);
}
    ...
    Buffer buffer, TransactionId priorXmax, Snapshot snapshot)
{
    ...
    do {
        ...
        if (rc == UNDO_TRAVERSAL_ABORT) {
            ...
            ereport(ERROR, (
                errmodule(MOD_UNDO),
                errmsg("snapshot too old! the undo record has been force discard. "
                "Reason: EPQ Internal. "
                "LogInfo: undo state %d, tuple flag %u. "
                "TDInfo: tdxid %lu, tdid %d, undoptr %lu. "
                "TransInfo: xid %lu, oid %u, tid(%u, %u), lastXid %lu, "
                ...
                case TM_Updated:
                    if (IsolationUsesXactSnapshot()) {
                        ...
                        ereport(ERROR, (
                            ...
                            errmodule(MOD_UPAGE),
                            errmsg(USTOREFORMAT("could not serialize access due to concurrent update."))));
                    }
    ...
    bool *hasTupLock, bool *multixidIsMySelf, int waitSec)
{
    ...
    if (UHeapTupleHasMultiLockers(utuple->disk_tuple->flag)) {
        ...
        ereport(DEBUG5, (
            errmodule(MOD_UPAGE),
            errmsg("curxid %ld, uheaptuple(flag=%d, tid(%d,%d)), wait multixid = %ld, xid is %d, multibase is %ld",
            ...
            if (nowait) {
                if (!ConditionalLockTuple(relation, &(utuple->ctid), tupleLockType)) {
                    ereport(ERROR, (
                        ...
                        errmodule(MOD_UPAGE),
                        errmsg(USTOREFORMAT("could not obtain lock on row in relation \"%s\", tupleLockType is %d, "
                        "tid(%d,%d)."),
                        RelationGetRelationName(relation), tupleLockType, ItemPointerGetBlockNumber(&utuple->ctid),
                        ItemPointerGetOffsetNumber(&utuple->ctid))));
                }
            } else {
        ...
        if (nowait) {
            if (!ConditionalMultiXactIdWait((MultiXactId)xwait, GetMXactStatusForLock(mode, false), NULL)) {
                ereport(ERROR, (
                    ...
                    errmodule(MOD_UPAGE),
                    errmsg(USTOREFORMAT("could not obtain lock on row in relation \"%s\", tupleLockType is %d, "
                    "tid(%d,%d), xwait is %lu, mode is %d."),
                    RelationGetRelationName(relation), tupleLockType, ItemPointerGetBlockNumber(&utuple->ctid),
                    ItemPointerGetOffsetNumber(&utuple->ctid), (MultiXactId)xwait, mode)));
            }
        } else {
        ...
        if (IsLockModeConflicting(curMode, mode)) {
            ...
            ereport(DEBUG5, (
                errmodule(MOD_UPAGE),
                errmsg("curxid %ld, uheaptuple(flag=%d, tid(%d,%d)), wait (xid = %ld subxid = %ld)",
                ...
                if (InvalidSubTransactionId != subXid) {
                    if (!ConditionalSubXactLockTableWait(topXid, subXid)) {
                        ereport(ERROR, (
                            ...
                            errmodule(MOD_UPAGE),
                            errmsg(USTOREFORMAT("could not obtain lock on row in relation \"%s\", tupleLockType is %d, "
                            "tid(%d,%d), topXid is %lu, subXid is %lu"),
                            RelationGetRelationName(relation), tupleLockType, ItemPointerGetBlockNumber(&utuple->ctid),
                            ItemPointerGetOffsetNumber(&utuple->ctid), topXid, subXid)));
                    }
                    ...
                    if (!ConditionalXactLockTableWait(topXid)) {
                        ereport(ERROR, (
                            errcode(ERRCODE_LOCK_NOT_AVAILABLE),
                            errmodule(MOD_UPAGE),
                            errmsg(USTOREFORMAT("could not obtain lock on row in relation \"%s\", "
                            "topXid is %lu, subXid is %lu"), RelationGetRelationName(relation), topXid, subXid)));
                    }
                }
            } else {
                if (InvalidSubTransactionId != subXid) {
    ...
    RowPtr *rp)
{
    ...
    if (mode == LockTupleExclusive) {
        ...
        if (SINGLE_LOCKER_XID_IS_SHR_LOCKED(oldinfomask) && TransactionIdIsInProgress(xidOnTup)) {
            ...
            ereport(DEBUG5, (
                errmodule(MOD_UPAGE),
                errmsg("locker %ld + locker %ld = multi %ld", curxid, xidOnTup, xid)));
            ...
            ereport(DEBUG5, (
                errmodule(MOD_UPAGE),
                errmsg("single shared locker %lu", xid)));
        }
    } else {
        Assert(0);
    }
    ...
    int waitSec)
{
    ...
    if (mode == LockTupleShared && !isSelectForUpdate) {
        ereport(ERROR, (
            ...
            errmsg("For Key Share and For No Key Update is not support for ustore.")));
    }
...
check_tup_satisfies_update:
    ...
    if (result == TM_Invisible) {
        PrevDumpUPage(t_thrd.ustore_cxt.rnode, page, blkno, "uheap");
        UHeapResetRelnode();
        UnlockReleaseBuffer(*buffer);
        ereport(defence_errlevel(), (
            errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
            errmodule(MOD_UPAGE),
            errmsg(USTOREFORMAT("attempted to lock invisible tuple"))));
    } else if (result == TM_BeingModified || result == TM_Ok) {
#ifdef ENABLE_WHITEBOX
        if (result != TM_Ok) {
            ereport(WARNING, (
                errmodule(MOD_UPAGE),
                errmsg("UHeapLockTuple returned %d", result)));
        }
#endif
    ...
    *relreplident = REPLICA_IDENTITY_NOTHING;
    ...
    if (!HeapTupleIsValid(tuple)) {
        ereport(ERROR, (
            errcode(ERRCODE_T_R_SERIALIZATION_FAILURE),
            errmodule(MOD_UPAGE),
            errmsg(USTOREFORMAT("pg_class entry for relid %u vanished during UHeapExtractReplicaIdentity."),
                relid)));
    }
    ...
    for (int natt = 0; natt < IndexRelationGetNumberOfKeyAttributes(indexRel); natt++) {
        ...
        if (attno < 0) {
            ...
            ereport(ERROR, (
                errcode(ERRCODE_DATA_CORRUPTED),
                errmodule(MOD_UPAGE),
                errmsg(USTOREFORMAT("system column in index, attno is %d"), attno)));
        }
        nulls[attno - 1] = false;
    }
...
check_tup_satisfies_update:
    ...
    if (result == TM_Invisible) {
        ...
        ereport(defence_errlevel(), (
            errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
            errmodule(MOD_UPAGE),
            errmsg(USTOREFORMAT("UHeapDelete: attempted to delete invisible tuple, tdinfo.xid is %lu"), tdinfo.xid)));
    } else if ((result == TM_BeingModified) && wait) {

#ifdef ENABLE_WHITEBOX
        ereport(WARNING, (
            errmodule(MOD_UPAGE),
            errmsg("UHeapDelete returned %d", result)));
#endif
        ...
        ereport(PANIC, (
            errmodule(MOD_USTORE),
            errmsg(USTOREFORMAT("UHeapDelete error. tuple flag %hu, xid %lu, oid %u,"
                "blockno %u offsetnum %hu, tdinfo.xid %lu."),
                utuple.disk_tuple->flag, GetTopTransactionId(), RelationGetRelid(relation),
                BufferGetBlockNumber(buffer), offnum, tdinfo.xid)));
    }
...
static bool PutInplaceUpdateTuple(Page page, UHeapTuple oldTup, UHeapTuple newTup, RowPtr *rp)
{
    /*
     ...
     */
    if (newTup->disk_tuple_size > RowPtrGetLen(rp)) {
        if (!RowPtrChangeLen(page, ItemPointerGetBlockNumber(&(oldTup->ctid)),
            rp, newTup->disk_tuple_size)) {
            return false;
        }
    }
...
void PutLinkUpdateTuple(Page page, Item item, RowPtr *lp, Size size)
{
    ...
    if (upper < uphdr->pd_lower || (int)uphdr->pd_upper < (int)alignedSize) {
        ...
        ereport(PANIC, (
            errmodule(MOD_UPAGE),
            errmsg(USTOREFORMAT("upper=%d, lower=%d, size=%d, tuplesize=%d, newupper=%d."
            "rowpointer->offset is %u, rowpointer->len is %u, rowpointer->flags is %u."),
            (int)uphdr->pd_upper, (int)uphdr->pd_lower, (int)size, (int)alignedSize, upper,
            lp->offset, lp->len, lp->flags)));
    }
...
bool VerifyRowPtr(Page page, OffsetNumber targetOffnum, Size newTupSize)
{
    ...
    for (i = FirstOffsetNumber; i <= nline; i++) {
        ...
        if (RowPtrIsNormal(rp)) {
            ...
            if (i == targetOffnum) {
                rpSortPtr->end = rpSortPtr->start + (int)newTupSize;
            } else {
            ...
            if (rpSortPtr->start < pdUpper || rpSortPtr->end > pdSpecial) {
                ereport(WARNING, (
                    errcode(ERRCODE_DATA_CORRUPTED),
                    errmodule(MOD_UPAGE),
                    errmsg(USTOREFORMAT("[PAGE_CORRUPT:PAGEVERIFY] Corrupted line pointer: offset = %d, rpstart = %u, "
                    "rplen = %u, pdlower = %d, pdupper = %d. targetOffnum = %d."),
    ...
    for (i = 0; i < nstorage - 1; i++) {
        ...
        if (tempPtr1->end > tempPtr2->start) {
            ereport(WARNING, (
                errcode(ERRCODE_DATA_CORRUPTED),
                errmodule(MOD_UPAGE),
                errmsg(USTOREFORMAT("[PAGE_CORRUPT:PAGEVERIFY] Corrupted line pointer: "
                "rp1offset %d, rp1start = %d, rp1end = %d, rp2offset = %d, rp2start = %d, rp2end = %d., "
                "targetOffnum = %d"), tempPtr1->offset, tempPtr1->start, tempPtr1->end,
...
check_tup_satisfies_update:
    ...
    if (result == TM_Invisible) {
        ...
        ereport(defence_errlevel(), (
            errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
            errmodule(MOD_UPAGE),
            errmsg(USTOREFORMAT("attempted to update invisible tuple, tdif.id is %lu"), txactinfo.xid)));
    } else if ((result == TM_BeingModified) && wait) {
#ifdef ENABLE_WHITEBOX
        ereport(WARNING, (
            errmodule(MOD_UPAGE),
            errmsg("UHeapUpdate returned %d", result)));
#endif
    ...
    */
    ...
    if (result != TM_Ok) {
        ...
        if (!RowPtrIsDeleted(lp) && oldtup.disk_tuple == NULL) {
            PrevDumpUTuple(&oldtup);
            ereport(ERROR, (
                errcode(ERRCODE_INTERNAL_ERROR),
                errmodule(MOD_UPAGE),
                errmsg(USTOREFORMAT("Undelete tuple is NULL, lp->flags is %u, lp->offset is %u, lp->len is %u"),
                    PtrGetVal(lp, flags), PtrGetVal(lp, offset), PtrGetVal(lp, len))));
        }
    ...
    if (needToast) {
        ...
        updateTrace |= UPDATE_SHORTER;
        useInplaceUpdate = true;
    } else {
        updateTrace |= UPDATE_LONGER;
    }
    ...
    if (!allow_inplace_update || needToast) {
        updateTrace |= UPDATE_NON_INPLACE;
        useInplaceUpdate = false;
        useLinkUpdate = false;
    } else if (!useInplaceUpdate) {
        updateTrace |= UPDATE_PRUNE;
        pruned = UHeapPagePruneOpt(relation, buffer, oldOffnum, newtupsize - oldtupsize);
        lp = UPageGetRowPtr(page, oldOffnum);
        if (pruned && VerifyRowPtr(page, oldOffnum, newtupsize)) {
            updateTrace |= UPDATE_PRUNE_SUCESS;
            useInplaceUpdate = true;
        }
    } else {
        updateTrace |= UPDATE_INPLACE;
    }

    /* The page might have been modified, so refresh disk_tuple */
    lp = UPageGetRowPtr(page, oldOffnum);
    oldtup.disk_tuple = (UHeapDiskTuple)UPageGetRowData(page, lp);
    if (useInplaceUpdate && RowPtrGetOffset(lp) + SHORTALIGN(newtup->disk_tuple_size) > BLCKSZ) {
        updateTrace |= UPDATE_VERIFY_FAIL;
        PrevDumpUPage(t_thrd.ustore_cxt.rnode, page, block, "uheap");
        ereport(WARNING, (
            errcode(ERRCODE_DATA_CORRUPTED),
            errmodule(MOD_UPAGE),
            errmsg(USTOREFORMAT("invalid inplace update1: ctid(%u, %u). "
            "newtupsize:%u, oldtupsize:%u, "
            "uheaptupsize:%u, newdisktupsize:%u, olddisktupsize:%u, lpsize:%u, "
            "lpoffset:%u, pruned:%d, updatetrace %d."),
            block, oldOffnum, (uint32)newtupsize, (uint32)oldtupsize, 0,
            newtup->disk_tuple_size, oldtup.disk_tuple_size, (uint32)RowPtrGetLen(lp),
            (uint32)RowPtrGetOffset(lp), pruned, updateTrace)));
        useInplaceUpdate = false;
    }
    oldtup.disk_tuple_size = RowPtrGetLen(lp);
    ...
    reacquire_buffer:
        ...
        t_thrd.ustore_cxt.oldBuffer = newbuf;
        /*
        ...
        if (oldblk == newblk) {
            ...
            if (lower < ((UHeapPageHeaderData *)npage)->pd_lower) {
                ereport(DEBUG5, (
                    errmodule(MOD_UPAGE),
                    errmsg("Do Reacquire1 Rel: %s, lower: %d, new_lower: %d, newbuf: %d",
            ...
            if (obufLower < ((UHeapPageHeaderData *)page)->pd_lower ||
                ...
                ereport(DEBUG5, (
                    errmodule(MOD_UPAGE),
                    errmsg("Do Reacquire2 Rel: %s, olower: %d, onew_lower: %d, nlower: %d, "
                    "nnew_lower: %d, oldbuf:%d, newbuf: %d",
    ...
    if (useInplaceUpdate && RowPtrGetOffset(lp) + SHORTALIGN(uheaptup->disk_tuple_size) > BLCKSZ) {
        PrevDumpUTuple(uheaptup);
        PrevDumpUPage(t_thrd.ustore_cxt.rnode, page, BufferGetBlockNumber(buffer), "uheap");
        ereport(ERROR, (
            errcode(ERRCODE_DATA_CORRUPTED),
            errmodule(MOD_UPAGE),
            errmsg(USTOREFORMAT("[PAGE_CORRUPT:UPDATE]invalid inplace update2: ctid(%u, %u). "
            "newtupsize:%u, oldtupsize:%u, "
            "uheaptupsize:%u, newdisktupsize:%u, olddisktupsize:%u, lpsize:%u, "
            "lpoffset:%u, pruned:%d, updatetrace %d."),
            block, oldOffnum, (uint32)newtupsize, (uint32)oldtupsize,
            uheaptup->disk_tuple_size, newtup->disk_tuple_size, oldtup.disk_tuple_size,
            (uint32)RowPtrGetLen(lp), (uint32)RowPtrGetOffset(lp), pruned, updateTrace)));
    }
    ...
    if (useInplaceUpdate) {
        ...
        if (undoXorDeltaSize != undorec->Rawdata()->len) {
            ereport(PANIC, (
                errmodule(MOD_USTORE),
                errmsg(USTOREFORMAT("xid %lu, oid %u, ctid(%u, %u). "
        ...
        if (!useLinkUpdate) {
            if (!PutInplaceUpdateTuple(page, &oldtup, uheaptup, lp)) {
                PrevDumpUPage(t_thrd.ustore_cxt.rnode, page, BufferGetBlockNumber(buffer), "uheap");
                ereport(PANIC, (
                    errcode(ERRCODE_DATA_CORRUPTED),
                    errmodule(MOD_UPAGE),
                    errmsg(USTOREFORMAT("invalid inplace update3: ctid(%u, %u). "
                    "newtupsize:%u, oldtupsize:%u, "
                    "uheaptupsize:%u, newdisktupsize:%u, olddisktupsize:%u, lpsize:%u, "
                    "lpoffset:%u, pruned:%d, updatetrace %d."),
                    block, oldOffnum, (uint32)newtupsize, (uint32)oldtupsize,
    ...
    if (unlikely(ConstructUstoreVerifyParam(USTORE_VERIFY_MOD_UPAGE, USTORE_VERIFY_COMPLETE,
        (char *) &verifyParams, relation, page, block, ItemPointerGetOffsetNumber(&oldtup.ctid),
        NULL, NULL, InvalidXLogRecPtr, NULL, NULL, DML_VERIFY))) {
        ExecuteUstoreVerify(USTORE_VERIFY_MOD_UPAGE, (char *) &verifyParams);
    }
...
reacquire_buffer:
        ...
        for (i = 0; i < ufreeOffsetRanges->nranges; i++) {
            ...
            ereport(DEBUG1, (
                errmodule(MOD_UPAGE),
                errmsg("start offset: %d, end offset: %d", ufreeOffsetRanges->startOffset[i],
    ...
    bool *lockReacquired, Buffer otherBuf, TransactionId *minXid, bool aggressiveSearch)
{
    ...
    *minXid = currMinXid;
    ...
    if (!aggressiveSearch && tdCount >= TD_THRESHOLD_FOR_PAGE_SWITCH) {
        ...
        ereport(DEBUG5, (
            errmodule(MOD_UPAGE),
            errmsg("Could not extend TD slots beyond threshold Rel: %s, blkno: %d",
    ...
    if (nExtended > 0) {
        ...
        ereport(DEBUG5, (
            errmodule(MOD_UPAGE),
            errmsg("TD array extended by %d slots for Rel: %s, blkno: %d",
    ...
    ereport(DEBUG5, (
        errmodule(MOD_UPAGE),
        errmsg("Could not extend TD array for Rel: %s, blkno: %d",
        RelationGetRelationName(relation), BufferGetBlockNumber(buf))));
...
cleanup:
    UPageVerifyParams verifyParams;
    if (unlikely(ConstructUstoreVerifyParam(USTORE_VERIFY_MOD_UPAGE, USTORE_VERIFY_COMPLETE,
        (char *) &verifyParams, relation, page, BufferGetBlockNumber(buf), InvalidOffsetNumber,
...
void GetTDSlotInfo(Buffer buf, int tdId, UHeapTupleTransInfo *tdinfo)
{
    if (tdId == UHEAPTUP_SLOT_FROZEN) {
        ...
        if (tdId < 1 || tdId > phdr->td_count) {
            ...
            ereport(ERROR, (
                errmodule(MOD_USTORE),
                errmsg(USTOREFORMAT("[PAGE_CORRUPT:GetTDSlotInfo]An out of bounds "
                "access was made to the array td_info! "
        ...
        for (int i = 0; i < t_thrd.ustore_cxt.undo_buffer_idx; i++) {
            if (BufferIsValid(t_thrd.ustore_cxt.undo_buffers[i].buf)) {
                ...
                if (LWLockHeldByMeInMode(BufferDescriptorGetContentLock(bufdesc), LW_EXCLUSIVE)) {
                    ...
                    ereport(PANIC, (
                        errmodule(MOD_USTORE),
                        errmsg(USTOREFORMAT("xid %lu, oid %u, blockno %u. buffer %d is not unlocked, lock state %lu."),
    ...
    UndoRecPtr prevurpInOneXact, BlockNumber blk, XlUndoHeader *xlundohdr, undo::XlogUndoMeta *xlundometa)
{
    ...
    if (status != UNDO_RET_SUCC) {
        ereport(ERROR, (
            ...
            errmsg(UNDOFORMAT("Failed to generate UndoRecord, persistence is %d, xlundohdr->flag is %u, "
            "xlundohdr->relOid is %u, xlundohdr->urecptr is %lu"),
            persistence, PtrGetVal(xlundohdr, flag), PtrGetVal(xlundohdr, relOid), PtrGetVal(xlundohdr, urecptr))));
    }
...
static void LogUHeapInsert(UHeapWALInfo *walinfo, Relation rel, bool isToast)
{
    ...
    if ((walinfo->flag & XLOG_UNDO_HEADER_HAS_BLK_PREV) != 0) {
        ereport(DEBUG5, (
            errmodule(MOD_UPAGE),
            errcode(ERRCODE_DATA_EXCEPTION),
            errmsg("blkprev=%lu", walinfo->blkprev)));
...
void UHeapAbortSpeculative(Relation relation, UHeapTuple utuple)
{
    ...
    /* Verify upage after abort. */
    if (unlikely(ConstructUstoreVerifyParam(USTORE_VERIFY_MOD_UPAGE, USTORE_VERIFY_COMPLETE,
        (char *) &verifyParams, relation, page, blkno, InvalidOffsetNumber,
        NULL, NULL, InvalidXLogRecPtr))) {
    ...
    TransactionId* tmfdXmin)
{
    ...
    switch (result) {
        ...
        case TM_SelfModified:
            ...
            ereport(ERROR, (
                errmodule(MOD_UPAGE),
                errmsg(USTOREFORMAT("xid %lu, oid %u, ctid(%u, %u). tuple already updated by self."
                "snapshot->xmin is %lu, snapshot->xmax is %lu, snapshot->gtm_snapshot_type is %d"),
                GetTopTransactionId(), RelationGetRelid(relation), ItemPointerGetOffsetNumber(tid),
                ItemPointerGetBlockNumber(tid),
                PtrGetVal(snapshot, xmin), PtrGetVal(snapshot, xmax), PtrGetVal(snapshot, gtm_snapshot_type))));
            break;
        ...
        case TM_Updated:
            ereport(ERROR, (
                errmodule(MOD_USTORE),
                errmsg(USTOREFORMAT("xid %lu, oid %u, ctid(%u, %u). tuple concurrently updated."
                "snapshot->xmin is %lu, snapshot->xmax is %lu, snapshot->gtm_snapshot_type is %d"),
                GetTopTransactionId(), RelationGetRelid(relation), ItemPointerGetOffsetNumber(tid),
                ItemPointerGetBlockNumber(tid),
                PtrGetVal(snapshot, xmin), PtrGetVal(snapshot, xmax), PtrGetVal(snapshot, gtm_snapshot_type))));
        ...
        case TM_Deleted:
            ereport(ERROR, (
                errmodule(MOD_USTORE),
                errmsg(USTOREFORMAT("xid %lu, oid %u, ctid(%u, %u). tuple concurrently deleted."
                "snapshot->xmin is %lu, snapshot->xmax is %lu, snapshot->gtm_snapshot_type is %d"),
                GetTopTransactionId(), RelationGetRelid(relation), ItemPointerGetOffsetNumber(tid),
                ItemPointerGetBlockNumber(tid),
                PtrGetVal(snapshot, xmin), PtrGetVal(snapshot, xmax), PtrGetVal(snapshot, gtm_snapshot_type))));
        ...
        default:
            ereport(ERROR, (
                errmodule(MOD_USTORE),
                errmsg(USTOREFORMAT("xid %lu, oid %u, ctid(%u, %u). unrecognized UHeapDelete status: %u."
                "snapshot->xmin is %lu, snapshot->xmax is %lu, snapshot->gtm_snapshot_type is %d"),
                GetTopTransactionId(), RelationGetRelid(relation), ItemPointerGetOffsetNumber(tid),
...
void UHeapResetRelnode()
{
    t_thrd.ustore_cxt.rnode = NULL;
    t_thrd.ustore_cxt.oldBuffer = 0;
}
...
uint8 UPageExtendTDSlots(Relation relation, Buffer buf)
{
    ...
    if (currTDSlots < TD_THRESHOLD_FOR_PAGE_SWITCH) {
        ...
        ereport(DEBUG5, (
            errmodule(MOD_UPAGE),
            errmsg("TD slot count exceeded max allowed. Rel: %s, blkno: %d",
            RelationGetRelationName(relation), BufferGetBlockNumber(buf))));
        return 0;
    }
    ...
    if (numExtended == 0) {
        ...
        ereport(DEBUG5, (
            errmodule(MOD_UPAGE),
            errmsg("TD slots cannot be extended due to insufficient space Rel: %s, blkno: %d",
            RelationGetRelationName(relation), BufferGetBlockNumber(buf))));
        return 0;
    }
