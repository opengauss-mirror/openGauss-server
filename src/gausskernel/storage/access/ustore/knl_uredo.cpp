    ...
    TupleBuffer &tbuf)
{
    ...
    t_thrd.ustore_cxt.rnode = &targetNode;
    t_thrd.ustore_cxt.oldBuffer = buf;

    if (UHeapPageGetMaxOffsetNumber(page) + 1 < xlrec->offnum) {
        PrevDumpUPage(&targetNode, page, blkno, "uheap");
        ereport(PANIC, (
            errcode(ERRCODE_DATA_CORRUPTED),
            errmodule(MOD_UPAGE),
            errmsg("Invalid max offset number, offnum in xlrec is %d, offnum in page head is %d"
            "pd_lsn %X/%X, pd_flags %d, pd_lower %d, pd_upper %d, potential_freespace %d, td_count %d",
            xlrec->offnum, UHeapPageGetMaxOffsetNumber(page),
            (uint32)(PageGetLSN((UHeapPageHeader) page) >> HIGH_BITS_LEN_OF_LSN),
            (uint32)PageGetLSN((UHeapPageHeader) page),
            ((UHeapPageHeaderData *)page)->pd_flags,
            ((UHeapPageHeaderData *)page)->pd_lower,
            ((UHeapPageHeaderData *)page)->pd_upper,
            ((UHeapPageHeaderData *)page)->potential_freespace,
            GetTDCount((UHeapPageHeaderData *)page))));
    }
    ...
    if (UPageAddItem(NULL, &bufpage, (Item)utup, newlen, xlrec->offnum, true) == InvalidOffsetNumber) {
        ...
        ereport(PANIC, (
            errcode(ERRCODE_DATA_CORRUPTED),
            errmodule(MOD_UPAGE),
            errmsg("Failed to add tuple, offnum is %d, newlen is %lu, offset in page %d"
            "pd_lsn %X/%X, pd_flags %d, pd_lower %d, pd_upper %d, potential_freespace %d, td_count %d",
            xlrec->offnum, newlen, UHeapPageGetMaxOffsetNumber(page),
            (uint32)(PageGetLSN((UHeapPageHeader) page) >> HIGH_BITS_LEN_OF_LSN),
            (uint32)PageGetLSN((UHeapPageHeader) page),
            ((UHeapPageHeaderData *)page)->pd_flags,
            ((UHeapPageHeaderData *)page)->pd_lower,
...
static void UHeapXlogClean(XLogReaderState *record)
{
     ...
     */
    action = XLogReadBufferForRedo(record, 0, &buffer);
    t_thrd.ustore_cxt.rnode = &rnode;
    t_thrd.ustore_cxt.oldBuffer = buffer.buf;
    if (action == BLK_NEEDS_REDO) {
    ...
    const UndoRecPtr urecptr, const bool sameBlock)
{
    ...
    if (xlrec->flags & XLZ_NON_INPLACE_UPDATE) {
        ...
        if (newlen >= RowPtrGetLen(rp) || (xlrec->flags & XLZ_UPDATE_PREFIX_FROM_OLD) != 0 ||
            ...
            if (!RowPtrChangeLen(oldpage, buffers->oldbuffer.blockinfo.blkno, rp, newlen)) {
                ereport(PANIC, (
                    errcode(ERRCODE_DATA_CORRUPTED),
                    errmodule(MOD_UPAGE),
                    errmsg(USTOREFORMAT("redo update error: lpoff %u."), xlrec->old_offnum)));
            }
        }
...
static void UHeapXlogBaseShift(XLogReaderState *record)
{
    ...
    if (XLogReadBufferForRedo(record, HEAP_BASESHIFT_ORIG_BLOCK_NUM, &buffer) == BLK_NEEDS_REDO) {
        t_thrd.ustore_cxt.rnode = &(buffer.blockinfo.rnode);
        t_thrd.ustore_cxt.oldBuffer = buffer.buf;
...
static void UHeapUndoXlogResetXid(XLogReaderState *record)
{
    ...
    if (action == BLK_NEEDS_REDO) {
        ...
        MarkBufferDirty(buf);
        if (unlikely(ConstructUstoreVerifyParam(USTORE_VERIFY_MOD_REDO, USTORE_VERIFY_FAST,
            (char *) &verifyParams, NULL, BufferGetPage(buf), blkno, InvalidOffsetNumber, NULL, NULL,
            t_thrd.shemem_ptr_cxt.XLogCtl->RedoRecPtr))) {
            ExecuteUstoreVerify(USTORE_VERIFY_MOD_REDO, (char *) &verifyParams);
        }
    }
