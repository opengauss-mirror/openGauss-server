...
bool UndoZone::CheckNeedSwitch(void)
{
    if (insertURecPtr_ < forceDiscardURecPtr_ || allocateTSlotPtr_ < recycleTSlotPtr_ ||
        ...
        ereport(WARNING, (errmsg("cannot use this zone, undometa check failed."
            "zoneid: %d, inserURecPtr_ %lu, forceDiscardURecPtr_ %lu, discardUrecPtr_ %lu, "
...
void UndoZone::RecoveryUndoZone(int fd)
{
    ...
    for (zoneId = 0; zoneId < PERSIST_ZONE_COUNT; zoneId++) {
        ...
        if (uspMetaInfo->insertURecPtr != UNDO_LOG_BLOCK_HEADER_SIZE) {
            ...
            /*
             * MAX_UINT44 is 17592186044416‬, which is 16TB. We don't want a single transaction
             * uses two different zones. So mark those zones as unavailable after recovery all
             * undometa file if offset exceed UNDO_LOG_MAX_SIZE - transUndoThresholdSize. This
             * threshold is also used in CheckNeedSwitch.
             */
...
static UndoZone *SwitchUndoZoneId(UndoPersistence upersistence, int *retZid)
{
    int zid = -1;
    UndoZone *newUzone = NULL;
    for (int i = 0; i < PERSIST_ZONE_COUNT; i++) {
        zid = i + (int)upersistence * PERSIST_ZONE_COUNT;
        g_instance.undo_cxt.uZoneBitmap[upersistence] =
            bms_del_member(g_instance.undo_cxt.uZoneBitmap[upersistence], i);
        newUzone = UndoZoneGroup::GetUndoZone(zid, true);
        if (newUzone == NULL) {
            zid = -1;
            ereport(WARNING, (errmsg(UNDOFORMAT("can not palloc undo zone memory for zone %d "
                "when switch zone."), zid)));
            g_instance.undo_cxt.uZoneBitmap[upersistence] =
                bms_add_member(g_instance.undo_cxt.uZoneBitmap[upersistence], i);
            continue;
        }
...
UndoZone *UndoZoneGroup::SwitchZone(int zid, UndoPersistence upersistence)
{
    ...
    UndoZone *newUzone = SwitchUndoZoneId(upersistence, &retZid);
