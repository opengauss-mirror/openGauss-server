 ...
 *
 * IDENTIFICATION
 *	  src/gausskernel/storage/access/gist/gistbuild.cpp
 *
 * -------------------------------------------------------------------------
 */
#include "postgres.h"
...
typedef enum {
    /* in regular build mode and aren't going to switch */
    GIST_BUFFERING_DISABLED,
    /* in regular build mode, but will switch to buffering build mode if the index grows too big */
    GIST_BUFFERING_AUTO,
    /* gathering statistics of index tuple size before switching to the buffering build mode */
    GIST_BUFFERING_STATS,
    /* in buffering build mode */
...
static int calculatePagesPerBuffer(const GISTBuildState *buildstate, int levelStep);
static void gistBuildCallback(Relation index, HeapTuple htup, Datum *values, const bool *isnull, bool tupleIsAlive,
                              void *state);
...
static BlockNumber gistbufferinginserttuples(GISTBuildState *buildstate, Buffer buffer, int level, IndexTuple *itup,
                                             int ntup, OffsetNumber oldoffnum, BlockNumber parentblk,
                                             OffsetNumber downlinkoffnum);
static Buffer gistBufferingFindCorrectParent(GISTBuildState *buildstate, BlockNumber childblkno, int level,
                                             const BlockNumber *parentblk, OffsetNumber *downlinkoffnum);
...
Datum gistbuild(PG_FUNCTION_ARGS)
{
    ...
    buildstate.freespace = (Size)(BLCKSZ * (100 - fillfactor) / 100);
    ...
    if (buildstate.bufferingMode == GIST_BUFFERING_ACTIVE) {
        ereport(DEBUG1, (errmsg("all tuples processed, emptying buffers")));
...
void gistValidateBufferingOption(const char *value)
{
    if (value == NULL || (strcmp(value, "on") != 0 && strcmp(value, "off") != 0 && strcmp(value, "auto") != 0)) {
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("invalid value for \"buffering\" option"),
...
static void gistInitBuffering(GISTBuildState *buildstate)
{
    ...
    pageFreeSpace = BLCKSZ - SizeOfPageHeaderData - sizeof(GISTPageOpaqueData) - sizeof(ItemIdData) -
     ...
     * statistics.
     */
    itupAvgSize = (Size)(buildstate->indtuplesSize / buildstate->indtuples);
    ...
    avgIndexTuplesPerPage = (double)pageFreeSpace / itupAvgSize;
    maxIndexTuplesPerPage = (double)pageFreeSpace / itupMinSize;
     ...
     *
     * The average size (in pages) of a subtree of depth n can be calculated as
     * a geometric series: B^0 + B^1 + B^2 + ... + B^n = (1 - B^(n + 1)) / (1 - B)
     *
     ...
     * g_instance.cost_cxt.effective_cache_size, with a safety factor of 4.
     *
     ...
     * constrained by g_instance.cost_cxt.effective_cache_size rather than maintenance_work_mem.
     *
    ...
    for (;;) {
        ...
        if (subtreesize > (double)u_sess->attr.attr_sql.effective_cache_size / 4)
            break;

        /* each node in the lowest level of a subtree has one page in memory */
        if (maxlowestlevelpages > ((double)u_sess->attr.attr_memory.maintenance_work_mem * 1024) / BLCKSZ)
            break;
     ...
     */
    if (levelStep <= 0) {
        ereport(DEBUG1, (errmsg("failed to switch to buffered GiST build")));
        buildstate->bufferingMode = GIST_BUFFERING_DISABLED;
        return;
    }
    ...
    ereport(DEBUG1, (errmsg("switched to buffered GiST build; level step = %d, pagesPerBuffer = %d", levelStep,
                            pagesPerBuffer)));
}
                              ...
                              void *state)
{
    ...
    if ((buildstate->bufferingMode == GIST_BUFFERING_AUTO &&
         ...
         (uint32)u_sess->attr.attr_sql.effective_cache_size < smgrnblocks(index->rd_smgr, MAIN_FORKNUM)) ||
        (buildstate->bufferingMode == GIST_BUFFERING_STATS &&
...
static void gistBufferingBuildInsert(GISTBuildState *buildstate, IndexTuple itup)
{
    ...
    (void)gistProcessItup(buildstate, itup, 0, buildstate->gfbb->rootlevel);
...
static bool gistProcessItup(GISTBuildState *buildstate, IndexTuple itup, BlockNumber startblkno, int startlevel)
{
    ...
    for (;;) {
        ...
        if (newtup) {
            blkno = gistbufferinginserttuples(buildstate, buffer, level, &newtup, 1, childoffnum, InvalidBlockNumber,
                                              InvalidOffsetNumber);
    ...
    if (LEVEL_HAS_BUFFERS(level, gfbb)) {
         ...
         */
        GISTNodeBuffer *childNodeBuffer = NULL;
                                             ...
                                             OffsetNumber downlinkoffnum)
{
    ...
    is_split = gistplacetopage(buildstate->indexrel, buildstate->freespace, buildstate->giststate, buffer, itup, ntup,
                               oldoffnum, &placed_to_blk, InvalidBuffer, &splitinfo, false);
    /*
    ...
    if (is_split && BufferGetBlockNumber(buffer) == GIST_ROOT_BLKNO) {
        ...
        ereport(DEBUG2, (errmsg("splitting GiST root page, now %d levels deep", gfbb->rootlevel)));
    ...
    if (splitinfo != NIL) {
        ...
        parentBuffer = gistBufferingFindCorrectParent(buildstate, BufferGetBlockNumber(buffer), level, &parentblk,
                                                      &downlinkoffnum);
        ...
        foreach (lc, splitinfo) {
            GISTPageSplitInfo *tmpSplitinfo = (GISTPageSplitInfo *)lfirst(lc);
             ...
             */
            if (level > 0)
                gistMemorizeParent(buildstate, BufferGetBlockNumber(tmpSplitinfo->buf),
             ...
             */
            if (level > 1)
                gistMemorizeAllDownlinks(buildstate, tmpSplitinfo->buf);

            /*
             * Since there's no concurrent access, we can release the lower
             * level buffers immediately. This includes the original page.
             */
            UnlockReleaseBuffer(tmpSplitinfo->buf);
            downlinks[i++] = tmpSplitinfo->downlink;
        }
 ...
 */
static Buffer gistBufferingFindCorrectParent(GISTBuildState *buildstate, BlockNumber childblkno, int level,
                                             const BlockNumber *parentblkno, OffsetNumber *downlinkoffnum)
{
    BlockNumber parent;
    Buffer buffer;
    Page page;
    ...
    if (level > 0) {
        ...
        if (*parentblkno == InvalidBlockNumber) {
            ereport(ERROR,
                    (errcode(ERRCODE_INDEX_CORRUPTED), errmsg("no parent buffer provided of child %u", childblkno)));
        }
        parent = *parentblkno;
    }
    ...
    if (parent == *parentblkno && *parentblkno != InvalidBlockNumber && *downlinkoffnum != InvalidOffsetNumber &&
    ...
    ereport(ERROR, (errcode(ERRCODE_INDEX_CORRUPTED), errmsg("failed to re-find parent for block %u", childblkno)));
...
static void gistProcessEmptyingQueue(GISTBuildState *buildstate)
{
    ...
    while (gfbb->bufferEmptyingQueue != NIL) {
        GISTNodeBuffer *emptyingNodeBuffer = NULL;
        ...
        while (gfbb->buffersOnLevels[i] != NIL) {
            ...
            if (nodeBuffer->blocksCount != 0) {
                ...
                if (!nodeBuffer->queuedForEmptying) {
                    (void)MemoryContextSwitchTo(gfbb->context);
                    nodeBuffer->queuedForEmptying = true;
                    gfbb->bufferEmptyingQueue = lcons(nodeBuffer, gfbb->bufferEmptyingQueue);
                    (void)MemoryContextSwitchTo(buildstate->giststate->tempCxt);
                }
                gistProcessEmptyingQueue(buildstate);
            } else
...
static void gistInitParentMap(GISTBuildState *buildstate)
{
    ...
    buildstate->parentMap = hash_create("gistbuild parent map", 1024, &hashCtl,
                                        HASH_ELEM | HASH_CONTEXT | HASH_FUNCTION);
}

static void gistMemorizeParent(GISTBuildState *buildstate, BlockNumber child, BlockNumber parent)
{
    ParentMapEntry *entry = NULL;
    bool found = false;
...
static BlockNumber gistGetParent(GISTBuildState *buildstate, BlockNumber child)
{
    ...
    if (!found)
        ereport(ERROR,
                (errcode(ERRCODE_INDEX_CORRUPTED), errmsg("could not find parent of block %u in lookup table", child)));

    return entry->parentblkno;
}

Datum gistmerge(PG_FUNCTION_ARGS)
{
    IndexBuildResult *result = NULL;

    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED), errmsg("gistmerge: unimplemented")));
