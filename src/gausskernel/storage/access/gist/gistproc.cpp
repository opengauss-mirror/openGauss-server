 ...
 */
static double box_penalty(BOX *original, BOX *newm)
{
    double union_width, union_height;

    union_width = Max(original->high.x, newm->high.x) - Min(original->low.x, newm->low.x);
    union_height = Max(original->high.y, newm->high.y) - Min(original->low.y, newm->low.y);
    return union_width * union_height - (original->high.x - original->low.x) * (original->high.y - original->low.y);
}
...
Datum gist_box_same(PG_FUNCTION_ARGS)
{
    ...
    if (b1 != NULL && b2 != NULL) {
        *result = (b1->low.x == b2->low.x && b1->low.y == b2->low.y && b1->high.x == b2->high.x &&
                   b1->high.y == b2->high.y);
    } else {
