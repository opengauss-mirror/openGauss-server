    ...
    if (enablePQ) {
        ...
        for (int lc = entryPoint->level; lc >= 1; lc--) {
            ...
            w = HnswSearchLayer(base, q, ep, 1, lc, index, procinfo, collation, m, false, NULL,
                                false, NULL, NULL, NULL, NULL, true, NULL, true, NULL, enablePQ, &pqinfo);
            ep = w;
        }
        pqinfo.lc = 0;
        w = HnswSearchLayer(base, q, ep, hnswEfSearch, 0, index, procinfo, collation, m, false, NULL,
                            false, NULL, NULL, &so->v,
        ...
        for (int lc = entryPoint->level; lc >= 1; lc--) {
            w = HnswSearchLayer(base, q, ep, 1, lc, index, procinfo, collation, m, false, NULL,
                                so->enableRabitQ, so->rbqParams, NULL, NULL, NULL, true, NULL, true);
            ep = w;
        }
        w = HnswSearchLayer(base, q, ep, hnswEfSearch, 0, index, procinfo, collation, m, false, NULL,
                            so->enableRabitQ, so->rbqParams, NULL, &so->v,
                            u_sess->datavec_ctx.hnsw_iterative_scan != HNSW_ITERATIVE_SCAN_OFF ? &so->discarded : NULL,
                            true, &so->tuples, true);
    }
    return w;
...
static List *ResumeScanItems(IndexScanDesc scan)
{
    ...
    return HnswSearchLayer(base, so->q, ep, batchSize, 0, index, so->procinfo, so->collation,
                           so->m, false, NULL, so->enableRabitQ, so->rbqParams, NULL, &so->v,
                           &so->discarded, false, &so->tuples, true);
}
