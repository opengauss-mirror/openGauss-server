 ...
 *
 * IDENTIFICATION
 *			src/gausskernel/storage/access/spgist/spgkdtreeproc.cpp
 *
 * -------------------------------------------------------------------------
 */
#include <math.h>
...
Datum spg_kd_choose(PG_FUNCTION_ARGS)
{
    ...
    out->result.matchNode.nodeN = (getSide(coord, in->level % 2, inPoint) > 0) ? 0 : 1;
...
Datum spg_kd_picksplit(PG_FUNCTION_ARGS)
{
    ...
    qsort(sorted, in->nTuples, sizeof(*sorted), (in->level % 2) ? x_cmp : y_cmp);
    middle = in->nTuples / 2;
