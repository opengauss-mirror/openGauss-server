...
void xact_redo_log_drop_segs(_in_ ColFileNodeRel *xnodes, _in_ int nrels, XLogRecPtr lsn)
{
    bool isNeedLogRemainSegs = IsNeedLogRemainSegs(lsn);
    if (!isNeedLogRemainSegs) {
        return;
 ...
 */
void unlink_relfiles(_in_ ColFileNodeRel *xnodes, _in_ int nrels, bool is_old_delay_ddl)
{
                                      ...
                                      uint64 csn, TransactionId newStandbyXmin, RepOriginId originId)
{
    ...
    if (nrels > 0) {
        ...
        if (IS_EXRTO_READ) {
            update_delay_ddl_files(xnodes, nrels, lsn);
        } else {
...
static void xact_redo_abort(xl_xact_abort *xlrec, TransactionId xid, XLogRecPtr lsn, bool abortXlogNewVersion)
{
    ...
    if (xlrec->nrels > 0) {
        ...
        /* Make sure files supposed to be dropped are dropped */
        if (IS_EXRTO_READ) {
            update_delay_ddl_files(xlrec->xnodes, xlrec->nrels, lsn);
        } else {
...
void ApplyUndoActions()
{
    ...
    if (!s->perform_undo) {
        if (!IsSubTransaction()) {
            for (int i = 0; i < UNDO_PERSISTENCE_LEVELS; i++) {
                if (t_thrd.undo_cxt.slotPtr[i] == INVALID_UNDO_SLOT_PTR) {
                    continue;
                }
                undo::TransactionSlot *slot = (undo::TransactionSlot *)t_thrd.undo_cxt.slots[i];
                if (GetTopTransactionIdIfAny() != slot->XactId()) {
                    continue;
                }
