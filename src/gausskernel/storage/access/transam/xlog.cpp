...
pg_crc32 GetXlogRecordCrc(XLogRecPtr RecPtr, bool &crcvalid, XLogPageReadCB pagereadfunc, Size bufAlignSize)
{
    ...
    if (IsInitdb && t_thrd.xlog_cxt.ThisTimeLineID != 0) {
        tli = t_thrd.xlog_cxt.ThisTimeLineID;
        if (!list_member_int(t_thrd.xlog_cxt.expectedTLIs, (int)tli)) {
            t_thrd.xlog_cxt.expectedTLIs = lcons_int((int)tli, t_thrd.xlog_cxt.expectedTLIs);
        }
    }
    ...
    if (rec != NULL) {
        ...
        crcvalid = false;
        ereport(LOG, (errmsg("error msg:%s.", errorMsg)));
    }
...
void ShareStorageInit()
{
    if (g_instance.attr.attr_storage.xlog_file_path != NULL && g_instance.attr.attr_storage.xlog_file_size > 0) {
        void *tmpBuf = MemoryContextAllocZero(INSTANCE_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_STORAGE),
            sizeof(ShareStorageXLogCtl) + MEMORY_ALIGNED_SIZE);
...
void DoShareStorageXLogCopy(XLogCopyMode mode, XLogRecPtr start)
{
    ...
    t_thrd.shemem_ptr_cxt.ControlFile =
        (ControlFileData *)MemoryContextAllocZero(INSTANCE_GET_MEM_CXT_GROUP(MEMORY_CONTEXT_STORAGE),
        sizeof(ControlFileData));
    errno_t ret = memset_s(t_thrd.shemem_ptr_cxt.ControlFile, sizeof(ControlFileData), 0, sizeof(ControlFileData));
    securec_check(ret, "", "");
    ReadControlFile();
    bool copySuccess = false;
    if (mode == XLOG_COPY_FROM_LOCAL) {
        copySuccess = XLogOverwriteFromLocal(false, start);
    } else if (mode == XLOG_FORCE_COPY_FROM_LOCAL) {
        copySuccess = XLogOverwriteFromLocal(true, start);
    } else if (mode == XLOG_COPY_FROM_SHARE) {
        copySuccess = XLogOverwriteFromShare();
    }

    ereport(LOG, (errmsg("xlog copy result: %s", copySuccess ? "success" : "fail")));
    if (copySuccess) {
        proc_exit(0);
    } else {
        proc_exit(1);
