...
void heap2_desc(StringInfo buf, XLogReaderState* record)
{
    ...
    if (info == XLOG_HEAP2_FREEZE) {
        ...
        if (bucket_id == -1) {
            appendStringInfo(buf,
                "bcm: rel %u/%u/%u; blk %lu status: %d, count: %d, col: %u",
                xlrec->node.spcNode,
                xlrec->node.dbNode,
                xlrec->node.relNode,
                xlrec->block,
            ...
            appendStringInfo(buf,
                "bcm: rel %u/%u/%u/%d; blk %lu status: %d, count: %d, col: %u",
                xlrec->node.spcNode,
                xlrec->node.dbNode,
                xlrec->node.relNode,
                bucket_id,
        ...
        if (bucket_id == -1) {
            appendStringInfo(buf,
                "logical newpage rel %u/%u/%u; blk %u",
                xlrec->node.spcNode,
                xlrec->node.dbNode,
                xlrec->node.relNode,
                xlrec->blkno);
            ...
            appendStringInfo(buf,
                "logical newpage rel %u/%u/%u/%d; blk %u",
                xlrec->node.spcNode,
                xlrec->node.dbNode,
                xlrec->node.relNode,
                bucket_id,
