    ...
    foreach (parsetree_item, parsetree_list) {
        ...
        if (runOpfusionCheck) {
            ...
            if (opFusionObj != NULL) {
                ...
                if (OpFusion::process(FUSION_SIMPLE_EXECUTE, NULL, completionTag, isTopLevel, NULL)) {
                    CommandCounterIncrement();
                    finish_xact_command();
                    EndCommand(completionTag, dest);
                    MemoryContextReset(OptimizerContext);
                    /* clean simple query's bypass */
...
/*
 * Reload pooler if necessary, just pass while in transaction or transaction block
 * Use condition IsTransactionOrTransactionBlock() because s->blockState maybe TBLOCK_STARTED and
 * u_sess->pgxc_cxt.XactWriteNodes/u_sess->pgxc_cxt.XactReadNodes are not NULL when in creating table.
 */
...
int PostgresMain(int argc, char* argv[], const char* dbname, const char* username)
{
    ...
    if (!IsUnderPostmaster && xlogCopyMode != XLOG_COPY_NOT) {
        DoShareStorageXLogCopy(xlogCopyMode, xlogCopyStart);
    }
...
void switch_and_copy_plan_for_batch_for_gpc(CachedPlanSource* psrc, int old_generation,
                                            MemoryContext lastCopyCxt, List* lastCopyStmts, Portal portal)
{
    List* copyStmts = lastCopyStmts;
    /* generated new gplan, copy it incase someone change it */
    if (old_generation != psrc->generation && psrc->gplan) {
        if (lastCopyCxt != NULL) {
            MemoryContextDelete(lastCopyCxt);
        }
        copyStmts = CopyLocalStmt(psrc->gplan->stmt_list, u_sess->top_portal_cxt, &portal->copyCxt);
    } else if (lastCopyStmts == NULL && psrc->gpc.status.InShareTable() && psrc->gplan) {
        /* copy for shared plan in first time */
        if (lastCopyCxt != NULL) {
            MemoryContextDelete(lastCopyCxt);
        }
        copyStmts = CopyLocalStmt(psrc->gplan->stmt_list, u_sess->top_portal_cxt, &portal->copyCxt);
    } else if (psrc->cplan) {
        /* if has cplan, we need to delete saved copy cxt. */
        if (lastCopyCxt != NULL) {
            MemoryContextDelete(lastCopyCxt);
        }
        copyStmts = NULL;
    } else {
        /* no need to copy plan in repeate gplan case, so we keep use last copy cxt for current portal. */
        portal->copyCxt = (copyStmts != NULL) ? lastCopyCxt : NULL;
    }
    if (copyStmts != NULL)
        portal->stmts = copyStmts;
}
/*
 * exec_one_in_batch
...
static void exec_one_in_batch(CachedPlanSource* psrc, ParamListInfo params, int numRFormats, int16* rformats,
    ...
    const char* stmt_name, PreparedStatement* pstmt, bool isFirst)
{
    CachedPlan* cplan = NULL;
    Portal portal;
    ...
    bool completed = false;
    bool hasGetLock = false;
    MemoryContext lastCopyCxt = NULL;
    List* lastCopyStmts = NULL;
    ...
    if (ENABLE_GPC && !isFirst) {
        Portal last_portal = GetPortalByName("");
        if (PortalIsValid(last_portal)) {
            // save last poral's copy cxt when last copy stmt is valid.
            lastCopyCxt = last_portal->copyCxt;
            lastCopyStmts = (last_portal->copyCxt != NULL) ? last_portal->stmts : NULL;
            last_portal->copyCxt = NULL;
        }
    }
    ...
    if (ENABLE_GPC) {
        switch_and_copy_plan_for_batch_for_gpc(psrc, generation, lastCopyCxt, lastCopyStmts, portal);
        lastCopyCxt = NULL;
        lastCopyStmts = NULL;
    }
