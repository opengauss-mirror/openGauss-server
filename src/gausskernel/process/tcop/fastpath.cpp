 ...
 *
 * IDENTIFICATION
 *	  src/gausskernel/process/tcop/fastpath.cpp
 *
 * NOTES
...
struct fp_info {
    ...
    Oid nmspace;     /* other stuff from pg_proc */
    Oid rettype;
    ...
    if (unlikely(!(OidIsValid(func_id))))
                ereport(ERROR,
                    (errmodule(MOD_EXECUTOR),
                        ...
                        errmsg("func_id not valid in %s line %d", __FILE__, __LINE__)));
    AssertEreport(fip != NULL, MOD_EXECUTOR, "fip is NULL");
    ...
    if (pp->pronargs > FUNC_MAX_ARGS)
        ereport(ERROR,
            (errcode(ERRCODE_TOO_MANY_ARGUMENTS),
                errmsg("function %s has more than %d arguments", NameStr(pp->proname), FUNC_MAX_ARGS)));
 ...
 * t_thrd.mem_cxt.msg_mem_cxt memory context, which will be automatically reset when
...
int HandleFunctionRequest(StringInfo msgBuf)
{
    ...
    if (aclresult != ACLCHECK_OK)
        aclcheck_error(aclresult, ACL_KIND_PROC, get_func_name(fid));
    ...
    switch (check_log_duration(msec_str, was_logged)) {
        ...
        case 2:
            ereport(
                LOG, (errmsg("duration: %s ms  fastpath function call: \"%s\" (OID %u)", msec_str, fip->fname, fid)));
            break;
        default:
            break;
    }
...
static int16 parse_fcall_arguments(StringInfo msgBuf, struct fp_info* fip, FunctionCallInfo fcinfo)
{
    ...
    if (numAFormats > 1 && numAFormats != nargs)
        ereport(ERROR,
            (errcode(ERRCODE_PROTOCOL_VIOLATION),
                errmsg("function call message contains %d argument formats but %d arguments", numAFormats, nargs)));
            ...
            if (argsize != -1 && abuf.cursor != abuf.len)
                ereport(ERROR,
                    (errcode(ERRCODE_INVALID_BINARY_REPRESENTATION),
                        errmsg("incorrect binary data format in function argument %d", i + 1)));
        } else
