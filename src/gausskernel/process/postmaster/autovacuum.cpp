...
static void relation_needs_vacanalyze(Oid relid, AutoVacOpts* relopts, bytea* rawRelopts, Form_pg_class classForm,
    HeapTuple tuple, PgStat_StatTabEntry* tabentry, bool allowAnalyze, bool allowVacuum, TupleDesc tupdesc,
    bool is_recheck, bool* dovacuum, bool* doanalyze, bool* need_freeze);
...
static void partition_needs_vacanalyze(Oid partid, AutoVacOpts* relopts, Form_pg_partition partForm,
    HeapTuple partTuple, TupleDesc partTupDesc, at_partitioned_table* ap_entry, PgStat_StatTabEntry* tabentry,
    bool is_recheck, bool* dovacuum, bool* doanalyze, bool* need_freeze);
...
static Oid do_start_worker(void)
{
    ...
    foreach (cell, dblist) {
         ...
         * Do freeze check unless we maintenance datafrozenxid64 in right way.
         * (e.g: sub partition set wrong datafrozenxid64 in single mode).
         */
#ifndef ENABLE_MULTIPLE_NODES
        xidForceLimit = FirstNormalTransactionId;
#endif
...
static void do_autovacuum(void)
{
    ...
    while ((tuple = (HeapTuple) tableam_scan_getnexttuple(relScan, ForwardScanDirection)) != NULL) {
        ...
        relation_needs_vacanalyze(relid, relopts, rawRelopts, classForm, tuple, tabentry, enable_analyze, enable_vacuum,
            pg_class_desc, false, &dovacuum, &doanalyze, &need_freeze);
    ...
    pg_partition_desc = CreateTupleDescCopy(RelationGetDescr(partRel));
    while (NULL != (partTuple = (HeapTuple) tableam_scan_getnexttuple(partScan, ForwardScanDirection))) {
        ...
        /* Check if it needs vacuum or analyze */
        partition_needs_vacanalyze(partOid, relopts, partForm, partTuple, pg_partition_desc, ap_entry, tabentry,
    ...
    while ((tuple = (HeapTuple) tableam_scan_getnexttuple(relScan, ForwardScanDirection)) != NULL) {
        ...
        relation_support_autoavac(tuple, &enable_analyze, &enable_vacuum, &is_internal_relation);
        relation_needs_vacanalyze(relid, relopts, rawRelopts, classForm, tuple, tabentry, enable_analyze, enable_vacuum,
            pg_class_desc, true, &dovacuum, &doanalyze, &need_freeze);
     ...
     bool is_recheck, bool* dovacuum, bool* doanalyze, bool* need_freeze)
{
    ...
    *dovacuum = *dovacuum && userEnabled;
    ...
    rawRelOptions = extractRelOptions(tuple, tupdesc, InvalidOid);
    ustoreTable = (rawRelOptions != NULL && RelationIsTableAccessMethodUStoreType(rawRelOptions));
    if (ustoreTable && allowVacuum && userEnabled) {
        *dovacuum = true;
    }
    pfree_ext(rawRelOptions);
    ...
    bool is_recheck, bool* dovacuum, bool* doanalyze, bool* need_freeze)
{
    ...
    *need_freeze = force_vacuum;
    ...
    partRelOptions = extractPartRelOptions(partTuple, partTupDesc);
    ustorePartTbl = (partRelOptions != NULL && RelationIsTableAccessMethodUStoreType(partRelOptions));
    if (ustorePartTbl && av_enabled && ap_entry->at_allowvacuum) {
        *dovacuum = true;
    }
    pfree_ext(partRelOptions);
    ...
    vacuum_object* vacObj, HTAB* table_relopt_map, HTAB* partitioned_tables_map, TupleDesc partTupDesc)
{
    ...
    partition_needs_vacanalyze(partid, avopts, partForm, partTuple, partTupDesc, ap_entry, tabentry, true,
