 ...
 *
 * IDENTIFICATION
 *	  src/gausskernel/optimizer/util/pgxcship_single.cpp
 *
 * ---------------------------------------------------------------------------------------
 */
#include "postgres.h"
...
bool pgxc_is_uuid_generate_func_unshippable(Oid funcid, shipping_context* context)
{
    DISTRIBUTED_FEATURE_NOT_SUPPORTED();
    return false;
}
...
ExecNodes* pgxc_merge_exec_nodes(ExecNodes* en1, ExecNodes* en2, int join_type)
{
    DISTRIBUTED_FEATURE_NOT_SUPPORTED();
    return NULL;
}
...
bool contain_unsupport_expression(Node* expr, void* context)
{
    DISTRIBUTED_FEATURE_NOT_SUPPORTED();
    return false;
}
...
static void pgxc_set_exprtype_shippability(Oid exprtype, Shippability_context* sc_context)
{
    ...
    if (typerelkind == RELKIND_SEQUENCE || typerelkind == RELKIND_VIEW || typerelkind == RELKIND_FOREIGN_TABLE)
...
static ExecNodes* pgxc_FQS_datanodes_for_rtr(Index varno, Shippability_context* sc_context)
{
    ...
    switch (rte->rtekind) {
        ...
        case RTE_SUBQUERY: {
             ...
             */
            exec_nodes = pgxc_is_query_shippable(rte->subquery, sc_context->sc_query_level + 1);
            /*
    ...
    switch (nodeTag(node)) {
        case T_FromExpr: {
            ...
            foreach (lcell, from_expr->fromlist) {
                 ...
                 */
                result_en = pgxc_is_join_shippable(result_en, en, JOIN_INNER, false, false, from_expr->quals);
...
static ExecNodes* pgxc_FQS_get_relation_nodes(RangeTblEntry* rte, Index varno, Query* query)
{
    ...
    if (list_length(query->rtable) == 1)
        rel_exec_nodes = GetRelationNodesByQuals(rte->relid, varno, query->jointree->quals, rel_access);
    else
        rel_exec_nodes = GetRelationNodes(rel_loc_info, (Datum)0, true, InvalidOid, rel_access);
...
bool pgxc_query_has_distcolgrouping(Query* query, ExecNodes* exec_nodes)
{
    ...
    foreach (lcell, query->groupClause) {
        ...
        if (IsA(sgc_expr, Var) && pgxc_is_var_distrib_column((Var*)sgc_expr, exec_nodes->en_dist_vars))
            return true;
    }
    return false;
}
...
static bool pgxc_shippability_walker(Node* node, Shippability_context* sc_context)
{
    ...
    switch (nodeTag(node)) {
        ...
        case T_Query: {
             ...
             * here
             */
            if (query_tree_walker(query, pgxc_shippability_walker, sc_context, QTW_IGNORE_RANGE_TABLE))
                return true;
             ...
             */
            if ((query->hasAggs || query->havingQual) && sc_context->sc_exec_nodes &&
        ...
        case T_FromExpr: {
             ...
             */
            if (sc_context->sc_query->commandType == CMD_INSERT && ((FromExpr*)node)->quals)
                pgxc_set_shippability_reason(sc_context, SS_UNSUPPORTED_EXPR);
        } break;
        ...
        case T_SubLink: {
            ...
            if (IsA(sublink.subselect, Query))
                sublink_en = pgxc_is_query_shippable((Query*)(sublink.subselect), sc_context->sc_query_level);
            else
                sublink_en = NULL;
            ...
            return expression_tree_walker((Node*)&sublink, pgxc_shippability_walker, sc_context);
        } break;
 ...
 */
static bool pgxc_is_var_distrib_column(Var* var, List* dist_vars)
{
 ...
 */
ExecNodes* pgxc_is_query_shippable(Query* query, int query_level)
{
    ...
    errno_t rc = memset_s(&sc_context, sizeof(sc_context), 0, sizeof(sc_context));
     ...
     * shipped.
     */
    (void)pgxc_shippability_walker((Node*)query, &sc_context);
    ...
    if (query->hasSubLinks) {
        if (exec_nodes && IsExecNodesReplicated(exec_nodes) && sc_context.sc_subquery_en &&
            IsExecNodesReplicated(sc_context.sc_subquery_en))
    ...
    if (bms_is_member(SS_NEED_SINGLENODE, shippability)) {
         ...
         * single node.
         */
        if (list_length(exec_nodes->nodeList) != 1 && !IsExecNodesReplicated(exec_nodes))
            canShip = false;
...
bool pgxc_is_expr_shippable(Expr* node, bool* has_aggs)
{
    ...
    (void)pgxc_shippability_walker((Node*)node, &sc_context);
 ...
 */
bool pgxc_is_func_shippable(Oid funcid)
{
    /*
 ...
 * Check equijoin conditions on given relations
 */
Expr* pgxc_find_dist_equijoin_qual(List* dist_vars1, List* dist_vars2, Node* quals)
{
    List* lquals = NIL;
    ListCell* qcell = NULL;
    ...
    foreach (qcell, lquals) {
        ...
        if (!((pgxc_is_var_distrib_column(lvar, dist_vars1) && pgxc_is_var_distrib_column(rvar, dist_vars2)) ||
                (pgxc_is_var_distrib_column(lvar, dist_vars2) && pgxc_is_var_distrib_column(rvar, dist_vars1))))
            continue;
         ...
         */
        if (!op_mergejoinable(op->opno, exprType((Node*)lvar)) && !op_hashjoinable(op->opno, exprType((Node*)lvar)))
            continue;
 ...
 */
ExecNodes* pgxc_merge_exec_nodes(ExecNodes* en1, ExecNodes* en2)
{
    ...
    if (en1->primarynodelist || en2->primarynodelist || en1->en_expr || en2->en_expr || OidIsValid(en1->en_relid) ||
        OidIsValid(en2->en_relid) || en1->accesstype != RELATION_ACCESS_READ || en2->accesstype != RELATION_ACCESS_READ)
        return NULL;
 ...
 */
Var* pgxc_get_dist_var(Index varno, RangeTblEntry* rte, List* tlist)
{
    ...
    foreach (lcell, tlist) {
        ...
        if (var && IsA(var, Var) && (var->varno == varno) && (var->varattno == rel_loc_info->partAttrNum))
     ...
     * it and return.
     */
    get_rte_attribute_type(rte, rel_loc_info->partAttrNum, &dist_var_type, &dist_var_typmod, &dist_var_collid);
    dist_var = makeVar(varno, rel_loc_info->partAttrNum, dist_var_type, dist_var_typmod, dist_var_collid, 0);
    return dist_var;
}
    ...
    bool outer_unshippable_tlist, JoinType jointype, Node* join_quals)
{
    ...
    /* If both sides are distributed, ... */
         ...
         */
        merge_nodes = true;
    } else if (IsExecNodesReplicated(outer_en) && IsExecNodesColumnDistributed(inner_en) && jointype == JOIN_INNER) {
        /*
 ...
 */
bool pgxc_find_nonshippable_row_trig(Relation rel, int16 tgtype_event, int16 tgtype_timing, bool ignore_timing)
{
    ...
    for (i = 0; i < trigdesc->numtriggers; i++) {
        ...
        if (ignore_timing) {
             ...
             * and timing.
             */
            if (!TRIGGER_TYPE_MATCHES(tgtype, TRIGGER_TYPE_ROW, tgtype_timing, tgtype_event))
                continue;
        }
