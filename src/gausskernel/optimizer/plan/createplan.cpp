...
Plan* make_modifytable(PlannerInfo* root, CmdType operation, bool canSetTag, List* resultRelations, List* subplans,
    ...
    List* mergeSourceTargetList, List* mergeActionList, UpsertExpr* upsertClause, HintState* hintState)
#else
...
#ifndef ENABLE_MULTIPLE_NODES
    ListCell* lc = NULL;
    if (has_material_subplan_hint(hintState) && (operation == CMD_MERGE || operation == CMD_INSERT)) {
        foreach (lc, subplans) {
            Plan* subplan = (Plan*)lfirst(lc);
            Material *mtplan = make_material(subplan, true);
            inherit_plan_locator_info((Plan *)mtplan, subplan);
            copy_plan_costsize((Plan *)mtplan, subplan);
            lfirst(lc) = (void*)mtplan;
            node->material_all_subplan = true; // to be judged by guc
        }
    }
#endif
