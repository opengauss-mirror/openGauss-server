...
static RemoteQuery* pgxc_FQS_create_remote_plan(Query* query, ExecNodes* exec_nodes, bool is_exec_direct);
static bool pgxc_locate_grouping_columns(PlannerInfo* root, List* tlist, AttrNumber* grpColIdx);
static List* pgxc_process_grouping_targetlist(List* local_tlist, bool single_node_grouping);
static List* pgxc_process_having_clause(
    List* remote_tlist, bool single_node_grouping, Node* havingQual, List** local_qual, List** remote_qual);
...
static Query* pgxc_build_shippable_query_recurse(
    PlannerInfo* root, RemoteQueryPath* rqpath, List** unshippable_quals, List** rep_tlist);
static RemoteQuery* make_remotequery(List* qptlist, List* qpqual, Index scanrelid);
static RangeTblEntry* make_dummy_remote_rte(char* relname, Alias* alias);
static List* pgxc_build_shippable_tlist(List* tlist, List* unshippabl_quals, bool has_aggs);
static List* pgxc_add_to_flat_tlist(List* remote_tlist, Node* expr, Index ressortgroupref);
...
static void pgxc_add_returning_list(RemoteQuery* rq, List* ret_list, int rel_index);
static void pgxc_build_dml_statement(
    PlannerInfo* root, CmdType cmdtype, Index resultRelationIndex, RemoteQuery* rqplan, List* sourceTargetList);
static void pgxc_dml_add_qual_to_query(Query* query, int param_num, AttrNumber sys_col_attno, Index varno);
static void pgxc_add_param_as_tle(Query* query, int param_num, Oid param_type, char* resname);
 ...
 */
static List* pgxc_separate_quals(List* quals, List** unshippabl_quals, bool has_aggs)
{
    ListCell* l = NULL;
...
static List* pgxc_build_shippable_tlist(List* tlist, List* unshippabl_quals, bool has_aggs)
{
    ...
    PVCAggregateBehavior pvc_agg_spec = has_aggs ? PVC_INCLUDE_AGGREGATES : PVC_REJECT_AGGREGATES;
    ...
    foreach (lcell, tlist) {
        ...
        if (!IsA(tle, TargetEntry))
            elog(ERROR, "Expected TargetEntry node, but got node with type %d", nodeTag(tle));
        expr = tle->expr;
        /*
        ...
        if (pgxc_is_expr_shippable((Expr*)tle, tmp_ptr))
            remote_tlist = pgxc_add_to_flat_tlist(remote_tlist, (Node*)expr, tle->ressortgroupref);
        else
    ...
    foreach (lcell, aggs_n_vars) {
        ...
        Assert(!IsA(agg_or_var, Aggref) || pgxc_is_expr_shippable((Expr*)agg_or_var, &tmp_has_aggs));
 ...
 */
static Query* pgxc_build_shippable_query_baserel(
    PlannerInfo* root, RemoteQueryPath* rqpath, List** unshippable_quals, List** rep_tlist)
{
    RelOptInfo* baserel = rqpath->path.parent;
    List* tlist = NIL;
    List* scan_clauses = NIL;
    List* shippable_quals = NIL;
    Query* query = NULL;
    RangeTblEntry* rte = rt_fetch(baserel->relid, root->parse->rtable);
    ListCell* lcell = NULL;
    RangeTblRef* rtr = NULL;

    if ((baserel->reloptkind != RELOPT_BASEREL && baserel->reloptkind != RELOPT_OTHER_MEMBER_REL) ||
    ...
    *unshippable_quals = NIL;
    ...
    scan_clauses = list_concat(extract_actual_clauses(scan_clauses, false), extract_actual_clauses(scan_clauses, true));
    ...
    tlist = pull_var_clause((Node*)tlist, PVC_REJECT_AGGREGATES, PVC_RECURSE_PLACEHOLDERS);
    tlist =
        list_concat(tlist, pull_var_clause((Node*)*unshippable_quals, PVC_REJECT_AGGREGATES, PVC_RECURSE_PLACEHOLDERS));
    ...
    *rep_tlist = add_to_flat_tlist(NIL, tlist);
    ...
    tlist = list_concat(pull_var_clause((Node*)query->targetList, PVC_REJECT_AGGREGATES, PVC_RECURSE_PLACEHOLDERS),
        pull_var_clause((Node*)query->jointree->quals, PVC_REJECT_AGGREGATES, PVC_RECURSE_PLACEHOLDERS));
    foreach (lcell, tlist) {
 ...
 */
static List* pgxc_generate_colnames(char* prefix, int num_cols)
{
    List* colnames = NIL;
 ...
 * mapping is provided by rep_tlist.
 */
static Query* pgxc_build_shippable_query_jointree(
    PlannerInfo* root, RemoteQueryPath* rqpath, List** unshippable_quals, List** rep_tlist)
{
    /* Variables for the part of the Query representing the JOIN */
    Query* join_query = NULL;
    FromExpr* from_expr = NULL;
    List* rtable = NIL;
    List* tlist = NIL;
    JoinExpr* join_expr = NULL;
    List* join_clauses = NIL;
    List* other_clauses = NIL;
    List* varlist = NIL;
    /* Variables for the left side of the JOIN */
    Query* left_query = NULL;
    List* left_us_quals = NIL;
    List* left_rep_tlist = NIL;
    RangeTblEntry* left_rte = NULL;
    Alias* left_alias = NULL;
    List* left_colnames = NIL;
    /* For the time being! Do we really care about unique alias names across the tree */
    char* left_aname = "l";
    RangeTblRef* left_rtr = NULL;
    /* Variables for the right side of the JOIN */
    Query* right_query = NULL;
    List* right_us_quals = NIL;
    List* right_rep_tlist = NIL;
    RangeTblEntry* right_rte = NULL;
    Alias* right_alias = NULL;
    List* right_colnames = NIL;
    char* right_aname = "r";
    RangeTblRef* right_rtr = NULL;
     ...
     */
    left_query = pgxc_build_shippable_query_recurse(root, rqpath->leftpath, &left_us_quals, &left_rep_tlist);
     ...
     * RTE with proper aliases
     */
    right_query = pgxc_build_shippable_query_recurse(root, rqpath->rightpath, &right_us_quals, &right_rep_tlist);
    right_colnames = pgxc_generate_colnames("a", list_length(right_rep_tlist));
    right_alias = makeAlias(right_aname, right_colnames);
    right_rte = addRangeTableEntryForSubquery(NULL, right_query, right_alias, true);
     ...
     */
    extract_actual_join_clauses(rqpath->join_restrictlist, &join_clauses, &other_clauses);
    ...
    other_clauses = list_concat(other_clauses, extract_actual_clauses(rqpath->join_restrictlist, true));
    ...
    varlist = list_concat(pull_var_clause((Node*)tlist, PVC_REJECT_AGGREGATES, PVC_RECURSE_PLACEHOLDERS),
        pull_var_clause((Node*)*unshippable_quals, PVC_REJECT_AGGREGATES, PVC_RECURSE_PLACEHOLDERS));
    *rep_tlist = add_to_flat_tlist(NIL, varlist);
    list_free(varlist);
    tlist = copyObject(*rep_tlist);
    varlist = list_concat(pull_var_clause((Node*)tlist, PVC_REJECT_AGGREGATES, PVC_RECURSE_PLACEHOLDERS),
        pull_var_clause((Node*)join_clauses, PVC_REJECT_AGGREGATES, PVC_RECURSE_PLACEHOLDERS));
    varlist =
        list_concat(varlist, pull_var_clause((Node*)other_clauses, PVC_REJECT_AGGREGATES, PVC_RECURSE_PLACEHOLDERS));
    foreach (lcell, varlist) {
        ...
        if (tle) {
            ...
            if (tle) {
                ...
                elog(ERROR, "can not find var with varno = %u and varattno = %d", var->varno, var->varattno);
            }
        }
    }
    list_free(varlist);
    ...
    PlannerInfo* root, RemoteQueryPath* rqpath, List** unshippable_quals, List** rep_tlist)
{
    ...
    switch (parent_rel->reloptkind) {
        ...
        case RELOPT_OTHER_MEMBER_REL:
            result_query = pgxc_build_shippable_query_baserel(root, rqpath, unshippable_quals, rep_tlist);
            break;

        case RELOPT_JOINREL:
            result_query = pgxc_build_shippable_query_jointree(root, rqpath, unshippable_quals, rep_tlist);
            break;

        default:
            elog(ERROR, "Creating remote query plan for relations of type %d is not supported", parent_rel->reloptkind);
    }
    return result_query;
}
...
static void pgxc_rqplan_adjust_vars(RemoteQuery* rqplan, Node* node)
{
    List* var_list = pull_var_clause(node, PVC_RECURSE_AGGREGATES, PVC_RECURSE_PLACEHOLDERS);
    ListCell* lcell_var = NULL;
    ...
    foreach (lcell_var, var_list) {
        ...
        TargetEntry* ref_tle = tlist_member((Node*)var, rqplan->coord_var_tlist);
        Assert(ref_tle);
        TargetEntry* qry_tle = get_tle_by_resno(rqplan->query_var_tlist, ref_tle->resno);
        Assert(qry_tle);
 ...
 * inside it.
 */
static void pgxc_rqplan_build_statement(RemoteQuery* rqplan)
{
    StringInfo sql = makeStringInfo();
    deparse_query(rqplan->remote_query, sql, NULL, rqplan->rq_finalise_aggs, rqplan->rq_sortgroup_colno);
...
extern void pgxc_rqplan_adjust_tlist(RemoteQuery* rqplan)
{
     ...
     */
    base_tlist = pgxc_build_shippable_tlist(plan->targetlist, plan->qual, rqplan->remote_query->hasAggs);
 ...
 * RemoteQuery plan.
 */
static void pgxc_build_shippable_query(PlannerInfo* root, RemoteQueryPath* covering_path, RemoteQuery* result_node)
{
    Query* query = NULL;
    List* rep_tlist = NIL;
    List* unshippable_quals = NIL;
     ...
     */
    query = pgxc_build_shippable_query_recurse(root, covering_path, &unshippable_quals, &rep_tlist);
...
Plan* create_remotequery_plan(PlannerInfo* root, RemoteQueryPath* best_path)
{
    RelOptInfo* rel = best_path->path.parent; /* relation for which plan is being built */

    RemoteQuery* result_node = NULL; /* the built plan */
    List* tlist = NIL; /* expected target list */
    ...
    dummy_rte = make_dummy_remote_rte(rte_name, makeAlias("_REMOTE_TABLE_QUERY_", NIL));
    ...
    if (IsExecNodesReplicated(result_node->exec_nodes))
        result_node->exec_nodes->nodeList = GetPreferredReplicationNode(result_node->exec_nodes->nodeList);
...
static void pgxc_add_returning_list(RemoteQuery* rq, List* ret_list, int rel_index)
{
     ...
     */
    varlist = pull_var_clause((Node*)ret_list, PVC_REJECT_AGGREGATES, PVC_REJECT_PLACEHOLDERS);
    ...
    foreach (lc, varlist) {
        ...
        if (var->varno == rel_index)
            shipableReturningList = add_to_flat_tlist(shipableReturningList, list_make1(var));
    }
...
static void pgxc_dml_add_qual_to_query(Query* query, int param_num, AttrNumber sys_col_attno, Index varno)
{
    ...
    qual = make_op(NULL, list_make1(makeString("=")), (Node*)lhs_var, (Node*)rhs_param, -1);

    /* Add the qual to the qual list */
    query->jointree->quals = (Node*)lappend((List*)query->jointree->quals, (Node*)qual);
}
...
static void pgxc_build_dml_statement(
    PlannerInfo* root, CmdType cmdtype, Index resultRelationIndex, RemoteQuery* rqplan, List* sourceTargetList)
{
    Query* query_to_deparse = NULL;
    RangeTblRef* target_table_ref = NULL;
    RangeTblEntry* res_rel = NULL;
    ListCell* elt = NULL;
     ...
     */
    if (IsTempTable(res_rel->relid) || (!res_rel->alias && res_rel->eref && res_rel->eref->aliasname &&
    ...
    foreach (elt, sourceTargetList) {
        ...
        if (cmdtype == CMD_INSERT) {
             ...
             */
            pgxc_add_param_as_tle(query_to_deparse, tle->resno, exprType((Node*)tle->expr), NULL);
        }
    }
    ...
    if (cmdtype == CMD_UPDATE) {
        ...
        bool br_triggers = pgxc_should_exec_br_trigger(rel, pgxc_get_trigevent(cmdtype));
        ...
        if (br_triggers) {
            ...
            for (attnum = 1; attnum <= natts; attnum++) {
                pgxc_add_param_as_tle(
                    query_to_deparse, attnum, get_atttype(res_rel->relid, attnum), get_attname(res_rel->relid, attnum));
            }
        } else {
            foreach (elt, root->parse->targetList) {
                ...
                if (param_num == InvalidAttrNumber) {
                    elog(ERROR, "cache lookup failed for attribute %s of relation %u", qtle->resname, res_rel->relid);
                    return;
                }
                 ...
                 */
                pgxc_add_param_as_tle(query_to_deparse, param_num, exprType((Node*)qtle->expr), qtle->resname);
            }
        }
    ...
    if (cmdtype == CMD_UPDATE || cmdtype == CMD_DELETE) {
         ...
         */
        pgxc_dml_add_qual_to_query(
            query_to_deparse, ctid_param_num, SelfItemPointerAttributeNumber, resultRelationIndex);

        if (node_id_found) {
            pgxc_dml_add_qual_to_query(
                query_to_deparse, ctid_param_num + 1, XC_NodeIdAttributeNumber, resultRelationIndex);
...
Plan* create_remotedml_plan(PlannerInfo* root, Plan* topplan, CmdType cmdtyp)
{
    ...
    foreach (rel, mt->resultRelations) {
        ...
        RemoteQuery* fstep = NULL;
        RangeTblEntry* dummy_rte = NULL; /* RTE for the remote query node */
        Plan* sourceDataPlan = NULL;     /* plan producing source data */
        char* relname = NULL;
        ...
        if (mt->returningLists)
            pgxc_add_returning_list(fstep, list_nth(mt->returningLists, relcount), resultRelationIndex);

        /* Get the plan that is supposed to supply source data to this plan */
        sourceDataPlan = list_nth(mt->plans, relcount);

        pgxc_build_dml_statement(root, cmdtyp, resultRelationIndex, fstep, sourceDataPlan->targetlist);

        fstep->combine_type = get_plan_combine_type(cmdtyp, rel_loc_info->locatorType);
        ...
        if (cmdtyp == CMD_INSERT) {
            ...
            fstep->exec_nodes = GetRelationNodes(rel_loc_info, 0, true, UNKNOWNOID, accessType);
        }
...
Plan* create_remotegrouping_plan(PlannerInfo* root, Plan* local_plan)
{
    ...
    Sort* sort_plan = NULL;
    RemoteQuery* remote_scan = NULL; /* remote query in the passed in plan */
    Plan* temp_plan = NULL;
    List* base_tlist = NIL;
    RangeTblEntry* dummy_rte = NULL;
    int numGroupCols;
    AttrNumber* grpColIdx = NULL;
    List* remote_qual = NIL;
    List* local_qual = NIL;
    bool single_node_grouping = false; /* Can grouping be entirely performed on a given datanode? */
    List* remote_scan_tlist = NIL;
    Plan* local_plan_left = NULL;
     ...
     */
    remote_scan_tlist = pgxc_process_grouping_targetlist(local_plan->targetlist, single_node_grouping);
    remote_scan_tlist = pgxc_process_having_clause(
        remote_scan_tlist, single_node_grouping, query->havingQual, &local_qual, &remote_qual);
    /*
    ...
    if (sort_plan) {
        ...
        sort_plan = make_sort_from_groupcols(root, query->groupClause, grpColIdx, (Plan*)remote_scan);
 ...
 */
static bool pgxc_locate_grouping_columns(PlannerInfo* root, List* tlist, AttrNumber* groupColIdx)
{
    int keyno = 0;
    ListCell* gl = NULL;
...
static List* pgxc_add_to_flat_tlist(List* remote_tlist, Node* expr, Index ressortgroupref)
{
    ...
    if (!remote_tle) {
        remote_tle = makeTargetEntry(copyObject(expr), list_length(remote_tlist) + 1, NULL, false);
...
static List* pgxc_process_grouping_targetlist(List* local_tlist, bool single_node_grouping)
{
    ...
    if (single_node_grouping) {
        ...
        List* aggs_n_vars = pull_var_clause((Node*)local_tlist, PVC_INCLUDE_AGGREGATES, PVC_RECURSE_PLACEHOLDERS);
        ListCell* lcell = NULL;
        bool tmp_has_aggs = false;
        ...
        foreach (lcell, aggs_n_vars) {
             ...
             */
            if (IsA(av_expr, Aggref) && !pgxc_is_expr_shippable(av_expr, &tmp_has_aggs))
                return NULL;
        }
        ...
        foreach (temp, local_tlist) {
            ...
            aggs_n_vars = pull_var_clause(expr, PVC_INCLUDE_AGGREGATES, PVC_RECURSE_PLACEHOLDERS);
    ...
    *local_qual = NIL;
    ...
    foreach (temp, quals) {
        ...
        if (!shippable_qual || (!single_node_grouping && has_aggs)) {
            ...
            vars_n_aggs = pull_var_clause(expr, PVC_INCLUDE_AGGREGATES, PVC_RECURSE_PLACEHOLDERS);
            /* copy the aggregates into the remote target list */
            foreach (lcell, vars_n_aggs) {
                Node* agg_or_var = lfirst(lcell);
                bool tmp_has_agg = false;

                Assert(IsA(agg_or_var, Aggref) || IsA(agg_or_var, Var));
                if (IsA(agg_or_var, Aggref) && !pgxc_is_expr_shippable((Expr*)agg_or_var, &tmp_has_agg))
                    return NULL;
...
static Expr* pgxc_set_en_expr(Oid tableoid, Index resultRelationIndex)
{
    ...
    tp = SearchSysCache(ATTNUM, ObjectIdGetDatum(tableoid), Int16GetDatum(rel_loc_info->partAttrNum), 0, 0);
 ...
 */
static Var* make_ctid_col_ref(Query* qry)
{
    ListCell* lc1 = NULL;
    ListCell* lc2 = NULL;
    RangeTblEntry* rte1 = NULL;
    RangeTblEntry* rte2 = NULL;
...
static CombineType get_plan_combine_type(CmdType commandType, char baselocatortype)
{
    ...
    switch (commandType) {
        ...
        case CMD_UPDATE:
        case CMD_DELETE:
            return baselocatortype == LOCATOR_TYPE_REPLICATED ? COMBINE_TYPE_SAME : COMBINE_TYPE_SUM;
        default:
 ...
 */
static Oid get_fn_oid(char* fn_name, Oid* p_rettype)
{
    Value* fn_nm = NULL;
    List* fn_name_list = NIL;
    FuncDetailCode fdc;
    bool retset = false ;
    int nvargs;
    Oid* true_typeids = NULL;
    Oid func_oid;
 ...
 */
static void fetch_ctid_of(Plan* subtree, Query* query)
{
...
static RemoteQuery* pgxc_FQS_create_remote_plan(Query* query, ExecNodes* exec_nodes, bool is_exec_direct)
{
     ...
     */
    query_step->combine_type = get_plan_combine_type(query->commandType, query_step->exec_nodes->baselocatortype);
    /*
 ...
 */
Plan* create_remotesort_plan(PlannerInfo* root, Plan* local_plan)
{
    ...
    foreach (lcell, remote_query->targetList) {
         ...
         */
        if (tle->ressortgroupref > 0 && next_ressortgrpref <= tle->ressortgroupref)
    ...
    for (cntCol = 0; cntCol < local_sort->numCols; cntCol++) {
        TargetEntry* remote_base_tle = NULL; /* TLE in the remote_scan's base_tlist */
        TargetEntry* remote_tle = NULL; /* TLE in the remote_scan's targetlist */
        SortGroupClause* orderby_entry = NULL;
        TargetEntry* remote_query_tle = NULL; /* TLE in targetlist of the query tobe sent to the datanodes */

        remote_tle = get_tle_by_resno(remote_scan->scan.plan.targetlist, local_sort->sortColIdx[cntCol]);
        ...
        remote_base_tle = tlist_member((Node*)remote_tle->expr, remote_scan->base_tlist);
        /*
        ...
        remote_query_tle = get_tle_by_resno(remote_query->targetList, remote_base_tle->resno);
        /*
         ...
         */
        if (!remote_query_tle || remote_query_tle->ressortgroupref != remote_base_tle->ressortgroupref ||
    ...
    Assert(result_plan == (Plan*)remote_scan || result_plan == (Plan*)local_sort);
...
Plan* create_remotelimit_plan(PlannerInfo* root, Plan* local_plan)
{
    ...
    if (remote_scan->exec_nodes && list_length(remote_scan->exec_nodes->nodeList) == 1) {
        /*
...
static List* pgxc_collect_RTE(Query* query)
{
    ...
    (void)query_tree_walker(query, pgxc_collect_RTE_walker, &crte_context, 0);
...
static bool pgxc_collect_RTE_walker(Node* node, collect_RTE_context* crte_context)
{
    ...
    if (IsA(node, Query)) {
        ...
        crte_context->crte_rtable = list_concat(crte_context->crte_rtable, query->rtable);
        return query_tree_walker(query, pgxc_collect_RTE_walker, crte_context, 0);
    }
