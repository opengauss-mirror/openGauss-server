...
typedef struct AsyncQueueControl {
    ...
    QueueBackendStatus backend[1]; /* actually of length g_instance.shmem_cxt.MaxBackends+1 */
...
static void asyncQueueUnregister(void)
{
    ...
    Assert(t_thrd.asy_cxt.listenChannels == NIL); /* else caller error */

    if (!t_thrd.asy_cxt.amRegisteredListener) /* nothing to do */
        return;
...
void AtSubStart_Notify(void)
{
    ...
    /* Keep the list-of-lists in u_sess->top_transaction_mem_cxt for simplicity */
