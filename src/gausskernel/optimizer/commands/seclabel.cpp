 ...
 *
 * IDENTIFICATION
 *	  src/gausskernel/optimizer/commands/seclabel.cpp
 *
 * -------------------------------------------------------------------------
 */
#include "postgres.h"
...
ObjectAddress ExecSecLabelStmt(SecLabelStmt* stmt)
{
    ...
    if (stmt->provider == NULL) {
        if (u_sess->cmd_cxt.label_provider_list == NIL)
            ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("no security label providers have been loaded")));
        if (lnext(list_head(u_sess->cmd_cxt.label_provider_list)) != NULL)
            ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("must specify provider when multiple security label providers have been loaded")));
        provider = (LabelProvider*)linitial(u_sess->cmd_cxt.label_provider_list);
    } else {
        foreach (lc, u_sess->cmd_cxt.label_provider_list) {
            LabelProvider* lp = (LabelProvider*)lfirst(lc);
        ...
        if (provider == NULL)
            ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("security label provider \"%s\" is not loaded", stmt->provider)));
    }
...
static char* GetSharedSecurityLabel(const ObjectAddress* object, const char* provider)
{
    ...
    ScanKeyInit(
        &keys[1], Anum_pg_shseclabel_classoid, BTEqualStrategyNumber, F_OIDEQ, ObjectIdGetDatum(object->classId));
...
char* GetSecurityLabel(const ObjectAddress* object, const char* provider)
{
    ...
    ScanKeyInit(
        &keys[2], Anum_pg_seclabel_objsubid, BTEqualStrategyNumber, F_INT4EQ, Int32GetDatum(object->objectSubId));
...
static void SetSharedSecurityLabel(const ObjectAddress* object, const char* provider, const char* label)
{
    ...
    errno_t errorno = memset_s(nulls, sizeof(nulls), false, sizeof(nulls));
    securec_check(errorno, "\0", "\0");
    errorno = memset_s(replaces, sizeof(replaces), false, sizeof(replaces));
    securec_check(errorno, "\0", "\0");
    ...
    if (newtup == NULL && label != NULL) {
        ...
        (void)simple_heap_insert(pg_shseclabel, newtup);
    }
...
void SetSecurityLabel(const ObjectAddress* object, const char* provider, const char* label)
{
    ...
    if (newtup == NULL && label != NULL) {
        ...
        (void)simple_heap_insert(pg_seclabel, newtup);
    }
...
void DeleteSecurityLabel(const ObjectAddress* object)
{
    ...
    if (object->objectSubId != 0) {
        ScanKeyInit(
            &skey[2], Anum_pg_seclabel_objsubid, BTEqualStrategyNumber, F_INT4EQ, Int32GetDatum(object->objectSubId));
        nkeys = 3;
    } else
        nkeys = 2;
...
void register_label_provider(const char* provider_name, check_object_relabel_type hook)
{
    ...
    provider->hook = hook;
    u_sess->cmd_cxt.label_provider_list = lappend(u_sess->cmd_cxt.label_provider_list, provider);
