...
static void intorel_startup(DestReceiver* self, int operation, TupleDesc typeinfo)
{
     ...
     */
    myState->hi_options = HEAP_INSERT_SKIP_FSM | (XLogIsNeeded() ? 0 : HEAP_INSERT_SKIP_WAL);
...
static void intorel_receive(TupleTableSlot* slot, DestReceiver* self)
{
    ...
    heap_insert(myState->rel, tuple, myState->output_cid, myState->hi_options, myState->bistate);
...
static void intorel_shutdown(DestReceiver* self)
{
    ...
    if ((myState->hi_options & HEAP_INSERT_SKIP_WAL) || enable_heap_bcm_data_replication())
