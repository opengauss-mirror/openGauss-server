...
if(${BUILD_TUPLE} STREQUAL "aarch64")
    if($ENV{DEBUG_TYPE} STREQUAL "release" AND  ${ENABLE_MULTIPLE_NODES} STREQUAL "OFF" AND ${ENABLE_LSE} STREQUAL "ON")
        ...
        set(OS_OPTIONS -march=armv8.2-a+crc+lse)
    else()
        set(OS_OPTIONS -march=armv8.2-a+crc)
    endif()
endif()
...
if(ENABLE_PYTHON3)
    ...
    if(IS_DIRECTORY ${3RD_PATH_PY})
        set(PYTHON_CMP_MAJOR_VERSION "3")
        set(PYTHON_CMP_MINOR_VERSION "7")
        set(PYTHON_CMP_PYMALLOC "0")
        set(PYTHON_LIB_NAME "\"libpython3.7m.so\"")
    else()
        ...
        if(Python3_Interpreter_FOUND)
            ...
            set(PYTHON_CMP_MAJOR_VERSION ${Python3_VERSION_MAJOR})
            message(STATUS "PYTHON_CMP_MAJOR_VERSION is ${Python3_VERSION_MAJOR}")
            set(PYTHON_CMP_MINOR_VERSION ${Python3_VERSION_MINOR})
            message(STATUS "PYTHON_CMP_MINOR_VERSION is ${Python3_VERSION_MINOR}")
            if( "${PYTHON_CMP_MAJOR_VERSION}.${PYTHON_CMP_MINOR_VERSION}" VERSION_LESS "3.7" )
                message(FATAL_ERROR "python version must not less than 3.7")
            endif()
            set(PYTHON_CMP_PYMALLOC "0")
            get_filename_component(PYTHON_LIB "${Python3_LIBRARY}" NAME)
            set(PYTHON_LIB_NAME \"${PYTHON_LIB}\")
            message(STATUS "PYTHON_LIB_NAME is ${PYTHON_LIB_NAME}")
        else()
            message(FATAL_ERROR "python3.x is not installed, please install 3.x (x >= 7) version and try again")
        endif()
    endif()
endif()
