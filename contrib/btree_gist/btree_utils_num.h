...
#define penalty_num(result, olower, oupper, nlower, nupper)                                                      \
    do {                                                                                                         \
        ...
        if (tmp > 0.0F) {                                                                                        \
            (*(result)) += FLT_MIN;                                                                              \
            (*(result)) += (float)(((double)(tmp)) /                                                             \
                                   ((double)(tmp) + (((double)(oupper)) * 0.49F - ((double)(olower)) * 0.49F))); \
...
#define INTERVAL_TO_SEC(ivp)                                                                 \
    (((double)(ivp)->time) / ((double)USECS_PER_SEC) + (ivp)->day * (24.0 * SECS_PER_HOUR) + \
        (ivp)->month * (30.0 * SECS_PER_DAY))
#else
#define INTERVAL_TO_SEC(ivp) ((ivp)->time + (ivp)->day * (24.0 * SECS_PER_HOUR) + (ivp)->month * (30.0 * SECS_PER_DAY))
#endif
...
extern bool gbt_num_consistent(const GBT_NUMKEY_R* key, const void* query, const StrategyNumber* strategy, bool is_leaf,
    const gbtree_ninfo* tinfo);

extern float8 gbt_num_distance(const GBT_NUMKEY_R* key, const void* query, bool is_leaf, const gbtree_ninfo* tinfo);

extern GIST_SPLITVEC* gbt_num_picksplit(const GistEntryVector* entryvec, GIST_SPLITVEC* v, const gbtree_ninfo* tinfo);

extern GISTENTRY* gbt_num_compress(GISTENTRY* retval, GISTENTRY* entry, const gbtree_ninfo* tinfo);

extern void* gbt_num_union(GBT_NUMKEY* out, const GistEntryVector* entryvec, const gbtree_ninfo* tinfo);

extern bool gbt_num_same(const GBT_NUMKEY* a, const GBT_NUMKEY* b, const gbtree_ninfo* tinfo);

extern void gbt_num_bin_union(Datum* u, GBT_NUMKEY* e, const gbtree_ninfo* tinfo);
