...
static void deparseColumnRef(StringInfo buf, Index varno, int varattno, RangeTblEntry *rte);
 ...
 */
void deparseSelectSql(StringInfo buf, PlannerInfo *root, RelOptInfo *baserel, Bitmapset *attrs_used,
    List **retrieved_attrs)
{
    ...
    deparseTargetList(buf, rte, baserel->relid, rel, false, attrs_used, retrieved_attrs);
    ...
    Bitmapset *attrs_used, List **retrieved_attrs)
{
    ...
    *retrieved_attrs = NIL;
    ...
    bool have_wholerow = bms_is_member(0 - FirstLowInvalidHeapAttributeNumber, attrs_used);
    ...
    for (i = 1; i <= tupdesc->natts; i++) {
        ...
        if (have_wholerow || bms_is_member(i - FirstLowInvalidHeapAttributeNumber, attrs_used)) {
            ...
            deparseColumnRef(buf, rtindex, i, rte);
    ...
    List *returningList, List **retrieved_attrs)
{
    ...
    if (targetAttrs) {
        ...
        foreach (lc, targetAttrs) {
            ...
            deparseColumnRef(buf, rtindex, attnum, rte);
        }
    ...
    List *returningList, List **retrieved_attrs)
{
    ...
    if (attrs_used != NULL) {
        deparseTargetList(buf, rte, rtindex, rel, true, attrs_used, retrieved_attrs);
    } else {
        *retrieved_attrs = NIL;
    }
}
...
static void deparseVar(Var *node, deparse_expr_cxt *context)
{
    ...
    if (node->varno == context->foreignrel->relid && node->varlevelsup == 0) {
        /* Var belongs to foreign table */
        deparseColumnRef(buf, node->varno, node->varattno, planner_rt_fetch(node->varno, context->root));
    } else {
        /* Treat like a Param */
