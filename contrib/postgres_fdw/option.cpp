...
extern "C" Datum postgres_fdw_validator(PG_FUNCTION_ARGS);
...
Datum postgres_fdw_validator(PG_FUNCTION_ARGS)
{
    ...
    foreach (cell, options_list) {
         ...
         */
        if (strcmp(def->defname, "use_remote_estimate") == 0 || strcmp(def->defname, "updatable") == 0) {
            /* these accept only boolean values */
            (void)defGetBoolean(def);
        } else if (strcmp(def->defname, "fdw_startup_cost") == 0 || strcmp(def->defname, "fdw_tuple_cost") == 0) {
            char *endp = NULL;
            /* these must have a non-negative numeric value */
            double val = strtod(defGetString(def), &endp);
            if (*endp || val < 0) {
                ereport(ERROR,
                    (errcode(ERRCODE_SYNTAX_ERROR), errmsg("%s requires a non-negative numeric value", def->defname)));
            }
        }
    }
...
static void InitPgFdwOptions(void)
{
     ...
     * string.
     */
    postgres_fdw_options = (PgFdwOption *)malloc(sizeof(PgFdwOption) * num_libpq_opts + sizeof(non_libpq_options));
    ...
    for (lopt = libpq_options; lopt->keyword; lopt++) {
        ...
        if (strchr(lopt->dispchar, 'D') || strcmp(lopt->keyword, "fallback_application_name") == 0 ||
    ...
    int rc = memcpy_s(popt, sizeof(non_libpq_options), non_libpq_options, sizeof(non_libpq_options));
    securec_check(rc, "", "");
}
