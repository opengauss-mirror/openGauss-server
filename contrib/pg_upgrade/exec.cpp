...
#define MAXCMDLEN (4 * MAXPGPATH)
    ...
    nRet = snprintf_s(
        cmd + written, MAXCMDLEN - written, MAXCMDLEN - written - 1, " >> \"%s\" 2>&1" SYSTEMQUOTE, log_file);
    ...
    {
        /*
         * "gs_ctl -w stop" might have reported that the server has
...
bool is_server_running(const char* datadir)
{
    ...
    nRet = snprintf_s(path, sizeof(path), sizeof(path) - 1, "%s/postmaster.pid", datadir);
    securec_check_ss_c(nRet, "\0", "\0");
...
#endif
        pg_log(PG_FATAL, "You must have read and write access in the current directory.\n");
...
static void check_data_dir(const char* pg_data)
{
    ...
    const char* requiredSubdirs[] = {
        "", "base", "global", "pg_clog", "pg_multixact", "pg_subtrans", "pg_tblspc", "pg_twophase", "pg_xlog"};

    for (subdirnum = 0; subdirnum < (int)(sizeof(requiredSubdirs) / sizeof(requiredSubdirs[0])); ++subdirnum) {
        struct stat statBuf;

        nRet = snprintf_s(subDirName,
            sizeof(subDirName),
            sizeof(subDirName) - 1,
            "%s%s%s",
            pg_data,
...
static void check_bin_dir(ClusterInfo* cluster)
{
    ...
    validate_exec(cluster->bindir, "gaussdb");
    validate_exec(cluster->bindir, "gs_ctl");
    validate_exec(cluster->bindir, "pg_resetxlog");
    if (cluster == &new_cluster) {
        /* these are only needed in the new cluster */
        validate_exec(cluster->bindir, "gsql");
        validate_exec(cluster->bindir, "gs_dumpall");
    }
}
...
static void validate_exec(const char* dir, const char* cmdName)
{
    ...
    nRet = snprintf_s(path, sizeof(path), sizeof(path) - 1, "%s/%s", dir, cmdName);
    securec_check_ss_c(nRet, "\0", "\0");
