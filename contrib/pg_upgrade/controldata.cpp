...
void get_control_data(ClusterInfo* cluster, bool live_check, bool xid_8B)
{
    ...
    while (fgets(bufin, sizeof(bufin), output)) {
        ...
        if ((p = strstr(bufin, "pg_control version number:")) != NULL) {
            ...
            if (strspn(p, "0123456789ABCDEF") != XLOG_NAME_LENGTH)
                pg_log(PG_FATAL, "%d: controldata retrieval problem\n", __LINE__);

            nRet = strncpy_s(cluster->controldata.nextxlogfile, XLOG_NAME_LENGTH + 1, p, XLOG_NAME_LENGTH + 1);
            ...
            op++; /* removing ':' char */
            cluster->controldata.chkpnt_nxtxid = str2uint64(op);
            got_xid = true;
            ...
            p++; /* removing ':' char */
            if (xid_8B) {
                p = strchr(p, ':'); /* the ole NextXID format is "x:x" */
                p++;
            }
    ...
    if (GET_MAJOR_VERSION(cluster->major_version) <= 902) {
        if (got_log_id && got_log_seg) {
            nRet = snprintf_s(cluster->controldata.nextxlogfile,
                XLOG_NAME_LENGTH + 1,
                XLOG_NAME_LENGTH,
                "%08X%08X%08X",
                tli,
    ...
    if (!got_xid || !got_oid || (!live_check && !got_nextxlogfile) || !got_tli || !got_align || !got_blocksz ||
        !got_largesz || !got_walsz || !got_walseg || !got_ident || !got_index || !got_toast || !got_date_is_int ||
        !got_float8_pass_by_value) {
        pg_log(PG_REPORT, "Some required control information is missing;  cannot find:\n");
...
void get_control_data_xlog_info(ClusterInfo* cluster)
{
    ...
    while (fgets(bufin, sizeof(bufin), output)) {
        ...
        if ((p = strstr(bufin, "First log file ID after reset:")) != NULL) {
            p = strchr(p, ':');
    ...
    /* verify that we got all the mandatory pg_control data */
    if (!got_nextxlogfile || !got_xid || !got_oid) {
        pg_log(PG_REPORT, "Some required control information is missing;  cannot find:\n");
...
void check_control_data(ControlData* oldctrl, ControlData* newctrl)
{
    if (oldctrl->align == 0 || oldctrl->align != newctrl->align)
        pg_log(PG_FATAL, "old and new pg_controldata alignments are invalid or do not match\n");

    if (oldctrl->blocksz == 0 || oldctrl->blocksz != newctrl->blocksz)
        pg_log(PG_FATAL, "old and new pg_controldata block sizes are invalid or do not match\n");

    if (oldctrl->largesz == 0 || oldctrl->largesz != newctrl->largesz)
        pg_log(PG_FATAL, "old and new pg_controldata maximum relation segment sizes are invalid or do not match\n");

    if (oldctrl->walsz == 0 || oldctrl->walsz != newctrl->walsz)
        pg_log(PG_FATAL, "old and new pg_controldata WAL block sizes are invalid or do not match\n");

    if (oldctrl->walseg == 0 || oldctrl->walseg != newctrl->walseg)
        pg_log(PG_FATAL, "old and new pg_controldata WAL segment sizes are invalid or do not match\n");

    if (oldctrl->ident == 0 || oldctrl->ident != newctrl->ident)
        pg_log(PG_FATAL, "old and new pg_controldata maximum identifier lengths are invalid or do not match\n");

    if (oldctrl->index == 0 || oldctrl->index != newctrl->index)
        pg_log(PG_FATAL, "old and new pg_controldata maximum indexed columns are invalid or do not match\n");

    if (oldctrl->toast == 0 || oldctrl->toast != newctrl->toast)
        pg_log(PG_FATAL, "old and new pg_controldata maximum TOAST chunk sizes are invalid or do not match\n");
...
void disable_old_cluster(void)
{
    ...
    nRet = snprintf_s(old_path, sizeof(old_path), sizeof(old_path) - 1, "%s/global/pg_control", old_cluster.pgdata);
    securec_check_ss_c(nRet, "\0", "\0");
    nRet = snprintf_s(new_path, sizeof(new_path), sizeof(new_path) - 1, "%s/global/pg_control.old", old_cluster.pgdata);
    securec_check_ss_c(nRet, "\0", "\0");

    if (pg_mv_file(old_path, new_path) != 0)
        pg_log(PG_FATAL, "Unable to rename %s to %s.\n", old_path, new_path);

    if ((out_file = fopen(new_path, "r")) == NULL)
        pg_log(PG_FATAL, "Unable to fopen file %s.\n", new_path);

    /* fsync the new_path file immediately, in case of an unfortunate system crash */
    if (fsync(fileno(out_file)) != 0) {
        (void)fclose(out_file);
        pg_log(PG_FATAL, "Unable to fsync file%s.\n", new_path);
    }

    if (fclose(out_file) != 0)
        pg_log(PG_FATAL, "Unable to flose file %s.\n", new_path);
