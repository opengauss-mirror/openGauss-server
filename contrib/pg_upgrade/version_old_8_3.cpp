...
void old_8_3_check_for_name_data_type_usage(ClusterInfo* cluster)
{
    ...
    nRet = snprintf_s(output_path, sizeof(output_path), sizeof(output_path) - 1, "tables_using_name.txt");
...
void old_8_3_check_for_tsquery_usage(ClusterInfo* cluster)
{
    ...
    nRet = snprintf_s(output_path, sizeof(output_path), sizeof(output_path) - 1, "tables_using_tsquery.txt");
    securec_check_ss_c(nRet, "\0", "\0");
...
void old_8_3_check_ltree_usage(ClusterInfo* cluster)
{
    ...
    nRet = snprintf_s(output_path, sizeof(output_path), sizeof(output_path) - 1, "contrib_ltree.txt");
    securec_check_ss_c(nRet, "\0", "\0");
...
void old_8_3_rebuild_tsvector_tables(ClusterInfo* cluster, bool check_mode)
{
    ...
    nRet = snprintf_s(output_path, sizeof(output_path), sizeof(output_path) - 1, "rebuild_tsvector_tables.sql");
    securec_check_ss_c(nRet, "\0", "\0");

    for (dbnum = 0; dbnum < cluster->dbarr.ndbs; dbnum++) {
        PGresult* res = NULL;
        bool db_used = false;
        char nspname[NAMEDATALEN] = "", relname[NAMEDATALEN] = "";
        int ntups;
        int rowno;
        int i_nspname, i_relname, i_attname;
        DbInfo* active_db = &cluster->dbarr.dbs[dbnum];
    ...
    if (found) {
        ...
        if (check_mode)
            ...
            pg_log(PG_WARNING,
                ...
                "    %s\n"
                "when executed by gsql by the database system admin will rebuild all tables\n"
...
void old_8_3_invalidate_hash_gin_indexes(ClusterInfo* cluster, bool check_mode)
{
    ...
    nRet = snprintf_s(output_path, sizeof(output_path), sizeof(output_path) - 1, "reindex_hash_and_gin.sql");
    securec_check_ss_c(nRet, "\0", "\0");
    ...
    for (dbnum = 0; dbnum < cluster->dbarr.ndbs; dbnum++) {
        ...
        res = executeQueryOrDie(conn,
            ...
            "		c.relam = a.oid AND "
            "		c.relnamespace = n.oid AND "
            "		a.amname IN ('hash', 'gin') AND " SKIP_TSVECTOR_TABLES);
    ...
    if (found) {
        ...
        if (check_mode)
            ...
            pg_log(PG_WARNING,
                ...
                "    %s\n"
                "when executed by gsql by the database system admin will recreate all invalid\n"
...
void old_8_3_invalidate_bpchar_pattern_ops_indexes(ClusterInfo* cluster, bool check_mode)
{
    ...
    nRet = snprintf_s(output_path, sizeof(output_path), sizeof(output_path) - 1, "reindex_bpchar_ops.sql");
    securec_check_ss_c(nRet, "\0", "\0");
    ...
    for (dbnum = 0; dbnum < cluster->dbarr.ndbs; dbnum++) {
        ...
        res = executeQueryOrDie(conn,
            ...
            "		= ANY (i.indclass) AND " SKIP_TSVECTOR_TABLES);
...
char* old_8_3_create_sequence_script(ClusterInfo* cluster)
{
    ...
    FILE* script = NULL;
    bool found = false;
    char* output_path = (char*)pg_malloc(MAXPGPATH);
    int nRet = 0;

    nRet = snprintf_s(output_path, MAXPGPATH, MAXPGPATH - 1, "adjust_sequences.sql");
    securec_check_ss_c(nRet, "\0", "\0");
    ...
    for (dbnum = 0; dbnum < cluster->dbarr.ndbs; dbnum++) {
        ...
        for (rowno = 0; rowno < ntups; rowno++) {
            ...
            fprintf(script,
                ...
                quote_identifier(nspname),
                quote_identifier(relname),
                PQgetvalue(seq_res, 0, i_last_value),
                PQgetvalue(seq_res, 0, i_is_called));
            PQclear(seq_res);
        }
        if (db_used)
            fprintf(script, "\n");
