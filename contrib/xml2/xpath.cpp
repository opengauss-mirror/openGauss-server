...
static xmlChar* pgxmlNodeSetToText(xmlNodeSetPtr nodeset, xmlChar* toptagname, xmlChar* septagname, xmlChar* plainsep)
{
    ...
    if (nodeset != NULL) {
        for (i = 0; i < nodeset->nodeNr; i++) {
            if (plainsep != NULL) {
                xmlBufferWriteCHAR(buf, xmlXPathCastNodeToString(nodeset->nodeTab[i]));
                ...
                xmlNodeDump(buf, nodeset->nodeTab[i]->doc, nodeset->nodeTab[i], 1, 0);
...
static xmlXPathObjectPtr pgxml_xpath(text* document, xmlChar* xpath, xpath_workspace* workspace)
{
    ...
    PG_TRY();
    {
        ...
        if (workspace->doctree != NULL) {
            ...
            if (comppath == NULL)
                xml_ereport(xmlerrcxt, ERROR, ERRCODE_EXTERNAL_ROUTINE_EXCEPTION, "XPath Syntax Error");
...
static text* pgxml_result_to_text(xmlXPathObjectPtr res, xmlChar* toptag, xmlChar* septag, xmlChar* plainsep)
{
    ...
    switch (res->type) {
        case XPATH_NODESET:
            xpresstr = pgxmlNodeSetToText(res->nodesetval, toptag, septag, plainsep);
            break;
...
Datum xpath_table(PG_FUNCTION_ARGS)
{
    ...
    int rownr;               /* For issuing multiple rows from one original
                              * document */
    bool had_values = false; /* To determine end of nodeset results */
    ...
    if (rsinfo->expectedDesc == NULL)
        ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR), errmsg("xpath_table must be called as a table function")));
    ...
    if (ret_tupdesc->natts < 1)
        ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR), errmsg("xpath_table must have at least one output column")));
    ...
    appendStringInfo(&query_buf, "SELECT %s, %s FROM %s WHERE %s", pkeyfield, xmlfield, relname, condition);
