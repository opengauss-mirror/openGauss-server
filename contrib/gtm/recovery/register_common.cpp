    ...
    for (i = 0; i < NODE_HASH_TABLE_SIZE; i++) {
        ...
        gtm_foreach(elem, bucket->nhb_list)
        {
            ...
            if (cur != NULL && cur->type == type) {
                data[node] = cur;
                elog(DEBUG1, "pgxcnode_find_by_type: ipaddress=%s", cur->ipaddress);
                node++;
            }
...
static GTM_PGXCNodeInfo* pgxcnode_find_info(GTM_PGXCNodeType type, char* node_name)
{
    ...
    {
        curr_nodeinfo = (GTM_PGXCNodeInfo*)gtm_lfirst(elem);
        if (pgxcnode_type_equal(curr_nodeinfo->type, type) && (strcmp(curr_nodeinfo->nodename, node_name) == 0)) {
            break;
        }
        curr_nodeinfo = NULL;
    }
 ...
 */
static int pgxcnode_remove_info(GTM_PGXCNodeInfo* nodeinfo)
{
 ...
 */
static int pgxcnode_add_info(GTM_PGXCNodeInfo* nodeinfo)
{
    uint32 hash = pgxcnode_gethash(nodeinfo->nodename);
    GTM_PGXCNodeInfoHashBucket* bucket;
    gtm_ListCell* elem;
    size_t len = strlen(nodeinfo->datafolder);
    ...
    gtm_foreach(elem, bucket->nhb_list)
    {
        ...
        if (pgxcnode_type_equal(curr_nodeinfo->type, nodeinfo->type) &&
            ...
            if (curr_nodeinfo->status == NODE_CONNECTED) {
                GTM_RWLockRelease(&bucket->nhb_lock);
                ereport(LOG, (EEXIST, errmsg("Node with the given ID number already exists")));
                return EEXIST;
            } else {
                /*
                ...
                if (strlen(curr_nodeinfo->datafolder) == strlen(nodeinfo->datafolder)) {
                    if (memcpy_s(curr_nodeinfo->datafolder, len, nodeinfo->datafolder, len) != EOK) {
...
static char* pgxcnode_copy_char(const char* str)
{
     ...
     * contextes.
     */
    retstr = (char*)MemoryContextAlloc(TopMostMemoryContext, strlen(str) + 1);

    if (retstr == NULL) {
        ereport(ERROR, (ENOMEM, errmsg("Out of memory")));
    }

    rc = memcpy_s(retstr, len, str, len);
...
int Recovery_PGXCNodeRegister(GTM_PGXCNodeType type, char* nodename, GTM_PGXCNodePort port, char* proxyname,
    GTM_PGXCNodeStatus status, char* ipaddress, char* datafolder, bool in_recovery, int socket)
{
    ...
    elog(DEBUG1,
        "Recovery_PGXCNodeRegister Request info: type=%d, nodename=%s, port=%d,"
        ...
        status);
    elog(DEBUG1,
        "Recovery_PGXCNodeRegister Node info: type=%d, nodename=%s, port=%d, "
        "datafolder=%s, ipaddress=%s, status=%d",
        nodeinfo->type,
        nodeinfo->nodename,
        nodeinfo->port,
...
#define Write(x, y, z)                                                 \
    do {                                                               \
        if (write(x, y, z) != (z))                                     \
            elog(WARNING, "Write error in Recovery_SaveRegisterInfo"); \
    } while (0)
    GTM_PGXCNodeInfoHashBucket* bucket;
    gtm_ListCell* elem;
    GTM_PGXCNodeInfo* nodeinfo = NULL;
    ...
    rc = sprintf_s(filebkp, GTM_NODE_FILE_MAX_PATH, "%s.bkp", GTMPGXCNodeFile);
    securec_check_ss(rc, "\0", "\0");

    ctlfd = open(filebkp, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
...
#define Write(x, y, z)                                                   \
    do {                                                                 \
        if (write(x, y, z) != (z))                                       \
            elog(WARNING, "Write error in Recovery_RecordRegisterInfo"); \
    } while (0)
    int ctlfd;
    int len;
    ...
    ctlfd = open(GTMPGXCNodeFile, O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR);
...
#define Read(x, y, z)                                                    \
    do {                                                                 \
        if (read(x, y, z) != (z))                                        \
            elog(WARNING, "Read error in Recovery_RestoreRegisterInfo"); \
    } while (0)
    ...
    while (read(ctlfd, &magic, sizeof(NodeRegisterMagic)) == sizeof(NodeRegisterMagic)) {
        ...
        if (magic == NodeRegisterMagic) {
            Recovery_PGXCNodeRegister(type, nodename, port, proxyname, status, ipaddress, datafolder, true, 0);
        } else {
...
void Recovery_SaveRegisterFileName(char* dir)
{
    ...
    rc = sprintf_s(GTMPGXCNodeFile, GTM_NODE_FILE_MAX_PATH, "%s/%s", dir, GTM_NODE_FILE);
    securec_check_ss(rc, "\0", "\0");  
}
...
void Recovery_PGXCNodeDisconnect(Port* myport)
{
    ...
    if (!myport->is_postmaster) { return; }
...
void ProcessPGXCNodeBackendDisconnect(Port* myport, StringInfo message)
{
    ...
    if (is_postmaster) {
        ...
        rc = memcpy_s(&type, sizeof(GTM_PGXCNodeType), pq_getmsgbytes(message, sizeof(GTM_PGXCNodeType)), 
        ...
        rc = memcpy_s(node_name, len, (char*)pq_getmsgbytes(message, len), len);
        securec_check(rc, "\0", "\0");
        node_name[len] = '\0';
    }
    ...
    if (GetMyThreadInfo->thr_conn->standby) {
        ...
        elog(DEBUG1, "forwarding MSG_BACKEND_DISCONNECT to standby GTM.");

    retry:
        _rc = backend_disconnect(GetMyThreadInfo->thr_conn->standby, is_postmaster, type, node_name);
