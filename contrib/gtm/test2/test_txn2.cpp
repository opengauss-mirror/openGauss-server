...
void test_txn_21()
{
    ...
    rc = begin_transaction_multi(
        conn, txn_count, txn_isolation_level, txn_read_only, txn_connid, &txn_count2, &start_gxid, &start_ts);
    _ASSERT(rc >= 0);
...
void test_txn_23()
{
    ...
    rc = snapshot_get_multi(
        conn, txn_count, gxid, &txn_count2, &status, &xmin_out, &xmax_out, &recent_global_xmin_out, &xcnt_out);
    _ASSERT(rc >= 0);
    _ASSERT(txn_count2 == 2);
