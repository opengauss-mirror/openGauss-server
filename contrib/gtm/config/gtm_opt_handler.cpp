 ...
 */
void ProcessConfigFile(GtmOptContext context)
{
    int elevel;
    ...
    for (item = head; item; item = item->next) {
        ...
        if (!set_config_option(item->name, item->value, context, GTMC_S_FILE, false)) {
            goto cleanup_list;
        }
    }
    ...
    for (i = 0; i < num_gtm_opt_variables; i++) {
        ...
        if (gconf->reset_source != GTMC_S_FILE || (gconf->status & GTMOPT_IS_IN_FILE)) {
            continue;
        }
        ...
        if (gconf->context < GTMC_SIGHUP) {
             ...
             */
            if (isStartUp) {
                write_stderr("parameter \"%s\" cannot be changed without restarting the server", gconf->name);
            } else {
                ereport(elevel,
                    (0, errmsg("parameter \"%s\" cannot be changed without restarting the server", gconf->name)));
            }
            continue;
        }
        ...
        set_config_option(gconf->name, NULL, context, GTMC_S_DEFAULT, true);
        ...
        if (context == GTMC_SIGHUP) {
            if (isStartUp) {
                ...
                ereport(elevel,
                    (errmsg("parameter \"%s\" removed from configuration file, reset to default", gconf->name)));
            }
        }
    }
    ...
    for (item = head; item; item = item->next) {
        ...
        if (set_config_option(item->name, item->value, context, GTMC_S_FILE, true)) {
 ...
 */
bool ParseConfigFile(const char* config_file, const char* calling_file, int depth, int elevel, ConfigVariable** head_p,
    ConfigVariable** tail_p)
{
    ...
    if (depth > 10) {
        if (isStartUp) {
            ...
            ereport(elevel,
                (0, errmsg("could not open configuration file \"%s\": maximum nesting depth exceeded", config_file)));
        }
        return false;
    }
    ...
    if (!fp) {
        if (isStartUp) {
            ...
            ereport(elevel, (0, errmsg("could not open configuration file \"%s\": %m", config_file)));
        }
        return false;
    }
    ...
    FILE* fp, const char* config_file, int depth, int elevel, ConfigVariable** head_p, ConfigVariable** tail_p)
{
    ...
    while ((token = GTMOPT_yylex())) {
        ...
        if (token != GTMOPT_ID && token != GTMOPT_STRING && token != GTMOPT_INTEGER && token != GTMOPT_REAL &&
            token != GTMOPT_UNQUOTED_STRING) {
            goto parse_error;
        }

        /* strip quotes and escapes */
        if (token == GTMOPT_STRING) {
        ...
        if (gtm_opt_name_compare(opt_name, "include") == 0) {
            ...
            if (!ParseConfigFile(opt_value, config_file, depth + 1, elevel, head_p, tail_p)) {
                free(opt_name);
                free(opt_value);
                OK = false;
                goto cleanup_exit;
             ...
             */
            item = *head_p;
            if (item != NULL && gtm_opt_name_compare(item->name, "custom_variable_classes") == 0) {
    ...
    if (token == GTMOPT_EOL || token == 0) {
        if (isStartUp) {
            ...
            ereport(elevel,
                (0,
                    errmsg(
                        "syntax error in file \"%s\" line %u, near end of line", config_file, ConfigFileLineno - 1)));
        }
    } else {
        if (isStartUp) {
...
static void* gtm_opt_malloc(int elevel, size_t size)
{
    ...
    if (data == NULL) {
        if (isStartUp) {
            ...
            ereport(elevel, (0, errmsg("out of memory")));
        }
    }
    return data;
}
...
void build_gtm_opt_variables(void)
{
    ...
    gtm_opt_vars = (struct config_generic**)gtm_opt_malloc(FATAL, size_vars * sizeof(struct config_generic*));
    ...
    qsort((void*)gtm_opt_variables, num_gtm_opt_variables, sizeof(struct config_generic*), gtm_opt_var_compare);
}
 ...
 */
void InitializeGTMOptions(void)
{
    int i;
...
bool SelectConfigFiles(const char* userDoption, const char* progname)
{
    ...
    if (GTMConfigFileName) {
        if (GTMConfigFileName[0] == '/') {
            ...
            if (configdir) {
                sz = strlen(configdir) + strlen(GTMConfigFileName) + 2;
                fname = gtm_opt_malloc(FATAL, sz);
                nRet = sprintf_s(fname, sz, "%s/%s", configdir, GTMConfigFileName);
        ...
        sz = strlen(configdir) + strlen(config_filename) + 2;
        fname = gtm_opt_malloc(FATAL, sz);
        nRet = sprintf_s(fname, sz, "%s/%s", configdir, config_filename);
        securec_check_ss(nRet, "\0", "\0");
    } else {
...
static void push_old_value(struct config_generic* gconf)
{
     ...
     */
    stack = (GtmOptStack*)MemoryContextAllocZero(TopMemoryContext, sizeof(GtmOptStack));
 ...
 */
bool set_config_option(const char* name, const char* value, GtmOptContext context, GtmOptSource source, bool changeVal)
{
    ...
    if (context == GTMC_SIGHUP || source == GTMC_S_DEFAULT) {
         ...
         */
        elevel = DEBUG3;
    } else if (source == GTMC_S_DATABASE || source == GTMC_S_USER || source == GTMC_S_DATABASE_USER)
        elevel = WARNING;
    else
        elevel = ERROR;
    ...
    if (record == NULL) {
        if (isStartUp) {
            ...
            ereport(elevel, (0, errmsg("unrecognized configuration parameter \"%s\"", name)));
        }
        return false;
    }
    ...
    switch (record->context) {
        ...
        case GTMC_STARTUP:
            if (context == GTMC_SIGHUP) {
                ...
                if (isStartUp) {
                    ...
                    ereport(
                        elevel, (0, errmsg("parameter \"%s\" cannot be changed without restarting the server", name)));
                }
                return false;
            }
            break;
        case GTMC_SIGHUP:
            if (context != GTMC_SIGHUP && context != GTMC_STARTUP) {
                if (isStartUp) {
                    ...
                    ereport(elevel, (0, errmsg("parameter \"%s\" cannot be changed now", name)));
                }
                return false;
            }
        ...
        default:
            if (isStartUp) {
                ...
                ereport(elevel, (0, errmsg("GtmOptContext invalid (%d)", context)));
            }
            return false;
    }
     ...
     */
    makeDefault = changeVal && (source <= GTMC_S_OVERRIDE) && ((value != NULL) || source == GTMC_S_DEFAULT);
            ...
            if (value) {
                if (!gtm_opt_parse_bool(value, &newval)) {
                    if (isStartUp) {
                        ...
                        ereport(elevel, (0, errmsg("parameter \"%s\" requires a Boolean value", name)));
                    }
                    return false;
                }
            ...
            if (prohibitValueChange) {
                if (*conf->variable != newval) {
                    if (isStartUp) {
                        ...
                        ereport(elevel,
                            (0, errmsg("parameter \"%s\" cannot be changed without restarting the server", name)));
                    }
                }
                return false;
            }
            ...
            if (value) {
                if (!parse_real(value, &newval)) {
                    if (isStartUp) {
                        ...
                        ereport(elevel, (0, errmsg("parameter \"%s\" requires a numeric value", name)));
                    }
                    return false;
                }
                if (newval < conf->min || newval > conf->max) {
 ...
 */
void SetConfigOption(const char* name, const char* value, GtmOptContext context, GtmOptSource source)
{
    (void)set_config_option(name, value, context, source, true);
}
 ...
 */
const char* GetConfigOption(const char* name, bool restrict_superuser)
{
    struct config_generic* record;
    ...
    if (record == NULL) {
        if (isStartUp) {
            ...
            ereport(ERROR, (0, errmsg("unrecognized configuration parameter \"%s\"", name)));
        }
    }
    switch (record->vartype) {
        case GTMC_BOOL:
        ...
        case GTMC_INT:
            nRet = snprintf_s(buffer, sizeof(buffer), sizeof(buffer) - 1,
                "%d", *((struct config_int*)record)->variable);
            securec_check_ss(nRet, "\0", "\0");
            return buffer;

        case GTMC_REAL:
            nRet = snprintf_s(buffer, sizeof(buffer), sizeof(buffer) - 1,
                "%g", *((struct config_real*)record)->variable);
            securec_check_ss(nRet, "\0", "\0");
            return buffer;
...
const char* GetConfigOptionResetString(const char* name)
{
    ...
    switch (record->vartype) {
        ...
        case GTMC_INT:
            nRet = snprintf_s(buffer, sizeof(buffer), sizeof(buffer) - 1,
                "%d", ((struct config_int*)record)->reset_val);
            securec_check_ss(nRet, "\0", "\0");
            return buffer;

        case GTMC_REAL:
            nRet = snprintf_s(buffer, sizeof(buffer), sizeof(buffer) - 1,
                "%g", ((struct config_real*)record)->reset_val);
            securec_check_ss(nRet, "\0", "\0");
            return buffer;
...
void EmitWarningsOnPlaceholders(const char* className)
{
    ...
    for (i = 0; i < num_gtm_opt_variables; i++) {
        ...
        if ((var->flags & GTMOPT_CUSTOM_PLACEHOLDER) != 0 && strncmp(className, var->name, classLen) == 0 &&
            var->name[classLen] == GTMOPT_QUALIFIER_SEPARATOR) {
            if (isStartUp) {
                write_stderr("unrecognized configuration parameter \"%s\"\n", var->name);
            } else {
                ereport(WARNING, (0, errmsg("unrecognized configuration parameter \"%s\"", var->name)));
            }
        }
    }
}
...
void GetConfigOptionByNum(int varnum, const char** values, bool* noshow)
{
    ...
    if (conf->vartype == GTMC_INT) {
        ...
        switch (conf->flags & (GTMOPT_UNIT_MEMORY | GTMOPT_UNIT_TIME)) {
            case GTMOPT_UNIT_KB:
                ...
                break;
            case GTMOPT_UNIT_BLOCKS:
                nRet = snprintf_s(buf, sizeof(buf), sizeof(buf) - 1,
                    "%dkB", BLCKSZ / 1024);
                securec_check_ss(nRet, "\0", "\0");
                values[2] = buf;
                break;
    ...
    switch (conf->vartype) {
        ...
        case GTMC_ENUM: {
            ...
            /* boot_val */
            values[12] = strdup(config_enum_lookup_by_value(lconf, lconf->boot_val));

            /* reset_val */
            values[13] = strdup(config_enum_lookup_by_value(lconf, lconf->reset_val));
        } break;
...
static char* _ShowOption(struct config_generic* record, bool use_units)
{
    ...
    switch (record->vartype) {
        ...
        case GTMC_INT: {
            ...
            if (use_units && result > 0 && (record->flags & GTMOPT_UNIT_MEMORY)) {
            ...
            } else if (use_units && result > 0 && (record->flags & GTMOPT_UNIT_TIME)) {
