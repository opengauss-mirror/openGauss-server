 ...
 */
static MemoryContextMethods AllocSetMethods = {AllocSetAlloc,
    AllocSetFree,
    AllocSetRealloc,
    AllocSetInit,
    AllocSetReset,
...
MemoryContext AllocSetContextCreate(
    MemoryContext parent, const char* name, Size minContextSize, Size initBlockSize, Size maxBlockSize, bool isShared)
{
    AllocSet context;

    /* Do the type-independent part of context creation */
    context = (AllocSet)MemoryContextCreate(sizeof(AllocSetContext), &AllocSetMethods, parent, name);
    ...
    while (context->allocChunkLimit > (Size)(maxBlockSize - ALLOC_BLOCKHDRSZ - ALLOC_CHUNKHDRSZ)) {
    ...
    if (minContextSize > ALLOC_BLOCKHDRSZ + ALLOC_CHUNKHDRSZ) {
        ...
        if (block == NULL) {
            ...
            ereport(ERROR,
                (ENOMEM, errmsg("out of memory"), errdetail("Failed while creating memory context \"%s\".", name)));
        }
        block->aset = context;
...
static void AllocSetReset(MemoryContext context)
{
    ...
    while (block != NULL) {
        ...
        if (block == set->keeper) {
            ...
            rc = memset_s(datastart, block->freeptr - datastart, 0x7F, block->freeptr - datastart);
...
static void AllocSetFree(MemoryContext context, void* pointer)
{
    ...
    if (chunk->size > set->allocChunkLimit) {
        ...
        if (block == NULL) {
            ...
            elog(ERROR, "could not find block containing chunk");
        }
...
#ifdef CLOBBER_FREED_MEMORY
        /* Wipe freed memory for debugging purposes */
        rc = memset_s(pointer, chunk->size, 0x7F, chunk->size);
        securec_check(rc, "\0", "\0");
#endif
