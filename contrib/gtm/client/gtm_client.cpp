...
static int commit_prepared_transaction_internal(
    GTM_Conn* conn, GlobalTransactionId gxid, GlobalTransactionId prepared_gxid, bool is_backup);
...
static int abort_transaction_multi_internal(
    GTM_Conn* conn, int txn_count, GlobalTransactionId* gxid, int* txn_count_out, int* status_out, bool is_backup);
static int open_sequence_internal(GTM_Conn* conn, GTM_SequenceKey key, GTM_Sequence increment, GTM_Sequence minval,
    GTM_Sequence maxval, GTM_Sequence startval, bool cycle, bool is_backup);
static int get_next_internal(GTM_Conn* conn, GTM_SequenceKey key, GTM_Sequence* result, bool is_backup);
...
static int alter_sequence_internal(GTM_Conn* conn, GTM_SequenceKey key, GTM_Sequence increment, GTM_Sequence minval,
    GTM_Sequence maxval, GTM_Sequence startval, GTM_Sequence lastval, bool cycle, bool is_restart, bool is_backup);
...
static GTM_Result* makeEmptyResultIfIsNull(GTM_Result* oldres)
{
    ...
    if (oldres == NULL) {
        ...
        rc = memset_s(res, sizeof(GTM_Result), 0, sizeof(GTM_Result));
 ...
 */
int begin_replication_initial_sync(GTM_Conn* conn)
{
    GTM_Result* res = NULL;
    time_t finish_time;
 ...
 * returns 1 on success, 0 on failure.
 */
int end_replication_initial_sync(GTM_Conn* conn)
{
    GTM_Result* res = NULL;
    time_t finish_time;

    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_NODE_END_REPLICATION_INIT, sizeof(GTM_MessageType), conn)) {
        goto send_failed;
    }
 ...
 */
size_t get_node_list(GTM_Conn* conn, GTM_PGXCNodeInfo* data, size_t maxlen)
{
    GTM_Result* res = NULL;
    time_t finish_time;
    size_t num_node;
    ...
    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_NODE_LIST, sizeof(GTM_MessageType), conn)) {
        goto send_failed;
    }
    ...
    for (i = 0; i < num_node; i++) {
        rc = memcpy_s(&data[i], sizeof(GTM_PGXCNodeInfo), res->gr_resdata.grd_node_list.nodeinfo[i],
            sizeof(GTM_PGXCNodeInfo));
        securec_check(rc, "\0", "\0");
    }
...
GlobalTransactionId get_next_gxid(GTM_Conn* conn)
{
    ...
    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_TXN_GET_NEXT_GXID, sizeof(GTM_MessageType), conn)) {
        goto send_failed;
    }
 ...
 */
uint32 get_txn_gxid_list(GTM_Conn* conn, GTM_Transactions* txn)
{
    GTM_Result* res = NULL;
    time_t finish_time;
    int txn_count;
    ...
    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_TXN_GXID_LIST, sizeof(GTM_MessageType), conn)) {
        goto send_failed;
    }
    ...
    txn_count =
        gtm_deserialize_transactions(txn, res->gr_resdata.grd_txn_gid_list.ptr, res->gr_resdata.grd_txn_gid_list.len);
 ...
 */
size_t get_sequence_list(GTM_Conn* conn, GTM_SeqInfo** seq_list)
{
    GTM_Result* res = NULL;
    time_t finish_time;

    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_SEQUENCE_LIST, sizeof(GTM_MessageType), conn)) {
        goto send_failed;
    }
...
int bkup_begin_transaction(
    GTM_Conn* conn, GTM_TransactionHandle txn, GTM_IsolationLevel isolevel, bool read_only, GTM_Timestamp timestamp)
{
    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_BKUP_TXN_BEGIN, sizeof(GTM_MessageType), conn) ||
        gtmpqPutInt(txn, sizeof(GTM_TransactionHandle), conn) ||
        gtmpqPutInt(isolevel, sizeof(GTM_IsolationLevel), conn) || gtmpqPutc(read_only, conn) ||
...
{
    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_BKUP_TXN_BEGIN_GETGXID, sizeof(GTM_MessageType), conn) ||
        gtmpqPutInt(txn, sizeof(GTM_TransactionHandle), conn) || gtmpqPutInt(gxid, sizeof(GlobalTransactionId), conn) ||
        gtmpqPutInt(isolevel, sizeof(GTM_IsolationLevel), conn) || gtmpqPutc(read_only, conn) ||
        gtmpqPutnchar((char*)&timestamp, sizeof(GTM_Timestamp), conn)) {
        goto send_failed;
    }
...
GlobalTransactionId begin_transaction(GTM_Conn* conn, GTM_IsolationLevel isolevel, GTM_Timestamp* timestamp)
{
    ...
    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_TXN_BEGIN_GETGXID, sizeof(GTM_MessageType), conn) ||
        gtmpqPutInt(isolevel, sizeof(GTM_IsolationLevel), conn) || gtmpqPutc(txn_read_only, conn)) {
        goto send_failed;
    }
...
int bkup_begin_transaction_autovacuum(
    GTM_Conn* conn, GTM_TransactionHandle txn, GlobalTransactionId gxid, GTM_IsolationLevel isolevel)
{
    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) ||
...
static int commit_transaction_internal(GTM_Conn* conn, GlobalTransactionId gxid, bool is_backup)
{
    ...
    if (gtmpqPutMsgStart('C', true, conn) ||
        ...
        gtmpqPutc(true, conn) || gtmpqPutnchar((char*)&gxid, sizeof(GlobalTransactionId), conn)) {
        goto send_failed;
    }
    ...
    GTM_Conn* conn, GlobalTransactionId gxid, GlobalTransactionId prepared_gxid, bool is_backup)
{
    ...
    if (gtmpqPutMsgStart('C', true, conn) ||
        gtmpqPutInt(
            is_backup ? MSG_BKUP_TXN_COMMIT_PREPARED : MSG_TXN_COMMIT_PREPARED, sizeof(GTM_MessageType), conn) ||
        gtmpqPutc(true, conn) || gtmpqPutnchar((char*)&gxid, sizeof(GlobalTransactionId), conn) ||
        gtmpqPutc(true, conn) || gtmpqPutnchar((char*)&prepared_gxid, sizeof(GlobalTransactionId), conn)) {
        goto send_failed;
    }
...
int backup_start_prepared_transaction(GTM_Conn* conn, GTM_TransactionHandle txn, char* gid, char* nodestring)
{
    Assert(nodestring && gid && conn);

    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_BKUP_TXN_START_PREPARED, sizeof(GTM_MessageType), conn) ||
        gtmpqPutc(false, conn) || gtmpqPutInt(txn, sizeof(GTM_TransactionHandle), conn) ||
        gtmpqPutInt(strlen(gid), sizeof(GTM_StrLen), conn) || gtmpqPutnchar(gid, strlen(gid), conn) ||
...
int start_prepared_transaction(GTM_Conn* conn, GlobalTransactionId gxid, char* gid, char* nodestring)
{
    GTM_Result* res = NULL;
    time_t finish_time;
    ...
    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_TXN_START_PREPARED, sizeof(GTM_MessageType), conn) ||
        gtmpqPutc(true, conn) || gtmpqPutnchar((char*)&gxid, sizeof(GlobalTransactionId), conn) ||
        /* Send also GID for an explicit prepared transaction */
        gtmpqPutInt(strlen(gid), sizeof(GTM_StrLen), conn) || gtmpqPutnchar((char*)gid, strlen(gid), conn) ||
        gtmpqPutInt(strlen(nodestring), sizeof(GTM_StrLen), conn) ||
...
int get_gid_data(GTM_Conn* conn, GTM_IsolationLevel isolevel, char* gid, GlobalTransactionId* gxid,
    GlobalTransactionId* prepared_gxid, char** nodestring)
{
    ...
    /* Start the message. Send also GID for an explicit prepared transaction */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_TXN_GET_GID_DATA, sizeof(GTM_MessageType), conn) ||
        gtmpqPutInt(isolevel, sizeof(GTM_IsolationLevel), conn) || gtmpqPutc(txn_read_only, conn) ||
        gtmpqPutInt(strlen(gid), sizeof(GTM_StrLen), conn) || gtmpqPutnchar((char*)gid, strlen(gid), conn)) {
        goto send_failed;
    }
 ...
 */
GTM_SnapshotData* get_snapshot(GTM_Conn* conn, GlobalTransactionId gxid, bool canbe_grouped)
{
    GTM_Result* res = NULL;
    time_t finish_time;

    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_SNAPSHOT_GET, sizeof(GTM_MessageType), conn) ||
        gtmpqPutc(canbe_grouped, conn) || gtmpqPutc(true, conn) ||
 ...
 */
int open_sequence(GTM_Conn* conn, GTM_SequenceKey key, GTM_Sequence increment, GTM_Sequence minval, GTM_Sequence maxval,
    GTM_Sequence startval, bool cycle)
{
    return open_sequence_internal(conn, key, increment, minval, maxval, startval, cycle, false);
}

int bkup_open_sequence(GTM_Conn* conn, GTM_SequenceKey key, GTM_Sequence increment, GTM_Sequence minval,
    ...
    GTM_Sequence maxval, GTM_Sequence startval, bool cycle, bool is_backup)
{
    ...
    if (gtmpqPutMsgStart('C', true, conn) ||
        ...
        gtmpqPutInt(key->gsk_keylen, 4, conn) || gtmpqPutnchar(key->gsk_key, key->gsk_keylen, conn) ||
        ...
        gtmpqPutnchar((char*)&startval, sizeof(GTM_Sequence), conn) || gtmpqPutc(cycle, conn)) {
        goto send_failed;
    }
...
int alter_sequence(GTM_Conn* conn, GTM_SequenceKey key, GTM_Sequence increment, GTM_Sequence minval,
...
int bkup_alter_sequence(GTM_Conn* conn, GTM_SequenceKey key, GTM_Sequence increment, GTM_Sequence minval,
    GTM_Sequence maxval, GTM_Sequence startval, GTM_Sequence lastval, bool cycle, bool is_restart)
{
    ...
    GTM_Sequence maxval, GTM_Sequence startval, GTM_Sequence lastval, bool cycle, bool is_restart, bool is_backup)
{
    ...
    if (gtmpqPutMsgStart('C', true, conn) ||
        ...
        gtmpqPutnchar((char*)&lastval, sizeof(GTM_Sequence), conn) || gtmpqPutc(cycle, conn) ||
...
static int rename_sequence_internal(GTM_Conn* conn, GTM_SequenceKey key, GTM_SequenceKey newkey, bool is_backup)
{
    ...
    if (gtmpqPutMsgStart('C', true, conn) ||
        ...
        gtmpqPutInt(key->gsk_keylen, 4, conn) || gtmpqPutnchar(key->gsk_key, key->gsk_keylen, conn) ||
        gtmpqPutInt(newkey->gsk_keylen, 4, conn) || gtmpqPutnchar(newkey->gsk_key, newkey->gsk_keylen, conn)) {
        goto send_failed;
    }
...
int set_val(GTM_Conn* conn, GTM_SequenceKey key, GTM_Sequence nextval, bool iscalled)
{
    return set_val_internal(conn, key, nextval, iscalled, false);
}

int bkup_set_val(GTM_Conn* conn, GTM_SequenceKey key, GTM_Sequence nextval, bool iscalled)
{
...
static int set_val_internal(GTM_Conn* conn, GTM_SequenceKey key, GTM_Sequence nextval, bool iscalled, bool is_backup)
{
    ...
    if (gtmpqPutMsgStart('C', true, conn) ||
        ...
        gtmpqPutInt(key->gsk_keylen, 4, conn) || gtmpqPutnchar(key->gsk_key, key->gsk_keylen, conn) ||
        gtmpqPutnchar((char*)&nextval, sizeof(GTM_Sequence), conn) || gtmpqPutc(iscalled, conn)) {
        goto send_failed;
    }
...
char* node_get_local_addr(GTM_Conn* conn, char* buf, size_t buflen, int* rc)
{
    ...
    *rc = 0;
    ...
    ret = memset_s(local_host, sizeof(local_host), 0, sizeof(local_host));
    securec_check(ret, "\0", "\0");
    ret = memset_s(local_port, sizeof(local_port), 0, sizeof(local_port));
    securec_check(ret, "\0", "\0");
    ret = memset_s(buf, buflen, 0, buflen);
    securec_check(ret, "\0", "\0");

    if (conn->remote_type != GTM_NODE_GTM_PROXY) {
        if (gtm_getnameinfo_all(&conn->laddr.addr,
                conn->laddr.salen,
                local_host,
                sizeof(local_host),
                local_port,
                sizeof(local_port),
                NI_NUMERICSERV)) {
            *rc = gtm_getnameinfo_all(&conn->laddr.addr,
                conn->laddr.salen,
                local_host,
                sizeof(local_host),
                local_port,
    ...
    if (local_host[0] != '\0') {
        ret = strncpy_s(buf, buflen, local_host, buflen - 1);
        securec_check(ret, "\0", "\0");
    }
...
{
    /* Start the message. */
    if (gtmpqPutMsgStart('C', true, conn) || gtmpqPutInt(MSG_BACKEND_DISCONNECT, sizeof(GTM_MessageType), conn) ||
...
int begin_transaction_multi(GTM_Conn* conn, int txn_count, GTM_IsolationLevel* txn_isolation_level, bool* txn_read_only,
    GTMProxy_ConnID* txn_connid, int* txn_count_out, GlobalTransactionId* gxid_out, GTM_Timestamp* ts_out)
{
    GTM_Result* res = NULL;
    time_t finish_time;
    int i;
    ...
    if (res->gr_status == GTM_RESULT_OK) {
        rc = memcpy_s(txn_count_out, sizeof(int), &res->gr_resdata.grd_txn_get_multi.txn_count, sizeof(int));
        securec_check(rc, "\0", "\0");
        rc = memcpy_s(gxid_out, sizeof(GlobalTransactionId), &res->gr_resdata.grd_txn_get_multi.start_gxid,
            sizeof(GlobalTransactionId));
        securec_check(rc, "\0", "\0");
        rc = memcpy_s(ts_out, sizeof(GTM_Timestamp), &res->gr_resdata.grd_txn_get_multi.timestamp,
            sizeof(GTM_Timestamp));
        securec_check(rc, "\0", "\0");
    }
...
int bkup_begin_transaction_multi(GTM_Conn* conn, int txn_count, GTM_TransactionHandle* txn,
    GlobalTransactionId start_gxid, GTM_IsolationLevel* isolevel, bool* read_only, GTMProxy_ConnID* txn_connid)
{
    int ii;
    ...
    for (ii = 0; ii < txn_count; ii++, gxid++) {
        ...
        if (gtmpqPutInt(txn[ii], sizeof(GTM_TransactionHandle), conn) ||
            ...
            gtmpqPutInt(isolevel[ii], sizeof(GTM_IsolationLevel), conn) || gtmpqPutc(read_only[ii], conn) ||
...
int bkup_commit_transaction_multi(GTM_Conn* conn, int txn_count, GTM_TransactionHandle* txn)
{
    int ii;
    ...
    for (ii = 0; ii < txn_count; ii++) {
        if (gtmpqPutc(false, conn) || gtmpqPutnchar((char*)&txn[ii], sizeof(GTM_TransactionHandle), conn))
            goto send_failed;
    }
...
int commit_transaction_multi(
    GTM_Conn* conn, int txn_count, GlobalTransactionId* gxid, int* txn_count_out, int* status_out)
{
    GTM_Result* res = NULL;
    time_t finish_time;
    int i;
    ...
    if (gtmpqPutInt(MSG_TXN_COMMIT_MULTI, sizeof(GTM_MessageType), conn) || gtmpqPutInt(txn_count, sizeof(int), conn)) {
        goto send_failed;
    }

    for (i = 0; i < txn_count; i++) {
        if (gtmpqPutc(true, conn) || gtmpqPutnchar((char*)&gxid[i], sizeof(GlobalTransactionId), conn)) {
            goto send_failed;
        }
    }
    ...
    if (res->gr_status == GTM_RESULT_OK) {
        rc = memcpy_s(txn_count_out, sizeof(int), &res->gr_resdata.grd_txn_get_multi.txn_count, sizeof(int));
        securec_check(rc, "\0", "\0");
        rc = memcpy_s(status_out, sizeof(int) * (*txn_count_out), &res->gr_resdata.grd_txn_rc_multi.status, 
            sizeof(int) * (*txn_count_out));
        securec_check(rc, "\0", "\0");
    }
...
int abort_transaction_multi(
    GTM_Conn* conn, int txn_count, GlobalTransactionId* gxid, int* txn_count_out, int* status_out)
{
    return abort_transaction_multi_internal(conn, txn_count, gxid, txn_count_out, status_out, false);
}

int bkup_abort_transaction_multi(GTM_Conn* conn, int txn_count, GlobalTransactionId* gxid)
{
    int txn_count_out;
...
int snapshot_get_multi(GTM_Conn* conn, int txn_count, GlobalTransactionId* gxid, int* txn_count_out, int* status_out,
    GlobalTransactionId* xmin_out, GlobalTransactionId* xmax_out, GlobalTransactionId* recent_global_xmin_out,
    int32* xcnt_out)
{
    GTM_Result* res = NULL;
    time_t finish_time;
    ...
    if (gtmpqPutInt(MSG_SNAPSHOT_GET_MULTI, sizeof(GTM_MessageType), conn) || gtmpqPutInt(txn_count, sizeof(int), conn)) {
        goto send_failed;
    }
    ...
    if (res->gr_status == GTM_RESULT_OK) {
        ...
        rc = memcpy_s(status_out, sizeof(int) * (*txn_count_out), &res->gr_resdata.grd_txn_rc_multi.status,
            sizeof(int) * (*txn_count_out));
        securec_check(rc, "\0", "\0");
        rc = memcpy_s(xmin_out, sizeof(GlobalTransactionId), &res->gr_snapshot.sn_xmin, sizeof(GlobalTransactionId));
        securec_check(rc, "\0", "\0");
        rc = memcpy_s(xmax_out, sizeof(GlobalTransactionId), &res->gr_snapshot.sn_xmax, sizeof(GlobalTransactionId));
        securec_check(rc, "\0", "\0");
        rc = memcpy_s(recent_global_xmin_out, sizeof(GlobalTransactionId), &res->gr_snapshot.sn_recent_global_xmin,
            sizeof(GlobalTransactionId));
        securec_check(rc, "\0", "\0");
        rc = memcpy_s(xcnt_out, sizeof(int32), &res->gr_snapshot.sn_xcnt, sizeof(int32));
        securec_check(rc, "\0", "\0");
    }
...
int report_barrier(GTM_Conn* conn, char* barrier_id)
{
...
static int report_barrier_internal(GTM_Conn* conn, char* barrier_id, bool is_backup)
{
    ...
    if (gtmpqPutInt(is_backup ? MSG_BKUP_BARRIER : MSG_BARRIER, sizeof(GTM_MessageType), conn) ||
        gtmpqPutInt(barrier_id_len, sizeof(int), conn) || gtmpqPutnchar(barrier_id, barrier_id_len, conn)) {
        goto send_failed;
    }
