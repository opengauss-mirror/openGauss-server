...
int main(int argc, char* argv[])
{
    ...
    for (ii = 0; ii < 5; ii++) {
        ...
        if (snapshot != NULL) {
            client_log(("Snapshot: GXID %u, xmin=%u, xmax=%u\n", gxid[ii], snapshot->sn_xmin, snapshot->sn_xmax));
            client_log(("xcnt=%d %s", snapshot->sn_xcnt, snapshot->sn_xcnt > 0 ? "xip=(" : ""));
            for (jj = 0; jj < snapshot->sn_xcnt; jj++) {
                client_log(("%d%c ", snapshot->sn_xip[jj], ((jj + 1) == snapshot->sn_xcnt) ? ')' : ','));
            }
            client_log(("\n"));
        }
    }
