 ...
 */
int gtmpqGetc(char* result, GTM_Conn* conn)
{
 ...
 */
int gtmpqPutc(char c, GTM_Conn* conn)
{
 ...
 */
static int gtmpqGets_internal(PQExpBuffer buf, GTM_Conn* conn, bool resetbuffer)
{
...
int gtmpqGets(PQExpBuffer buf, GTM_Conn* conn)
{
    return gtmpqGets_internal(buf, conn, true);
}

int gtmpqGets_append(PQExpBuffer buf, GTM_Conn* conn)
{
 ...
 */
int gtmpqPuts(const char* s, GTM_Conn* conn)
{
 ...
 */
int gtmpqGetnchar(char* s, size_t len, GTM_Conn* conn)
{
    errno_t rc = EOK;
 ...
 */
int gtmpqPutnchar(const char* s, size_t len, GTM_Conn* conn)
{
 ...
 *	to local byte order
 */
int gtmpqGetInt(int* result, size_t bytes, GTM_Conn* conn)
{
    uint16 tmp2;
    uint32 tmp4;
    errno_t rc = EOK;
    ...
    switch (bytes) {
        case 2:
            ...
            rc = memcpy_s(&tmp2, 2, conn->inBuffer + conn->inCursor, 2);
            securec_check(rc, "\0", "\0");
            conn->inCursor += 2;
        ...
        case 4:
            ...
            rc = memcpy_s(&tmp4, 4, conn->inBuffer + conn->inCursor, 4);
            securec_check(rc, "\0", "\0");
            conn->inCursor += 4;
 ...
 */
int gtmpqPutInt(int value, size_t bytes, GTM_Conn* conn)
{
    uint16 tmp2;
    uint32 tmp4;
 ...
 */
int gtmpqPutMsgStart(char msg_type, bool force_len, GTM_Conn* conn)
{
    int lenPos;
    int endPos;
...
static int gtmpqPutMsgBytes(const void* buf, size_t len, GTM_Conn* conn)
{
    ...
    /* okay, save the data */
    rc = memcpy_s(conn->outBuffer + conn->outMsgEnd, len, buf, len);
    securec_check(rc, "\0", "\0");
    conn->outMsgEnd += len;
