...
void ProcessPGXCNodeRegister(Port* myport, StringInfo message, bool is_backup)
{
    ...
    if (len >= NI_MAXHOST)
        ereport(ERROR, (EINVAL, errmsg("Invalid name length.")));
    ...
    /* Read Host name */
    len = pq_getmsgint(message, sizeof(int));
    rc = memcpy_s(remote_host, len, (char*)pq_getmsgbytes(message, len), len);
    securec_check(rc, "\0", "\0");
    remote_host[len] = '\0';
    ipaddress = remote_host;

    /* Read Port Number */
    rc = memcpy_s(&port, sizeof(GTM_PGXCNodePort), pq_getmsgbytes(message, sizeof(GTM_PGXCNodePort)), 
    ...
    if (len >= NI_MAXHOST)
        ereport(ERROR, (EINVAL, errmsg("Invalid proxy name length.")));
    rc = memcpy_s(proxyname, len, (char*)pq_getmsgbytes(message, len), len);
    securec_check(rc, "\0", "\0");
    proxyname[len] = '\0';
    ...
    rc = memcpy_s(datafolder, len, (char*)pq_getmsgbytes(message, len), len);
    securec_check(rc, "\0", "\0");
    datafolder[len] = '\0';
    ...
    if ((type != GTM_NODE_GTM_PROXY) && (type != GTM_NODE_GTM_PROXY_POSTMASTER) && (type != GTM_NODE_COORDINATOR) &&
        (type != GTM_NODE_DATANODE) && (type != GTM_NODE_GTM) && (type != GTM_NODE_DEFAULT))
        ereport(ERROR, (EINVAL, errmsg("Unknown node type.")));
    ...
    if (Recovery_PGXCNodeRegister(
            type, node_name, port, proxyname, NODE_CONNECTED, ipaddress, datafolder, false, myport->sock)) {
    ...
    if (!is_backup) {
        ...
        if (GetMyThreadInfo->thr_conn->standby) {
            ...
            elog(DEBUG1, "calling node_register_internal() for standby GTM.");

        retry:
            _rc = bkup_node_register_internal(
                GetMyThreadInfo->thr_conn->standby, type, ipaddress, port, node_name, datafolder, status);
...
void ProcessPGXCNodeUnregister(Port* myport, StringInfo message, bool is_backup)
{
    ...
    len = pq_getmsgint(message, sizeof(int));
    if (len >= NI_MAXHOST)
        ereport(ERROR, (EINVAL, errmsg("Invalid node name length")));
    rc = memcpy_s(node_name, len, (char*)pq_getmsgbytes(message, len), len);
    securec_check(rc, "\0", "\0");
    node_name[len] = '\0';
    ...
    if (!is_backup) {
        ...
        if (GetMyThreadInfo->thr_conn->standby) {
            ...
            elog(DEBUG1, "calling node_unregister() for standby GTM.");

        retry:
            _rc = bkup_node_unregister(GetMyThreadInfo->thr_conn->standby, type, node_name);
...
void ProcessPGXCNodeList(Port* myport, StringInfo message)
{
    ...
    rc = memset_s(data, sizeof(GTM_PGXCNodeInfo*) * MAX_NODES, 0, sizeof(GTM_PGXCNodeInfo*) * MAX_NODES);
    securec_check(rc, "\0", "\0");
    rc = memset_s(s_data, sizeof(char*) * MAX_NODES, 0, sizeof(char*) * MAX_NODES);
    securec_check(rc, "\0", "\0");
    ...
    for (i = 0; i < num_node; i++) {
        ...
        rc = memset_s(s_data[i], s_len + 1, 0, s_len + 1);
        securec_check(rc, "\0", "\0");
