...
int StreamServerPort(int family, char* hostName, unsigned short portNumber, int ListenSocket[], int MaxListen)
{
    ...
    {
        ret = snprintf_s(portNumberStr, sizeof(portNumberStr), sizeof(portNumberStr) - 1,
            "%d", portNumber);
    ...
    for (addr = addrs; addr; addr = addr->ai_next) {
...
#endif
            default:
                ret = snprintf_s(familyDescBuf, sizeof(familyDescBuf), sizeof(familyDescBuf) - 1,
        ...
        if (!IS_AF_UNIX(addr->ai_family)) {
            if ((setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (char*)&one, sizeof(one))) == -1) {
                ereport(LOG, (EACCES, errmsg("setsockopt(SO_REUSEADDR) failed: %m")));
                close(fd);
                continue;
            }
        }
        ...
        if (addr->ai_family == AF_INET6) {
            if (setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&one, sizeof(one)) == -1) {
                ereport(LOG, (EACCES, errmsg("setsockopt(IPV6_V6ONLY) failed: %m")));
                close(fd);
                continue;
            }
        }
        ...
        if (err < 0) {
            ereport(LOG,
                (EACCES,
                    /* translator: %s is IPv4, IPv6, or Unix */
                    errmsg("could not bind %s socket: %m", familyDesc),
...
int StreamConnection(int server_fd, Port* port)
{
    ...
    if ((port->sock = accept(server_fd, (struct sockaddr*)&port->raddr.addr, (socklen_t*)&port->raddr.salen)) < 0) {
        ereport(LOG, (EACCES, errmsg("could not accept new connection: %m")));
...
static int pq_recvbuf(Port* myport)
{
    ...
    for (;;) {
        ...
        r = recv(myport->sock, myport->PqRecvBuffer + myport->PqRecvLength, PQ_BUFFER_SIZE - myport->PqRecvLength, 0);
        ...
        if (r < 0) {
             ...
             */
            ereport(COMMERROR, (EACCES, errmsg("could not receive data from client: %m")));
            return EOF;
        } else {
            myport->last_errno = 0;
        }
...
int pq_getbytes(Port* myport, char* s, size_t len)
{
    ...
    while (len > 0) {
        ...
        rc = memcpy_s(s, amount, myport->PqRecvBuffer + myport->PqRecvPointer, amount);
        securec_check(rc, "\0", "\0");
 ...
 * --------------------------------
 */
int pq_getstring(Port* myport, StringInfo s)
{
    int i;
    ...
    for (;;) {
        ...
        for (i = myport->PqRecvPointer; i < myport->PqRecvLength; i++) {
            if (myport->PqRecvBuffer[i] == '\0') {
                ...
                appendBinaryStringInfo(s, myport->PqRecvBuffer + myport->PqRecvPointer, i - myport->PqRecvPointer + 1);
        ...
        appendBinaryStringInfo(
            s, myport->PqRecvBuffer + myport->PqRecvPointer, myport->PqRecvLength - myport->PqRecvPointer);
 ...
 * --------------------------------
 */
int pq_getmessage(Port* myport, StringInfo s, int maxlen)
{
    int32 len;
    ...
    if (pq_getbytes(myport, (char*)&len, 4) == EOF) {
        ereport(COMMERROR, (EPROTO, errmsg("unexpected EOF within message length word")));
        return EOF;
    }
    ...
    if (len < 4 || (maxlen > 0 && len > maxlen)) {
        ereport(COMMERROR, (EPROTO, errmsg("invalid message length")));
        return EOF;
    }
    ...
    if (len > 0) {
        ...
        if (pq_getbytes(myport, s->data, len) == EOF) {
            ereport(COMMERROR, (EPROTO, errmsg("incomplete message from client")));
            return EOF;
        }
        s->len = len;
...
static int internal_putbytes(Port* myport, const char* s, size_t len)
{
    ...
    while (len > 0) {
        ...
        rc = memcpy_s(myport->PqSendBuffer + myport->PqSendPointer, amount, s, amount);
        securec_check(rc, "\0", "\0");
    ...
    while (bufptr < bufend) {
        ...
        if (r <= 0) {
            ...
            if (errno != last_reported_send_errno) {
                ...
                ereport(COMMERROR, (EACCES, errmsg("could not send data to client: %m")));
            }
