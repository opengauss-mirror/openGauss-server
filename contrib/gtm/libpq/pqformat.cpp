 ...
 * --------------------------------
 */
void pq_endmessage(Port* myport, StringInfo buf)
{
 ...
 * --------------------------------
 */
void pq_puttextmessage(Port* myport, char msgtype, const char* str)
{
    int slen = strlen(str);
 ...
 * --------------------------------
 */
void pq_putemptymessage(Port* myport, char msgtype)
{
...
int pq_getmsgbyte(StringInfo msg)
{
    if (msg->cursor >= msg->len)
        ereport(ERROR, (EPROTO, errmsg("no data left in message")));
...
int64 pq_getmsgint64(StringInfo msg)
{
    ...
    if ((result < 0) ? (h32 != -1) : (h32 != 0))
        ereport(ERROR, (ERANGE, errmsg("binary value is out of range for type bigint")));
#else
    result = h32;
    result <<= 32;
    result |= l32;
...
const char* pq_getmsgbytes(StringInfo msg, int datalen)
{
    ...
    if (datalen < 0 || datalen > (msg->len - msg->cursor))
        ereport(ERROR, (EPROTO, errmsg("insufficient data left in message")));
...
void pq_copymsgbytes(StringInfo msg, char* buf, int datalen)
{
    ...
    if (datalen < 0 || datalen > (msg->len - msg->cursor))
        ereport(ERROR, (EPROTO, errmsg("insufficient data left in message")));
    rc = memcpy_s(buf, datalen, &msg->data[msg->cursor], datalen);
...
char* pq_getmsgtext(StringInfo msg, int rawbytes, int* nbytes)
{
    ...
    rc = memcpy_s(p, rawbytes, str, rawbytes);
    securec_check(rc, "\0", "\0");
    p[rawbytes] = '\0';
    *nbytes = rawbytes;
    return p;
...
const char* pq_getmsgstring(StringInfo msg)
{
    ...
    if (msg->cursor + slen >= msg->len)
        ereport(ERROR, (EPROTO, errmsg("invalid string in message")));
    msg->cursor += slen + 1;
...
{
    if (msg->cursor != msg->len)
        ereport(ERROR, (EPROTO, errmsg("invalid message format")));
}
 ...
 * --------------------------------
 */
int pq_getmsgunreadlen(StringInfo msg)
{
