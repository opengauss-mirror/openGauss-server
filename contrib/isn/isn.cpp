 ...
 */
#ifdef ISN_DEBUG
static bool check_table(const char* (*TABLE)[2], const unsigned TABLE_index[10][2])
{
    ...
    while (TABLE[i][0] && TABLE[i][1]) {
        ...
        while (*aux1 && *aux2) {
            if (!(isdigit((unsigned char)*aux1) && isdigit((unsigned char)*aux2)) && (*aux1 != *aux2 || *aux1 != '-'))
                goto invalidtable;
            aux1++;
            aux2++;
        }
...
invalidtable:
    elog(DEBUG1, "invalid table near {\"%s\", \"%s\"} (pos: %d)", TABLE[i][0], TABLE[i][1], i);
    return false;
...
static bool ean2isn(ean13 ean, bool errorOK, ean13* result, enum isn_type accept)
{
    ...
    *aux = '\0'; /* terminate string; aux points to last digit */
    ...
    if (strncmp("978", buf, 3) == 0) { /* ISBN */
        type = ISBN;
    } else if (strncmp("977", buf, 3) == 0) { /* ISSN */
        type = ISSN;
    } else if (strncmp("9790", buf, 4) == 0) { /* ISMN */
        type = ISMN;
    } else if (strncmp("979", buf, 3) == 0) { /* ISBN-13 */
        type = ISBN;
    } else if (*buf == '0') { /* UPC */
        type = UPC;
    } else {
        ...
        if (type != EAN13) {
            ereport(ERROR,
                (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                    errmsg("cannot cast EAN13(%s) to %s for number: \"%s\"", isn_names[type], isn_names[accept], buf)));
        } else {
            ereport(ERROR,
                (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                    errmsg("cannot cast %s to %s for number: \"%s\"", isn_names[type], isn_names[accept], buf)));
        }
    }
    return false;
    ...
    if (!errorOK) {
        ...
        ereport(ERROR,
            (errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
                errmsg("value \"%s\" is out of range for %s type", eanbuf, isn_names[type])));
    }
    return false;
}
 ...
 */
static inline void ean2ISBN(char* isn)
{
    char* aux = NULL;
    unsigned check;
...
static inline void ean2ISMN(char* isn)
{
...
static inline void ean2ISSN(char* isn)
{
    unsigned check;
...
static inline void ean2UPC(char* isn)
{
...
eanbadcheck:
    ...
    if (!errorOK) {
        if (rcheck == (unsigned)-1) {
            ereport(ERROR,
                (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                    errmsg("invalid %s number: \"%s\"", isn_names[accept], str)));
        } else {
            ereport(ERROR,
                (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
...
eaninvalid:
    if (!errorOK)
        ereport(ERROR,
            (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                errmsg("invalid input syntax for %s number: \"%s\"", isn_names[accept], str)));
    return false;
...
eanwrongtype:
    if (!errorOK)
        ereport(ERROR,
            (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
                errmsg("cannot cast %s to %s for number: \"%s\"", isn_names[type], isn_names[accept], str)));
    return false;
...
eantoobig:
    if (!errorOK)
        ereport(ERROR,
            (errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
                errmsg("value \"%s\" is out of range for %s type", str, isn_names[accept])));
    return false;
}
