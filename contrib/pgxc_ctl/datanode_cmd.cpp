...
cmd_t* prepare_initDatanodeMaster(char* nodeName)
{
    ...
    snprintf(newCommand(cmdInitdb),
        MAXLINE,
        "rm -rf %s; mkdir -p %s; gs_initdb --nodename %s -D %s",
    ...
    snprintf(newCommand(cmdPgConf), MAXLINE, "cat >> %s/postgresql.conf", aval(VAR_datanodeMasterDirs)[idx]);
    ...
    if (isVarYes(VAR_datanodeSlave) && !is_none(aval(VAR_datanodeSlaveServers)[idx])) {
        ...
        snprintf(newCommand(cmd_cleanDir),
            MAXLINE,
        ...
        snprintf(newCommand(cmd_PgConf), MAXLINE, "cat >> %s/postgresql.conf", aval(VAR_datanodeMasterDirs)[idx]);
    ...
    snprintf(newCommand(cmdPgHba), MAXLINE, "cat >> %s/pg_hba.conf", aval(VAR_datanodeMasterDirs)[idx]);
...
cmd_t* prepare_initDatanodeSlave(char* nodeName)
{
    ...
    snprintf(newCommand(cmdRecovConf), MAXLINE, "cat >> %s/recovery.conf", aval(VAR_datanodeSlaveDirs)[idx]);
    ...
    snprintf(newCommand(cmdPgConf), MAXPATH, "cat >> %s/postgresql.conf", aval(VAR_datanodeSlaveDirs)[idx]);
...
cmd_t* prepare_startDatanodeMaster(char* nodeName)
{
    ...
    snprintf(newCommand(cmdStartDatanodeMaster),
        MAXLINE,
        "gs_ctl start -Z datanode -D %s -o -i",
        aval(VAR_datanodeMasterDirs)[idx]);
...
cmd_t* prepare_startDatanodeSlave(char* nodeName)
{
    ...
    snprintf(
        newCommand(cmdStartDatanodeSlave), MAXLINE, "gs_ctl start -Z datanode -D %s", aval(VAR_datanodeSlaveDirs)[idx]);
    ...
    snprintf(newCommand(cmdMasterToSyncMode), MAXLINE, "cat >> %s/postgresql.conf", aval(VAR_datanodeMasterDirs)[idx]);
...
{
    cmd_t* cmdStopDatanodeMaster = NULL;
    int idx;
    ...
    if (immediate)
        snprintf(newCommand(cmdStopDatanodeMaster),
            MAXLINE,
            "gs_ctl stop -Z datanode -D %s -m %s",
            aval(VAR_datanodeMasterDirs)[idx],
            immediate);
    else
        snprintf(newCommand(cmdStopDatanodeMaster),
            MAXLINE,
            "gs_ctl stop -Z datanode -D %s",
            aval(VAR_datanodeMasterDirs)[idx]);
...
cmd_t* prepare_stopDatanodeSlave(char* nodeName, char* immediate)
{
    ...
    snprintf(newCommand(cmdMasterToAsyncMode), MAXLINE, "cat >> %s/postgresql.conf", aval(VAR_datanodeMasterDirs)[idx]);
    ...
    if (pingNode(aval(VAR_datanodeSlaveServers)[idx], aval(VAR_datanodePorts)[idx]) == 0) {
        cmd_t* cmdReloadMaster = NULL;

        appendCmdEl(cmdMasterToAsyncMode, (cmdReloadMaster = initCmd(aval(VAR_datanodeMasterServers)[idx])));
        snprintf(
            newCommand(cmdReloadMaster), MAXLINE, "gs_ctl reload -Z datanode -D %s", aval(VAR_datanodeMasterDirs)[idx]);
    }

    /* Stop the slave */
    appendCmdEl(cmdMasterToAsyncMode, (cmdStopSlave = initCmd(aval(VAR_datanodeSlaveServers)[idx])));
    if (immediate)
        snprintf(newCommand(cmdStopSlave),
            MAXLINE,
            "gs_ctl stop -Z datanode -D %s -m %s",
            aval(VAR_datanodeSlaveDirs)[idx],
            immediate);
    else
        snprintf(newCommand(cmdStopSlave), MAXLINE, "gs_ctl stop -Z datanode -D %s", aval(VAR_datanodeSlaveDirs)[idx]);
    return (cmd);
}
...
#define checkRc()                       \
    ...
    if (gtmPxyIdx >= 0)
        elog(NOTICE,
            "Failover datanode %s using gs_gtm %s\n",
    ...
    /* Promote the slave */
    rc_local = doImmediate(aval(VAR_datanodeSlaveServers)[datanodeIdx],
        NULL,
        "gs_ctl promote -Z datanode -D %s",
    ...
    rc_local = doImmediate(aval(VAR_datanodeSlaveServers)[datanodeIdx],
        NULL,
        "gs_ctl restart -w -Z datanode -D %s -o -i; sleep 1",
        aval(VAR_datanodeSlaveDirs)[datanodeIdx]);
    checkRc();
    /*
    ...
    for (jj = 0; aval(VAR_coordNames)[jj]; jj++) {
        ...
        if ((f = pgxc_popen_wRaw("gsql -p %d -h %s %s %s",
...
int add_datanodeMaster(char* name, char* host, int port, char* dir)
{
    ...
    if ((arraySizeName(VAR_datanodePorts) != size) || (arraySizeName(VAR_datanodeMasterServers) != size) ||
        (arraySizeName(VAR_datanodeMasterDirs) != size) || (arraySizeName(VAR_datanodeMaxWALSenders) != size) ||
    ...
    /* Lock ddl */
    if ((lockf = pgxc_popen_wRaw("gsql -h %s -p %d %s",
    ...
    for (ii = 0; aval(VAR_coordNames)[ii]; ii++) {
        if (!is_none(aval(VAR_coordNames)[ii])) {
            if ((f = pgxc_popen_wRaw("gsql -h %s -p %s %s",
...
int add_datanodeSlave(char* name, char* host, char* dir, char* archDir)
{
    ...
    if (isVarYes(VAR_datanodeSlave) && doesExist(VAR_datanodeSlaveServers, idx) &&
    ...
    if ((f = pgxc_popen_w(
             aval(VAR_datanodeMasterServers)[idx], "cat >> %s/postgresql.conf", aval(VAR_datanodeMasterDirs)[idx])) ==
        NULL) {
        elog(ERROR,
            "ERROR: Cannot open datanodenator master's configuration file, %s/postgresql.conf",
            aval(VAR_datanodeMasterDirs)[idx]);
        return 1;
    }
    fprintf(f,
    ...
    if ((f = pgxc_popen_w(
             aval(VAR_datanodeMasterServers)[idx], "cat >> %s/pg_hba.conf", aval(VAR_datanodeMasterDirs)[idx])) ==
        NULL) {
        elog(ERROR,
            "ERROR: Cannot open datanode master's pg_hba.conf file, %s/pg_hba.conf, %s\n",
            aval(VAR_datanodeMasterDirs)[idx],
            strerror(errno));
        return 1;
    }
    ...
    doImmediate(aval(VAR_coordMasterServers)[idx],
        NULL,
        "gs_ctl stop -Z datanode -D %s -m fast",
        aval(VAR_datanodeMasterDirs)[idx]);
    doImmediate(
        aval(VAR_coordMasterServers)[idx], NULL, "gs_ctl start -Z datanode -D %s", aval(VAR_datanodeMasterDirs)[idx]);
    /* pg_basebackup */
    doImmediate(host,
        NULL,
        "pg_basebackup -p %s -h %s -D %s -x",
    ...
    fclose(f);
    /* Start the slave */
    doImmediate(host, NULL, "gs_ctl start -Z datanode -D %s", dir);
    return 0;
}
...
{
    /*
      1. Transfer the data from the datanode to be removed to the rest of the datanodes for all the tables in all the
      databases. For example to shift data of the table rr_abc to the rest of the nodes we can use command
    ...
    for (ii = 0; aval(VAR_datanodeNames)[ii]; ii++) {
        if ((ii != idx) && !is_none(aval(VAR_datanodeNames)[ii]) &&
            (pingNode(aval(VAR_datanodeMasterServers)[ii], aval(VAR_datanodePorts)[ii]) != 0)) {
...
int remove_datanodeSlave(char* name, int clean_opt)
{
    ...
    {
        FILE* f;
        if ((f = pgxc_popen_w(aval(VAR_datanodeMasterServers)[idx],
                 "cat >> %s/postgresql.conf",
                 aval(VAR_datanodeMasterDirs)[idx])) == NULL) {
            elog(ERROR,
    ...
    doImmediate(aval(VAR_datanodeMasterServers)[idx],
        NULL,
        "gs_ctl restart -Z datanode -D %s",
        aval(VAR_datanodeMasterDirs)[idx]);
...
int show_config_datanodeMaster(int flag, int idx, char* hostname)
{
    ...
    elog(NOTICE,
        ...
        sval(VAR_datanodeExtraConfig),
        aval(VAR_datanodeSpecificExtraConfig)[idx]);
    ...
    elog(NOTICE,
        ...
        sval(VAR_datanodeExtraPgHba),
        aval(VAR_datanodeSpecificExtraPgHba)[idx]);
    unlockLogFile();
    return 0;
}
...
cmd_t* prepare_killDatanodeMaster(char* nodeName)
{
    ...
    if ((postmasterPid = get_postmaster_pid(
             aval(VAR_datanodeMasterServers)[dnIndex], aval(VAR_datanodeMasterDirs)[dnIndex])) > 0) {
