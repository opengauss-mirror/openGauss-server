    ...
    switch (password_type) {
        case PASSWORD_TYPE_MD5:
            ...
            if (strcmp(password, encrypted) == 0)
                ereport(
                    ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("password must not contain user name")));
            break;
        ...
        case PASSWORD_TYPE_PLAINTEXT:
            ...
            if (pwdlen < MIN_PWD_LENGTH)
                ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("password is too short")));
